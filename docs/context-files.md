...
Elidia Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Elidia instance and is loaded from `ELIDIA_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.elidia.md** / **ELIDIA.md** | Project instructions (highest priority) | Walks to git root |
| **AGENTS.md** | Project instructions, conventions, architecture | CWD at startup + subdirectories progressively |
| **CLAUDE.md** | Claude Code context files (also detected) | CWD at startup + subdirectories progressively |
| **SOUL.md** | Global personality and tone customization for this Elidia instance | `ELIDIA_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

> [!NOTE] Priority system
> Only **one** project context type is loaded per session (first match wins): `.elidia.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
`SOUL.md` controls the agent's personality, tone, and communication style. See the [Personality](https://aiutils.io/elidia/user-guide/features/personality) page for full details.

**Location:**

- `~/.elidia/SOUL.md`
- or `$ELIDIA_HOME/SOUL.md` if you run Elidia with a custom home directory

Important details:

- Elidia seeds a default `SOUL.md` automatically if one does not exist yet
- Elidia loads `SOUL.md` only from `ELIDIA_HOME`
- Elidia does not probe the working directory for `SOUL.md`
...
Elidia is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.elidia.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Elidia.
...
1. **Scan working directory** — checks for `.elidia.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
