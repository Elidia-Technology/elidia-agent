...
This page is the top-level map of Elidia Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ elidia_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ElidiaCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── elidia_state.py           # SQLite session/state database with FTS5
├── elidia_constants.py       # ELIDIA_HOME, profile-aware paths
...
│
├── elidia_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `elidia` subcommands (large file)
...
│   ├── skills_config.py      # elidia skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # elidia tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # elidia gateway start/stop
│
...
```text
User input → ElidiaCLI.process_input()
...
2. **[Agent Loop Internals](https://aiutils.io/elidia/developer-guide/agent-loop)** — how AIAgent works
3. **[Prompt Assembly](https://aiutils.io/elidia/developer-guide/prompt-assembly)** — system prompt construction
4. **[Provider Runtime Resolution](https://aiutils.io/elidia/developer-guide/provider-runtime)** — how providers are selected
5. **[Adding Providers](https://aiutils.io/elidia/developer-guide/adding-providers)** — practical guide to adding a new provider
6. **[Tools Runtime](https://aiutils.io/elidia/developer-guide/tools-runtime)** — tool registry, dispatch, environments
7. **[Session Storage](https://aiutils.io/elidia/developer-guide/session-storage)** — SQLite schema, FTS5, session lineage
8. **[Gateway Internals](https://aiutils.io/elidia/developer-guide/gateway-internals)** — messaging platform gateway
9. **[Context Compression & Prompt Caching](https://aiutils.io/elidia/developer-guide/context-compression-and-caching)** — compression and caching
10. **[ACP Internals](https://aiutils.io/elidia/developer-guide/acp-internals)** — IDE integration
...
→ [Agent Loop Internals](https://aiutils.io/elidia/developer-guide/agent-loop)
...
→ [Prompt Assembly](https://aiutils.io/elidia/developer-guide/prompt-assembly), [Context Compression & Prompt Caching](https://aiutils.io/elidia/developer-guide/context-compression-and-caching)
...
→ [Provider Runtime Resolution](https://aiutils.io/elidia/developer-guide/provider-runtime)
...
→ [Tools Runtime](https://aiutils.io/elidia/developer-guide/tools-runtime)
...
→ [Session Storage](https://aiutils.io/elidia/developer-guide/session-storage)
...
→ [Gateway Internals](https://aiutils.io/elidia/developer-guide/gateway-internals)

### Plugin System

Three discovery sources: `~/.elidia/plugins/` (user), `.elidia/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `elidia plugins` or `config.yaml`.

→ [Plugin Guide](https://aiutils.io/elidia/guides/build-a-elidia-plugin), [Memory Provider Plugin](https://aiutils.io/elidia/developer-guide/memory-provider-plugin)
...
→ [Cron Internals](https://aiutils.io/elidia/developer-guide/cron-internals)

### ACP Integration

Exposes Elidia as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.

→ [ACP Internals](https://aiutils.io/elidia/developer-guide/acp-internals)

### Trajectories

Generates ShareGPT-format trajectories from agent sessions for training data generation.

→ [Trajectories & Training Format](https://aiutils.io/elidia/developer-guide/trajectory-format)
...
| **Profile isolation** | Each profile (`elidia -p <name>`) gets its own ELIDIA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
