...
When running Elidia inside Docker, the default `network_mode: host` gives the
...
The Elidia [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ELIDIA_UID=$(id -u) ELIDIA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=elidia,elidia-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://elidia-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Elidia trust model and vulnerability reporting
