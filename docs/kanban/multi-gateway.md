...
Elidia supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.elidia/config.yaml`:
...
Or set the env var: `ELIDIA_KANBAN_DISPATCH_IN_GATEWAY=false`
