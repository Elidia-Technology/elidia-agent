...
Elidia Agent has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
Both are stored in `~/.elidia/memories/` and are injected into the system prompt as a frozen snapshot at session start. The agent manages its own memory via the `memory` tool — it can add, replace, or remove entries.
...
- All CLI and messaging sessions are stored in SQLite (`~/.elidia/state.db`) with FTS5 full-text search
...
```bash
elidia sessions list    # Browse past sessions
```

See [Session Search Tool](https://aiutils.io/elidia/user-guide/sessions) for the three calling shapes (discovery / scroll / browse) and the response format.
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Elidia ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
elidia memory setup      # pick a provider and configure it
elidia memory status     # check what's active
```

See the [Memory Providers](https://aiutils.io/elidia/user-guide/features/memory-providers) guide for full details on each provider, setup instructions, and comparison.
