...
All skills live in **`~/.elidia/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Elidia at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.

See also:

- [Bundled Skills Catalog](https://aiutils.io/elidia/reference/skills-catalog)
- [Official Optional Skills Catalog](https://aiutils.io/elidia/reference/optional-skills-catalog)

## Starting with a blank slate

By default every profile is seeded with the bundled skill catalog, and each `elidia update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.elidia` profile):

```bash
curl -fsSL https://aiutils.io/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
elidia profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
elidia skills opt-out            # stop future seeding — nothing on disk is touched
elidia skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
elidia skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `elidia update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `elidia skills opt-in`) to re-enable.

> [!NOTE] Safe by default
> `elidia skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Elidia installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Elidia to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.elidia/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
elidia chat --toolsets skills -q "What skills do you have?"
elidia chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.elidia/cache/chart-q4-2025.png
...
When a missing value is encountered, Elidia asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `elidia setup` or `~/.elidia/.env` locally instead.

Once set, declared env vars are **automatically passed through** to `execute_code` and `terminal` sandboxes — the skill's scripts can use `$TENOR_API_KEY` directly. For non-skill env vars, use the `terminal.env_passthrough` config option. See [Environment Variable Passthrough](security.md) for details.
...
Settings are stored under `skills.config` in your config.yaml. `elidia config migrate` prompts for unconfigured settings, and `elidia config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.

See [Skill Settings](configuration.md) and [Creating Skills — Config Settings](https://aiutils.io/elidia/developer-guide/creating-skills) for details.

## Skill Directory Structure

```text
~/.elidia/skills/                  # Single source of truth
...
If you maintain skills outside of Elidia — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Elidia to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.elidia/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.elidia/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Elidia process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Elidia ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.elidia/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.elidia/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `elidia bundles list`.
...
elidia bundles create research

# Overwrite an existing bundle
elidia bundles create backend-dev --skill ... --force

# Delete a bundle
elidia bundles delete backend-dev

# Re-scan ~/.elidia/skill-bundles/ and report changes
elidia bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.elidia/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.elidia/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
elidia skills browse                              # Browse all hub skills (official first)
elidia skills browse --source official            # Browse only official optional skills
elidia skills search kubernetes                   # Search all sources
elidia skills search react --source skills-sh     # Search the skills.sh directory
elidia skills search https://mintlify.com/docs --source well-known
elidia skills inspect openai/skills/k8s           # Preview before installing
elidia skills install openai/skills/k8s           # Install with security scan
elidia skills install official/security/1password
elidia skills install skills-sh/vercel-labs/json-render/json-render-react --force
elidia skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
elidia skills list --source hub                   # List hub-installed skills
elidia skills check                               # Check installed hub skills for upstream updates
elidia skills update                              # Reinstall hub skills with upstream changes when needed
elidia skills audit                               # Re-scan all hub skills for security
elidia skills uninstall k8s                       # Remove a hub skill
elidia skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
elidia skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
elidia skills publish skills/my-skill --to github --repo owner/repo
elidia skills snapshot export setup.json          # Export skill config
elidia skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Elidia. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `elidia skills search <query> --source skills-sh`. Elidia resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Elidia currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Elidia repository itself and install with built-in trust.

- Catalog: [Official Optional Skills Catalog](https://aiutils.io/elidia/reference/optional-skills-catalog)
...
This is Vercel's public skills directory. Elidia can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
elidia skills search https://mintlify.com/docs --source well-known
elidia skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
elidia skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Elidia can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
elidia skills install openai/skills/k8s
elidia skills tap add myorg/skills-repo
```
...
[Skills Hub](https://aiutils.io/elidia) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Elidia-side changes required.
...
- Elidia source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Elidia supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Elidia source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Elidia can search and convert agent entries from LobeHub's public catalog into installable Elidia skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Elidia source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Elidia integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Elidia' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Elidia source id: `browse-sh`
- Trust level: `community`

```bash
elidia skills search airbnb --source browse-sh
elidia skills inspect browse-sh/airbnb.com/search-listings-ddgioa
elidia skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Elidia fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Elidia source id: `url`
...
```bash
elidia skills install https://sharethis.chat/SKILL.md
elidia skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
elidia skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `elidia skills update` re-fetches from the same URL automatically when you want to refresh.
...
`elidia skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Elidia | Always trusted |
...
```bash
elidia skills check          # Report which installed hub skills changed upstream
elidia skills update         # Reinstall only the skills with updates available
elidia skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Elidia users add with `elidia skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.elidia.tags`, `version`, `author`, `platforms`, `metadata.elidia.config`).
...
Elidia discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/elidia-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Elidia user can subscribe and install:

```bash
elidia skills tap add my-org/elidia-skills
elidia skills search deploy
elidia skills install my-org/elidia-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.elidia/.hub/taps.json`:
...
The `elidia skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `elidia skills tap list` shows the effective path per tap.
...
```bash
elidia skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.elidia/.hub/taps.json` (created on demand).

## Bundled skill updates (`elidia skills reset`)

Elidia ships with a set of bundled skills in `skills/` inside the repo. On install and on every `elidia update`, a sync pass copies those into `~/.elidia/skills/` and records a manifest at `~/.elidia/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Elidia recomputes the hash of your local copy and compares it to the origin hash:
...
elidia skills reset google-workspace --restore
...
> [!NOTE] Profiles
> Each profile has its own `.bundled_manifest` under its own `ELIDIA_HOME`, so `elidia -p coder skills reset <name>` only affects that profile.
