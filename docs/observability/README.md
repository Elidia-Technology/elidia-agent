# Elidia Observer Hooks

Elidia observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Elidia' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Elidia catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Elidia only builds
