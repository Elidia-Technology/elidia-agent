...
Schedule tasks to run automatically with natural language or cron expressions. Elidia exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Elidia itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

> [!TIP]
> Cron jobs use whatever provider `elidia model` selected. `elidia setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Elidia Portal](elidia-portal.md).


> [!WARNING]
> Cron-run sessions cannot recursively create more cron jobs. Elidia disables cron management tools inside cron executions to prevent runaway scheduling loops.
...
```bash
elidia cron create "every 2h" "Check server status"
elidia cron create "every 1h" "Summarize new feed items" --skill blogwatcher
elidia cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Elidia will use the unified `cronjob` tool internally.
...
elidia cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever Elidia profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `ELIDIA_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
elidia cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root Elidia profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
> [!NOTE] Serialization
> Jobs with a `profile` set also run sequentially, for the same reason as `workdir`-pinned jobs: switching `ELIDIA_HOME` is a process-global mutation, so two profile-pinned jobs running in parallel would race each other. Unpinned jobs still run in the normal parallel pool.
...
```bash
elidia cron edit <job_id> --schedule "every 4h"
elidia cron edit <job_id> --prompt "Use the revised task"
elidia cron edit <job_id> --skill blogwatcher --skill maps
elidia cron edit <job_id> --add-skill maps
elidia cron edit <job_id> --remove-skill blogwatcher
elidia cron edit <job_id> --clear-skills
```
...
```bash
elidia cron list
elidia cron pause <job_id_or_name>
elidia cron resume <job_id_or_name>
elidia cron run <job_id_or_name>
elidia cron remove <job_id_or_name>
elidia cron edit <job_id_or_name> [...flags]
elidia cron status
elidia cron tick
```
...
```bash
elidia gateway install     # Install as a user service
sudo elidia gateway install --system   # Linux: boot-time system service for servers
elidia gateway             # Or run in foreground
...
1. loads jobs from `~/.elidia/cron/jobs.json`
...
A file lock at `~/.elidia/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.elidia/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.elidia/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ELIDIA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.elidia/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Elidia directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Elidia will write the check script to `~/.elidia/scripts/` via `write_file`, then call:
...
See the [Script-Only Cron Jobs guide](https://aiutils.io/elidia/guides/cron-script-only) for worked examples.
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.elidia/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.elidia/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.elidia/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.elidia/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Elidia reads Job 1's most recent output from `~/.elidia/cron/output/{job1_id}/*.md`
...
- **Rotate to the next credential** in your [credential pool](configuration.md) for the same provider

This means cron jobs that run at high frequency or during peak hours are more resilient — a single rate-limited key won't fail the entire run.

## Schedule formats

The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Elidia skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `elidia tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `elidia tools` cron-platform config wins; otherwise Elidia falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Elidia should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.elidia/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.elidia/scripts/.feed-changed.last"
...
> [!TIP]
> Elidia's own `~/.elidia/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.


Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/Elidia-Technology/elidia-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.elidia/cron/jobs.json`. Output from job runs is saved to `~/.elidia/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Elidia resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
