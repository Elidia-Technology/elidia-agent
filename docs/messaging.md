...
Chat with Elidia from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](https://aiutils.io/elidia/user-guide/features/voice-mode) and [Use Voice Mode with Elidia](https://aiutils.io/elidia/guides/use-voice-mode-with-elidia).

> [!TIP]
> Bots need both a model provider and tool providers (TTS, web). A [Elidia Portal](elidia-portal.md) subscription bundles all of them.
...
```mermaid
flowchart TB
    subgraph Gateway["Elidia Gateway"]
...
```bash
elidia gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
elidia gateway              # Run in foreground
elidia gateway setup        # Configure messaging platforms interactively
elidia gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo elidia gateway install --system   # Linux only: install a boot-time system service
elidia gateway start        # Start the default service
elidia gateway stop         # Stop the default service
elidia gateway status       # Check default service status
elidia gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Elidia Agent to the latest version |
...
Configure per-platform overrides in `~/.elidia/gateway.json`:
...
elidia pairing approve telegram XKGH5N7P

# Other pairing commands:
elidia pairing list          # View pending + approved users
elidia pairing revoke telegram 123456789  # Remove access
```
...
Use `/whoami` from any platform to see the active scope, your tier (admin / user / unrestricted), and which slash commands you can run. See the [Telegram](https://aiutils.io/elidia/user-guide/messaging/telegram) and [Discord](https://aiutils.io/elidia/user-guide/messaging/discord) pages for platform-specific examples.
...
The first time you message a busy agent on any platform, Elidia appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.elidia/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.elidia/config.yaml`:
...
```bash
ELIDIA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
elidia gateway install               # Install as user service
elidia gateway start                 # Start the service
elidia gateway stop                  # Stop the service
elidia gateway status                # Check status
journalctl --user -u elidia-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo elidia gateway install --system
sudo elidia gateway start --system
sudo elidia gateway status --system
journalctl -u elidia-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Elidia will warn if it detects both because start/stop/status behavior gets ambiguous.

> [!NOTE] Multiple installations
> If you run multiple Elidia installations on the same machine (with different `ELIDIA_HOME` directories), each gets its own systemd service name. The default `~/.elidia` uses `elidia-gateway`; other installations use `elidia-gateway-<hash>`. The `elidia gateway` commands automatically target the correct service for your current `ELIDIA_HOME`.


### macOS (launchd)

```bash
elidia gateway install               # Install as launchd agent
elidia gateway start                 # Start the service
elidia gateway stop                  # Stop the service
elidia gateway status                # Check status
tail -f ~/.elidia/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.elidia.gateway.plist`. It includes three environment variables:
...
- **ELIDIA_HOME** — scopes the gateway to your Elidia installation.

> [!TIP] PATH changes after install
> launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `elidia gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.


> [!NOTE] Multiple installations
> Like the Linux systemd service, each `ELIDIA_HOME` directory gets its own launchd label. The default `~/.elidia` uses `ai.elidia.gateway`; other installations use `ai.elidia.gateway-<suffix>`.
...
|----------|---------|--------------|
| CLI | `elidia-cli` | Full access |
| Telegram | `elidia-telegram` | Full tools including terminal |
| Discord | `elidia-discord` | Full tools including terminal |
| WhatsApp | `elidia-whatsapp` | Full tools including terminal |
| Slack | `elidia-slack` | Full tools including terminal |
| Google Chat | `elidia-google_chat` | Full tools including terminal |
| Signal | `elidia-signal` | Full tools including terminal |
| SMS | `elidia-sms` | Full tools including terminal |
| Email | `elidia-email` | Full tools including terminal |
| Home Assistant | `elidia-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `elidia-mattermost` | Full tools including terminal |
| Matrix | `elidia-matrix` | Full tools including terminal |
| DingTalk | `elidia-dingtalk` | Full tools including terminal |
| Feishu/Lark | `elidia-feishu` | Full tools including terminal |
| WeCom | `elidia-wecom` | Full tools including terminal |
| WeCom Callback | `elidia-wecom-callback` | Full tools including terminal |
| Weixin | `elidia-weixin` | Full tools including terminal |
| BlueBubbles | `elidia-bluebubbles` | Full tools including terminal |
| QQBot | `elidia-qqbot` | Full tools including terminal |
| Yuanbao | `elidia-yuanbao` | Full tools including terminal |
| Microsoft Teams | `elidia-teams` | Full tools including terminal |
| API Server | `elidia-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `elidia-webhook` | Full tools including terminal |
...
See also the broader status summary command [`/platforms`](https://aiutils.io/elidia/reference/slash-commands).
...
1. **Gateway log** (`~/.elidia/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
...
- [Telegram Setup](https://aiutils.io/elidia/user-guide/messaging/telegram)
- [Discord Setup](https://aiutils.io/elidia/user-guide/messaging/discord)
- [Slack Setup](https://aiutils.io/elidia/user-guide/messaging/slack)
- [Google Chat Setup](https://aiutils.io/elidia/user-guide/messaging/google_chat)
- [WhatsApp Setup](https://aiutils.io/elidia/user-guide/messaging/whatsapp)
- [Signal Setup](https://aiutils.io/elidia/user-guide/messaging/signal)
- [SMS Setup (Twilio)](https://aiutils.io/elidia/user-guide/messaging/sms)
- [Email Setup](https://aiutils.io/elidia/user-guide/messaging/email)
- [Home Assistant Integration](https://aiutils.io/elidia/user-guide/messaging/homeassistant)
- [Mattermost Setup](https://aiutils.io/elidia/user-guide/messaging/mattermost)
- [Matrix Setup](https://aiutils.io/elidia/user-guide/messaging/matrix)
- [DingTalk Setup](https://aiutils.io/elidia/user-guide/messaging/dingtalk)
- [Feishu/Lark Setup](https://aiutils.io/elidia/user-guide/messaging/feishu)
- [WeCom Setup](https://aiutils.io/elidia/user-guide/messaging/wecom)
- [WeCom Callback Setup](https://aiutils.io/elidia/user-guide/messaging/wecom-callback)
- [Weixin Setup (WeChat)](https://aiutils.io/elidia/user-guide/messaging/weixin)
- [BlueBubbles Setup (iMessage)](https://aiutils.io/elidia/user-guide/messaging/bluebubbles)
- [QQBot Setup](https://aiutils.io/elidia/user-guide/messaging/qqbot)
- [Yuanbao Setup](https://aiutils.io/elidia/user-guide/messaging/yuanbao)
- [Microsoft Teams Setup](https://aiutils.io/elidia/user-guide/messaging/teams)
- [Teams Meetings Pipeline](https://aiutils.io/elidia/user-guide/messaging/teams-meetings)
- [Open WebUI + API Server](https://aiutils.io/elidia/user-guide/messaging/open-webui)
- [Webhooks](https://aiutils.io/elidia/user-guide/messaging/webhooks)
