...
Elidia Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

> [!TIP] First-time setup
> One command — `elidia setup --portal` — and you're ready to `elidia chat`. See [Elidia Portal](elidia-portal.md).


> [!TIP]
> Elidia also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `elidia --tui` — see the [TUI](https://aiutils.io/elidia/user-guide/tui) guide.
...
elidia chat --model "anthropic/claude-sonnet-4"

# With a specific provider
elidia chat --provider elidia        # Use Elidia Portal
elidia chat --provider openrouter  # Force OpenRouter

# With specific toolsets
elidia chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
elidia -s elidia-agent-dev,github-auth
elidia chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
elidia --continue             # Resume the most recent CLI session (-c)
elidia --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
elidia chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
elidia -w                         # Interactive mode in worktree
elidia -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Elidia CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Elidia CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ELIDIA_YOLO_MODE` is on (either `elidia --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`elidia -c` or `elidia --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](https://aiutils.io/elidia/user-guide/sessions) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Elidia to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Elidia supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Elidia replies |
...
For the full built-in CLI and messaging lists, see [Slash Commands Reference](https://aiutils.io/elidia/reference/slash-commands).

For setup, providers, silence tuning, and messaging/Discord voice usage, see [Voice Mode](https://aiutils.io/elidia/user-guide/features/voice-mode).
...
Then type `/status`, `/gpu`, or `/restart` in any chat. See the [Configuration guide](configuration.md) for more examples.
...
```bash
elidia -s elidia-agent-dev,github-auth
elidia chat -s github-pr-workflow -s github-auth
```

Elidia loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.elidia/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.elidia/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Elidia..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Elidia — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
> The very first time you press Enter while Elidia is working, Elidia prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.


### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Elidia to the background — just like any terminal process. The shell prints a confirmation:

```
Elidia Agent has been suspended. Run `fg` to bring Elidia Agent back.
```
...
Cycle through display modes with `/verbose`: `off → new → all → verbose`. This command can also be enabled for messaging platforms — see [configuration](configuration.md).
...
Resume this session with:
  elidia --resume 20260225_143052_a1b2c3
...
```bash
elidia --continue                          # Resume the most recent CLI session
elidia -c                                  # Short form
elidia -c "my project"                     # Resume a named session (latest in lineage)
elidia --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
elidia --resume "refactoring auth"         # Resume by title
elidia -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `elidia sessions rename <id> <title>` from the command line. Use `elidia sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Elidia's SQLite state database under `~/.elidia/state.db`. The database keeps:
...
Elidia immediately confirms the task and gives you back the prompt:
