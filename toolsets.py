...
_ELIDIA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ELIDIA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `elidia tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ELIDIA_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Elidia toolsets (CLI + messaging platforms)
    #
    ...
    "elidia-cli": {
        ...
        "tools": _ELIDIA_CORE_TOOLS,
        "includes": []
    },

    "elidia-cron": {
        # Mirrors elidia-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `elidia tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as elidia-cli; gated by `elidia tools`",
        "tools": _ELIDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "elidia-email": {
        "description": "Email bot toolset - interact with Elidia via email (IMAP/SMTP)",
        "tools": _ELIDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "elidia-sms": {
        "description": "SMS bot toolset - interact with Elidia via SMS (Twilio)",
        "tools": _ELIDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "elidia-webhook": {
        ...
        "tools": _ELIDIA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "elidia-gateway": {
        ...
        "tools": [],
        "includes": ["elidia-telegram", "elidia-discord", "elidia-whatsapp", "elidia-slack", "elidia-signal", "elidia-bluebubbles", "elidia-homeassistant", "elidia-email", "elidia-sms", "elidia-mattermost", "elidia-matrix", "elidia-dingtalk", "elidia-feishu", "elidia-wecom", "elidia-wecom-callback", "elidia-weixin", "elidia-qqbot", "elidia-webhook", "elidia-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (elidia-<name>).
        # Gives them _ELIDIA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("elidia-"):
            platform_name = name[len("elidia-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ELIDIA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
