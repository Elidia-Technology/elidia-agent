...
React + Ink terminal UI for Elidia. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ELIDIA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.elidia/.elidia_history` or under `ELIDIA_HOME`.
...
```text
ui-tui/
  packages/elidia-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      elidia-ink.d.ts    type declarations for @elidia/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ElidiaCLI subprocess for slash commands
```
