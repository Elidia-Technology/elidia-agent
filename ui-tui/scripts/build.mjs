...
await build({
  ...
  // Skip the prebuilt @elidia/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@elidia/ink': resolve(root, 'packages/elidia-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The elidia_cli launcher
