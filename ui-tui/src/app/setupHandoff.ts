import type { RunExternalProcess } from '@elidia/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`elidia ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching elidia: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`elidia ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
