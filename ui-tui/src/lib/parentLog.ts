...
const logDir = join(process.env.ELIDIA_HOME?.trim() || join(homedir(), '.elidia'), 'logs')
...
// a real ~/.elidia (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('elidia-tui: parent lifecycle log unavailable\n')
    }
  }
}
