import { Box, Text, useStdout } from '@elidia/ink'
import { useEffect, useState } from 'react'
import unicodeSpinners from 'unicode-animations'

import { artWidth, elidiaMark, ELIDIA_MARK_WIDTH, logo, LOGO_WIDTH } from '../banner.js'
...
const TAG_FULL = 'AiUtils · Messenger of the Digital Gods'
const TAG_MID = 'Messenger of the Digital Gods'
const TAG_TINY = 'AiUtils'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const heroLines = elidiaMark(t.color, t.bannerHero || undefined)
  const leftW = Math.min((artWidth(heroLines) || ELIDIA_MARK_WIDTH) + 4, Math.floor(cols * 0.4))
      ...
      {wide && (
        <Box flexDirection="column" marginRight={2} width={leftW}>
          ...
          <Text color={t.color.accent}>
            ...
            <Text color={t.color.muted}> · AiUtils</Text>
          </Text>
      ...
      <Box flexDirection="column" width={w}>
        ...
        {typeof info.update_behind === 'number' && info.update_behind > 0 && (
          <Text bold color={t.color.warn}>
            ...
            <Text bold color={t.color.warn}>
              {info.update_command || 'elidia update'}
            </Text>
            <Text bold={false} color={t.color.warn} dimColor>
              {' '}
              to update
