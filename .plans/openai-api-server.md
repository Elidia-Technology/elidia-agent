# OpenAI-Compatible API Server for Elidia Agent
...
SSE streaming. By exposing this endpoint, elidia-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  elidia-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.elidia/.env`
2. Run `elidia gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with elidia-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns elidia-agent as a model) |
...
Authorization: Bearer elidia-api-key-here
...
But elidia-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "elidia-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured elidia-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of elidia-agent's core prompt
...
Once implemented, elidia-agent works as a drop-in backend for:
