...
# 5. Symlinks the 'elidia' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}✦ Elidia Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/elidia-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink elidia into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up elidia command..."

ELIDIA_BIN="$SCRIPT_DIR/venv/bin/elidia"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ELIDIA_BIN" "$COMMAND_LINK_DIR/elidia"
echo -e "${GREEN}✓${NC} Symlinked elidia → $COMMAND_LINK_DISPLAY_DIR/elidia"
...
# ============================================================================
# Seed bundled skills into ~/.elidia/skills/
# ============================================================================

ELIDIA_SKILLS_DIR="${ELIDIA_HOME:-$HOME/.elidia}/skills"
mkdir -p "$ELIDIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.elidia/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ELIDIA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  elidia status        # Check configuration"
if is_termux; then
    echo "  elidia gateway       # Run gateway in foreground"
else
    echo "  elidia gateway install # Install gateway service (messaging + cron)"
fi
echo "  elidia cron list     # View scheduled jobs"
echo "  elidia doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m elidia_cli.main setup
fi
