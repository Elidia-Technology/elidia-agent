...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      elidia-agent = self'.packages.default;
      elidiaVenv = elidia-agent.elidiaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "elidia-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${elidiaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "elidia-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "elidia-build-package" { } ''
          echo "PASS: package built at ${elidia-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "elidia-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "elidia-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${elidia-agent}/bin/elidia || (echo "FAIL: elidia binary missing"; exit 1)
          test -x ${elidia-agent}/bin/elidia-agent || (echo "FAIL: elidia-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${elidia-agent}/bin/elidia version 2>&1 | grep -qi "elidia" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "elidia-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in elidia elidia-agent elidia-acp; do
            test -x ${elidia-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "elidia-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking elidia --help ==="
          ${elidia-agent}/bin/elidia --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${elidia-agent}/bin/elidia --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "elidia-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${elidia-agent}/share/elidia-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${elidia-agent}/share/elidia-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ELIDIA_BUNDLED_SKILLS" ${elidia-agent}/bin/elidia || \
            (echo "FAIL: ELIDIA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ELIDIA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "elidia-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${elidia-agent}/share/elidia-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${elidia-agent}/share/elidia-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ELIDIA_BUNDLED_PLUGINS" ${elidia-agent}/bin/elidia || \
            (echo "FAIL: ELIDIA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ELIDIA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "elidia-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${elidia-agent}/share/elidia-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${elidia-agent}/share/elidia-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${elidia-agent}/share/elidia-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ELIDIA_BUNDLED_LOCALES" ${elidia-agent}/bin/elidia || \
            (echo "FAIL: ELIDIA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ELIDIA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ELIDIA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ELIDIA_BUNDLED_LOCALES=${elidia-agent}/share/elidia-agent/locales \
            ${elidiaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ELIDIA_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${elidiaVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${elidiaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ELIDIA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "elidia-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${elidia-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${elidia-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ELIDIA_TUI_DIR" ${elidia-agent}/bin/elidia || \
            (echo "FAIL: ELIDIA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ELIDIA_TUI_DIR set in wrapper"
        ...
        # Verify ELIDIA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        elidia-node = pkgs.runCommand "elidia-node-version" { } ''
          set -e
          echo "=== Checking ELIDIA_NODE in wrapper ==="
          grep -q "ELIDIA_NODE" ${elidia-agent}/bin/elidia || \
            (echo "FAIL: ELIDIA_NODE not set in wrapper"; exit 1)
          echo "PASS: ELIDIA_NODE present in wrapper"

          ELIDIA_NODE=$(sed -n "s/^export ELIDIA_NODE='\(.*\)'/\1/p" ${elidia-agent}/bin/elidia)
          test -x "$ELIDIA_NODE" || (echo "FAIL: ELIDIA_NODE=$ELIDIA_NODE not executable"; exit 1)
          echo "PASS: ELIDIA_NODE executable at $ELIDIA_NODE"

          NODE_MAJOR=$("$ELIDIA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ELIDIA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ELIDIA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "elidia-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ELIDIA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ELIDIA_MANAGED guards ==="
          check_blocked "config set" ${elidia-agent}/bin/elidia config set model foo
          check_blocked "config edit" ${elidia-agent}/bin/elidia config edit
        ...
        extra-python-packages = let
          ...
          elidiaWithExtra = elidia-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "elidia-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${elidiaWithExtra}/bin/elidia || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${elidiaWithExtra}/bin/elidia || \
          ...
          if grep -q "PYTHONPATH" ${elidia-agent}/bin/elidia; then
        ...
        extra-dependency-groups = let
          elidiaWithGroups = elidia-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "elidia-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${elidiaWithGroups}"
          echo "venv: ${elidiaWithGroups.elidiaVenv}"
        ...
        messaging-variant = pkgs.runCommand "elidia-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.elidiaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "elidia-config-roundtrip" {
          ...
          merge_and_load() {
            local elidia_home="$1"
            export ELIDIA_HOME="$elidia_home"
            ${configMergeScript} ${nixSettings} "$elidia_home/config.yaml"
            ${elidiaVenv}/bin/python3 -c '
import json, sys
