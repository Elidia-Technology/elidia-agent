# nix/desktop.nix — Elidia Desktop (Electron) app build + wrapper
#
# `elidiaAgent` is the fully-built `.#default` package — it ships the
# `elidia` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ELIDIA_DESKTOP_ELIDIA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Elidia CLI").
# No reimplementation of the agent resolution in this wrapper.
{ pkgs, lib, stdenv, makeWrapper, elidiaNpmLib, electron, elidiaAgent, ... }:
let
  npm = elidiaNpmLib.mkNpmPassthru { folder = "apps/desktop"; attr = "desktop"; pname = "elidia-desktop"; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/elidia-desktop $out/bin
    cp -r ${renderer}/* $out/share/elidia-desktop/

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ELIDIA_DESKTOP_ELIDIA to the absolute path of the nix-built `elidia`
    # binary so the desktop's resolver step 4 ("existing Elidia CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/elidia-desktop \
      --add-flags "$out/share/elidia-desktop" \
      --set ELIDIA_DESKTOP_ELIDIA "${lib.getExe elidiaAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Elidia Agent";
    homepage = "https://github.com/Elidia-Technology/elidia-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "elidia-desktop";
  };
}
