# nix/nixosModules.nix — NixOS module for elidia-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: elidia runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ELIDIA_HOME/.env
# and read by elidia at startup — no container recreation needed for env changes.
#
# Tool resolution: the elidia wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.elidia-agent;
    effectivePackage =
    ...
    elidia-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-elidia-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "elidia-config-attrs";
      description = "Elidia YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "elidia-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "elidia-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ELIDIA_DOC_EOF'\n${value}\nELIDIA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "elidia-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/elidia";
    ...
    # elidia user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "elidia-container-entrypoint" ''
      set -eu

      ELIDIA_UID="''${ELIDIA_UID:?ELIDIA_UID must be set}"
      ELIDIA_GID="''${ELIDIA_GID:?ELIDIA_GID must be set}"

      # ── Group: ensure a group with GID=$ELIDIA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ELIDIA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="elidia"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ELIDIA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ELIDIA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ELIDIA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ELIDIA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="elidia"
        TARGET_HOME="/home/elidia"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ELIDIA_UID" -g "$ELIDIA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ELIDIA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ELIDIA_UID:$ELIDIA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ELIDIA_HOME is owned by the target user.
      ...
      if [ -n "''${ELIDIA_HOME:-}" ] && [ -d "$ELIDIA_HOME" ]; then
        find "$ELIDIA_HOME" \! -user "$ELIDIA_UID" -exec chown "$ELIDIA_UID:$ELIDIA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/elidia-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/elidia-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/elidia ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/elidia
        chmod 0440 /etc/sudoers.d/elidia
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ELIDIA_UID" --regid="$ELIDIA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ELIDIA_HOME/.env.
    ...
    # Default: /var/lib/elidia/workspace → /data/workspace.
  ...
  in {
    options.services.elidia-agent = with lib; {
      enable = mkEnableOption "Elidia Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = elidia-agent;
        description = "The elidia-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/elidia";
        description = "State directory. Contains .elidia/ subdir (ELIDIA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Elidia config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ELIDIA_HOME/.env at activation time.
          Elidia reads this file on every startup via load_elidia_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ELIDIA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ELIDIA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `elidia gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the elidia user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the elidia plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Elidia discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          elidia_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as elidia (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in elidia-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the elidia CLI to environment.systemPackages and export
          ELIDIA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.elidia symlink to the service
            stateDir. These users are automatically added to the elidia group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.elidia-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the elidia CLI to system PATH and export ELIDIA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.elidia/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ELIDIA_HOME = "${cfg.stateDir}/.elidia";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.elidia-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the elidia user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.elidia-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed elidia
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure elidia is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.elidia        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elidia/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elidia/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elidia/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elidia/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elidia/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."elidia-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.elidia
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.elidia ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.elidia ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.elidia -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.elidia/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.elidia/$_subdir"
            chmod 2770 "${cfg.stateDir}/.elidia/$_subdir"
            find "${cfg.stateDir}/.elidia/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.elidia/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.elidia/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elidia/config.yaml
            chmod 0640 ${cfg.stateDir}/.elidia/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.elidia/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elidia/.managed
          chmod 0644 ${cfg.stateDir}/.elidia/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.elidia/.container-mode <<'ELIDIA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    elidia_bin=${containerDataDir}/current-package/bin/elidia
    ELIDIA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elidia/.container-mode
            chmod 0644 ${cfg.stateDir}/.elidia/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.elidia/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.elidia";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.elidia" ]; then
                  rm -f "${symlinkPath}"
                  echo "elidia-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.elidia -> stateDir/.elidia for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.elidia";
                target = "${cfg.stateDir}/.elidia";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "elidia-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.elidia/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.elidia/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.elidia/auth.json
              fi
            ''}
          ''}
          ...
          # Elidia reads $ELIDIA_HOME/.env at startup via load_elidia_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.elidia/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ELIDIA_NIX_ENV_EOF'
    ${envFileContent}
    ELIDIA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.elidia/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.elidia/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elidia/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.elidia-agent = {
          description = "Elidia Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            ELIDIA_HOME = "${cfg.stateDir}/.elidia";
            ELIDIA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ELIDIA_HOME/.env by the activation script. load_elidia_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/elidia"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the elidia group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.elidia-agent = {
          description = "Elidia Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ELIDIA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ELIDIA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ELIDIA_UID="$ELIDIA_UID" \
                --env ELIDIA_GID="$ELIDIA_GID" \
                --env ELIDIA_HOME=${containerDataDir}/.elidia \
                --env ELIDIA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/elidia gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
