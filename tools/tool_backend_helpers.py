...
def managed_elidia_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Elidia Tool Gateway.

    Entitlement is paid Elidia Portal service access OR a live free tool pool
...
def elidia_tool_gateway_unavailable_message(
    capability: str = "the Elidia Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Elidia Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `elidia model` to refresh your "
        "Elidia Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_elidia_tools_enabled()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.elidia/.env`` (via
    ``elidia_cli.config.get_env_value`` when available) so tool-side
