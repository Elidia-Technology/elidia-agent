...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ELIDIA_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ELIDIA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ELIDIA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ELIDIA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ELIDIA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ELIDIA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ELIDIA_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ELIDIA_CRON_SESSION"):
        return False
    if env_var_enabled("ELIDIA_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ELIDIA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ELIDIA_ENV_PATH = (
    r'(?:~\/\.elidia/|'
    r'(?:\$home|\$\{home\})/\.elidia/|'
    r'(?:\$elidia_home|\$\{elidia_home\})/)'
    r'\.env\b'
)
# ~/.elidia/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ELIDIA_ENV_PATH; matches the ELIDIA_HOME override form as
# well as ~/.elidia/.
_ELIDIA_CONFIG_PATH = (
    r'(?:~\/\.elidia/|'
    r'(?:\$home|\$\{home\})/\.elidia/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\belidia\s+gateway\s+(stop|restart)\b', "stop/restart elidia gateway (kills running agents)"),
    (r'\belidia\s+update\b', "elidia update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `elidia gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart elidia-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart elidia-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(elidia|gateway|cli\.py)\b', "kill elidia/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill elidia` but not
    # `kill -9 $(pgrep -f elidia)` because the substitution is opaque
    ...
    # In-place edit of an Elidia-managed security file (~/.elidia/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ELIDIA_CONFIG_PATH}|{_ELIDIA_ENV_PATH})', "in-place edit of Elidia config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ELIDIA_CONFIG_PATH}|{_ELIDIA_ENV_PATH})', "in-place edit of Elidia config/env (long flag)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ELIDIA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ELIDIA_INTERACTIVE or ELIDIA_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ELIDIA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ELIDIA_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ELIDIA_EXEC_ASK")
