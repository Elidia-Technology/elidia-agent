...
supports multiple backends — **Browser Use** (cloud, default for Elidia
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    elidia_node_bin = str(elidia_home / "node" / "bin")
    elidia_node_root = str(elidia_home / "node")
    elidia_nm_bin = str(elidia_home / "node_modules" / ".bin")
    return [elidia_node_bin, elidia_node_root, elidia_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.elidia/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Elidia gateway or
    ...
    browser plugins (``~/.elidia/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed Elidia gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Elidia silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Elidia CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-elidia_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed elidia processes — this way every clean elidia exit sweeps
    ...
    # Sweep orphans from other crashed elidia processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live elidia processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current elidia PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live elidia process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning elidia process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced elidia PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent elidia instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-elidia_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live elidia process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Elidia-managed
    ...
    # Build an extended search PATH including Elidia-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                elidia_nm = str(get_elidia_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=elidia_nm)
            if not recheck:
                elidia_node_bin = str(get_elidia_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=elidia_node_bin)
            if not recheck:
                elidia_node_root = str(get_elidia_home() / "node")
                recheck = shutil.which("agent-browser", path=elidia_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this elidia PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = elidia_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Elidia
    ...
    from elidia_constants import get_elidia_dir
    screenshots_dir = get_elidia_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/aiutils/elidia-agent:latest")
                else:
