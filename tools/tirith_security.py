...
it is automatically downloaded from GitHub releases to $ELIDIA_HOME/bin/tirith.
...
def _get_elidia_home() -> str:
    """Return the Elidia home directory, respecting ELIDIA_HOME env var."""
    return str(get_elidia_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_elidia_home(), ".tirith-install-failed")
...
def _elidia_bin_dir() -> str:
    """Return $ELIDIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_elidia_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ELIDIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_elidia_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ELIDIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ELIDIA_HOME/bin/tirith
    ...
    elidia_bin = os.path.join(_elidia_bin_dir(), "tirith")
    if os.path.isfile(elidia_bin) and os.access(elidia_bin, os.X_OK):
        _resolved_path = elidia_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return elidia_bin
