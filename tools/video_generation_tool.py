...
provider** — turn on a backend by enabling a plugin (``elidia plugins
enable video_gen/<name>``) and selecting it in ``elidia tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/elidia-agent-dev/references/dynamic-tool-schemas.md.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `elidia tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `elidia tools` → "
...
# `elidia tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`elidia tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `elidia tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
