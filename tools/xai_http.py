...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``elidia tools`` repaint, tool-registration scans,
    ...
    2. ``~/.elidia/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_elidia_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.elidia/.env`` first, then ``os.environ``.

    Wraps :func:`elidia_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from elidia_cli.config import get_env_value as _elidia_get_env_value

        value = _elidia_get_env_value(name)
...
def elidia_xai_user_agent() -> str:
    """Return a stable Elidia-specific User-Agent for xAI HTTP calls."""
    try:
        from elidia_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Elidia-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Elidia-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``elidia_cli.config.get_env_value`` so keys
    stored in ``~/.elidia/.env`` (the standard Elidia location) are honored —
