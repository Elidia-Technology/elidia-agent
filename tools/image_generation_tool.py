...
selectable via ``elidia tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Elidia Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Elidia
    ...
    # existing FAL key / Elidia Portal subscription rather than register
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Elidia Subscription)
# ---------------------------------------------------------------------------
...
def _get_managed_fal_client(managed_gateway):
    ...
    client_config = (
        ...
        managed_gateway.elidia_user_token,
    )
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.elidia_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Elidia Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Elidia Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `elidia tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Elidia tools are enabled)
    ...
    if managed_elidia_tools_enabled():
        ...
        gateway_message = elidia_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_elidia_tools_enabled():
        lines.append(
            "  2. Sign in to an Elidia account that has the managed FAL "
            "gateway enabled (`elidia setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `elidia tools` "
        "→ Image Generation (run `elidia plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping elidia with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `elidia plugins list` to see "
