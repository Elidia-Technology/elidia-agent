...
Lets the LLM write a Python script that calls Elidia tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `elidia_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `elidia_tools.py` with file-based RPC stubs
...
# available on every platform Elidia itself runs on.
...
# match a safe prefix, the operational ELIDIA_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ELIDIA_" prefix was deliberately removed (#27303) — it leaked
# ELIDIA_*-named config that lacks a secret substring (e.g. ELIDIA_BASE_URL,
# ELIDIA_KANBAN_DB, ELIDIA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ELIDIA_CHILD_ALLOWED below; ELIDIA_RPC_SOCKET /
# ELIDIA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ELIDIA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set elidia_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_ELIDIA_CHILD_ALLOWED = frozenset({
    "ELIDIA_HOME",
    "ELIDIA_PROFILE",
    "ELIDIA_CONFIG",
    "ELIDIA_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ELIDIA_* vars (_ELIDIA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ELIDIA_* vars dropped by the tightened allowlist (#27303). The
    # broad "ELIDIA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ELIDIA_KANBAN_DB / ELIDIA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ELIDIA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ELIDIA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ELIDIA_* var.
            _dropped_elidia.append(k)
    if _dropped_elidia:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ELIDIA_* var(s) from the "
...
# ---------------------------------------------------------------------------
# elidia_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Elidia-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Elidia-profile soft guard."""',
...
def generate_elidia_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the elidia_tools.py stub module.
...
"""Auto-generated Elidia tools RPC stubs."""
...
def _connect():
    ...
    ELIDIA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ELIDIA_RPC_SOCKET"]
...
"""Auto-generated Elidia tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ELIDIA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "elidia_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated elidia_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/elidia_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_elidia_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/elidia_tools.py", tools_src)
        ...
        env_prefix = (
            f"ELIDIA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ELIDIA_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with elidia_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="elidia_sandbox_")
    ...
    # selector from ELIDIA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"elidia_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated elidia_tools module.
        ...
        tools_src = generate_elidia_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "elidia_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ELIDIA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ELIDIA_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the elidia-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from elidia_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _elidia_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _elidia_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ELIDIA_TIMEZONE is an internal Elidia setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ELIDIA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ELIDIA_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ELIDIA_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.elidia/.env')).
...
def _load_config() -> dict:
    ...
    ``elidia --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (elidia-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``elidia tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and elidia-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.elidia/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Elidia tools programmatically. "
        ...
        f"Available via `from elidia_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into elidia_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from elidia_tools import {import_str}` "
