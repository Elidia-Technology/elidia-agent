...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Elidia exit
...
# display_elidia_home imported lazily at call site (stale-module safety during elidia update)
...
from tools.tool_backend_helpers import (
    ...
    elidia_tool_gateway_unavailable_message,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of elidia directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "elidia-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ELIDIA_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ELIDIA_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ELIDIA_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ELIDIA_INTERACTIVE=1).
    ...
    try:
        os.environ["ELIDIA_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "ELIDIA_SPINNER_PAUSE" in os.environ:
            del os.environ["ELIDIA_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (ELIDIA_INTERACTIVE=1):
    ...
    # interactive Elidia password prompt or the sudo -S password-pipe path.
    ...
    # silently without Elidia prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("ELIDIA_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Elidia can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``elidia-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Elidia processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Elidia processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Elidia processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, ElidiaSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.elidia/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for elidia-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Elidia processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Elidia Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Elidia can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # elidia-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in elidia-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ELIDIA_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ELIDIA_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ELIDIA_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ELIDIA_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ELIDIA_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("ELIDIA_SESSION_MESSAGE_ID", "")
            ...
            try:
                from elidia_cli.plugins import invoke_hook
