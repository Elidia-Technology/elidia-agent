"""Generic managed-tool gateway helpers for Elidia-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "aiutils.io"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_ELIDIA_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    elidia_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the Elidia auth store path, respecting ELIDIA_HOME overrides."""
    return get_elidia_home() / "auth.json"


def _read_elidia_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        elidia_provider = providers.get("elidia", {})
        if isinstance(elidia_provider, dict):
            return elidia_provider
    except Exception:
        pass
    return None
...
def peek_elidia_access_token() -> Optional[str]:
    """Cheap probe for an Elidia gateway token without triggering refresh.

    Availability scans (`elidia tools`, banner/status paint, provider
    ...
    that call :func:`read_elidia_access_token`.
    """
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    elidia_provider = _read_elidia_provider_state() or {}
    access_token = elidia_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_elidia_access_token() -> Optional[str]:
    """Read an Elidia Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    elidia_provider = _read_elidia_provider_state() or {}
    cached_token = peek_elidia_access_token()

    if cached_token and not _access_token_is_expiring(
        elidia_provider.get("expires_at"),
        _ELIDIA_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from elidia_cli.auth import resolve_elidia_access_token

        refreshed_token = resolve_elidia_access_token(
            refresh_skew_seconds=_ELIDIA_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Elidia access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_elidia_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    elidia_user_token = resolved_token_reader()
    if not gateway_origin or not elidia_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        elidia_user_token=elidia_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Elidia token are present.

    Defaults to :func:`peek_elidia_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_elidia_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_elidia_access_token,
    ) is not None
