...
def _is_elidia_provider_credential(name: str) -> bool:
    """True if ``name`` is an Elidia-managed provider credential (API key,
    token, or similar) per ``_ELIDIA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Elidia API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _ELIDIA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Elidia-managed provider credentials (from
    ``_ELIDIA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to an Elidia-managed
    ...
    Non-Elidia third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_elidia_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Elidia provider "
                "credential %r (blocked by _ELIDIA_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Elidia-managed provider credentials must not be passed
                ...
                if _is_elidia_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Elidia "
                        "provider credential %r from config.yaml (blocked "
                        "by _ELIDIA_PROVIDER_ENV_BLOCKLIST). Operator "
