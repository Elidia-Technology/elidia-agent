#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Elidia Skills Hub.
...
Used by elidia_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            elidia_meta = metadata.get("elidia", {})
            if isinstance(elidia_meta, dict):
                tags = elidia_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by AiUtils) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.elidia/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                elidia_meta = meta_block.get("elidia", {})
                if isinstance(elidia_meta, dict):
                    tags = elidia_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Elidia centralized index source
# ---------------------------------------------------------------------------

ELIDIA_INDEX_URL = "https://share.aiutils.io/elidia-hermes/skills-index.json"
ELIDIA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "elidia-index.json"
ELIDIA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_elidia_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on aiutils.io, rebuilt daily by CI.
    We cache it locally for ELIDIA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ELIDIA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ELIDIA_INDEX_CACHE_FILE.stat().st_mtime
            if age < ELIDIA_INDEX_TTL:
                return json.loads(ELIDIA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ELIDIA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Elidia index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Elidia index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ELIDIA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ELIDIA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ElidiaIndexSource(SkillSource):
    """Skill source backed by the centralized Elidia Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_elidia_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "elidia-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "elidia-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ElidiaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
