...
This module provides the glue:
    - ``ElidiaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Elidia Agent"
"""
...
from elidia_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__elidia_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ELIDIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``ELIDIA_HOME/mcp-tokens/``
    """
    try:
        from elidia_constants import get_elidia_home
        base = Path(get_elidia_home())
    except ImportError:
        base = Path(os.environ.get("ELIDIA_HOME", str(Path.home() / ".elidia")))
...
# ---------------------------------------------------------------------------
# ElidiaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ElidiaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ELIDIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        ELIDIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ELIDIA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Elidia records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Elidia.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://aiutils.io/elidia/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `elidia mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Elidia Agent")
    scope = cfg.get("scope")
