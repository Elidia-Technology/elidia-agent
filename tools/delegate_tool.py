...
# Must match elidia_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (elidia-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("elidia-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``elidia-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Elidia Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. elidia-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Elidia Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.elidia/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, elidia, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'elidia auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (elidia_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Elidia subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
