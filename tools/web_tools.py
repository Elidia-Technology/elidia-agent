...
Backend is selected during ``elidia tools`` setup (web.backend in config.yaml).
When available, Elidia can route Firecrawl calls through an Elidia-hosted tool-gateway
for Elidia Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Elidia Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_elidia_access_token as _peek_elidia_access_token,
    read_elidia_access_token as _read_elidia_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.elidia/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``elidia tools``).
    ...
    # tool gateway is configured for Elidia subscribers.
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `elidia tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_elidia_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Elidia portal on every CLI startup (invoked at
...
def _is_elidia_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Elidia Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "aiutils.io" or host.endswith(".aiutils.io")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_elidia_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.portal_tags import elidia_portal_tags
        extra_body = get_auxiliary_extra_body() or {"tags": elidia_portal_tags()}
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `elidia tools` to set one up."
                ),
            }
        else:
            logger.info(
...
if __name__ == "__main__":
    ...
    elidia_available = check_auxiliary_model()
    ...
    if not elidia_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Elidia Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
