...
_elidia_config_resolved: str | None = None
_elidia_config_resolved_loaded = False


def _get_elidia_config_resolved() -> str | None:
    """Return the resolved absolute path of the Elidia config file (cached)."""
    global _elidia_config_resolved, _elidia_config_resolved_loaded
    if _elidia_config_resolved_loaded:
        return _elidia_config_resolved
    _elidia_config_resolved_loaded = True
    try:
        from elidia_cli.config import get_config_path
        _elidia_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _elidia_config_resolved = str(Path("~/.elidia/config.yaml").expanduser().resolve())
        except Exception:
            _elidia_config_resolved = None
    return _elidia_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Elidia config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    elidia_config = _get_elidia_config_resolved()
    if elidia_config and (resolved == elidia_config or normalized == elidia_config):
        return (
            f"Refusing to write to Elidia config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.elidia/config.yaml directly or use 'elidia config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Elidia mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Elidia
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Elidia state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.elidia/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Elidia process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.elidia/…`` path.

    Returns ``None`` when the write is in-scope or outside Elidia scope.
    ...
    # in a session that cd'd into ``~/.elidia/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Elidia internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ELIDIA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ELIDIA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Elidia-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Elidia-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Elidia profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Elidia profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with elidia_tools.write_file() for very "
            "large files."
        )
