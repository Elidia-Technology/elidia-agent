...
Elidia, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ELIDIA_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``elidia``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "elidia", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_elidia_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ELIDIA_HOME/cache/vision/ and cleaned up
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → elidia).
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Elidia, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_elidia_dir("cache/video", "temp_video_files")
