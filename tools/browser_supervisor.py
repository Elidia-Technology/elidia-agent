...
One ``CDPSupervisor`` runs per Elidia ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "elidia-dialog-bridge.invalid"
...
(() => {
  if (window.__elidiaDialogBridgeInstalled) return;
  window.__elidiaDialogBridgeInstalled = true;
  const ENDPOINT = "http://elidia-dialog-bridge.invalid/";
