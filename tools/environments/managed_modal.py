...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with Elidia-compatible execute/cleanup."""
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and Elidia user token")

        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._elidia_user_token = gateway.elidia_user_token
    ...
    def _request(self, method: str, path: str, *,
        ...
        headers = {
            "Authorization": f"Bearer {self._elidia_user_token}",
