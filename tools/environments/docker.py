...
from tools.environments.local import _ELIDIA_PROVIDER_ENV_BLOCKLIST
...
def _load_elidia_env_vars() -> dict[str, str]:
    """Load ~/.elidia/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Elidia profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale elidia-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=elidia-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Elidia process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=elidia-profile=<profile_filter>`` (sweep only the
      caller's profile by default; an elidia process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Elidia
    process leaves its container behind permanently because there's no
    subsequent Elidia process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=elidia-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=elidia-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ELIDIA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ELIDIA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ELIDIA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'elidia'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``elidia-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.elidia/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. elidia-agent:latest) already use /init as PID 1
        ...
        container_name = f"elidia-{uuid.uuid4().hex[:8]}"
        # Labels make elidia-created containers identifiable to:
        #   * the orphan reaper (`elidia-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`elidia-task-id`, `elidia-profile`)
        #   * operators running `docker ps --filter label=elidia-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Elidia profile is captured at
        ...
        label_args = [
            "--label", "elidia-agent=1",
            "--label", f"elidia-task-id={task_label}",
            "--label", f"elidia-profile={profile_name}",
        ]
        self._labels = {
            "elidia-agent": "1",
            "elidia-task-id": task_label,
            "elidia-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Elidia process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Elidia secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ELIDIA_PROVIDER_ENV_BLOCKLIST)
        elidia_env = _load_elidia_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = elidia_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``elidia-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=elidia-agent=1",
                    "--filter", f"label=elidia-task-id={task_label}",
                    "--filter", f"label=elidia-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Elidia process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Elidia exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Elidia process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Elidia startup. That covers the
        ...
        # outlives Elidia processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"elidia-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``elidia /quit``
