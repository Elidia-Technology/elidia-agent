...
#
# Wired into the image as /etc/cont-init.d/01-elidia-setup by the
...
ELIDIA_HOME="${ELIDIA_HOME:-/opt/data}"
INSTALL_DIR="/opt/elidia"

# Drop to elidia via s6-setuidgid, but skip it when already non-root.
as_elidia() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid elidia "$@"; }

# --- Bootstrap ELIDIA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid elidia mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ELIDIA_HOME paths whose parents only
# root can create (e.g. `ELIDIA_HOME=/home/elidia/.elidia` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ELIDIA_UID/ELIDIA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ELIDIA_UID/
# ELIDIA_GID still win when both are set.  See #15290, salvages #25872.
ELIDIA_UID="${ELIDIA_UID:-${PUID:-}}"
ELIDIA_GID="${ELIDIA_GID:-${PGID:-}}"

if [ -n "${ELIDIA_UID:-}" ] && validate_uid_gid "$ELIDIA_UID" && [ "$ELIDIA_UID" != "$(id -u elidia)" ]; then
    echo "[stage2] Changing elidia UID to $ELIDIA_UID"
    usermod -u "$ELIDIA_UID" elidia
fi
if [ -n "${ELIDIA_GID:-}" ] && validate_uid_gid "$ELIDIA_GID" && [ "$ELIDIA_GID" != "$(id -g elidia)" ]; then
    echo "[stage2] Changing elidia GID to $ELIDIA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ELIDIA_GID" elidia 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised elidia user (UID 10000)
...
# elidia process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes elidia. Idempotent across container
...
#   - elidia is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G elidia 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] elidia already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" elidia 2>/dev/null; then
        echo "[stage2] Added elidia to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group elidia failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ELIDIA_UID is remapped or the top-level $ELIDIA_HOME isn't owned by
# the runtime elidia UID, restore ownership to elidia — but ONLY for the
# directories elidia actually writes to. The full $ELIDIA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of elidia-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_elidia_uid=$(id -u elidia)
needs_chown=false
if [ "$(stat -c %u "$ELIDIA_HOME" 2>/dev/null)" != "$actual_elidia_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ELIDIA_HOME (targeted) to elidia ($actual_elidia_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ELIDIA_HOME: chown the directory itself (not its contents)
    # so elidia can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown elidia:elidia "$ELIDIA_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $ELIDIA_HOME failed (rootless container?) — continuing"
    # Elidia-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by elidia (see the s6-setuidgid mkdir
    # -p block below for the canonical list).
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles; do
        if [ -e "$ELIDIA_HOME/$sub" ]; then
            chown -R elidia:elidia "$ELIDIA_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ELIDIA_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
    # Elidia-owned trees under $INSTALL_DIR must be re-chowned when the UID
    ...
    #     the source mtime is newer than dist/ or when ELIDIA_TUI_FORCE_BUILD
    #     is set) and writes to ui-tui/dist/. Without this chown the new
    #     elidia UID can't write the build output (#28851).
    #   - node_modules: root-level dependencies (puppeteer, web tooling)
    #     that runtime code may walk/update.
    # The set mirrors the build-time `chown -R elidia:elidia` line in the
    # Dockerfile — keep them in sync if the Dockerfile chown set changes.
    # These are under $INSTALL_DIR (not $ELIDIA_HOME), so the bind-mount
    # concern doesn't apply — recursive is fine.
    chown -R elidia:elidia \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
...
# Always reset ownership of $ELIDIA_HOME/profiles to elidia on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> elidia …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as elidia and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ELIDIA_HOME/profiles" ]; then
    chown -R elidia:elidia "$ELIDIA_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of elidia-owned top-level state files on every boot.
# The targeted data-volume chown above only covers elidia-owned
# *subdirectories*; loose state files living directly under $ELIDIA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> elidia …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged elidia runtime then hits
...
# sweep so host-owned files in a bind-mounted $ELIDIA_HOME are never
...
# entries of elidia_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ELIDIA_HOME/$f" ]; then
        chown elidia:elidia "$ELIDIA_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the elidia runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ELIDIA_HOME/config.yaml" ]; then
    chown elidia:elidia "$ELIDIA_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$ELIDIA_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as elidia user ---
# Run as elidia via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ELIDIA_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_elidia mkdir -p \
    "$ELIDIA_HOME/cron" \
    "$ELIDIA_HOME/sessions" \
    "$ELIDIA_HOME/logs" \
    "$ELIDIA_HOME/hooks" \
    "$ELIDIA_HOME/memories" \
    "$ELIDIA_HOME/skills" \
    "$ELIDIA_HOME/skins" \
    "$ELIDIA_HOME/plans" \
    "$ELIDIA_HOME/workspace" \
    "$ELIDIA_HOME/home"

# --- Install-method stamp (read by detect_install_method() in elidia status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the elidia user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_elidia tee "$ELIDIA_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ELIDIA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_elidia cp "$INSTALL_DIR/$src" "$ELIDIA_HOME/$dest"
    fi
}
...
if [ -f "$ELIDIA_HOME/.env" ]; then
    chown elidia:elidia "$ELIDIA_HOME/.env" 2>/dev/null || true
    chmod 600 "$ELIDIA_HOME/.env" 2>/dev/null || true
fi

# auth.json: bootstrap from env on first boot only. Same semantics as the
# pre-s6 entrypoint — the [ ! -f ] guard is critical to avoid clobbering
# rotated refresh tokens on container restart.
if [ ! -f "$ELIDIA_HOME/auth.json" ] && [ -n "${ELIDIA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$ELIDIA_AUTH_JSON_BOOTSTRAP" > "$ELIDIA_HOME/auth.json"
    chown elidia:elidia "$ELIDIA_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$ELIDIA_HOME/auth.json"
fi
...
# can set ELIDIA_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ELIDIA_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ELIDIA_HOME/gateway_state.json" ] && \
        [ "${ELIDIA_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$ELIDIA_HOME/gateway_state.json"
    chown elidia:elidia "$ELIDIA_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$ELIDIA_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_elidia "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/elidia/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Elidia spawns for the
...
# main-wrapper.sh propagates it into the supervised ``elidia`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-elidia, dashboard, etc.).
