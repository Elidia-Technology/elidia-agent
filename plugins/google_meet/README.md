...
Let the elidia agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where elidia runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `elidia meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `elidia meet` CLI |
...
| `cli.py` | `elidia meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ELIDIA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `elidia meet node {run,list,approve,remove,status,ping}` |
...
elidia meet install                                      # pip + Chromium
elidia meet setup                                        # preflight
elidia meet auth                                         # optional
elidia meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
elidia meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.elidia/.env
elidia meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
elidia meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
elidia meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.elidia/.env
elidia meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, elidia will **not** switch your system audio input automatically — the
...
elidia plugins enable google_meet
elidia meet node run --display-name my-mac --host 0.0.0.0 --port 18789
