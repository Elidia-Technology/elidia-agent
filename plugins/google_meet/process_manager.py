...
session-scoped state file under ``$ELIDIA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ELIDIA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_elidia_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this elidia install, leave it first —
    ...
    env = os.environ.copy()
    env["ELIDIA_MEET_URL"] = url
    env["ELIDIA_MEET_OUT_DIR"] = str(out)
    env["ELIDIA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ELIDIA_MEET_HEADED"] = "1"
    if auth_state:
        env["ELIDIA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ELIDIA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ELIDIA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ELIDIA_MEET_MODE"] = mode
    if realtime_model:
        env["ELIDIA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ELIDIA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ELIDIA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ELIDIA_MEET_REALTIME_KEY"] = realtime_api_key
