...
``$ELIDIA_HOME/workspace/meetings/<meeting-id>/``. The main elidia process
...
Run standalone for debugging::
    ...
    ELIDIA_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ELIDIA_MEET_OUT_DIR=/tmp/meet-debug \\
    ELIDIA_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``ELIDIA_MEET_OUT_DIR``.
...
# and exposes ``window.__elidiaMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__elidiaMeetInstalled) return;
  window.__elidiaMeetInstalled = true;
  window.__elidiaMeetQueue = [];
  ...
  window.__elidiaMeetDrain = () => {
    const out = window.__elidiaMeetQueue.slice();
    window.__elidiaMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "elidia_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("ELIDIA_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ELIDIA_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ELIDIA_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ELIDIA_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ELIDIA_MEET_GUEST_NAME", "Elidia Agent")
    duration_s = _parse_duration(os.environ.get("ELIDIA_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ELIDIA_MEET_MODE=realtime.
    mode = os.environ.get("ELIDIA_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ELIDIA_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ELIDIA_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ELIDIA_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ELIDIA_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — ELIDIA_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: ELIDIA_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in ELIDIA_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ELIDIA_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__elidiaMeetDrain && window.__elidiaMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__elidiaMeetInstalled) {
