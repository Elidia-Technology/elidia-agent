...
Resolution order for config file:
  1. $ELIDIA_HOME/honcho.json  (instance-local, enables isolated Elidia instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for an Elidia profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Elidia profile.

    Resolution order:
      1. ELIDIA_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``elidia.<profile>``
      3. Fallback: ``"elidia"`` (default profile)
    """
    explicit = os.environ.get("ELIDIA_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ELIDIA_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.elidia/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_elidia_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_elidia_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Elidia system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.elidia
    ...
    def from_global_config(
        ...
        Resolution: $ELIDIA_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Elidia profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.elidia block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Elidia session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Elidia session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Elidia session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'elidia honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run elidia honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `elidia honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            elidia_cfg = load_config()
            honcho_cfg = elidia_cfg.get("honcho", {})
