...
Handles: elidia honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "elidia" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Elidia profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'elidia honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'elidia honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `elidia update` -- no output, no exceptions.
    """
    try:
        from elidia_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Elidia profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ELIDIA_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_elidia_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        elidia_host.get("pinUserPeer"),
        elidia_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in elidia_host:
        aliases_src = elidia_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in elidia_host:
        prefix_src = elidia_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(elidia_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Elidia persistent cross-session memory.")
    ...
    elidia_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        current_host_key = elidia_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            elidia_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'elidia honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = elidia_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        elidia_host["peerName"] = new_peer

    current_ai = elidia_host.get("aiPeer") or cfg.get("aiPeer", "elidia")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        elidia_host["aiPeer"] = new_ai

    current_workspace = elidia_host.get("workspace") or cfg.get("workspace", "elidia")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        elidia_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, elidia_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = elidia_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(elidia_host)
        elidia_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{elidia_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            elidia_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            elidia_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'elidia honcho status' to inspect.")
        ...
        if existing_aliases:
            elidia_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = elidia_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        elidia_host["observationMode"] = new_obs
    else:
        elidia_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(elidia_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        elidia_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        elidia_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = elidia_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        elidia_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = elidia_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        elidia_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                elidia_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(elidia_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            elidia_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        elidia_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        elidia_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        elidia_host["dialecticReasoningLevel"] = new_reasoning
    else:
        elidia_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = elidia_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        elidia_host["sessionStrategy"] = new_strat

    elidia_host["enabled"] = True
    elidia_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        elidia_config = load_config()
        elidia_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(elidia_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: elidia config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    elidia honcho status     -- show full config")
    print("    elidia honcho mode       -- change recall/observation mode")
    print("    elidia honcho tokens     -- tune context and dialectic budgets")
    print("    elidia honcho peer       -- update peer names")
    print("    elidia honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Elidia profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: elidia honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'elidia honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: elidia honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        elidia = hosts.get(_host_key(), {})
        user = elidia.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = elidia.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = elidia.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = elidia.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Elidia' identity in Honcho. Seed with 'elidia honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: elidia honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: elidia honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = elidia.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = elidia.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = elidia.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Elidia asks Honcho's AI peer a question")
        ...
        print("\n  Set with: elidia honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'elidia honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'elidia honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    elidia honcho identity --show        — show both peer representations")
        print("    elidia honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Elidia + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Elidia\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Elidia persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  elidia honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'elidia honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'elidia honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  elidia honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'elidia'")
        print("  with Honcho configured and no prior session history.")
        print("  (Elidia calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    elidia honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'elidia honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Elidia, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'elidia honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    elidia honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  elidia honcho identity <path/to/SOUL.md>")
    ...
    print("    Elidia:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Elidia:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Elidia:   Honcho observes every message and updates representations")
    ...
    print("    Elidia:   per-session by default — each run gets its own session")
    print("              Map a custom name:  elidia honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. elidia honcho setup              — configure API key (required)")
        print("  2. elidia honcho migrate            — re-run this walkthrough")
    else:
        print("  1. elidia honcho status             — verify Honcho connection")
        print("  2. elidia                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. elidia honcho identity --show    — verify AI peer representation")
        print("  4. elidia honcho tokens             — tune context and dialectic budgets")
        print("  5. elidia honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'elidia memory setup'...\n")
        from elidia_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``elidia honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``elidia honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to elidia memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Elidia Honcho",
    )
