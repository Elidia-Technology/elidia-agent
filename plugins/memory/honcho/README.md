...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/elidia>
...
```bash
elidia memory setup honcho   # configure Honcho directly (works on a fresh install)
elidia memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
elidia config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.elidia/.env
```

> `elidia honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `elidia memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ELIDIA_HOME/honcho.json` | Profile-local (isolated Elidia instances) |
| 2 | `~/.elidia/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Elidia profile: `elidia` (default) or `elidia_<profile>`.
...
**Deployment shapes** (`elidia memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Elidia to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Elidia session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`elidia-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`elidia`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-elidia-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening elidia in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Elidia session ID (timestamp + hex). Every `elidia` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Elidia profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`elidia`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Elidia profile: `elidia` (default) or `elidia_<profile>` (e.g. `elidia -p coder` -> host key `elidia_coder`). Older `elidia.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ELIDIA_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `elidia memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `elidia honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `elidia memory setup`) |
| `elidia honcho status` | Show resolved config for active profile |
| `elidia honcho enable` / `disable` | Toggle Honcho for active profile |
| `elidia honcho mode <mode>` | Change recall or observation mode |
| `elidia honcho peer --user <name>` | Update user peer name |
| `elidia honcho peer --ai <name>` | Update AI peer name |
| `elidia honcho tokens --context <N>` | Set context token budget |
| `elidia honcho tokens --dialectic <N>` | Set dialectic max chars |
| `elidia honcho map <name>` | Map current directory to a session name |
| `elidia honcho sync` | Create host blocks for all Elidia profiles |
