...
def _load_supermemory_config(elidia_home: str) -> dict:
    config = _default_config()
    config_path = Path(elidia_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, elidia_home: str) -> None:
    config_path = Path(elidia_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `elidia memory setup`.
        # All other options are documented for $ELIDIA_HOME/supermemory.json
    ...
    def save_config(self, values, elidia_home):
        ...
        _save_supermemory_config(sanitized, elidia_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from elidia_constants import get_elidia_home
        self._elidia_home = kwargs.get("elidia_home") or str(get_elidia_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._elidia_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "elidia", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "elidia_memory", "target": target, "type": "explicit_memory"},
