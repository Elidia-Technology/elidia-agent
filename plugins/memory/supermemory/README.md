...
Config file: `$ELIDIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `elidia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `elidia-{identity}` → `elidia-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Elidia profile:

```json
{
  "container_tag": "elidia-{identity}"
}
```

For a profile named `coder`, this resolves to `elidia-coder`. The default profile resolves to `elidia-default`. Without `{identity}`, all profiles share the same container.
