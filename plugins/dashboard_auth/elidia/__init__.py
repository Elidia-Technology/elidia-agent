"""ElidiaDashboardAuthProvider — Elidia Portal OAuth (authorization-code + PKCE).

Implements ``elidia-account-service/docs/agent-dashboard-oauth-contract.md``
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      ELIDIA_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      ELIDIA_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://developer.aiutils.io``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``elidia-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``elidia_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ELIDIA_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via ELIDIA_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://developer.aiutils.io"
...
class ElidiaDashboardAuthProvider(DashboardAuthProvider):
    """Elidia Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"elidia_session_pkce\"]``
        ...
        cookie_payload = {
            "elidia_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Elidia Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-elidia-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-elidia-refresh-
                ...
                headers={
                    ...
                    "x-elidia-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between ELIDIA_DASHBOARD_PORTAL_URL / ELIDIA_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Elidia Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    """
    try:
        from elidia_cli.config import cfg_get, load_config

        cfg = load_config()
    except Exception as exc:  # noqa: BLE001 — broad catch is intentional
        logger.debug(
            "dashboard-auth-elidia: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ELIDIA_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ELIDIA_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    Registers ``ElidiaDashboardAuthProvider`` only when a client_id is
    configured (either via ``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set ELIDIA_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``elidia_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ELIDIA_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Elidia Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys an Elidia Agent "
        ...
        logger.debug("dashboard-auth-elidia: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ELIDIA_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Elidia Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-elidia: %s", LAST_SKIP_REASON)
        return

    try:
        provider = ElidiaDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"ElidiaDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-elidia: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-elidia: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
