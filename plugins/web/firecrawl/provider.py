...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Elidia tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Elidia Subscriber token are available.

    Reads ``peek_elidia_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_elidia_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_elidia_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``elidia tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_elidia_tools_enabled():
        return ""
    return (
        ", or use the Elidia Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_elidia_tools_enabled():
        message += (
            " With your Elidia subscription you can also use the Tool Gateway. "
            "run `elidia tools` and select Elidia Subscription as the web provider."
        )
    else:
        message += " " + _wt.elidia_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    ``_read_elidia_access_token``, ``Firecrawl``) are also looked up via
    ...
    if direct_config is not None and not _wt.prefers_gateway("web"):
        ...
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_elidia_access_token
        )
        ...
        kwargs = {
            "api_key": managed_gateway.elidia_user_token,
