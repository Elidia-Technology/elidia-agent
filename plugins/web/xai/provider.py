...
``{title, url, description, position}`` rows every other Elidia web
provider produces.
...
prefers Elidia-managed xAI Grok OAuth (via ``elidia auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.elidia/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``elidia tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `elidia auth` to sign in with "
