# Elidia Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: elidia-achievements plugin
...
### `GET /api/plugins/elidia-achievements/achievements`
...
### `POST /api/plugins/elidia-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/elidia-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/elidia-achievements/overview`
...
- Persist plugin data under:
  - `~/.elidia/plugins/elidia-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.elidia/plugins/elidia-achievements/scan_checkpoint.json`
...
- `~/.elidia/plugins/elidia-achievements/`
