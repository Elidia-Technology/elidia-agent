...
Optional Elidia observability plugin that maps Elidia observer hooks to
...
not replace Elidia Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Elidia emit NeMo Relay lifecycle events for
the work Elidia already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Elidia Agent can:

- Preserve Elidia execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
elidia plugins enable observability/nemo_relay
```

The `ELIDIA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ELIDIA_HOME` that the
agent run will use:

```bash
env ELIDIA_HOME=/tmp/elidia-nemo-relay-test \
  elidia plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ELIDIA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ELIDIA_HOME` is the Elidia profile/config home used by both
`elidia plugins enable ...` and the later `elidia chat ...` run. If unset,
Elidia uses the user's default home, usually `~/.elidia`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ELIDIA_HOME=/tmp/elidia-nemo-relay-test
elidia plugins enable observability/nemo_relay
elidia chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `elidia` command you run is built from the
...
uv run elidia plugins enable observability/nemo_relay
uv run elidia chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/elidia_agent-*.whl
...
The plugin can configure exporters directly from `ELIDIA_NEMO_RELAY_*`
...
```bash
export ELIDIA_NEMO_RELAY_ATOF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ELIDIA_NEMO_RELAY_ATIF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ELIDIA_NEMO_RELAY_ATOF_FILENAME`
- `ELIDIA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ELIDIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ELIDIA_NEMO_RELAY_ATIF_AGENT_NAME`
- `ELIDIA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ELIDIA_NEMO_RELAY_ATIF_MODEL_NAME`
- `ELIDIA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Elidia at it:

```bash
export ELIDIA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Elidia Agent"
agent_version = "local"
```

When `ELIDIA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ELIDIA_NEMO_RELAY_ATOF_*` fallback setup is skipped.
...
export ELIDIA_HOME=/tmp/elidia-nemo-relay-docs/elidia-home
mkdir -p "$ELIDIA_HOME"

cat > "$ELIDIA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Elidia session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ELIDIA_NEMO_RELAY_ATOF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/elidia-nemo-relay-docs/subagent/atof
export ELIDIA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ELIDIA_NEMO_RELAY_ATOF_MODE=overwrite
export ELIDIA_NEMO_RELAY_ATIF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/elidia-nemo-relay-docs/subagent/atif
export ELIDIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ELIDIA_NEMO_RELAY_ATIF_AGENT_NAME='Elidia Agent E2E'
export ELIDIA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ELIDIA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"elidia.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Elidia Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Elidia dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/elidia-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/elidia-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/elidia-nemo-relay-docs/workdir/beta.txt
cd /tmp/elidia-nemo-relay-docs/workdir

export ELIDIA_NEMO_RELAY_ATOF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/elidia-nemo-relay-docs/parallel/atof
export ELIDIA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ELIDIA_NEMO_RELAY_ATOF_MODE=overwrite
export ELIDIA_NEMO_RELAY_ATIF_ENABLED=1
export ELIDIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/elidia-nemo-relay-docs/parallel/atif
export ELIDIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ELIDIA_NEMO_RELAY_ATIF_AGENT_NAME='Elidia Agent E2E'
export ELIDIA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Elidia sessions map to `agent` scopes.
- Elidia API request hooks map to `llm` scope start/end events.
- Elidia tool hooks map to `tool` scope start/end events.
