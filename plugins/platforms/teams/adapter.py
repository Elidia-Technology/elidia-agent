"""
Microsoft Teams platform adapter for Elidia Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Elidia"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        elidia_action = data.get("elidia_action", "")
        session_key = data.get("session_key", "")

        if not elidia_action or not session_key:
        ...
        choice = choice_map.get(elidia_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="elidia_approve",
                    data={**btn_data_base, "elidia_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="elidia_approve",
                    data={**btn_data_base, "elidia_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="elidia_approve",
                    data={**btn_data_base, "elidia_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="elidia_approve",
                    data={**btn_data_base, "elidia_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Elidia\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.elidia/.env")
