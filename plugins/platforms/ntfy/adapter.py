"""ntfy platform adapter (Elidia plugin).
...
an Elidia dependency.

This adapter ships as an Elidia platform plugin under
``plugins/platforms/ntfy/``. The Elidia plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "elidia-in"              # subscribe topic (incoming)
          publish_topic: "elidia-out"     # optional — defaults to topic
...
_ECHO_TAG = "elidia-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``elidia cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already an Elidia dependency",
