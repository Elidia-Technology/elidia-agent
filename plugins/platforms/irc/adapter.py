"""
IRC Platform Adapter for Elidia Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Elidia agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "elidia-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Elidia Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Elidia Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: elidia_, elidia_1, elidia_2...
...
def check_requirements() -> bool:
    ...
    # elidia setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `elidia gateway setup` flow for the IRC platform.

    Lazy-imports ``elidia_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Elidia to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #elidia — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.elidia/.env")
    print_info("Restart the gateway for changes to take effect: elidia gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "elidia-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "elidia-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Elidia Agent (cron)")
