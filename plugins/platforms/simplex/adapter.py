"""SimpleX Chat platform adapter (Elidia plugin).
...
This adapter ships as an Elidia platform plugin under
``plugins/platforms/simplex/``. The Elidia plugin loader scans the
...
discoverable and `elidia setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Elidia — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"elidia-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``elidia setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.elidia/.env`` via ``elidia_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("elidia_cli.config not available; set SIMPLEX_* vars manually in ~/.elidia/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `elidia gateway status` without
