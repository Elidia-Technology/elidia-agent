...
# ``model_tools`` import time, so ``elidia status``, ``elidia chat``, etc.
...
# ``elidia_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Elidia is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to an elidia MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ELIDIA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _elidia_home = _get_elidia_home()
        except (ModuleNotFoundError, ImportError):
            _elidia_home = _Path.home() / ".elidia"
        self._thread_count_store = _ThreadCountStore(
            _elidia_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Elidia is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ELIDIA_HOME", str(_Path.home() / ".elidia"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ELIDIA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into an elidia MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Elidia is thinking…' marker message.
        ...
        "Elidia is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Elidia is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Elidia is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Elidia is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Elidia is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``elidia setup``.

    The setup wizard at ``elidia_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.elidia/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. elidia-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.elidia/.env")
    print_info("Restart the gateway: elidia gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``elidia cron`` running as a
    separate process from ``elidia gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Elidia plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'elidia-agent[google_chat]'",
        ...
        # the "Elidia is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Elidia is thinking…' marker message "
