"""
LINE Messaging API platform adapter for Elidia Agent.
...
to Elidia Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ELIDIA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            elidia_home = Path.home().joinpath(".elidia").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``elidia status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``elidia status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``elidia setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.elidia/.env`` via ``elidia_cli.config``.
    """
    print()
    ...
    try:
        from elidia_cli.config import get_env_var, set_env_var
    except ImportError:
        print("elidia_cli.config not available; set LINE_* vars manually in ~/.elidia/.env")
        return
