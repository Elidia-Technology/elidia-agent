...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Elidia tool
gateway (which Elidia uses to bill Browser Use sessions to an Elidia
...
Auth env vars (one of)::
    ...
    # OR a managed Elidia gateway entry (configured via 'elidia setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Elidia
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Elidia tool gateway when ``tool_gateway.browser`` config
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        ...
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_elidia_access_token,
        )
        if managed is None:
            return None

        return {
            "api_key": managed.elidia_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Elidia only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"elidia_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
