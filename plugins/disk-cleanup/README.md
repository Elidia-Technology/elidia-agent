...
Auto-tracks and cleans up ephemeral files created during Elidia Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ELIDIA_HOME` and `/tmp/elidia-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ELIDIA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ELIDIA_HOME` or `/tmp/elidia-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ELIDIA_HOME/disk-cleanup/` is itself excluded
- `$ELIDIA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
