"""disk_cleanup — ephemeral file cleanup for Elidia Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ELIDIA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ELIDIA_HOME and /tmp/elidia-*
Never touches: ~/.elidia/logs/ or any system directory.
"""
...
try:
    ...
    def get_elidia_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ELIDIA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".elidia").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ELIDIA_HOME/logs/``."""
    return get_elidia_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ELIDIA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ELIDIA_HOME or ``/tmp/elidia-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    elidia_home = get_elidia_home()
    try:
        path.resolve().relative_to(elidia_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/elidia-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("elidia-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ELIDIA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ELIDIA_HOME (but leave ELIDIA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "elidia-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(elidia_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == elidia_home:
                continue
            try:
                rel_parts = dirpath.relative_to(elidia_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    elidia_home = get_elidia_home()
    try:
        rel = path.resolve().relative_to(elidia_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "elidia-agent",
        }:
            return None
        ...
        # Path isn't under ELIDIA_HOME (e.g. /tmp/elidia-*) — fall through.
        pass
