...
const STORAGE_KEY = "elidia-dashboard-theme";
...
const THEME_NAME_ALIASES: Record<string, string> = {
  // Renamed during the LENS_5I port + Elidia-blue rebrand.
  "lens-5i": "elidia-blue",
};
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `elidia_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "elidia-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-elidia-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-elidia-theme-font", "true");
...
function applyTheme(theme: DashboardTheme) {
  ...
  // (e.g. Elidia Blue) doesn't leave its values behind when the user
