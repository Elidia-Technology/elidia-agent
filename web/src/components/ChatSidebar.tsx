...
import { Card } from "@aiutils/ui/ui/components/card";
...
export function ChatSidebar({ channel, className }: ChatSidebarProps) {
  ...
  // /api/events in elidia_cli/web_server.py for the broadcast hop.
  //
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      ws = new WebSocket(
        `${proto}//${window.location.host}${ELIDIA_BASE_PATH}/api/events?${qs.toString()}`,
      );
