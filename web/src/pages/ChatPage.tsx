/**
 * ChatPage — embeds `elidia --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (elidia_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { Typography } from "@aiutils/ui/ui/components/typography/index";
import { ELIDIA_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${ELIDIA_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ELIDIA_SESSION_TOKEN__ &&
    !window.__ELIDIA_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `elidia dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ELIDIA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Elidia TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[elidia-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ELIDIA_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Elidia. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="elidia-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
