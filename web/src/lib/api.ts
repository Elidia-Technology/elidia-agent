...
// (e.g. https://mission-control.tilos.com/elidia/). The Python backend
// injects ``window.__ELIDIA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ELIDIA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ELIDIA_BASE_PATH = readBasePath();
const BASE = ELIDIA_BASE_PATH;
...
declare global {
  interface Window {
    __ELIDIA_SESSION_TOKEN__?: string;
    __ELIDIA_BASE_PATH__?: string;
    ...
    __ELIDIA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Elidia-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ELIDIA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``elidia update``, ``elidia gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ELIDIA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ELIDIA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("elidia.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("elidia.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ELIDIA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("elidia.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ELIDIA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Elidia dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``elidia_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ELIDIA_AUTH_REQUIRED__) {
...
export const api = {
  ...
  updateElidia: () =>
    fetchJSON<ActionResponse>("/api/elidia/update", { method: "POST" }),
  checkElidiaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/elidia/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Elidia Portal contract V1 (the access token has no email/name claims —
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["elidia"]``).
