...
//!
//! Driven when the installer is launched as `Elidia-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Elidia desktop process to fully exit (so the venv
//!      shim is free; otherwise `elidia update` aborts with exit code 2),
//!   2. run `elidia update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in elidia_cli/main.py),
//!   3. run `elidia desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `elidia update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_elidia) and
...
/// `elidia update` exit code meaning "another elidia process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// elidia_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `elidia update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Elidia"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let elidia_home = crate::paths::elidia_home();
    let install_root = elidia_home.join("elidia-agent");
    ...
    let elidia = resolve_elidia(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the elidia CLI under {}. Is Elidia installed? \
    ...
    // async on Windows. If it still holds the venv shim, `elidia update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: elidia update -----------------------------------------
    // Pass --branch so `elidia update` targets the branch this installer was
    ...
    // without --branch, `elidia update` switches the checkout to `main` (a
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Elidia is still running. Close all Elidia windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "elidia update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::elidia_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: elidia desktop --build-only ----------------------------
    // `elidia update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `elidia desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Elidia manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_elidia_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_elidia(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Elidia to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            emit_log(
                ...
                Some("update"),
                LogStream::Stdout,
                "[update] timed out waiting for Elidia to exit; proceeding anyway",
            );
            return;
        }
...
/// Spawn `elidia <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv elidia shim under an install root, regardless of existence.
fn venv_elidia(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("elidia.exe")
    } else {
        install_root.join("venv").join("bin").join("elidia")
    }
}

/// Resolve the elidia CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `elidia` on PATH.
fn resolve_elidia(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_elidia(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "elidia.exe" } else { "elidia" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let elidia_home = crate::paths::elidia_home();
    let mut envs = vec![(
        "ELIDIA_HOME".to_string(),
        elidia_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        elidia_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_elidia_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Elidia.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.elidia-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.elidia-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::elidia_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_elidia_is_under_install_root() {
        let root = Path::new("/x/elidia-agent");
        let shim = venv_elidia(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Elidia.app"]),
            Some(PathBuf::from("/Applications/Elidia.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Elidia.app");
        let tmp = base.join("Elidia.app.elidia-update-new");
        let old = base.join("Elidia.app.elidia-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Elidia.app");
        let tmp = base.join("Elidia.app.elidia-update-new"); // intentionally absent
        let old = base.join("Elidia.app.elidia-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Elidia.app");
        let tmp = base.join("Elidia.app.elidia-update-new"); // absent
        let old = base.join("Elidia.app.elidia-update-old");
        write_marker(&target, "OLD");
