...
//!
//! Mirrors `elidia_constants.get_elidia_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\elidia
//!   macOS:   ~/.elidia
//!   Linux:   ~/.elidia  (override via $ELIDIA_HOME)
//!
//! NOTE (macOS): Python's get_elidia_home(), scripts/install.sh, and the
//! Electron desktop's resolveElidiaHome() ALL use ~/.elidia on macOS — there
...
/// Returns the canonical Elidia home directory, respecting $ELIDIA_HOME if set.
pub fn elidia_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ELIDIA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\elidia — matches scripts/install.ps1's $ElidiaHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("elidia");
        }
    }

    // macOS + Linux + fallback: ~/.elidia (matches Python get_elidia_home(),
    // install.sh, and the Electron desktop's resolveElidiaHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".elidia");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    elidia_home().join("bootstrap-cache")
}
...
/// ELIDIA_HOME so it survives repo checkout deletion (unlike anything under
/// elidia-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\elidia\elidia-setup.exe`; on other
...
pub fn copy_self_to_elidia_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ELIDIA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ELIDIA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".elidia-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ELIDIA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[elidia-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ELIDIA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_elidia_home() -> String {
    elidia_home().to_string_lossy().into_owned()
}
