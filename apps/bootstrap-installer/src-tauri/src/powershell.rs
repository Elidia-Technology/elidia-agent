...
///
/// `elidia_home_override` propagates to the child as $ELIDIA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, elidia_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = elidia_home_override {
        cmd.env("ELIDIA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, elidia_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = elidia_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_elidia_home() {
