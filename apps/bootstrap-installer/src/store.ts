...
export const $elidiaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, elidiaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_elidia_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $elidiaHome.set(elidiaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Elidia" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Elidia-Setup.exe --update);
...
export async function launchElidiaDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_elidia_desktop', { installRoot })
}
