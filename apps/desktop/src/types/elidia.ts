...
export type ElidiaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
  ...
  /** Elidia only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Elidia only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
  ...
  requires_elidia_auth: boolean
  /** True when this is the provider currently written to config (mirrors the
   *  CLI `elidia tools` active-provider detection). */
  is_active: boolean
}
...
export interface ModelAssignmentResponse {
  /** Toolset keys auto-routed through the Elidia Tool Gateway as a result of
   *  switching the main provider to Elidia. Empty unless provider === 'elidia'
