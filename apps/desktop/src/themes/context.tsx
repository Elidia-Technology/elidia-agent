...
const SKIN_KEY = 'elidia-desktop-theme-v2'
const MODE_KEY = 'elidia-desktop-mode-v1'
...
const normalizeSkin = (name: string | null | undefined): string => {
  const resolved = name ? (SKIN_ALIASES[name] ?? name) : undefined

  return resolved && BUILTIN_THEMES[resolved] && !RETIRED_SKINS.has(resolved) ? resolved : DEFAULT_SKIN_NAME
}
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = BUILTIN_THEMES[skinName] ?? elidiaTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...elidiaTheme.typography, ...theme.typography }
  ...
  root.dataset.elidiaTheme = skinName
  root.dataset.elidiaMode = rendered
  ...
  window.elidiaDesktop?.setTitleBarTheme?.({
...
/** Sync the desktop skin with the active Elidia backend theme on connect. */
export function useSyncThemeFromBackend(backendThemeName: string | undefined, setTheme: (name: string) => void) {
  useEffect(() => {
    ...
    const resolved = SKIN_ALIASES[backendThemeName] ?? backendThemeName

    if (BUILTIN_THEMES[resolved]) {
      setTheme(resolved)
    }
