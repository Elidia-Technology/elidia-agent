...
export const ELIDIA_PATHS_MIME = 'application/x-elidia-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Elidia sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.elidiaDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(ELIDIA_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const pickContextPaths = useCallback(
    async (kind: 'file' | 'folder') => {
      const paths = await window.elidiaDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(async (filePath: string) => {
    ...
    try {
      const previewUrl = await window.elidiaDesktop?.readFileDataUrl(filePath)
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.elidiaDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(async () => {
    try {
      const path = await window.elidiaDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.elidiaDesktop?.getPathForFile ? window.elidiaDesktop.getPathForFile(file) : ''
