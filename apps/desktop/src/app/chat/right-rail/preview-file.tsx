...
async function readTextPreview(filePath: string) {
  if (window.elidiaDesktop.readFileText) {
    try {
      return await window.elidiaDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'elidia:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.elidiaDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(ELIDIA_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
      ...
      <PreviewEmptyState
        body={
          binary
            ...
            : `${target.label} is ${formatBytes(size)}. Elidia will only show the first 512 KB.`
        }
