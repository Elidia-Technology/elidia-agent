    ...
    <PreviewEmptyState
        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.elidiaDesktop?.openExternal(error.url)
            }}
          >
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Elidia is restarting...' : 'Ask Elidia to restart the server',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Elidia to restart the server" looked like it did nothing —
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `Elidia is looking for a preview server to restart (${taskId})`
      })

      notify({
        kind: 'info',
        title: 'Restarting preview server',
        message: 'Elidia is working in the background. Watch the preview console for progress.',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ? `Elidia finished restarting the preview server${
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'Elidia could not restart the server.',
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'Elidia is still working, but no restart result has arrived yet. The server command may be running in the foreground.'
      )
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.elidiaDesktop?.watchPreviewFile ||
      !window.elidiaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.elidiaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.elidiaDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.elidiaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.elidiaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:elidia-preview')
