...
import { elidiaDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
export function ChatBar({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, ELIDIA_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(ELIDIA_PATHS_MIME)) {
