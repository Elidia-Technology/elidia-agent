...
import type { ElidiaReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ElidiaReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { elidiaDesktop: { readDir: typeof readDir } }).elidiaDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { elidiaDesktop?: unknown }).elidiaDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ElidiaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ElidiaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ElidiaReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.elidiaDesktop is missing', async () => {
    delete (window as unknown as { elidiaDesktop?: unknown }).elidiaDesktop
