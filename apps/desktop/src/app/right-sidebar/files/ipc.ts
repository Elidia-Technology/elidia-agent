...
import type { ElidiaReadDirEntry, ElidiaReadDirResult } from '@/global'

export type ProjectTreeEntry = ElidiaReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.elidiaDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.elidiaDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.elidiaDesktop?.readDir || !window.elidiaDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.elidiaDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.elidiaDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: ElidiaReadDirEntry) {
...
async function filterIgnored(entries: ElidiaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ElidiaReadDirResult> {
  if (!window.elidiaDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.elidiaDesktop.readDir(dirPath)
