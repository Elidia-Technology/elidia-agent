...
const ELIDIA_PATHS_MIME = 'application/x-elidia-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ELIDIA_PATHS_MIME)) {return true}
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ELIDIA_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.elidiaDesktop?.terminal
    ...
    try {
      ...
      console.warn('[elidia-terminal] WebGL unavailable; falling back to DOM', err)
    }
