...
import {
  getElevenLabsVoices,
  getElidiaConfigDefaults,
  getElidiaConfigRecord,
  getElidiaConfigSchema,
  saveElidiaConfig
} from '@/elidia'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ElidiaConfigRecord } from '@/types/elidia'
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ElidiaConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<ElidiaConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getElidiaConfigRecord(), getElidiaConfigDefaults(), getElidiaConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: ElidiaConfigRecord) => {
  ...
  if (!config || !schema) {
    return <LoadingState label="Loading Elidia configuration..." />
  }
