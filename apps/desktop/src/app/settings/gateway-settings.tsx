...
export function GatewaySettings() {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.elidiaDesktop
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.elidiaDesktop.applyConnectionConfig(payload())
        : await window.elidiaDesktop.saveConnectionConfig(payload())
      ...
      notify({
        ...
        message: apply ? 'Elidia Desktop will reconnect using the saved settings.' : 'Saved for the next restart.'
      })
    } catch (err) {
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.elidiaDesktop.testConnectionConfig({
        mode: 'remote',
        remoteToken: remoteToken.trim() || undefined,
        remoteUrl: state.remoteUrl.trim()
      })

      const message = `Connected to ${result.baseUrl}${result.version ? ` · Elidia ${result.version}` : ''}`
      setLastTest(message)
  ...
  if (!window.elidiaDesktop?.getConnectionConfig) {
    return (
      <EmptyState
    ...
    <SettingsContent>
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-2xl text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
          Elidia Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control
          an already-running Elidia backend on another machine or behind a trusted proxy.
        </p>
      </div>
        ...
        <div className="mb-5 flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2.5 text-[length:var(--conversation-caption-font-size)] text-destructive">
          ...
          <div>
            ...
            <div className="mt-1 leading-5">
              Unset <code>ELIDIA_DESKTOP_REMOTE_URL</code> and <code>ELIDIA_DESKTOP_REMOTE_TOKEN</code> to use the saved
              setting below.
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ModeCard
          ...
          description="Start a private Elidia backend on localhost. This is the default and works offline."
        ...
        <ModeCard
          ...
          description="Connect this desktop shell to a remote Elidia backend using its session token."
          disabled={state.envOverride}
          icon={Globe}
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/elidia"
              value={state.remoteUrl}
            />
          }
          description="Base URL for the remote dashboard backend. Path prefixes are supported, for example /elidia."
          title="Remote URL"
        />
        <ListRow
          action={
      ...
      <div className="mt-6 divide-y divide-border/40">
        <ListRow
          action={
            <Button onClick={() => void window.elidiaDesktop?.revealLogs()} variant="outline">
