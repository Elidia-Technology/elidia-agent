...
import { deleteEnvVar, getToolsetConfig, revealEnvVar, selectToolsetProvider, setEnvVar } from '@/elidia'
import { Check, ExternalLink, Eye, EyeOff, Loader2, Save, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ToolEnvVar, ToolProvider, ToolsetConfig } from '@/types/elidia'
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // panel highlighted the first keyless provider (e.g. Elidia Portal) even when
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isActive && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_elidia_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">
                    Included with an Elidia subscription — sign in to Elidia Portal to activate.
                  </p>
                )}
                ...
                {provider.post_setup && (
                  <p className="text-[0.72rem] text-muted-foreground">
                    ...
                    <code className="font-mono">elidia tools</code> for now.
                  </p>
                )}
              </div>
            )}
