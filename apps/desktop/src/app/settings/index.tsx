...
import { getElidiaConfigDefaults, getElidiaConfigRecord, saveElidiaConfig } from '@/elidia'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getElidiaConfigRecord()
  ...
  const resetConfig = async () => {
    if (!window.confirm('Reset all settings to Elidia defaults?')) {
      return
    }

    try {
      await saveElidiaConfig(await getElidiaConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
