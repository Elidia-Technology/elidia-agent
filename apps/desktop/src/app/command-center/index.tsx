...
const NAVIGATION_SEARCH_ENTRIES: readonly NavigationSearchEntry[] = [
  ...
  { id: 'nav-settings', route: SETTINGS_ROUTE, title: 'Settings', detail: 'Configure Elidia desktop' },
...
export function CommandCenterView({
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateElidia()
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          Elidia {status.version} · Active sessions {status.active_sessions}
                        </div>
                      </div>
