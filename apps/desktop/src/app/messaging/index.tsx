...
const FIELD_COPY: Record<string, { advanced?: boolean; help?: string; label: string; placeholder?: string }> = {
  ...
  MATRIX_USER_ID: {
    label: 'Bot user ID',
    placeholder: '@elidia:example.org'
  },
...
const PLATFORM_INTRO: Record<string, string> = {
  ...
  signal:
    'Run a signal-cli REST bridge somewhere reachable, then point Elidia at the URL and the registered phone number.',
  whatsapp:
    'Start the WhatsApp bridge that ships with Elidia, scan the QR code on first run, then enable the platform.',
  bluebubbles:
    'Run BlueBubbles Server on a Mac with iMessage, expose its API, then point Elidia at the URL with the server password.',
  homeassistant:
  ...
  weixin:
    'Sign in to the WeChat Official Account platform, copy the AppID and Token, and point the message callback URL at Elidia.',
  qqbot: 'Register an app on the QQ Open Platform (q.qq.com) and copy the App ID and Client Secret.',
  api_server:
    'Expose Elidia as an OpenAI-compatible API. Set an auth key, then point Open WebUI / LobeChat / etc. at the host:port.',
  webhook:
