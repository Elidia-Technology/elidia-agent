...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<ElidiaGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.elidiaDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ElidiaGateway | null> | null>(null)
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Elidia gateway unavailable')
      }
