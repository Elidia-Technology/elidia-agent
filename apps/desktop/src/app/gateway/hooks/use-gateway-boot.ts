...
import type { RpcEvent } from '@/types/elidia'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.elidiaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ElidiaGateway | null) => void
  refreshElidiaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ElidiaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Elidia...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        await callbacksRef.current.refreshElidiaConfig().catch(() => undefined)
    ...
    const gateway = new ElidiaGateway()
    ...
    const offExit = desktop.onBackendExit(() => {
      if ($desktopBoot.get().running || $desktopBoot.get().visible) {
        failDesktopBoot('Elidia background process exited during startup.')
      }

      notify({
        kind: 'error',
        title: 'Backend stopped',
        message: 'Elidia background process exited.',
        durationMs: 0
      })
    })
    ...
    async function boot() {
      try {
        ...
        setDesktopBootStep({
          ...
          message: 'Loading Elidia settings',
          progress: 97
        })
        await callbacksRef.current.refreshElidiaConfig()
