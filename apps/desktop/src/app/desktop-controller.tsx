...
import { useElidiaConfig } from './session/hooks/use-elidia-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.elidiaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.elidiaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.elidiaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshElidiaConfig, sttEnabled, voiceMaxRecordingSeconds } = useElidiaConfig({
    activeSessionIdRef,
