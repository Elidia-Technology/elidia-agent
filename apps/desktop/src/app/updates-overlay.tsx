  ...
  return (
    <Dialog onOpenChange={handleClose} open={open}>
      ...
      >
        ...
        {phase === 'manual' && (
          <ManualView command={apply.command ?? 'elidia update'} onDone={() => handleClose(false)} />
        )}
  ...
  if (!status.supported) {
    return (
      <CenteredStatus
        ...
        body={status.message ?? 'This version of Elidia can’t update itself from inside the app.'}
        icon={<AlertCircle className="size-6 text-muted-foreground" />}
    ...
    <div className="grid gap-5 px-6 pb-6 pt-7 pr-8">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <DialogDescription className="text-center text-sm">
          A new version of Elidia is ready to install.
        </DialogDescription>
      </div>
  ...
  return (
    <div className="grid gap-5 px-6 pb-6 pt-7 pr-8">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <DialogDescription className="text-center text-sm">
          You installed Elidia from the command line, so updates run there too. Paste this into your terminal:
        </DialogDescription>
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground">
        Elidia will pick up the new version next time you launch it.
      </p>
...
function ApplyingView({ apply }: { apply: UpdateApplyState }) {
  const label = STAGE_LABELS[apply.stage] ?? 'Updating Elidia…'
  ...
  return (
    <div className="grid gap-5 px-6 pb-6 pt-7">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <DialogDescription className="text-center text-sm">
          The Elidia updater will take over in its own window and reopen Elidia when it&rsquo;s done.
        </DialogDescription>
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground">Elidia will close to apply the update.</p>
    </div>
  )
}
