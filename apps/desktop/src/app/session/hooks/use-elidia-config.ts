...
import { getElidiaConfig, getElidiaConfigDefaults } from '@/elidia'
...
interface ElidiaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useElidiaConfig({ activeSessionIdRef, refreshProjectBranch }: ElidiaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshElidiaConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getElidiaConfig(), getElidiaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshElidiaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
