...
const UPDATE_TOAST_SNOOZE_KEY = 'elidia:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: 'Update Elidia', onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
    message: 'Your Elidia backend is older than this desktop build and may not work correctly. Update to align them.',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.elidiaDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.elidiaDesktop?.updates
  ...
  try {
    ...
    // The update check pulls the latest elidia_cli + bundled package metadata
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `elidia update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'elidia update',
        command: result.command ?? 'elidia update'
      })
    }
...
function onFocus() {
  ...
  // updated Elidia from another window/CLI between focuses, and About should
