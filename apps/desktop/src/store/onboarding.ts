...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/elidia'
...
const CONFIGURED_CACHE_KEY = 'elidia-desktop-onboarded-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Elidia is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Elidia Tool Gateway,
// mirroring elidia_cli/elidia_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Elidia auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Elidia subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `elidia model` does (for Elidia it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Elidia still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Elidia still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.elidiaDesktop?.openExternal) {
    try {
      await window.elidiaDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Elidia still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
