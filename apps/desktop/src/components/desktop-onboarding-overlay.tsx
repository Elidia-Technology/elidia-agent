...
import { getGlobalModelOptions } from '@/elidia'
import {
  Check,
  ChevronDown,
  ChevronLeft,
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    description: 'Point Elidia at a local or self-hosted OpenAI-compatible endpoint (vLLM, llama.cpp, Ollama, etc).',
    docsUrl: 'https://github.com/Elidia-Technology/elidia-agent#bring-your-own-endpoint',
...
const FLOW_SUBTITLES: Record<OAuthProvider['flow'], string> = {
  ...
  device_code: 'Opens a verification page in your browser — Elidia connects automatically',
  loopback: 'Opens your browser to sign in — Elidia connects automatically',
  ...
  return (
    <div className="grid gap-3" role="status">
      <p className="text-sm text-muted-foreground">
        {installing
          ? 'Elidia is finishing install. This usually takes under a minute on first run.'
          : 'Starting Elidia…'}
      </p>
  ...
  return (
    <div className="border-b border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-[0.9375rem] font-semibold tracking-tight">Let's get you setup with Elidia Agent</h2>
...
function FlowPanel({ ctx, flow }: { ctx: OnboardingContext; flow: OnboardingFlow }) {
  ...
  if (flow.status === 'awaiting_browser') {
    return (
      <Step title={`Sign in with ${title}`}>
        <p className="text-sm text-muted-foreground">
          We opened {title} in your browser. Authorize Elidia there and you'll be connected
