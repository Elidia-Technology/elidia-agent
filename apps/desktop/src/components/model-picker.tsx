...
import type { ModelOptionProvider, ModelOptionsResponse, ModelPricing } from '@/types/elidia'
...
export function ModelPickerDialog({
  ...
  // the `elidia model` CLI picker, which shows the curated list verbatim.
          ...
          <CommandGroup heading={<ProviderHeading provider={provider} />} key={provider.slug}>
            ...
            {unavailable.size > 0 && (
              <div className="px-6 pb-2 pt-1 text-[0.62rem] leading-relaxed text-muted-foreground">
                Pro models need a paid Elidia subscription.
              </div>
            )}
          </CommandGroup>
        )
...
function ProviderHeading({ provider }: { provider: ModelOptionProvider }) {
  // free_tier is only set for Elidia. true → "Free tier", false → "Pro".
  const tierBadge =
