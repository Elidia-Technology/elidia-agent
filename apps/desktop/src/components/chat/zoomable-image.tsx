...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'elidia:saveImageFromUrl'")
}
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      if (window.elidiaDesktop?.saveImageFromUrl) {
        const saved = await window.elidiaDesktop.saveImageFromUrl(src)
      ...
      if (isMissingIpcHandler(error)) {
        try {
          ...
          notify({
            ...
            message: 'Restart Elidia Desktop to use Save Image.'
          })
        } catch (fallbackError) {
          notifyError(fallbackError, 'Restart Elidia Desktop to save images')
        }
