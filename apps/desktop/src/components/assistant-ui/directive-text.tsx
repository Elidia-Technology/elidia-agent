...
const ELIDIA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal'] as const
type ElidiaRefType = (typeof ELIDIA_REF_TYPES)[number]
...
const ICON_PATHS: Record<ElidiaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ElidiaRefType] ?? ICON_FALLBACK
...
const ELIDIA_DIRECTIVE_RE = new RegExp(
...
export const elidiaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ELIDIA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ElidiaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ElidiaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Elidia directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => elidiaDirectiveFormatter.parse(cleanedText), [cleanedText])
