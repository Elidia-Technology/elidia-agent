...
describe('elidiaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = elidiaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = elidiaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
