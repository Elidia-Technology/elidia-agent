...
import { ELIDIA_PATHS_MIME, extractDroppedFiles } from '@/app/chat/hooks/use-composer-actions'
import { ClarifyTool } from '@/components/assistant-ui/clarify-tool'
import { DirectiveContent, elidiaDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
const ResponseLoadingIndicator: FC = () => {
  ...
  return (
    <StatusRow data-slot="aui_response-loading" label="Elidia is loading a response">
...
const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      const serialized = elidiaDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ELIDIA_PATHS_MIME)) {
      return
    }
