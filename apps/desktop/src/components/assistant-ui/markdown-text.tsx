...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.elidiaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
    return mediaStreamUrl(path)
  }

  if (!window.elidiaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.elidiaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.elidiaDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
