...
export const ClarifyTool = (props: ToolCallMessagePartProps) => {
  ...
  // Once Elidia records an answer, fall back to the standard tool block so
...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  const respond = useCallback(
    async (answer: string) => {
      ...
      if (!gateway) {
        notifyError(new Error('Elidia gateway is not connected'), 'Could not send clarify response')
