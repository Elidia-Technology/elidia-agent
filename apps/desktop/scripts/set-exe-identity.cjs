#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Elidia icon + version metadata onto the
// built Elidia.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Elidia.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `elidia desktop`, the installer's
...
// path (which rebuilds via `elidia desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Elidia.exe>
//
...
// Stamp the Elidia icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Elidia',
      FileDescription: 'Elidia',
      CompanyName: 'AiUtils',
      LegalCopyright: 'Copyright (c) 2026 AiUtils'
    }
  })

  console.log('[set-exe-identity] done — Elidia icon + identity stamped')
}
