...
// no longer bundles the Elidia Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Elidia.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Elidia'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Elidia.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'elidia'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ELIDIA_HOME for non-sandboxed runs -- matches main.cjs's
// resolveElidiaHome(). On Windows it's %LOCALAPPDATA%\elidia; elsewhere
// it's ~/.elidia. The fresh-install sandbox launchFresh() sets its own
// ELIDIA_HOME and never touches this.
const DEFAULT_ELIDIA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'elidia')
  }
  return path.join(os.homedir(), '.elidia')
})()
const VENV_ROOT = path.join(DEFAULT_ELIDIA_HOME, 'elidia-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'elidia-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ELIDIA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Elidia-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Elidia-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Elidia-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Elidia-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ELIDIA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ELIDIA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const elidiaHome = path.join(sandbox, 'elidia-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(elidiaHome, { recursive: true })
  ...
  env.ELIDIA_DESKTOP_CWD = cwd
  env.ELIDIA_DESKTOP_IGNORE_EXISTING = '1'
  env.ELIDIA_DESKTOP_TEST_MODE = 'fresh-install'
  env.ELIDIA_DESKTOP_USER_DATA_DIR = userDataDir
  env.ELIDIA_HOME = elidiaHome
  delete env.ELIDIA_DESKTOP_ELIDIA
  delete env.ELIDIA_DESKTOP_ELIDIA_ROOT
  ...
  console.log(`  ELIDIA_HOME: ${elidiaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(elidiaHome, 'elidia-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Elidia Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of elidia_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'elidia-agent', 'elidia_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Elidia
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ELIDIA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ELIDIA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
