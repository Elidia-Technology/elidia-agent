 ...
 *
 * Drives apps/desktop's first-launch install of Elidia Agent by spawning
...
function bootstrapCacheDir(elidiaHome) {
  return path.join(elidiaHome, 'bootstrap-cache')
}

function cachedScriptPath(elidiaHome, commit) {
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, elidiaHome, emit }) {
  ...
  const cached = cachedScriptPath(elidiaHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, elidiaHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass ELIDIA_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        ELIDIA_HOME: elidiaHome || process.env.ELIDIA_HOME || ''
      }
    })
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, elidiaHome } = {}) {
  return new Promise((resolve, reject) => {
...
// instead of falling back to install.ps1's default ($Branch = "master").
...
function buildPosixPinArgs({ installStamp, activeRoot, elidiaHome }) {
  const args = ['--dir', activeRoot, '--elidia-home', elidiaHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, elidiaHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, elidiaHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, elidiaHome, activeRoot, abortSignal, installStamp }) {
  const startedAt = Date.now()
  emit({ type: 'stage', name: stage.name, state: 'running' })

  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--stage', stage.name, '--non-interactive', '--json', ...buildPosixPinArgs({ installStamp, activeRoot, elidiaHome })]
    : ['-Stage', stage.name, '-NonInteractive', '-Json', ...buildPinArgs(installStamp)]
  const result = await (isPosix ? spawnBash : spawnPowerShell)(
    scriptPath,
    args,
    { emit, stageName: stage.name, abortSignal, elidiaHome }
  )
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(elidiaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, elidiaHome, emit })
