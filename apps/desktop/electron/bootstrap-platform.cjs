 ...
 * null to keep it enabled. `ELIDIA_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.ELIDIA_DESKTOP_DISABLE_GPU || '').trim().toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (ELIDIA_DESKTOP_DISABLE_GPU)'
