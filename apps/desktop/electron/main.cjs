...
const USER_DATA_OVERRIDE = process.env.ELIDIA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ELIDIA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ELIDIA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[elidia] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[elidia] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[elidia] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[elidia] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ELIDIA_HOME — the user-facing root for everything Elidia-related. Mirrors
// scripts/install.ps1's $ElidiaHome and scripts/install.sh's $ELIDIA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\elidia (matches install.ps1)
//   macOS / Linux: ~/.elidia (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.elidia directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\elidia yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ELIDIA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ELIDIA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.elidia / %LOCALAPPDATA%\elidia.
function resolveElidiaHome() {
  if (process.env.ELIDIA_HOME) return path.resolve(process.env.ELIDIA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'elidia-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'elidia')
    const legacy = path.join(app.getPath('home'), '.elidia')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.elidia setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.elidia')
}

const ELIDIA_HOME = resolveElidiaHome()
// ACTIVE_ELIDIA_ROOT — the canonical mutable Elidia install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ELIDIA_ROOT = path.join(ELIDIA_HOME, 'elidia-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ELIDIA_ROOT, 'venv')
...
// boots, going straight to `resolveElidiaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ELIDIA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ELIDIA_ROOT, '.elidia-bootstrap-complete')
...
// elidiaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ELIDIA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by elidia_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ELIDIA_HOME, 'logs', 'desktop.log')
const DESKTOP_LOG_FLUSH_MS = 120
const DESKTOP_LOG_BUFFER_MAX_CHARS = 64 * 1024
const BOOT_FAKE_MODE = process.env.ELIDIA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ELIDIA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Elidia'
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 AiUtils'
})
...
const MEDIA_PROTOCOL = 'elidia-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(elidia-media://…)`.
...
// onto the error so subsequent startElidia() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Elidia backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[elidia] ${line}`)
  elidiaLog.push(...lines)
  if (elidiaLog.length > 300) {
    elidiaLog.splice(0, elidiaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-elidia-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('elidia:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the elidia:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:bootstrap:event', ev)
}
...
function isElidiaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'elidia_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ELIDIA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Elidia deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Elidia' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\elidia\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'elidia', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'elidia', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\elidia\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'elidia', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'elidia', 'git', 'bin', 'git.exe'))
  }
...
function recentElidiaLog() {
  return elidiaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's elidia root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ELIDIA_ROOT.
// ELIDIA_DESKTOP_ELIDIA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ELIDIA_DESKTOP_ELIDIA_ROOT && path.resolve(process.env.ELIDIA_DESKTOP_ELIDIA_ROOT),
    !IS_PACKAGED && isElidiaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isElidiaSourceRoot(ACTIVE_ELIDIA_ROOT) ? ACTIVE_ELIDIA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ELIDIA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('elidia:updates:progress', merged)
  }
}
...
// ELIDIA_HOME/elidia-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_elidia_home). That binary owns
// ALL repo mutation — running `elidia update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'elidia-setup.exe' : 'elidia-setup'
  const candidate = path.join(ELIDIA_HOME, name)
...
// `elidia update`). Instead we spawn the staged Elidia-Setup binary with
// --update and quit, so it can run `elidia update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri elidia-setup. Unlike Windows
    ...
    if (!updater) {
      ...
      // `elidia desktop`, never the Tauri installer that self-copies
      // elidia-setup.exe into ELIDIA_HOME). They DO have a working `elidia`
      ...
      // checkout they're on — bare `elidia update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'elidia update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `elidia update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `elidia update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, elidiaRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Elidia updater…', percent: 100 })
    ...
    // `elidia update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ELIDIA_HOME,
      env: {
        ...process.env,
        ELIDIA_HOME,
        PATH: [path.join(ELIDIA_HOME, 'node', 'bin'), venvBin, process.env.PATH]
          .filter(Boolean)
          .join(path.delimiter)
      },
      detached: true,
...
// Resolve the elidia CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `elidia` on PATH.
function resolveElidiaCliBinary(updateRoot) {
  const venvElidia = path.join(updateRoot, 'venv', 'bin', 'elidia')
  if (fileExists(venvElidia)) return venvElidia
  return findOnPath('elidia') || null
}
...
// macOS/Linux in-app update: backend (`elidia update`) + OS-aware GUI rebuild
...
async function applyUpdatesPosixInApp(opts = {}) {
  ...
  if (!elidia) {
    emitUpdateProgress({ stage: 'manual', message: 'elidia update', percent: null })
    return { ok: true, manual: true, command: 'elidia update', elidiaRoot: updateRoot }
  }

  // Put the Elidia-managed Node and the venv on PATH so `elidia desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(ELIDIA_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    ELIDIA_HOME,
  ...
  // `elidia update` reaps stale `elidia dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ELIDIA_DESKTOP_CHILD_PID and excludes
  // it while still reaping any genuinely-orphaned dashboards. (#37532)
  if (elidiaProcess && Number.isInteger(elidiaProcess.pid)) {
    env.ELIDIA_DESKTOP_CHILD_PID = String(elidiaProcess.pid)
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Elidia (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(elidia, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'elidia update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'elidia update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(elidia, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Elidia to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Elidia.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Elidia.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Elidia to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.elidia-update-new"; then
    rm -rf "$DST.elidia-update-old" 2>/dev/null || true
    mv "$DST" "$DST.elidia-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.elidia-update-new" "$DST"
    rm -rf "$DST.elidia-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `elidia-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Elidia to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ELIDIA_ROOT actually
...
function isActiveRuntimeUsable() {
  return isElidiaSourceRoot(ACTIVE_ELIDIA_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `elidia
...
// HEAD commit of ACTIVE_ELIDIA_ROOT so an adopted marker carries the same
...
function readActiveHeadCommit() {
  try {
    const sha = execFileSync(resolveGitBinary(), ['-C', ACTIVE_ELIDIA_ROOT, 'rev-parse', 'HEAD'], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore']
    }).trim()
...
function resolveWebDist() {
  const override = process.env.ELIDIA_DESKTOP_WEB_DIST
...
function resolveElidiaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Elidia.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ELIDIA_DESKTOP_CWD,
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'elidia_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_ELIDIA_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Elidia at ${ACTIVE_ELIDIA_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'elidia_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_ELIDIA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_ELIDIA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveElidiaBackend(dashboardArgs) {
  // 1. Explicit override -- ELIDIA_DESKTOP_ELIDIA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ELIDIA_DESKTOP_ELIDIA_ROOT && path.resolve(process.env.ELIDIA_DESKTOP_ELIDIA_ROOT)
  if (overrideRoot && isElidiaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Elidia source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `elidia` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isElidiaSourceRoot.)
  if (!IS_PACKAGED && isElidiaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Elidia source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ELIDIA_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\elidia\elidia-agent (Windows) or ~/.elidia/elidia-agent.
  ...
  //    to spawning elidia. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `elidia update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 3b. Existing-but-unmarked install at ACTIVE_ELIDIA_ROOT. The marker is
  ...
  //     the WHOLE install on top of a working one. ACTIVE_ELIDIA_ROOT is our
  //     canonical location (unlike a random `elidia` on PATH), so adopt it:
  ...
  if (!forceBootstrapRepair && isActiveRuntimeUsable()) {
    rememberLog(`[bootstrap] adopting existing install at ${ACTIVE_ELIDIA_ROOT}; skipping first-launch setup`)
    try {
  ...
  // 4. Existing `elidia` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ELIDIA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ELIDIA_DESKTOP_IGNORE_EXISTING !== '1') {
    let elidiaCommand = null
    const elidiaOverride = process.env.ELIDIA_DESKTOP_ELIDIA

    if (elidiaOverride) {
      const resolvedOverride = findOnPath(elidiaOverride)
      if (resolvedOverride) {
        elidiaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(elidiaOverride, { isWsl: IS_WSL })) {
        elidiaCommand = elidiaOverride
      } else {
        rememberLog(`Ignoring Windows Elidia override under WSL: ${elidiaOverride}`)
      }
    } else {
      elidiaCommand = findOnPath('elidia')
    }

    if (elidiaCommand) {
      if (looksLikeDesktopAppBinary(elidiaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Elidia CLI: ${elidiaCommand}`)
        elidiaCommand = null
      }
    }

    if (elidiaCommand) {
      // Smoke-test the candidate before trusting it. A `elidia` shim
      ...
      const shellForProbe = isCommandScript(elidiaCommand)
      if (verifyElidiaCli(elidiaCommand, { shell: shellForProbe })) {
        return {
          label: `existing Elidia CLI at ${elidiaCommand}`,
          command: elidiaCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Elidia CLI at ${elidiaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed elidia_cli module via system Python.
  ...
  if (python) {
    ...
    // having elidia_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\elidia\elidia-agent\venv.
    if (canImportElidiaCli(python)) {
      return {
        kind: 'python',
        label: `installed elidia_cli module via ${python}`,
        command: python,
        args: ['-m', 'elidia_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: elidia_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveElidiaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Elidia Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveElidiaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Elidia install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      elidiaHome: ELIDIA_HOME,
      logRoot: path.join(ELIDIA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Elidia install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Elidia bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ELIDIA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startElidia() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // elidia:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveElidiaBackend(backend.args))
  }
  ...
  if (!isElidiaSourceRoot(ACTIVE_ELIDIA_ROOT)) {
    throw new Error(
      `Elidia install at ${ACTIVE_ELIDIA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Elidia' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\elidia\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `elidia` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Elidia on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Elidia.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isElidiaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Elidia venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Elidia at ${ACTIVE_ELIDIA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Elidia runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Elidia backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Elidia backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Elidia backend after ${timeoutMs}ms`))
    })
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('elidia:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveElidiaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:preview-file-changed', payload)
}
...
async function waitForElidia(baseUrl, token) {
  ...
  throw new Error(`Elidia backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Elidia...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('elidia:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('elidia:window-state-changed', state)
}
...
function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig()) {
  ...
  return {
    ...
    envOverride: Boolean(process.env.ELIDIA_DESKTOP_REMOTE_URL)
  }
}
...
function resolveRemoteBackend() {
  const rawEnvUrl = process.env.ELIDIA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ELIDIA_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ELIDIA_DESKTOP_REMOTE_URL is set but ELIDIA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Elidia backend.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Elidia gateway is selected, but no session token is saved. ' +
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  const remote =
    config.mode === 'remote'
      ...
      : resolveRemoteBackend() || (await startElidia())
...
function resetElidiaConnection() {
  connectionPromise = null

  if (elidiaProcess && !elidiaProcess.killed) {
    elidiaProcess.kill('SIGTERM')
  }
...
async function startElidia() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startElidia() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Elidia backend', 8)
    const remote = resolveRemoteBackend()
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Elidia backend at ${remote.baseUrl}`, 24)
      await waitForElidia(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Elidia backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Elidia runtime', 28)
    const backend = await ensureRuntime(resolveElidiaBackend(dashboardArgs))
    const elidiaCwd = resolveElidiaCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Elidia backend via ${backend.label}`, 84)
    rememberLog(`Starting Elidia backend via ${backend.label}`)

    elidiaProcess = spawn(backend.command, backend.args, {
      cwd: elidiaCwd,
      env: {
        ...process.env,
        // Explicitly pin ELIDIA_HOME for the child so Python's get_elidia_home()
        // resolves to the SAME location our resolveElidiaHome() picked. Without
        // this pin, Python falls back to ~/.elidia on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\elidia, which differs from C:\Users\<u>\.elidia.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets ELIDIA_HOME via setx; the desktop
        // can't reliably do that, so we set it inline for every spawn.
        ELIDIA_HOME,
        ...backend.env,
        ELIDIA_DASHBOARD_SESSION_TOKEN: token,
        ELIDIA_DASHBOARD_TUI: '1',
        ELIDIA_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    elidiaProcess.stdout.on('data', rememberLog)
    elidiaProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    elidiaProcess.once('error', error => {
      rememberLog(`Elidia backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Elidia backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    elidiaProcess.once('exit', (code, signal) => {
      rememberLog(`Elidia backend exited (${signal || code})`)
      elidiaProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Elidia backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Elidia backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentElidiaLog()}`
          )
        )
      }
    })
    ...
    await advanceBootProgress('backend.wait', 'Waiting for Elidia backend to become ready', 90)
    await Promise.race([waitForElidia(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Elidia backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startElidia().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('elidia:connection', async () => startElidia())
ipcMain.handle('elidia:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startElidia() call restarts the
...
ipcMain.handle('elidia:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startElidia() re-runs the full installer (refreshing a broken/partial
  ...
  resetElidiaConnection()
  return { ok: true }
})
ipcMain.handle('elidia:bootstrap:cancel', async () => {
...
ipcMain.handle('elidia:boot-progress:get', async () => bootProgressState)
ipcMain.handle('elidia:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('elidia:connection-config:get', async () => sanitizeDesktopConnectionConfig())
ipcMain.handle('elidia:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('elidia:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config)
})
ipcMain.handle('elidia:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  resetElidiaConnection()
...
ipcMain.on('elidia:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('elidia:requestMicrophoneAccess', async () => {
...
ipcMain.handle('elidia:api', async (_event, request) => {
  const connection = await startElidia()
...
ipcMain.handle('elidia:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'Elidia',
    body: payload?.body || '',
...
ipcMain.handle('elidia:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('elidia:readFileText', async (_event, filePath) => {
...
ipcMain.handle('elidia:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('elidia:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('elidia:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('elidia:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('elidia:saveClipboardImage', async () => {
...
ipcMain.handle('elidia:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('elidia:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('elidia:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('elidia:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('elidia:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveElidiaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('elidia:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: path.join(app.getPath('home'), 'elidia-projects')
}))

ipcMain.handle('elidia:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('elidia:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('elidia:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('elidia:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('elidia:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: elidiaLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Elidia' TUI into
...
function terminalChannel(id, suffix) {
  return `elidia:terminal:${id}:${suffix}`
}
...
ipcMain.handle('elidia:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('elidia:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('elidia:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Elidia.')
  }
...
ipcMain.handle('elidia:terminal:write', (_event, id, data) => {
...
ipcMain.handle('elidia:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('elidia:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('elidia:updates:check', async () =>
...
ipcMain.handle('elidia:updates:apply', async (_event, payload) =>
...
ipcMain.handle('elidia:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('elidia:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Elidia version (the one `release.py` bumps in
// elidia_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Elidia version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveElidiaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'elidia_cli', '__init__.py')
...
ipcMain.handle('elidia:version', async () => ({
  appVersion: resolveElidiaVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  elidiaRoot: resolveUpdateRoot()
}))
...
// mount, ~/Downloads, ~/.elidia/...) -- which means Gatekeeper translocation,
...
function maybeRelocateToApplications() {
  if (!IS_MAC || !IS_PACKAGED || process.env.ELIDIA_DESKTOP_NO_AUTO_MOVE === '1') return false
  try {
...
function maybePinToDock() {
  if (!IS_MAC || !IS_PACKAGED || process.env.ELIDIA_DESKTOP_NO_DOCK_PIN === '1') return
  ...
  // file:///Applications/Elidia.app/ -- NOT a raw POSIX path. A type-0/raw-path
