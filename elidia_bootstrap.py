"""Windows UTF-8 bootstrap for Elidia entry points.
...
should be imported at the very top of every Elidia entry point
(``elidia``, ``elidia-agent``, ``elidia-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import elidia_bootstrap``
# (or ``from elidia_bootstrap import apply_windows_utf8_bootstrap``) at
