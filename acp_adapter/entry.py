"""CLI entry point for the elidia-agent ACP adapter.

Loads environment variables from ``~/.elidia/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ELIDIA_HOME (default ``~/.elidia``)."""
    from elidia_cli.env_loader import load_elidia_dotenv

    elidia_home = get_elidia_home()
    loaded = load_elidia_dotenv(elidia_home=elidia_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", elidia_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="elidia-acp",
        description="Run Elidia Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Elidia version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(elidia_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ElidiaACPAgent  # noqa: F401

    print("Elidia ACP check OK")


def _run_setup() -> None:
    from elidia_cli.main import main as elidia_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "elidia", "model"]
        elidia_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``elidia postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting elidia-agent ACP adapter")
    ...
    import acp
    from .server import ElidiaACPAgent
