"""ACP auth helpers — detect and advertise Elidia authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "elidia-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Elidia runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Elidia can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Elidia.
    ...
    not have Elidia provider credentials configured yet, so Elidia always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Elidia using the currently configured "
