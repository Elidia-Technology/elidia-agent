"""Anthropic Messages API adapter for Elidia Agent.

Translates between Elidia's internal OpenAI-style message format and
...
# Elidia effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Elidia historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Elidia)
    ...
    # 1. Elidia-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Elidia configs.
...
# ── Elidia-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.elidia/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ELIDIA_OAUTH_FILE = get_elidia_home() / ".anthropic_oauth.json"
...
def run_elidia_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Elidia-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Elidia with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from elidia_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_elidia_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Elidia-managed OAuth credentials from ~/.elidia/.anthropic_oauth.json."""
    if _ELIDIA_OAUTH_FILE.exists():
        try:
            data = json.loads(_ELIDIA_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Elidia OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See elidia-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See elidia-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See elidia-agent#13848 (Kimi) and
    elidia-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Elidia Agent", "Claude Code")
                text = text.replace("Elidia agent", "Claude Code")
                text = text.replace("elidia-agent", "claude-code")
                text = text.replace("AiUtils", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Elidia surfaces in its CLI. We
