...
def _state_file() -> Path:
    return get_elidia_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.elidia/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `elidia update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``elidia update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``elidia curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `elidia curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Elidia' background skill CURATOR. This is an "
    ...
    "into ~/.elidia/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: elidia-config-*, elidia-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.elidia/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.elidia/logs/curator/``)
    ...
    authored skill data in ``~/.elidia/skills/``.

    ``ensure_elidia_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_elidia_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every elidia install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: elidia curator status
        keep an umbrella stable: elidia curator pin document-tools
    ...
    lines.append("full report: elidia curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: elidia curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.elidia/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.elidia/skills/.archive/` for "
            "safety and can be restored via `elidia curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.elidia/skills/.archive/`. "
            "Restore any via `elidia curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `elidia curator restore <name>`")
    lines.append("- All archives live under `~/.elidia/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `elidia curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `elidia update` only if "
        ...
        # recorded in state so `elidia curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``elidia model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `elidia model` → auxiliary
