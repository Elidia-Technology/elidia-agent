"""Unified removal contract for every credential source Elidia reads from.

Elidia seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.elidia/.env
    claude_code   — ~/.claude/.credentials.json
    elidia_pkce   — ~/.elidia/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (elidia, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `elidia auth add`
...
unify here is **removal**:
    ...
    ``elidia auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, elidia device_code (partial), elidia_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"elidia"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.elidia/.env  → clear it
    ...
    """
    from elidia_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Elidia.",
            f"  The pool entry is now suppressed — Elidia will ignore "
            f"{env_var} until you run `elidia auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Elidia stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `elidia auth add anthropic` to re-enable if needed.",
    ])


def _remove_elidia_pkce(provider: str, removed) -> RemovalResult:
    """~/.elidia/.anthropic_oauth.json is ours — delete it outright."""
    from elidia_constants import get_elidia_home

    result = RemovalResult()
    oauth_file = get_elidia_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Elidia Anthropic OAuth credentials")
...
def _remove_elidia_device_code(provider: str, removed) -> RemovalResult:
    """Elidia OAuth lives in auth.json providers.elidia — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `elidia auth add elidia` (or any other path that writes providers.elidia)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `elidia auth add elidia` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Elidia: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``elidia auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``elidia auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `elidia model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Elidia auth store is not enough — _seed_from_singletons() would
    ...
    via ``elidia auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `elidia auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `elidia auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Elidia stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `elidia auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="elidia_pkce",
        remove_fn=_remove_elidia_pkce,
        description="~/.elidia/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="elidia", source_id="device_code",
        remove_fn=_remove_elidia_device_code,
        description="auth.json providers.elidia",
    ))
    register(RemovalStep(
