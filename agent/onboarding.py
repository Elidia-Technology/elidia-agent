...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Elidia starts and finds ``~/.openclaw/``.

    Points users at ``elidia claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``elidia claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Elidia, run "
        "`elidia claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `elidia claw cleanup` (renames it to ~/.openclaw.pre-migration — "
