...
as ``kind: backend``) or ``~/.elidia/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``elidia tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ELIDIA_HOME/cache/videos/``, creating parents as needed."""
    from elidia_constants import get_elidia_home

    path = get_elidia_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ELIDIA_HOME/cache/videos/``.
