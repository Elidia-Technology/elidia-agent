...
upstream prefix cache warm.  See ``elidia-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, elidia subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_elidia_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Elidia never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ELIDIA_HOME persona while keeping
    ...
    # Pointer to the elidia-agent skill + docs for user questions about Elidia itself.
    stable_parts.append(ELIDIA_AGENT_HELP_GUIDANCE)
    ...
    elidia_subscription_prompt = _r.build_elidia_subscription_prompt(agent.valid_tool_names)
    if elidia_subscription_prompt:
        stable_parts.append(elidia_subscription_prompt)
    ...
    # Active-profile hint — names the Elidia profile the agent is running
    # under so it doesn't conflate ~/.elidia/skills/ (default profile) with
    # ~/.elidia/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Elidia profile: default. Other profiles (if any) live "
            "under ~/.elidia/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Elidia profile: {active_profile}. This session reads "
            f"and writes ~/.elidia/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.elidia/skills/, ~/.elidia/plugins/, "
            f"~/.elidia/cron/, ~/.elidia/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Elidia never rebuilds or reinjects parts of it
