...
def _elidia_home_path() -> Path:
    """Resolve the active ELIDIA_HOME (profile-aware) without circular imports."""
    try:
        from elidia_constants import get_elidia_home  # local import to avoid cycles
        return get_elidia_home()
    except Exception:
        return Path(os.path.expanduser("~/.elidia"))


def _elidia_root_path() -> Path:
    """Resolve the Elidia root dir (always the parent of any profile, never per-profile)."""
    try:
        from elidia_constants import get_default_elidia_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    elidia_home = _elidia_home_path()
    elidia_root = _elidia_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(elidia_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(elidia_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ELIDIA_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ELIDIA_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Elidia control-plane files: block both the ACTIVE profile's view
    # (elidia_home) AND the global root view. Without the root pass, a
    ...
    elidia_dirs = []
    for base in (_elidia_home_path(), _elidia_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in elidia_dirs:
                elidia_dirs.append(real)
        except Exception:
            continue

    for base_real in elidia_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Elidia path.

    Three categories are blocked:

      * Internal Elidia cache files under ``ELIDIA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ELIDIA_HOME and the global Elidia
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.elidia/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ELIDIA_HOME (profile-aware) AND the global
    # Elidia root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ELIDIA_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    elidia_dirs: list[Path] = []
    for base in (_elidia_home_path(), _elidia_root_path()):
        try:
            real = base.resolve()
            if real not in elidia_dirs:
    ...
    for hd in elidia_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Elidia cache file "
    ...
    for hd in elidia_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is an Elidia credential store "
    ...
    for hd in elidia_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Elidia MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is an Elidia MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Elidia profiles are separate ELIDIA_HOME dirs under
...
#
# Reference: May 2026 incident where an elidia-security profile session
# edited skills under both ``~/.elidia/profiles/elidia-security/skills/``
# AND ``~/.elidia/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ELIDIA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ELIDIA_HOME.
    ...
    """
    try:
        home_real = _elidia_home_path().resolve()
        root_real = _elidia_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Elidia scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Elidia entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Elidia profile {info['target_profile']!r}, but the "
...
#
#   <ELIDIA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.elidia/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.elidia/…`` segment and warns
# regardless of which Elidia profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.elidia/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.elidia`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.elidia/…`` and returns the
    index where the inner Elidia-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .elidia / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".elidia":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Elidia state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.elidia``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.elidia`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Elidia resolver to
    succeed, so it works correctly even when called from contexts where
    ELIDIA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Elidia process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ELIDIA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.elidia/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.elidia/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ELIDIA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Elidia state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Elidia "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ELIDIA_HOME. Use the "
