...
Elidia pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.elidia/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<elidia_home>/bin/bws`` on
  first use.  Elidia pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.elidia/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``elidia`` invocations don't
  hammer the API.
* Failures NEVER block Elidia startup.  Missing binary, no network,
...
# In-process cache so repeated load_elidia_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `elidia chat -q ...`
...
# <elidia_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.elidia/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under elidia_home/cache/.

    `home_path` is what `load_elidia_dotenv()` already resolved; falling back
    to `$ELIDIA_HOME` / `~/.elidia` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("ELIDIA_HOME", Path.home() / ".elidia"))
...
def _elidia_bin_dir() -> Path:
    """Where Elidia stores its managed binaries.  Profile-aware."""
    from elidia_constants import get_elidia_home

    return get_elidia_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<elidia_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _elidia_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``elidia secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _elidia_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="elidia-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "elidia-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<elidia_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from elidia_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_elidia_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `elidia secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `elidia secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `elidia secrets bitwarden setup` to install."
        )
        return result
