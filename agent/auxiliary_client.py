...
Resolution order for text tasks (auto mode):
  ...
  3. Elidia Portal (~/.elidia/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Elidia triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://aiutils.io",
    "X-Title": "Elidia Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ELIDIA_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ELIDIA_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ELIDIA_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ELIDIA_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ElidiaAgent",
}
...
# Elidia Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Elidia Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with elidia_cli.__version__ across every Portal call site
...
def _elidia_extra_body() -> dict:
    """Return a fresh Elidia Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``elidia_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _elidia_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``ELIDIA_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_elidia_extra_body()`` or import ``elidia_portal_tags`` directly.
ELIDIA_EXTRA_BODY = _elidia_extra_body()

# Set at resolve time — True if the auxiliary client points to Elidia Portal
auxiliary_is_elidia: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_ELIDIA_MODEL = "google/gemini-3-flash-preview"
_ELIDIA_DEFAULT_BASE_URL = "https://inference-api.aiutils.io/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_elidia_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Elidia Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for elidia).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. elidia prefers inference_base_url).
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``elidia_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_elidia_auth() -> Optional[dict]:
    """Read and validate ~/.elidia/auth.json for an active Elidia provider.

    Returns the provider state dict if Elidia is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("elidia")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _ELIDIA_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "elidia":
            return None
        provider = data.get("providers", {}).get("elidia", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Elidia auth: %s", exc)
        return None


def _elidia_api_key(provider: dict) -> str:
    """Extract a usable Elidia inference JWT from stored auth state."""
    from elidia_cli.auth import _elidia_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _elidia_invoke_jwt_is_usable(
            token,
...
def _elidia_base_url() -> str:
    """Resolve the Elidia inference base URL from env or default."""
    return os.getenv("ELIDIA_INFERENCE_BASE_URL", _ELIDIA_DEFAULT_BASE_URL)


def _resolve_elidia_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Elidia runtime credentials when available.
    ...
    """
    try:
        from elidia_cli.auth import resolve_elidia_runtime_credentials

        creds = resolve_elidia_runtime_credentials(
            timeout_seconds=float(os.getenv("ELIDIA_AIUTILS_TIMEOUT_SECONDS", "15")),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Elidia runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``elidia auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``elidia_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from elidia_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Elidia auth store.
    ...
    try:
        from elidia_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from elidia_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from elidia_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from elidia_cli.models import copilot_default_headers
...
def _try_elidia(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Elidia —
    # if another session already recorded a 429, skip Elidia entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.elidia_rate_guard import elidia_rate_limit_remaining
        _remaining = elidia_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Elidia Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("elidia", ttl=_remaining)
            return None, None
    except Exception:
        pass

    elidia = _read_elidia_auth()
    runtime = _resolve_elidia_runtime_api(force_refresh=False)
    if runtime is None and not elidia:
        logger.warning(
            "Auxiliary Elidia client unavailable: no Elidia authentication found "
            "(run: elidia auth)."
        )
        _mark_provider_unhealthy("elidia", ttl=60)
        return None, None
    if runtime is None and elidia:
        logger.debug(
            "Auxiliary Elidia: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_elidia
    auxiliary_is_elidia = True
    logger.debug("Auxiliary client: Elidia Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/elidia/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_elidia_recommended_aux_model
    # auto-detects the caller's tier via check_elidia_free_tier().  Fall back to
    # _ELIDIA_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _ELIDIA_MODEL
    try:
        from elidia_cli.models import get_elidia_recommended_aux_model
        recommended = get_elidia_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _elidia_api_key(elidia or {})
        if not api_key:
            logger.warning(
                "Auxiliary Elidia client unavailable: no usable inference JWT found "
                "(run: elidia auth add elidia)."
            )
            _mark_provider_unhealthy("elidia", ttl=60)
            return None, None
        base_url = str((elidia or {}).get("inference_base_url") or _elidia_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _refresh_elidia_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the Elidia Portal's recommended model after a stale-model 404.
    ...
    the Elidia → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_ELIDIA_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_elidia_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "Elidia recommended-model refresh failed (%s); using default %s",
            exc, _ELIDIA_MODEL,
        )
    ...
    if _ELIDIA_MODEL.strip().lower() != stale:
        return _ELIDIA_MODEL
    return None
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `elidia setup` or `elidia model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_elidia`` shape but delegates to
    :func:`elidia_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from elidia_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second elidia
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``elidia config aux reset``)."""
...
def _elidia_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Elidia account API says paid access is allowed."""
    try:
        from elidia_cli.elidia_account import get_elidia_portal_account_info

        account_info = get_elidia_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Elidia paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Elidia → OpenRouter catalog. The Elidia
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.aiutils.io"):
        return "elidia"
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "elidia": "elidia",
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Elidia get their chosen chat model; users
      ...
      2. OpenRouter → Elidia → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_elidia, _stale_base_url_warned
    auxiliary_is_elidia = False  # Reset — _try_elidia() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `elidia model` but the
    #    old OPENAI_BASE_URL lingers in ~/.elidia/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: elidia model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.elidia/.env",
    ...
    # on aggregators (OpenRouter, Elidia) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from elidia_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from elidia_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "elidia", "openai-codex" (or "codex"),
    ...
    if provider == "elidia":
        ...
        client, default = _try_elidia(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: elidia requested "
                           "but Elidia Portal not configured (run: elidia auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: elidia model)")
                return None, None
    ...
    # OpenRouter / Elidia bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: elidia model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from elidia_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``elidia``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: elidia doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("elidia_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from elidia_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "elidia":
            return resolve_provider_client("elidia", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "elidia":
        return _try_elidia(vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Elidia → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Elidia — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Elidia Portal product tags when the auxiliary client is backed
    by Elidia Portal. Returns empty dict otherwise.
    """
    return _elidia_extra_body() if auxiliary_is_elidia else {}
...
def _refresh_elidia_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Elidia runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_elidia_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`elidia_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if provider == "elidia" or auxiliary_is_elidia:
        merged_extra.setdefault("tags", []).extend(_elidia_portal_tags())
    if merged_extra:
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `elidia model`."
                )
    ...
    try:
        ...
        # ── Stale-model self-heal (Elidia Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Elidia → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Elidia-routed calls.
        _heal_is_elidia = (
            resolved_provider == "elidia"
            or base_url_host_matches(_base_info, "inference-api.aiutils.io")
        )
        if _is_model_not_found_error(first_err) and _heal_is_elidia:
            healed_model = _refresh_elidia_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Elidia catalog; "
        ...
        # ── Elidia auth refresh parity with main agent ──────────────────
        client_is_elidia = (
            resolved_provider == "elidia"
            or base_url_host_matches(_base_info, "inference-api.aiutils.io")
        )
        if (
            ...
            and client_is_elidia
            and _elidia_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_elidia_auxiliary_client(
                cache_provider=resolved_provider or "elidia",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Elidia runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_elidia:
            refreshed_client, refreshed_model = _refresh_elidia_auxiliary_client(
                cache_provider=resolved_provider or "elidia",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Elidia runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the Elidia → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_elidia = (
            resolved_provider == "elidia"
            or base_url_host_matches(_client_base, "inference-api.aiutils.io")
        )
        if _is_model_not_found_error(first_err) and _heal_is_elidia:
            healed_model = _refresh_elidia_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Elidia catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Elidia runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_elidia:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Elidia runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
