...
``kind: backend``) or ``~/.elidia/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``elidia tools``. Defaults to ``name``."""
        return self.name
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`elidia_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Elidia gating, and the
