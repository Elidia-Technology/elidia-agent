...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Elidia also honours ELIDIA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ELIDIA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "elidia", "openai-codex", "copilot", "copilot-acp",
...
# Minimum context length required to run Elidia Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/Elidia minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Elidia from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.aiutils.io": "elidia",
    ...
    # (often 8K) makes it unusable for Elidia' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    return get_elidia_home() / "context_length_cache.yaml"
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/Elidia minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Elidia grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Elidia uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_elidia_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Elidia Portal model context length.

    Tries the live Elidia inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Elidia model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Elidia /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, Elidia path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Elidia URLs
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Elidia: live /v1/models probe first (authoritative), then OR
    ...
    if custom_providers and base_url and model:
        try:
            from elidia_cli.config import get_custom_provider_context_length
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Elidia Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "elidia":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Elidia portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from elidia_cli.models import get_copilot_model_context
    ...
    if effective_provider == "elidia":
        ctx, source = _resolve_elidia_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Elidia URLs.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Elidia sends to providers:
