...
Storage
-------
Bundles live in ``~/.elidia/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ELIDIA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``elidia bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ELIDIA_HOME.

    Honors ``ELIDIA_BUNDLES_DIR`` for tests; falls back to
    ``<ELIDIA_HOME>/skill-bundles``.
    """
    override = os.environ.get("ELIDIA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_elidia_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `elidia bundles` CLI subcommand.
# ---------------------------------------------------------------------------
