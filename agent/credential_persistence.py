...
``auth.json``.  They intentionally have no dependency on ``elidia_cli.auth`` so
...
# Sources Elidia owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Elidia-owned OAuth/device-code state)
