...
Owns one Codex thread per Elidia session. Drives `turn/start`, consumes
...
# Elidia' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ELIDIA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Elidia/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Elidia session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ELIDIA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ELIDIA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="elidia",
            client_title="Elidia Agent",
            client_version=_get_elidia_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Elidia' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Elidia' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own elidia-tools callback we
            # auto-accept — the user already approved Elidia' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Elidia approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Elidia returns 'once', 'session', 'always', or 'deny'.
...
def _get_elidia_version() -> str:
    """Best-effort Elidia version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("elidia-agent")
