...
from elidia_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ELIDIA_PLATFORM`` or
            ``ELIDIA_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``elidia`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.elidia/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``elidia`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ELIDIA_HOME, not cwd
        if not p.is_absolute():
            p = (elidia_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.elidia/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    elidia = metadata.get("elidia") or {}
    if not isinstance(elidia, dict):
        elidia = {}
    return {
        "fallback_for_toolsets": elidia.get("fallback_for_toolsets", []),
        "requires_toolsets": elidia.get("requires_toolsets", []),
        "fallback_for_tools": elidia.get("fallback_for_tools", []),
        "requires_tools": elidia.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    elidia = metadata.get("elidia")
    if not isinstance(elidia, dict):
        return []
    raw = elidia.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Elidia metadata, VCS, virtualenv/dependency, and cache
