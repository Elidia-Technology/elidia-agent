...
Plugins built on Elidia Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Elidia
...
:class:`~elidia_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Elidia' main config
...
handles every provider, fallback chain, and per-task override Elidia
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`elidia_cli.plugins.PluginContext`
