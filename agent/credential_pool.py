...
from elidia_cli.config import load_env
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "elidia":
            # Elidia stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._elidia_invoke_jwt_is_usable(
                        token,
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``elidia model`` / ``elidia auth`` which
        ...
        Mirrors the Elidia/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Elidia process
    ...
    def _sync_elidia_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync an Elidia pool entry from auth.json if tokens differ.

        Elidia OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_elidia_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Elidia Portal.
        """
        if self.provider != "elidia" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Elidia state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Elidia entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Elidia, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Elidia/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``elidia`` invocation that defaults to the active provider
        (e.g. setup wizard, ``elidia auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                if self.provider == "elidia":
                    ...
                    _store_provider_state(auth_store, "elidia", state, set_active=False)
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("elidia_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Elidia
            ...
            elif self.provider == "elidia":
                synced = self._sync_elidia_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_elidia_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_elidia_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("elidia_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as elidia — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the Elidia quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/elidia — another Elidia process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Elidia quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For elidia: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "elidia":
                synced = self._sync_elidia_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Elidia refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_elidia_refresh_error(exc):
                    logger.debug("Elidia refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "elidia") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_elidia_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_elidia_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Elidia OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.ELIDIA_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.ELIDIA_DEVICE_CODE_SOURCE}",
                    }
                    self._entries = [
                        item for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "elidia":
            # Elidia refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Elidia profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For elidia entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_elidia_runtime_credentials(), making this entry's
            ...
            # re-authed via `elidia model` / `elidia auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `elidia model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``elidia auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `elidia auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `elidia auth remove <provider> <N>` is stable across all source types.
    try:
        from elidia_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Elidia PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `elidia setup` ("Claude
        ...
        # Claude Code CLI, elidia_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.elidia/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `elidia setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"elidia_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_elidia_oauth_credentials

        for source_name, creds in (
            ("elidia_pkce", read_elidia_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_elidia_credentials(label=...) when the user ran
            # `elidia auth add elidia --label <name>`).  Fall back to the
        ...
        # Elidia auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from elidia_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.elidia/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `elidia auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `elidia auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Elidia auth store.  Without this gate the removal is instantly
        ...
        # Elidia owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``elidia model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``elidia auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.elidia/.env over os.environ — the user's config file is the
    # authoritative source for Elidia credentials. Stale env vars from parent
    ...
    # Honour user suppression — `elidia auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.elidia/.env.
    ...
    try:
        from elidia_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from elidia_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.elidia/.env over os.environ
