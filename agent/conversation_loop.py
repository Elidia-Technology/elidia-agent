...
from elidia_constants import PARTIAL_STREAM_STUB_ID
from elidia_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Elidia tool use: "
    ...
    return (
        ...
        f"context, but Elidia needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Elidia config, set `model.ollama_num_ctx: 65536` "
...
def _elidia_entitlement_message(capability: str) -> str:
    try:
...
def _print_elidia_entitlement_guidance(agent, capability: str) -> bool:
    message = _elidia_entitlement_message(capability)
    if not message:
        return False
...
def _is_elidia_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.aiutils.io")
        or base_url_host_matches(base, "inference.aiutils.io")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_elidia_inference_route(provider, base_url):
        return _elidia_entitlement_message(capability)
...
def _try_refresh_elidia_paid_entitlement_credentials(agent) -> bool:
    """Refresh Elidia runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_elidia_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def _provider_profile_streaming_disabled(agent) -> bool:
    """True when the provider's declarative profile disables OpenAI-style SSE.

    Some providers (e.g. AiUtils Developer API) stream only on a dedicated
    endpoint and return a single non-stream object for ``stream=true`` on the
    standard ``/chat/completions`` path — which the OpenAI SDK then yields as
    zero chunks (empty reply). Honor the profile's ``supports_streaming`` flag
    so these providers use the non-streaming path.
    """
    try:
        ...
        return False
    return profile is not None and not getattr(profile, "supports_streaming", True)
...
def run_conversation(
    ...
    # ``elidia logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Elidia's territory; plugins
    ...
    # all run inside Codex). Default Elidia path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Elidia internals.
        #
        # Elidia invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Elidia tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        elidia_auth_retry_attempted=False
        elidia_paid_entitlement_refresh_attempted=False
        ...
        while retry_count < max_retries:
            # ── Elidia Portal rate limit guard ──────────────────────
            # If another session already recorded that Elidia is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "elidia":
                try:
                    from agent.elidia_rate_guard import (
                        elidia_rate_limit_remaining,
                        format_remaining as _fmt_elidia_remaining,
                    )
                    _elidia_remaining = elidia_rate_limit_remaining()
                    if _elidia_remaining is not None and _elidia_remaining > 0:
                        _elidia_msg = (
                            f"Elidia Portal rate limit active — "
                            f"resets in {_fmt_elidia_remaining(_elidia_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_elidia_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_elidia_msg}")
            ...
            try:
                ...
                if env_var_enabled("ELIDIA_DUMP_REQUESTS"):
                ...
                # Declarative profile flag: provider has no OpenAI-style SSE
                # on /chat/completions (e.g. AiUtils Developer API streams on a
                # dedicated endpoint only) — use non-streaming.
                elif _provider_profile_streaming_disabled(agent):
                    _use_streaming = False
                ...
                # Clear Elidia rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Elidia.
                if agent.provider == "elidia":
                    try:
                        from agent.elidia_rate_guard import clear_elidia_rate_limit
                        clear_elidia_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    and _is_elidia_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not elidia_paid_entitlement_refresh_attempted
                ):
                    elidia_paid_entitlement_refresh_attempted = True
                    if _try_refresh_elidia_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Elidia paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "elidia"
                    and status_code == 401
                    and not elidia_auth_retry_attempted
                ):
                    elidia_auth_retry_attempted = True
                    if agent._try_refresh_elidia_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Elidia agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Elidia 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_elidia_entitlement_guidance(agent, "Elidia model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: elidia auth add elidia")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://developer.aiutils.io")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `elidia doctor` for credential-chain diagnostics, or")
                    ...
                    from elidia_constants import display_elidia_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Elidia-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: elidia config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: elidia config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Elidia Portal: record rate limit & skip retries ─────
                # When Elidia returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Elidia Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Elidia).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Elidia model for minutes.  We use
                # ``is_genuine_elidia_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_elidia_rate_limit = False
                    try:
                        from agent.elidia_rate_guard import (
                            is_genuine_elidia_rate_limit,
                            record_elidia_rate_limit,
                        )
                        ...
                        _genuine_elidia_rate_limit = is_genuine_elidia_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_elidia_rate_limit:
                            record_elidia_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Elidia 429 looks like upstream capacity "
                ...
                # which Elidia' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Elidia' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`elidia",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "elidia" and _print_elidia_entitlement_guidance(
                            agent,
                            "Elidia model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "elidia"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `elidia auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `elidia model`.", force=True)
                            else:  # elidia
                                agent._vprint(f"{agent.log_prefix}   💡 Elidia Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: elidia portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://developer.aiutils.io", force=True)
                                # ``:free`` is OpenRouter slug syntax; Elidia Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Elidia Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Elidia catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: elidia setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        elidia fallback add   (interactive picker — same as `elidia model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not an Elidia/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `elidia fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("ELIDIA_KANBAN_TASK")
        if _kanban_task:
            try:
