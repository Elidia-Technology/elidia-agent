...
as ``kind: backend``) or ``~/.elidia/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``elidia tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``elidia tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``elidia tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ELIDIA_HOME/cache/images/``, creating parents as needed."""
    from elidia_constants import get_elidia_home

    path = get_elidia_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ELIDIA_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$ELIDIA_HOME/cache/images/``.
