...
from elidia_constants import get_elidia_home, get_skills_dir, is_wsl
...
_ELIDIA_MD_NAMES = (".elidia.md", "ELIDIA.md")


def _find_elidia_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.elidia.md`` or ``ELIDIA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Elidia Agent, an intelligent AI assistant created by AiUtils. "
...
ELIDIA_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Elidia Agent "
    "itself, load the `elidia-agent` skill with skill_view(name='elidia-agent') "
    "before answering. Docs: https://aiutils.io/elidia"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.elidia/kanban.db`. Your task id is in "
    "`$ELIDIA_KANBAN_TASK`; your workspace is `$ELIDIA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ELIDIA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `elidia kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Elidia WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Elidia itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Elidia runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Elidia itself is running. The host OS, home, and cwd "
                f"of the Elidia process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Elidia "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Elidia
    ...
    extra = (os.getenv("ELIDIA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_elidia_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.elidia/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ELIDIA_PLATFORM")
        or get_session_env("ELIDIA_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Elidia Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `elidia-agent` skill "
            "first. It has the actual commands (e.g. `elidia config set …`, `elidia tools`, "
            "`elidia setup`) so you don't have to guess or invent workarounds.\n"
...
def build_elidia_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Elidia subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Elidia subscription helper: %s", exc)
        return ""

    if not managed_elidia_tools_enabled():
        return ""
    ...
    features = get_elidia_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_elidia:
            return f"- {feature.label}: active via Elidia subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.elidia_auth_present:
            return f"- {feature.label}: included with Elidia subscription, not currently selected"
        if feature.key == "modal" and features.elidia_auth_present:
            return f"- {feature.label}: optional via Elidia subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Elidia Subscription",
        "Elidia subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When an Elidia-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Elidia subscription would unlock or simplify, suggest Elidia subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: elidia setup, elidia setup tools, elidia setup terminal, elidia status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ELIDIA_HOME and return its content, or None.
    ...
    """
    try:
        from elidia_cli.config import ensure_elidia_home
        ensure_elidia_home()
    except Exception as e:
        logger.debug("Could not ensure ELIDIA_HOME before loading SOUL.md: %s", e)

    soul_path = get_elidia_home() / "SOUL.md"
...
def _load_elidia_md(cwd_path: Path) -> str:
    """.elidia.md / ELIDIA.md — walk to git root."""
    elidia_md_path = _find_elidia_md(cwd_path)
    if not elidia_md_path:
        return ""
    try:
        content = elidia_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = elidia_md_path.name
        try:
            rel = str(elidia_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".elidia.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", elidia_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .elidia.md / ELIDIA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ELIDIA_HOME is independent and always included when present.
    ...
    # SOUL.md from ELIDIA_HOME only — skip when already loaded as identity
    if not skip_soul:
