"""Cross-session rate limit guard for Elidia Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Elidia Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Elidia triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Elidia retries), and every one of those calls counts
...
_STATE_FILENAME = "elidia.json"


def _state_path() -> str:
    """Return the path to the Elidia rate limit state file."""
    try:
        from elidia_constants import get_elidia_home
        base = get_elidia_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".elidia")
...
def record_elidia_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Elidia Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Elidia rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Elidia rate limit state: %s", exc)


def elidia_rate_limit_remaining() -> Optional[float]:
    """Check if Elidia Portal is currently rate-limited.
...
def clear_elidia_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Elidia request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Elidia rate limit state: %s", exc)
...
def is_genuine_elidia_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Elidia Portal is a real account rate limit.

    Elidia Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Elidia, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Elidia is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Elidia.

    Tripping the cross-session breaker on (b) blocks ALL Elidia requests
    (and all models, since Elidia is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Elidia emits
