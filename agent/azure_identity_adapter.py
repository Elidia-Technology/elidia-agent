...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Elidia does
...
class EntraIdentityConfig:
    ...
    Captures the Elidia-managed Entra knobs we need outside Azure SDK
    ...
    ``elidia_cli/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Elidia-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.elidia/.env`` or the deployment environment.
    """
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``elidia doctor`` /
    ``elidia auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``elidia doctor`` and the wizard preflight — never
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``elidia_cli/azure_detect.py``).
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `elidia doctor` or `az login` to recover.",
                exc,
            )
