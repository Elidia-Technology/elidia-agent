...
Usage (via cron with --no-agent):
    ...
    elidia cron create elidia-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ELIDIA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name elidia-issues --repo AiUtils/elidia-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.elidia/.env to avoid the 60 req/hr
