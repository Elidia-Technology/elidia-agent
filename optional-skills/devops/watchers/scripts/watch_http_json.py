...
Usage (via cron with --no-agent):
    ...
    elidia cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ELIDIA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Elidia-Watcher/1.0"})
