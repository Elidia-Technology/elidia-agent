...
Maps each role archetype to the Elidia skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public elidia-agent
...
## Elidia skills relevant to video production

### Visual / rendering skills (`elidia-agent/skills/creative/`)
...
### Design / pre-production skills (`elidia-agent/skills/creative/`)
...
### Audio / media skills (`elidia-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`elidia-agent/skills/devops/`)
...
These are **not** Elidia skills but external CLIs / APIs that profiles invoke.
...
## Built-in Elidia tools for media review

These are native Elidia tools — not invoked via terminal but through their own
...
key is present in `~/.elidia/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Elidia profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.elidia/.env` → environment variable.
...
When a new Elidia-public video skill ships:
