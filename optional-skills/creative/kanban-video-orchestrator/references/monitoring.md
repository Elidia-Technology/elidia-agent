...
elidia kanban watch --tenant <project-slug>

# Snapshot of the board
elidia kanban list --tenant <project-slug>
elidia kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
elidia kanban stats --tenant <project-slug>

# Visual dashboard (browser)
elidia dashboard

# Inspect a specific task (includes comments + events)
elidia kanban show <task-id>

# Follow a single task's event stream
elidia kanban tail <task-id>
```

Verify available subcommands with `elidia kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `elidia kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `elidia kanban block <id>` then `elidia kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `elidia kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `elidia kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `elidia profile list` (profile exists?), `elidia status` (gateway/dispatcher up?), and verify tenant. |
...
elidia kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
elidia kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(elidia kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
elidia kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
elidia kanban block <task-id>
elidia kanban archive <task-id>

# Diagnose what happened
elidia kanban show <task-id>      # task body, comments, recent events
elidia kanban tail <task-id>      # follow the live event stream
elidia kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `elidia kanban create` for the director
...
while true; do
    clear
    elidia kanban list --tenant <slug>
    echo "---"
    elidia kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `elidia kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `elidia profile list` and start
