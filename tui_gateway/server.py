...
load_elidia_dotenv(
    elidia_home=_elidia_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.elidia/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_elidia_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("ELIDIA_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ELIDIA_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent ElidiaCLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `elidia` shows the prompt before constructing AIAgent; the TUI used
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ELIDIA_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from elidia_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ELIDIA_MODEL", "")
        or os.environ.get("ELIDIA_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("ELIDIA_TUI_PROVIDER", "").strip()
    ...
    try:
        from elidia_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ELIDIA_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ELIDIA_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ELIDIA_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ELIDIA_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from elidia_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ELIDIA_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ELIDIA_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ELIDIA_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ELIDIA_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from elidia_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from elidia_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["ELIDIA_MODEL"] = result.new_model
    os.environ["ELIDIA_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # ELIDIA_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["ELIDIA_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["ELIDIA_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from elidia_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from elidia_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from elidia_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from elidia_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ELIDIA_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ELIDIA_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ELIDIA_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ELIDIA_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ELIDIA_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ELIDIA_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ELIDIA_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Elidia profile home
    # (~/.elidia/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ELIDIA_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_elidia_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from elidia_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from elidia_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: elidia_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from elidia_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _elidia_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Elidia Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Elidia/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Elidia Desktop. If it serves Elidia/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "yolo":
        ...
        try:
            if session:
                ...
                current = is_truthy_value(os.environ.get("ELIDIA_YOLO_MODE"))
                if current:
                    os.environ.pop("ELIDIA_YOLO_MODE", None)
                    nv = "0"
                else:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from elidia_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_elidia_home), "display": display_elidia_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from elidia_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from elidia_cli.runtime_provider import resolve_runtime_provider
        from elidia_cli.auth import has_usable_secret
        from elidia_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Elidia provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.elidia/.env`` into the gateway process via
    ``elidia_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `elidia` is interactive — use `/elidia chat -q …` or run `elidia` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`elidia setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`elidia gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`elidia sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`elidia config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m elidia_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "elidia_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from elidia_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from elidia_cli.config import is_managed, save_env_value
        from elidia_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `elidia model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.elidia/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from elidia_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from elidia_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ELIDIA_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ELIDIA_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ELIDIA_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from elidia_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["ELIDIA_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from elidia_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from elidia_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from elidia_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from elidia_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ELIDIA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ELIDIA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_elidia_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from elidia_cli.skills_hub import do_install
        ...
        if action == "browse":
            from elidia_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from elidia_cli.skills_hub import inspect_skill
