"""Persistent slash-command worker — one ElidiaCLI per TUI session.
...
def _run(cli: ElidiaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ELIDIA_SESSION_KEY"] = args.session_key
    os.environ["ELIDIA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ElidiaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
