...
def _profile_user_agent() -> str:
    """Return a ``elidia-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from elidia_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"elidia-cli/{_ver}"
    except Exception:
        return "elidia-cli"
...
class ProviderProfile:
    ...
    supports_streaming: bool = True  # False → transport uses non-streaming; provider has no OpenAI-style SSE on /chat/completions
    ...
    # default_model: the model used when the user hasn't picked one for this
    # provider. Empty = fall back to fallback_models[0] (or the static
    # _PROVIDER_MODELS entry) when resolving get_default_model_for_provider().
    default_model: str = ""
    ...
    def fetch_models(
        ...
        *,
        ...
        # elidia-cli UA so the catalog endpoint is reachable.
