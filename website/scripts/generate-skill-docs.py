...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Elidia-Technology/elidia-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    elidia_meta = (fm.get("metadata") or {}).get("elidia") or {}
    tags = elidia_meta.get("tags") or []
    related = elidia_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `elidia skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Elidia Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Elidia ships with a large built-in skill library copied into `~/.elidia/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Elidia also syncs bundled skills on `elidia update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.elidia/skills/` tree, it is still shipped with Elidia; restore it with `elidia skills reset <name> --restore`.",
        "",
