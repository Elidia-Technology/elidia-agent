#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Elidia docs site.
...
Both publish at:
  https://aiutils.io/elidia/llms.txt
  https://aiutils.io/elidia/llms-full.txt
...
SITE_BASE = "https://aiutils.io/elidia"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Elidia as a Python Library", None),
        ("guides/use-mcp-with-elidia", "Use MCP with Elidia", None),
        ("guides/use-voice-mode-with-elidia", "Use Voice Mode with Elidia", None),
        ("guides/use-soul-with-elidia", "Use SOUL.md with Elidia", None),
        ("guides/build-a-elidia-plugin", "Build an Elidia Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Elidia"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Elidia Agent")
    lines.append("")
    lines.append(
        "> The multi-platform AI agent from AiUtils.io. A terminal-native "
        ...
        "backends. Works with Elidia Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Elidia-Technology/"
        "elidia-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Elidia-Technology/elidia-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Elidia Agent — Full Documentation\n",
        (
            "This file is the entire Elidia Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Elidia, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://aiutils.io/elidia\n",
        "Short index: https://aiutils.io/elidia/llms.txt\n",
        "\n---\n\n",
    ]
