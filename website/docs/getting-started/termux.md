...
description: "Run Elidia Agent directly on an Android phone with Termux"
---

# Elidia on Android with Termux

This is the tested path for running Elidia Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Elidia from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Elidia now ships a Termux-aware installer path:
...
- links `elidia` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `elidia` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/elidia" "$PREFIX/bin/elidia"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `elidia` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.elidia/.env`.
...
### `elidia doctor` says ripgrep or Node is missing
