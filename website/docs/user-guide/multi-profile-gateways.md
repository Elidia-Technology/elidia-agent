...
If you only run one Elidia agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Elidia agents that should all
...
(`ai.elidia.gateway-<name>.plist`) or systemd user service
(`elidia-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
elidia profile create coder
elidia profile create personal-bot
elidia profile create research
...
`~/.local/bin/elidia-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: elidia-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    elidia gateway "$action"
  else
    elidia -p "$profile" gateway "$action"
  fi
}
...
```bash
elidia-gateways start      # start every configured profile
elidia-gateways stop       # stop every configured profile
elidia-gateways restart    # restart all
elidia-gateways status     # status across all
elidia-gateways list       # delegates to `elidia gateway list`
```

:::tip
The `default` profile is targeted with `elidia gateway <action>` (no `-p`),
not `elidia -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `elidia -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.elidia.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/elidia-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.elidia.gateway.plist` /
`elidia-gateway.service`.
...
```bash
# Default profile
tail -f ~/.elidia/logs/gateway.log
tail -f ~/.elidia/logs/gateway.error.log

# Named profile
tail -f ~/.elidia/profiles/<name>/logs/gateway.log
tail -f ~/.elidia/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.elidia/logs/gateway.log ~/.elidia/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
elidia logs --tail              # follow default profile
elidia -p coder logs --tail     # follow one profile
elidia logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
elidia profile list             # profiles + model + gateway state
elidia-gateways status          # full status across every profile
launchctl list | grep elidia    # macOS — PIDs and labels
systemctl --user list-units 'elidia-gateway-*'   # Linux — units
```
...
```
~/.elidia/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.elidia/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
elidia config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.elidia/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=elidia --why="gateways running" \
  sleep infinity &
...
`elidia-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.elidia/.env ~/.elidia/profiles/*/.env
```

## Updating the code

`elidia update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `elidia gateway start` after a previous `elidia gateway stop`. The
...
```bash
ps -ef | grep "elidia_cli.*-p <profile>"
cat ~/.elidia/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.elidia.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.elidia.gateway-<profile>.plist

# Linux
systemctl --user restart elidia-gateway-<profile>.service
```

### Health check

```bash
elidia doctor                  # default profile
elidia -p <profile> doctor     # one profile
```
