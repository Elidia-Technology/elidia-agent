...
title: "Git Worktrees"
description: "Run multiple Elidia agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Elidia Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Elidia so each session has a clean, isolated working directory.

## Why Use Worktrees with Elidia?

Elidia treats the **current working directory** as the project root:

- CLI: the directory where you run `elidia` or `elidia chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.elidia/config.yaml`
...
git worktree add ../repo-feature feature/elidia-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/elidia-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Elidia there:

```bash
cd ../repo-feature

# Start Elidia in the worktree
elidia
```
...
git worktree add ../repo-experiment-a feature/elidia-a
git worktree add ../repo-experiment-b feature/elidia-b
```
...
- Works on its own branch (`feature/elidia-a` vs `feature/elidia-b`).
...
- Elidia checkpoint data under `~/.elidia/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Elidia experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/elidia-checkpoints-docs`, `feature/elidia-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Elidia from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `elidia -w` (Automatic Worktree Mode)

Elidia has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `elidia/elidia-<hash>`).
...
```bash
elidia -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `elidia -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Elidia session its own clean checkout.
