...
Elidia Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.elidia/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Elidia stores session history so it can resume conversations, but it does not
...
Elidia explicitly injects for that turn.
...
For example, if a user sends an image and asks Elidia to make a meme from it,
Elidia may inspect that image once with vision and run an image-processing
...
`elidia sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`elidia` or `elidia chat`) |
...
elidia -c "my project"   # → resumes "my project #3"
```
...
elidia --resume 20250305_091523_a1b2c3d4
elidia -r 20250305_091523_a1b2c3d4

# Resume by title
elidia --resume "refactoring auth"

# Or with the chat subcommand
elidia chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `elidia sessions list`.

### Conversation Recap on Resume

When you resume a session, Elidia displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming an Elidia session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.elidia/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `elidia -r "<title>"` from the shell) and pick up where the platform left off.
...
Elidia automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `elidia sessions list` or `elidia sessions browse`.
...
```bash
elidia sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Elidia creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`elidia -c "my project"`), it automatically picks the most recent session in the lineage.
...
Elidia provides a full set of session management commands via `elidia sessions`:
...
# Filter by platform
elidia sessions list --source telegram

# Show more sessions
elidia sessions list --limit 50
```
...
elidia sessions export backup.jsonl

# Export sessions from a specific platform
elidia sessions export telegram-history.jsonl --source telegram

# Export a single session
elidia sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
elidia sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
elidia sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
elidia sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
elidia sessions prune

# Custom age threshold
elidia sessions prune --older-than 30

# Only prune sessions from a specific platform
elidia sessions prune --source telegram --older-than 60

# Skip confirmation
elidia sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`elidia insights`](/reference/cli-commands#elidia-insights).
...
When Elidia cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Elidia uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Elidia in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.elidia/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.elidia/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.elidia/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.elidia/sessions/`. They are no longer written or
read by Elidia. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Elidia process in the same `ELIDIA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.elidia/config.yaml`:
...
elidia sessions delete <session_id>
