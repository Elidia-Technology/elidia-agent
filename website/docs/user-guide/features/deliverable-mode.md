...
When Elidia Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.elidia/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.elidia/config.yaml` (switchable per session
via `/personality`).
...
If you use Elidia' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.elidia/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Elidia Agent's deliverable
