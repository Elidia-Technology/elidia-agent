...
The web dashboard is a browser-based UI for managing your Elidia Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Elidia Portal OAuth; if you also want the dashboard to talk to a real backend, `elidia setup --portal` wires up the model and tool gateway too. See [Elidia Portal](/integrations/elidia-portal).
:::
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `elidia --tui` via PTY/WebSocket). Alternatively set `ELIDIA_DASHBOARD_TUI=1`. |

```bash
# Custom port
elidia dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
elidia dashboard --host 0.0.0.0

# Start without opening browser
elidia dashboard --no-open
...
The default `elidia-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'elidia-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install elidia-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `elidia dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `elidia dashboard` launch. Start the dashboard with `elidia dashboard --tui` or set `ELIDIA_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `elidia --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'elidia-agent[web,pty]'`, or `[all]` covers both)
...
1. **The embedded chat must be enabled.** `/api/ws` and `/api/pty` close immediately with WS code **4403** unless the dashboard was started with `--tui` (or `ELIDIA_DASHBOARD_TUI=1`). A plain `elidia dashboard` or `elidia gateway` serves the status page but refuses the chat socket.
2. **The session token must match.** Even with chat enabled, the socket closes with WS code **4401** if the token Desktop sends doesn't match the dashboard's session token. By default the dashboard generates a **fresh random token on every restart**, so a token you saved in Desktop yesterday is invalid after the service restarts. Pin it by setting `ELIDIA_DASHBOARD_SESSION_TOKEN` to a stable value.
...
```ini
[Service]
Environment="ELIDIA_DASHBOARD_SESSION_TOKEN=<long-random-token>"
ExecStart=/path/to/venv/bin/python -m elidia_cli.main dashboard \
...
```bash
curl -i -H "X-Elidia-Session-Token: <long-random-token>" http://VM_IP:9119/api/config
```
...
The REST API reads the token from the `X-Elidia-Session-Token` header; the WebSocket reads the same token from the `?token=` query parameter. Both compare against `ELIDIA_DASHBOARD_SESSION_TOKEN`, so a 200 here means the WS handshake will authenticate too.
:::
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `elidia config set` and the gateway read from.
:::
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.elidia/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `elidia skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `elidia mcp` reads from.
...
**Catalog:** browse the Elidia-approved MCP servers (the bundled `optional-mcps/`
...
`elidia mcp catalog` / `elidia mcp install` use.
...
Connect Elidia to any messaging platform from the browser — full parity with
`elidia setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.elidia/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Elidia versions, CPU core count + utilization, memory, disk usage of the Elidia home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Elidia version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `elidia update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Elidia Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `elidia portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `elidia curator`.
...
![System admin page — host stats and Elidia Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.elidia/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/mcp/catalog` | Browse the Elidia-approved MCP catalog |
...
| `GET /api/elidia/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Elidia Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public address — anything other than `127.0.0.1` / `localhost` — Elidia Agent engages an OAuth-based auth gate. Every request must carry a verified session cookie or it's bounced through a full OAuth round-trip via the Elidia Portal.
...
|-------|-----------|----------|
| `elidia dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `elidia dashboard --host 0.0.0.0` | **ON** | Production / Fly.io deployment |
| `elidia dashboard --host 192.168.1.10 --insecure` | OFF | Trusted LAN; user opts into legacy session-token auth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Elidia plugin, no custom plugin), `elidia dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

### Default provider: AiUtils

The bundled `plugins/dashboard_auth/elidia` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `elidia` when a client ID is configured.
...
dashboard:
  oauth:
    ...
    portal_url: https://developer.aiutils.io  # optional; defaults to production
```
...
|---------|-----------|--------|----------------|
| `ELIDIA_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | Elidia Portal at Fly.io provisioning time |
| `ELIDIA_DASHBOARD_PORTAL_URL` | `dashboard.oauth.portal_url` | URL (default: `https://developer.aiutils.io`) | Portal — override only for staging or a custom deployment |

Per the Elidia Agent convention (`~/.elidia/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so Fly.io's platform-secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • elidia: ELIDIA_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The Elidia Portal
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys an Elidia Agent instance — set it to your provisioned
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain Fly deploys with partial proxy chains), set `dashboard.public_url` (or `ELIDIA_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/elidia"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `ELIDIA_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `ELIDIA_DASHBOARD_PUBLIC_URL` env var | — | Fly.io platform secrets / CI |
...
The provider implements the [Elidia Portal OAuth contract v1](https://github.com/Elidia-Technology/elidia-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with AiUtils" button → `/auth/login?provider=elidia`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://developer.aiutils.io/oauth/authorize?…`.
4. User authenticates with Portal, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `elidia_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `elidia_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `elidia_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `elidia_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via elidia` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$ELIDIA_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-Elidia OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.elidia/plugins/dashboard-auth-myidp/__init__.py
from elidia_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path (Fly.io shape). ELIDIA_DASHBOARD_PORTAL_URL is
# optional — defaults to production.
ELIDIA_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  elidia dashboard --host 0.0.0.0
...
Elidia Desktop can drive an Elidia backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a **Session token**. (For the desktop app itself — install, settings, chat — see the [Elidia Desktop](/user-guide/desktop) page.)

The "session token" is the dashboard's session token — the same secret the local web UI uses for `/api` and WebSocket calls. **Elidia does not print it for you to copy.** By default the dashboard mints a fresh random token on every boot and injects it straight into the served HTML, so there is nothing sitting in `config.yaml`, in `/gateway`, or in the logs to grab. For a remote connection you set the token yourself on the backend, then paste that same value into the desktop app.

The desktop app sends the token as an `X-Elidia-Session-Token` header. The backend accepts it only in legacy session-token mode — i.e. when bound non-loopback **with `--insecure`**. A non-loopback bind *without* `--insecure` engages the [OAuth gate](#oauth-authentication-gated-mode) instead, which ignores the session token. So a remote desktop connection means: `--insecure` + a token you control.

The backend must also be started with **`--tui`** (or `ELIDIA_DASHBOARD_TUI=1`). The desktop's chat runs over the `/api/ws` + `/api/pty` WebSockets, and those are refused unless the embedded-chat surface is enabled. Without `--tui` the desktop still passes the `/api/status` health check (so the app reports the backend "ready") but the chat WebSocket is closed on connect — connects, looks ready, chat stays dead. A plain `elidia dashboard` or `elidia gateway` is not enough.

### On the backend (the remote machine)

```bash
# 1. Mint a stable token and store it in ~/.elidia/.env (secrets file, 0600).
#    Setting ELIDIA_DASHBOARD_SESSION_TOKEN pins the token so it survives
#    restarts and is the value the desktop app will use — without this,
#    the token is random per boot and uncopyable.
TOKEN=$(openssl rand -base64 32)
echo "ELIDIA_DASHBOARD_SESSION_TOKEN=$TOKEN" >> ~/.elidia/.env
chmod 600 ~/.elidia/.env
...
elidia dashboard --tui --no-open --insecure --host 0.0.0.0 --port 9119
```

If you run the dashboard as a systemd service, `~/.elidia/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.elidia/.env`, so the token is in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/elidia` are supported if you front it with a reverse proxy)
...
Instead of the in-app settings, you can point the desktop at a backend with two env vars before launching it. When `ELIDIA_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app settings (the Gateway settings panel shows an "env override" badge and disables editing):

| Env var | Value |
|---------|-------|
| `ELIDIA_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
| `ELIDIA_DESKTOP_REMOTE_TOKEN` | the same token as `ELIDIA_DASHBOARD_SESSION_TOKEN` on the backend |
...
- **Test remote fails with 401** — the token doesn't match the backend's `ELIDIA_DASHBOARD_SESSION_TOKEN`, or the backend is running *without* `--insecure` on a non-loopback bind (the OAuth gate is on and ignores the session token). Confirm `--insecure` and that the env var is actually loaded (`curl -s -H "X-Elidia-Session-Token: $TOKEN" http://<host>:9119/api/status` should return JSON, not 401).
- **Backend reports "ready" but chat does nothing** — the backend was started without `--tui` (or `ELIDIA_DASHBOARD_TUI=1`), so `/api/status` answers but the chat WebSocket (`/api/ws` / `/api/pty`) is refused. Restart the backend with `--tui`.
...
- **No token anywhere to copy** — expected. You mint it (`ELIDIA_DASHBOARD_SESSION_TOKEN`); Elidia never auto-surfaces the default ephemeral one.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `elidia_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `elidia update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `elidia dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Elidia Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Elidia Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
