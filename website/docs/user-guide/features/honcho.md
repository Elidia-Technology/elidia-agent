...
**Multi-agent profiles**: When multiple Elidia instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
elidia memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `elidia` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Elidia syncs them back at session init.
...
The `elidia honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `elidia memory setup honcho` (or run `elidia memory setup` and pick it from the list); the `elidia honcho` subcommand then appears on the next invocation.

```bash
elidia memory setup honcho    # Configure Honcho directly (works before activation)
elidia honcho status          # Connection status, config, and key settings
elidia honcho setup           # Redirects to `elidia memory setup` (post-activation alias)
elidia honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
elidia honcho peer            # Show or update peer names + dialectic reasoning level
elidia honcho mode            # Show or set recall mode (hybrid/context/tools)
elidia honcho tokens          # Show or set token budget for context and dialectic
elidia honcho identity        # Seed or show the AI peer's Honcho identity
elidia honcho sync            # Sync Honcho config to all existing profiles
elidia honcho peers           # Show peer identities across all profiles
elidia honcho sessions        # List known Honcho session mappings
elidia honcho map             # Map current directory to a Honcho session name
elidia honcho enable          # Enable Honcho for the active profile
elidia honcho disable         # Disable Honcho for the active profile
elidia honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `elidia honcho`

If you previously used the standalone `elidia honcho setup`:
...
No re-login or re-setup needed. Run `elidia memory setup` and select "honcho" — the wizard detects your existing config.
