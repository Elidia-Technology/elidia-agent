...
description: "Plugins shipped with Elidia Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Elidia ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.elidia/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build an Elidia Plugin](/guides/build-a-elidia-plugin) to write your own.
...
2. **User** — `~/.elidia/plugins/<name>/`
3. **Project** — `./.elidia/plugins/<name>/` (requires `ELIDIA_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `elidia_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `elidia memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `elidia plugins list` and the interactive `elidia plugins` UI), but none load until you explicitly enable them:

```bash
elidia plugins enable disk-cleanup
```

Or via `~/.elidia/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Elidia. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
elidia plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `elidia plugins enable <name>`.
...
| `elidia-achievements` | dashboard tab | Steam-style collectible badges generated from your real Elidia session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `elidia memory` and `elidia plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ELIDIA_HOME` or `/tmp/elidia-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ELIDIA_HOME | always | Never |
...
**State** — everything lives at `$ELIDIA_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ELIDIA_HOME` or `/tmp/elidia-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `elidia plugins enable disk-cleanup` (or check the box in `elidia plugins`).

**Disabling again:** `elidia plugins disable disk-cleanup`.
...
**Enabling:** `elidia plugins enable security-guidance` (or check the box in `elidia plugins`).

**Disabling again:** `elidia plugins disable security-guidance`.
...
Traces Elidia turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Elidia' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `elidia logs`.
...
```bash
elidia tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Elidia and the next turn ships a trace.
...
Then put the credentials in `~/.elidia/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Elidia turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ELIDIA_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Elidia session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `elidia chat` session lives under one Langfuse session.

**Verify:**

```bash
elidia plugins list                 # observability/langfuse should show "enabled"
elidia chat -q "hello"              # check the Langfuse UI for a "Elidia turn" trace
```
...
| `ELIDIA_LANGFUSE_RELEASE` | — | Release/version tag |
| `ELIDIA_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ELIDIA_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ELIDIA_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Elidia-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Elidia-prefixed wins when both are set.
...
**Disabling:** `elidia plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.elidia/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
elidia plugins enable google_meet
...
**Disabling:** `elidia plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.elidia/cache/google_meet/` until you remove them.

### elidia-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Elidia session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Elidia feature changes.

**How it works:**

- Scans your entire `~/.elidia/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ELIDIA_HOME/plugins/elidia-achievements/state.json`
...
| Secret | Hidden until Elidia detects the first related signal in your history |

**API** — routes mount under `/api/plugins/elidia-achievements/`:
...
**State files** — live under `$ELIDIA_HOME/plugins/elidia-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Elidia updates. |
...
**Enabling:** Nothing to enable — `elidia-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `elidia dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/elidia-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.elidia/plugins/elidia-achievements/` that ships no dashboard. The plugin's state files under `$ELIDIA_HOME/plugins/elidia-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Elidia plugin — see [Build an Elidia Plugin](/guides/build-a-elidia-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.elidia/plugins/<name>/`
- Manifest source is reported as `bundled` in `elidia plugins list`
