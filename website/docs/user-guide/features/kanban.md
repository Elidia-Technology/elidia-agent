...
description: "Durable SQLite-backed task board for coordinating multiple Elidia profiles"
---
...
Elidia Kanban is a durable task board, shared across all your Elidia profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.elidia/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.elidia/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `elidia kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `elidia kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/elidia-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ELIDIA_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
elidia kanban boards list

# Create a new board.
elidia kanban boards create atm10-server \
    --name "ATM10 Server" \
...
elidia kanban --board atm10-server list
elidia kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
elidia kanban boards switch atm10-server
elidia kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
elidia kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ELIDIA_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.elidia/kanban/current` — the slug persisted by `elidia kanban
   boards switch`.
4. `default`.
...
`elidia dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<elidia-home>/kanban/attachments/<task_id>/` for the default board, or
  `<elidia-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `ELIDIA_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
elidia kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `elidia kanban show t_abcd`.
...
Override the config flag at runtime via `ELIDIA_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `elidia gateway
...
until one comes up — `elidia kanban create` warns about this at creation
time.

Running `elidia kanban daemon` as a separate process is **deprecated**;
...
elidia kanban create "nightly ops review" \
    --assignee ops \
...
```bash
elidia kanban complete t_abc t_def t_hij --result "batch wrap"
elidia kanban archive  t_abc t_def t_hij
elidia kanban unblock  t_abc t_def
elidia kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `elidia kanban`.** When the dispatcher spawns a worker it sets `ELIDIA_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `elidia kanban` CLI.
...
kanban_show()                                     # no args — uses ELIDIA_KANBAN_TASK
...
### Why tools instead of shelling to `elidia kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `elidia kanban complete` *inside* the container, where `elidia` isn't installed and `~/.elidia/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.elidia/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `elidia chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `ELIDIA_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/elidia_cli/test_kanban_db.py -q"],
...
2. `cd $ELIDIA_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
elidia -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
elidia -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
elidia kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

elidia kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
elidia kanban create "Translate the docs site to French" \
...
```bash
elidia -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
elidia -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `elidia kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `elidia kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `elidia profile create --description "..."`, `elidia profile describe <name> --text "..."`, `elidia profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.elidia/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `elidia profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.elidia/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Elidia dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
elidia kanban init                                     # create kanban.db + print daemon hint
elidia kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
elidia kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
elidia kanban show <id> [--json]
elidia kanban assign <id> <profile>                    # or 'none' to unassign
elidia kanban link <parent_id> <child_id>
elidia kanban unlink <parent_id> <child_id>
elidia kanban claim <id> [--ttl SECONDS]
elidia kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
elidia kanban complete <id>... [--result "..."]
elidia kanban block <id> "<reason>" [--ids <id>...]
elidia kanban unblock <id>...
elidia kanban archive <id>...

elidia kanban tail <id>                                # follow a single task's event stream
elidia kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
elidia kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
elidia kanban runs <id> [--json]                       # attempt history (one row per run)
elidia kanban assignees [--json]                       # profiles on disk + per-assignee task counts
elidia kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
elidia kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `elidia gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
elidia kanban stats [--json]                           # per-status + per-assignee counts
elidia kanban log <id> [--tail BYTES]                  # worker log from ~/.elidia/kanban/logs/
elidia kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
elidia kanban notify-list [<id>] [--json]
elidia kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
elidia kanban context <id>                             # what a worker sees
elidia kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
elidia kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
elidia kanban create "nightly backup audit" \
...
Every `elidia kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `elidia chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `elidia_cli.kanban.run_slash()` entry point that reuses the `elidia kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `elidia kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.elidia/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`elidia kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `elidia kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/elidia-kanban-v1-spec.pdf`.
...
```bash
elidia kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$ELIDIA_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
elidia kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
elidia kanban notify-list
elidia kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
elidia kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
elidia kanban runs t_abcd
...
**Bulk close caveat.** `elidia kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`elidia kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `elidia kanban heartbeat $TASK` to signal liveness during long operations. |
...
`elidia kanban tail <id>` shows these for a single task. `elidia kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.elidia/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/elidia-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
