...
Elidia Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
elidia memory setup      # interactive picker + configuration
elidia memory status     # check what's active
elidia memory off        # disable external provider
```

You can also select the active memory provider via `elidia plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.elidia/config.yaml`:
...
When a memory provider is active, Elidia automatically:
...
**Setup Wizard:**
```bash
elidia memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

On a fresh install, configure Honcho directly with `elidia memory setup honcho`. The legacy `elidia honcho setup` command still works (it now redirects to `elidia memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$ELIDIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ELIDIA_HOME/honcho.json` > `~/.elidia/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Elidia-Technology/elidia-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/elidia).
...
:::tip Migrating from `elidia honcho`
If you previously used `elidia honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Elidia profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Elidia profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Elidia profile. Host key `elidia` → default; `elidia.<profile>` for others. |
...
```bash
elidia profile create coder --clone
```

`--clone` creates a `elidia.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Elidia profile, creates host blocks for any profile without one, inherits settings from the default `elidia` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/Elidia-Technology/elidia-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/elidia).
...
elidia memory setup    # select "openviking"
# Or manually:
elidia config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.elidia/.env
```
...
**Setup:**
```bash
elidia memory setup    # select "mem0"
# Or manually:
elidia config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.elidia/.env
```

**Config:** `$ELIDIA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `elidia-user` | User identifier |
| `agent_id` | `elidia` | Agent identifier |
...
**Setup:**
```bash
elidia memory setup    # select "hindsight"
# Or manually:
elidia config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.elidia/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p elidia ui start`

**Config:** `$ELIDIA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `elidia` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Elidia Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Elidia-Technology/elidia-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
elidia memory setup    # select "holographic"
# Or manually:
elidia config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.elidia-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ELIDIA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
elidia memory setup    # select "retaindb"
# Or manually:
elidia config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.elidia/.env
```
...
# Then configure Elidia
elidia memory setup    # select "byterover"
# Or manually:
elidia config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ELIDIA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
elidia memory setup    # select "supermemory"
# Or manually:
elidia config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.elidia/.env
```

**Config:** `$ELIDIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `elidia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `elidia-{identity}` → `elidia-coder`) to isolate memories per Elidia profile
...
| **Requires** | `pip install elidia-memori` + `elidia-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install elidia-memori
elidia-memori install
elidia config set memory.provider memori
elidia memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `elidia-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ELIDIA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ELIDIA_HOME/` so each profile has its own credentials
