...
Elidia has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.elidia/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.elidia/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.elidia/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".elidia" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.elidia/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.elidia/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".elidia" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.elidia/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.elidia/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/elidia-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.elidia/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Elidia does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.elidia/BOOT.md` with natural-language startup instructions.
...
Create `~/.elidia/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `elidia cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.elidia/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.elidia/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.elidia/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.elidia/hooks/boot-md/handler.py`**

```python
"""Run ~/.elidia/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".elidia" / "BOOT.md"
...
```bash
elidia logs --follow --level INFO | grep boot-md
```
...
Delete `~/.elidia/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Elidia's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/guides/build-a-elidia-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Elidia will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`elidia_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.elidia/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.elidia/agent-hooks/` (by convention) | `~/.elidia/plugins/<name>/` | `~/.elidia/hooks/<name>/` |
...
Each time the event fires, Elidia spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Elidia-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.elidia/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.elidia/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.elidia/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.elidia/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.elidia/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.elidia/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Elidia event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.elidia/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.elidia/agent-hooks/log-orchestration.sh
log=~/.elidia/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Elidia sees it, then persists the decision to `~/.elidia/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `elidia --accept-hooks chat`)
2. `ELIDIA_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `elidia hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `elidia hooks` CLI

| Command | What it does |
|---------|--------------|
| `elidia hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `elidia hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `elidia hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `elidia hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.elidia/agent-hooks/` so the path is easy to audit.
- Re-run `elidia hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
