...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Elidia automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Elidia Portal](/integrations/elidia-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Elidia auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
elidia auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
elidia auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
elidia auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  elidia_pkce          oauth   elidia_pkce ←
...
Run `elidia auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Elidia, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `elidia auth` | Interactive pool management wizard |
| `elidia auth list` | Show all pools and credentials |
| `elidia auth list <provider>` | Show a specific provider's pool |
| `elidia auth add <provider>` | Add a credential (prompts for type and key) |
| `elidia auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `elidia auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `elidia auth remove <provider> <index>` | Remove credential by 1-based index |
| `elidia auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `elidia auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `elidia model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via elidia model:
elidia auth list
# Shows:
...
elidia auth add Together.ai --api-key sk-together-second-key
```
...
Elidia automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, Elidia device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Elidia PKCE OAuth | `~/.elidia/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `elidia auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `elidia auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Elidia can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Elidia-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`elidia_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`elidia_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.elidia/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Elidia' credential store, so its token remains persistable.
