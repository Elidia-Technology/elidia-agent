...
description: "Connect Elidia Agent to external tool servers via MCP — and control exactly which MCP tools Elidia loads"
---
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `elidia mcp login <server>`.
