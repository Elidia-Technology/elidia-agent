...
description: "Customize Elidia Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Elidia Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ELIDIA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Elidia is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Elidia now seeds a default `SOUL.md` automatically in:

```text
~/.elidia/SOUL.md
```

More precisely, it uses the current instance's `ELIDIA_HOME`, so if you run Elidia with a custom home directory, it will use:
...
- Elidia creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Elidia loads `SOUL.md` only from `ELIDIA_HOME`
- Elidia does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Elidia falls back to a built-in default identity
...
If Elidia loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ELIDIA_HOME`, the personality belongs to the Elidia instance itself.

That also makes it easier to teach users:
- "Edit `~/.elidia/SOUL.md` to change Elidia' default personality."
...
- how Elidia should handle uncertainty, disagreement, or ambiguity
...
## What Elidia injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Elidia falls back to a built-in default identity ("You are Elidia Agent, an intelligent AI assistant created by AiUtils..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Elidia ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Elidia, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Elidia its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.elidia/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.elidia/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-elidia)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Elidia speaks
- `display.skin` and `/skin` affect how Elidia looks in the terminal
