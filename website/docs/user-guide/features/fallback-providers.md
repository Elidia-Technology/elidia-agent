...
Elidia Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Elidia can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`elidia fallback` reuses the provider picker from `elidia model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.elidia/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Elidia still honors it for back-compat, but `elidia fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Elidia Portal | `elidia` | `elidia setup --portal` (fresh) or `elidia auth add elidia` (OAuth) |
| OpenAI Codex | `openai-codex` | `elidia model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `elidia model` (Google OAuth; optional: `ELIDIA_GEMINI_PROJECT_ID`) |
| Google AI Studio | `gemini` | `GOOGLE_API_KEY` (alias: `GEMINI_API_KEY`) |
| xAI (Grok) | `xai` (alias `grok`) | `XAI_API_KEY` (optional: `XAI_BASE_URL`) |
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `elidia model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `elidia model` (Qwen Portal OAuth; optional: `ELIDIA_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `elidia model` (MiniMax portal OAuth) |
...
**Elidia Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `elidia fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Elidia uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `elidia kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Elidia tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter → Elidia Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → Elidia Portal →
Codex OAuth → Anthropic → Custom endpoint → give up
```

If the resolved provider fails at call time, Elidia also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | elidia | codex | main | anthropic
...
| `"codex"` | Force Codex OAuth | `elidia model` → Codex |
...
`base_url` takes precedence over `provider`. Elidia uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Elidia treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Elidia falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Elidia logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Elidia recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Elidia doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | elidia | main
    model: "google/gemini-3-flash-preview"
```
...
If no provider is available for compression, Elidia drops middle conversation turns without generating a summary rather than failing the session.
