...
Type `@` followed by a reference to inject content directly into your message. Elidia expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Elidia env: `$ELIDIA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ELIDIA_HOME/skills/.hub/`
