...
Elidia Agent includes a full browser automation toolset with multiple backend options:
...
:::tip Elidia Subscribers
If you have a paid [Elidia Portal](https://developer.aiutils.io) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `elidia setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Elidia Subscription** as the browser provider via `elidia model` or `elidia tools`.
:::
...
When a cloud provider is configured, Elidia auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`elidia setup tools → Browser Automation`
...
Or configure via `elidia tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.elidia/config.yaml`:
...
Then fully restart Elidia so the new config is picked up.

:::warning Nested path matters
Elidia reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Elidia ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Elidia silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Elidia profile, so different Elidia profiles get different browser profiles (profile isolation).

##### What Elidia does not do
- It does not force persistence on the Camofox server. Elidia only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Elidia cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Elidia and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Elidia after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Elidia derives the stable `userId` from the profile-scoped directory `~/.elidia/browser_auth/camofox/` (or the equivalent under `$ELIDIA_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Elidia profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Elidia to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Elidia uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Elidia calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Elidia skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Elidia does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Elidia issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Elidia adopts the most recently created one in that group.
3. Otherwise, Elidia adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Elidia falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Elidia does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Elidia to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Elidia generates a per-task `session_key` (`task_<id>`) — Elidia will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Elidia can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Elidia runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Elidia automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Elidia browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Elidia from the terminal (`elidia` or `elidia chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.elidia/chrome-debug \
  --no-first-run \
...
Then launch the Elidia CLI and run `/browser connect`.
...
If Elidia runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Elidia itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Elidia talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Elidia MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Elidia can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). Elidia auto-injects
...
# Elidia doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `elidia config set toolsets '["elidia-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/Elidia-Technology
```
...
```
Type "elidia agent" into the search field @e3
```
...
Screenshots are stored in `~/.elidia/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.elidia/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Elidia automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
