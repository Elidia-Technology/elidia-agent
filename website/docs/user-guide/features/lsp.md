...
Elidia runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Elidia
...
1. Elidia captures a baseline of current diagnostics for the file.
...
…). Elidia auto-detects the binary on PATH or in
`<ELIDIA_HOME>/lsp/bin/`.
...
`node_modules` tree — Elidia installs both packages together when you
run `elidia lsp install typescript` or auto-install fires on first
use.

## CLI

```
elidia lsp status          # service state + per-server install status
elidia lsp list            # registry, optionally --installed-only
elidia lsp install <id>    # eagerly install one server
elidia lsp install-all     # try every server with a known recipe
elidia lsp restart         # tear down running clients
elidia lsp which <id>      # print resolved binary path
```

`elidia lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ELIDIA_HOME>/lsp/bin
...
When `install_strategy: auto`, Elidia installs binaries into
`<ELIDIA_HOME>/lsp/bin/`. NPM packages land in
`<ELIDIA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Elidia-owned and is
...
Servers are kept alive for the life of the Elidia process. There's
...
**`elidia lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ELIDIA_HOME>/lsp/bin/`. Run
`elidia lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `elidia lsp status`**
...
When `elidia lsp status` shows a `Backend warnings` section, install
...
`~/.elidia/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.elidia/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `elidia lsp restart` to clear the set;
