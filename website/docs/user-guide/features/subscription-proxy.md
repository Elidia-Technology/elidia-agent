...
description: "Use your Elidia Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Elidia-managed provider subscription as their
...
| Use case | "Use Elidia as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Elidia Portal OAuth flow. Elidia stores
the refresh token in `~/.elidia/auth.json` — the same place all Elidia
...
Currently shipped: `elidia` (Elidia Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `elidia_cli/proxy/adapters/`.
...
If you see `not logged in`, run `elidia portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Elidia
Portal:
...
supports; check `developer.aiutils.io/models`.
...
[developer.aiutils.io](https://developer.aiutils.io).
...
`elidia_cli/proxy/adapters/<provider>.py` and registering it in
