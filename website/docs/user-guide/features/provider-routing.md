...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Elidia Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through [Elidia Portal](/integrations/elidia-portal) still respects per-model routing and priority configs — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.elidia/config.yaml`:
...
- **CLI mode** — configured in `~/.elidia/config.yaml`, loaded at startup
