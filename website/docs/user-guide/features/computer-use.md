...
Elidia Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `elidia computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `elidia tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.elidia/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Elidia refreshes the binary in two
places so you don't get stuck on a stale release:

- **`elidia update`** — when you update Elidia itself, if `cua-driver` is
...
- **`elidia computer-use install --upgrade`** — manual force-refresh.
...
`elidia computer-use status` shows the installed version next to the
binary path.
...
Elidia applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.elidia/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Elidia applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ELIDIA_CUA_DRIVER_VERSION`
...
`elidia computer-use install` to fetch the cua-driver binary, or run
`elidia tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/Elidia-Technology/elidia-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
