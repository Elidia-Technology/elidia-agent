...
- Give it a name like "elidia-agent"
- Select scopes:
...
- Give it a title like "elidia-agent-&lt;machine-name>"
...
elif [ -f ~/.elidia/.env ] && grep -q "^GITHUB_TOKEN=" ~/.elidia/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.elidia/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
