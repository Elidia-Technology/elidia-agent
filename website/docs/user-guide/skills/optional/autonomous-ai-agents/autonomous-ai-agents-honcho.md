...
description: "Configure and use Honcho memory with Elidia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Elidia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `elidia skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Elidia

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Elidia profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/elidia#running-honcho-locally-with-elidia

### Verify

```bash
elidia honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Elidia profile's AI peer
...
Honcho models conversations as interactions between **peers**. Elidia creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Elidia instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Elidia run |
| `global` | Single session across all directories |

Manual override: `elidia honcho map my-project-name`
...
Each Elidia profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block elidia.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `elidia.coder` host block in `honcho.json`
...
```bash
elidia honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Elidia when Honcho memory is active.
...
Config file: `$ELIDIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `elidia honcho setup`. Ensure `memory.provider: honcho` is in `~/.elidia/config.yaml`.

### Memory not persisting across sessions
Check `elidia honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `elidia profile create <name> --clone`. For existing profiles: `elidia honcho sync`.
...
| Command | Description |
|---------|-------------|
| `elidia honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `elidia honcho status` | Show resolved config, connection test, peer info for active profile |
| `elidia honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `elidia honcho disable` | Disable Honcho for the active profile |
| `elidia honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `elidia honcho peers` | Show peer identities across all profiles |
| `elidia honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `elidia honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `elidia honcho sessions` | List known directory-to-session-name mappings |
| `elidia honcho map <name>` | Map current working directory to a Honcho session name |
| `elidia honcho identity` | Seed AI peer identity or show both peer representations |
| `elidia honcho sync` | Create host blocks for all Elidia profiles that don't have one yet |
| `elidia honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Elidia + Honcho |
| `elidia memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `elidia memory status` | Show active memory provider and config |
| `elidia memory off` | Disable external memory provider |
