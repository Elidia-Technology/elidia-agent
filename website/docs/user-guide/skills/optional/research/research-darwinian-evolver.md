...
| | |
|---|---|
| Source | Optional — install with `elidia skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Elidia Agent |
| License | MIT |
...
import upstream classes into Elidia itself.
...
```bash
mkdir -p ~/.elidia/cache/darwinian-evolver && cd ~/.elidia/cache/darwinian-evolver
...
```bash
cd ~/.elidia/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.elidia/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.elidia/skills/research/darwinian-evolver
DE_DIR=~/.elidia/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Elidia core.
   Custom driver scripts under `~/.elidia/skills/...` are user-side and fine.
