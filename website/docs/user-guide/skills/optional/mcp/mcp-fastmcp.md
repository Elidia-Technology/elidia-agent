...
| | |
|---|---|
| Source | Optional — install with `elidia skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Elidia or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Elidia. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.elidia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.elidia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Elidia integration, either:

- configure the server in `~/.elidia/config.yaml` using the `native-mcp` skill, or
...
### Elidia cannot see the deployed server

The server-building part may be correct while the Elidia config is not. Load the `native-mcp` skill and configure the server in `~/.elidia/config.yaml`, then restart Elidia.
