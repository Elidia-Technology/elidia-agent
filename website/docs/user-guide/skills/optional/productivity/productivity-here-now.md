...
| | |
|---|---|
| Source | Optional — install with `elidia skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ELIDIA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ELIDIA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ELIDIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client elidia
```
...
```bash
PUBLISH="${ELIDIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client elidia
```
...
```bash
DRIVE="${ELIDIA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `elidia`)    |
