...
| | |
|---|---|
| Source | Optional — install with `elidia skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Elidia Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.elidia/.env`:
...
```bash
CANVAS="python $ELIDIA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
