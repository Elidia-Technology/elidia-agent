...
| | |
|---|---|
| Source | Optional — install with `elidia skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Elidia Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Elidia terminal calls (desktop app flow only)
...
### Service Account (recommended for Elidia)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.elidia/.env` (the skill will prompt for this on first load).
...
## Elidia Execution Pattern (desktop app flow)

Elidia terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/elidia-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/elidia-op.sock"
