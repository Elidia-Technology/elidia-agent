...
sidebar_position: 12
title: "Google Chat"
description: "Set up Elidia Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Elidia Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Elidia
...
> Run `elidia gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Elidia can read (e.g.,
`~/.elidia/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives an elidia restart)
...
- Principal: `elidia-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Elidia calls
...
- **App name**: whatever you want users to see ("Elidia" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/elidia-chat-events`.
...
`ADDED_TO_SPACE` event that Elidia uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Elidia

Add the Google Chat section to `~/.elidia/.env`:
...
Install the dependencies the Google Chat adapter needs (no Elidia extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Elidia is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Elidia detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Elidia session.
...
3. Download the JSON. Move it onto the host that runs Elidia.
4. On the host, register the client with Elidia:
...
That writes `~/.elidia/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.elidia/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.elidia/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Elidia isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `elidia gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Elidia will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.elidia/google_chat_user_tokens/<sanitized_email>.json` (filesystem
