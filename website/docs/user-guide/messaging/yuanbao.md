...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Elidia Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Elidia to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.elidia/.env`:
...
All standard Elidia commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.elidia/logs/gateway.log`
...
4. Consider enabling verbose logging: `ELIDIA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Elidia automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ELIDIA_LOG_LEVEL=debug elidia gateway
```
...
```bash
elidia chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
