...
Connect Elidia to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Elidia supports two WeCom integration modes:
...
> Run `elidia gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Elidia decrypts the message, queues it for the agent
...
(Use `elidia gateway start` only after `elidia gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Elidia and the token the
...
in `~/.elidia/.env` values around `=` will also break signature checks. After
fixing, restart `elidia gateway run`.
...
Check `elidia gateway run` logs for the bound host/port. If the adapter bound to
