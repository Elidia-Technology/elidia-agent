...
Connect Elidia to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Elidia and the BlueBubbles server
...
Or set environment variables directly in `~/.elidia/.env`:
...
By default, Elidia responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Elidia uses conservative defaults for `Elidia` and `@Elidia agent` variants.
...
When someone messages your iMessage, Elidia automatically sends them a pairing code. Approve it with:
```bash
elidia pairing approve bluebubbles <CODE>
```
Use `elidia pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.elidia/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.elidia/.env`):
```bash
...
Elidia will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Elidia
Elidia → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Elidia sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Elidia wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.elidia/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Elidia resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `elidia logs gateway` for webhook errors (or `elidia logs -f` to follow in real-time)
