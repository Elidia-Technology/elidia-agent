...
This page covers all commands related to [Elidia profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `elidia profile` without a subcommand shows help.
...
```bash
elidia profile use <name>
```

Sets `<name>` as the active profile. All subsequent `elidia` commands (without `-p`) will use this profile.
...
elidia profile use default
```

## `elidia profile create`

```bash
elidia profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `elidia profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `elidia update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.elidia`), use `elidia skills opt-out` / `elidia skills opt-in`. |
...
elidia profile create mybot

# Clone config only from current profile
elidia profile create work --clone

# Clone everything from current profile
elidia profile create backup --clone-all

# Clone config from a specific profile
elidia profile create work2 --clone --clone-from work
```

## `elidia profile describe`

```bash
elidia profile describe [<name>] [options]
```
...
elidia profile describe researcher

# Set it explicitly
elidia profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
elidia profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
elidia profile describe --all --auto
```

## `elidia profile delete`

```bash
elidia profile delete <name> [options]
```
...
```bash
elidia profile delete mybot
elidia profile delete mybot --yes
```
...
```bash
elidia profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Elidia home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ elidia profile show work
Profile: work
Path:    ~/.elidia/profiles/work
...
## `elidia profile alias`

```bash
elidia profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Elidia installation.
...
```bash
elidia profile alias work
# Creates/updates ~/.local/bin/work

elidia profile alias work --name mywork
# Creates ~/.local/bin/mywork

elidia profile alias work --remove
# Removes the wrapper script
```

## `elidia profile rename`

```bash
elidia profile rename <old-name> <new-name>
```
...
```bash
elidia profile rename mybot assistant
# ~/.elidia/profiles/mybot → ~/.elidia/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `elidia profile export`

```bash
elidia profile export <name> [options]
```
...
```bash
elidia profile export work
# Creates work.tar.gz in the current directory

elidia profile export work -o ./work-2026-03-29.tar.gz
```

## `elidia profile import`

```bash
elidia profile import <archive> [options]
```
...
```bash
elidia profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

elidia profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`elidia profile export` / `import` are still the right commands for
...
### `elidia profile install`

```bash
elidia profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `elidia -p telemetry`). |
...
elidia profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
elidia profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
elidia profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
elidia profile install ./telemetry/
```

### `elidia profile update`

```bash
elidia profile update <name> [--force-config] [--yes]
```
...
```bash
elidia profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Elidia version, author, env var requirements, the source URL/path, and
...
`elidia profile list` also shows the distribution name and version in a
`Distribution` column, and `elidia profile show <name>` / `delete <name>`
...
elidia profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
elidia profile install https://github.com/your-org/internal-assistant.git
```
...
`elidia_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Elidia version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Elidia can clone from.
3. Tell recipients to run `elidia profile install <your-repo-url>`.
...
## `elidia -p` / `elidia --profile`

```bash
elidia -p <name> <command> [options]
elidia --profile <name> <command> [options]
```

Global flag to run any Elidia command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
elidia -p work chat -q "Check the server status"
elidia --profile dev gateway start
elidia -p personal skills list
elidia -p work config edit
```

## `elidia completion`

```bash
elidia completion <shell>
```
...
- `elidia profile <TAB>` — subcommands (list, use, create, etc.)
- `elidia profile use <TAB>` — profile names
- `elidia -p <TAB>` — profile names
