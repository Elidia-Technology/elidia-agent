...
description: "Complete reference of all environment variables used by Elidia Agent"
---
...
Environment variables for the bundled [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) plugin. Set these in `~/.elidia/.env`. The plugin must also be enabled (`elidia plugins enable observability/langfuse`, or check the box in `elidia plugins`) before any of these take effect.
...
These variables configure the [Tool Gateway](/user-guide/features/tool-gateway) for paid Elidia subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `elidia model` or `elidia tools`.
...
| `GATEWAY_PROXY_URL` | URL of a remote Elidia API server to forward messages to ([proxy mode](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
Auth for the [web dashboard](/user-guide/features/web-dashboard) and for connecting [Elidia Desktop to a remote backend](/user-guide/features/web-dashboard#connecting-elidia-desktop-to-a-remote-backend). Per the secrets-only convention, the token belongs in `~/.elidia/.env`; the OAuth `client_id`/`portal_url` are better set under `dashboard.oauth` in `config.yaml` (env wins when set).
...
| `CODEX_HOME` | When [Codex app-server runtime](../user-guide/features/codex-app-server-runtime) is enabled, override the directory Codex CLI reads its config + auth from (default: `~/.codex`). Elidia' migration writes the managed block to `<CODEX_HOME>/config.toml`. |
...
| Variable | Description |
|----------|-------------|
| `ELIDIA_TUI` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`. |
