...
description: "How Elidia stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `elidia_cli/cron.py` | CLI `elidia cron` subcommands |
...
Jobs are stored in `~/.elidia/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `elidia cron` commands are run or during active CLI sessions.
...
```python
# ~/.elidia/scripts/check_competitors.py
...
2. **Environment variable** — `ELIDIA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.elidia/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `elidia cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `elidia cron` CLI provides direct job management:

```bash
elidia cron list                    # Show all jobs
elidia cron create                  # Interactive job creation (alias: add)
elidia cron edit <job_id>           # Edit job configuration
elidia cron pause <job_id>          # Pause a running job
elidia cron resume <job_id>         # Resume a paused job
elidia cron run <job_id>            # Trigger immediate execution
elidia cron remove <job_id>         # Delete a job
```
