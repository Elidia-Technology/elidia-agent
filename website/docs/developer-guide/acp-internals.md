...
The ACP adapter wraps Elidia' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
elidia acp / elidia-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.elidia/.env
  -> configure stderr logging
  -> construct ElidiaACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'elidia-agent[acp]==<version>' elidia-acp`, pointed at the `elidia-agent` PyPI release.
...
- `allow_once` -> Elidia `once`
- `allow_always` -> Elidia `always`
- reject options -> Elidia `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Elidia tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["elidia-acp"])
...
Instead it reuses Elidia' runtime resolver:

- `acp_adapter/auth.py`
- `elidia_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Elidia provider/credentials. It also always advertises a terminal setup auth method (`elidia-setup`, args `--setup`) so first-run registry clients can open Elidia' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.elidia/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `elidia-acp` toolset definition
- `elidia_cli/main.py` — `elidia acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `elidia-acp` script
