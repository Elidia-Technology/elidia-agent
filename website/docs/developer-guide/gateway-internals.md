...
The messaging gateway is the long-running process that connects Elidia to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `elidia_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.elidia/.env` | API keys, bot tokens, platform credentials |
| `~/.elidia/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`elidia send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.elidia/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `elidia gateway start` / `elidia gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.elidia/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `elidia gateway stop` stops only the current profile's gateway. `elidia gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
