...
Elidia Agent uses a SQLite database (`~/.elidia/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `elidia_state.py`


## Architecture Overview

```
~/.elidia/state.db (SQLite, WAL mode)
...
Multiple elidia processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.elidia/state.db
...
Default path: `~/.elidia/state.db`

This is derived from `elidia_constants.get_elidia_home()` which resolves to
`~/.elidia/` by default, or the value of `ELIDIA_HOME` environment variable.
