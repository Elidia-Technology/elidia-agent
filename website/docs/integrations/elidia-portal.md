...
sidebar_position: 1
title: "Elidia Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Elidia Chat — the recommended way to run Elidia Agent"
---
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Elidia Agent's tool calls through Elidia-managed infrastructure. Five backends, one login:
...
If you use [Elidia profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Elidia tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Elidia configurations sharing one Portal login
