...
```bash
elidia teams-pipeline validate
```
...
```bash
elidia teams-pipeline token-health
elidia teams-pipeline token-health --force-refresh
```
...
```bash
elidia teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
elidia teams-pipeline maintain-subscriptions
elidia teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Elidia cron (recommended if you already run the Elidia gateway)

Elidia ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.elidia/scripts/`. First create the script:

```bash
mkdir -p ~/.elidia/scripts
cat > ~/.elidia/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec elidia teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.elidia/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
elidia cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/elidia-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Elidia Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=elidia
EnvironmentFile=/etc/elidia/env
ExecStart=/usr/local/bin/elidia teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/elidia-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Elidia Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now elidia-teams-pipeline-maintain.timer
systemctl list-timers elidia-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/elidia teams-pipeline maintain-subscriptions >> /var/log/elidia/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.elidia/.env` at the top of a wrapper script that crontab calls.
...
```bash
elidia teams-pipeline subscriptions   # should show expirationDateTime advanced
elidia teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
elidia teams-pipeline list
elidia teams-pipeline list --status failed
elidia teams-pipeline show <job-id>
```

### Replay a stored job

```bash
elidia teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
elidia teams-pipeline fetch --meeting-id <meeting-id>
elidia teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `elidia teams-pipeline maintain-subscriptions --dry-run`
- inspect `elidia teams-pipeline list --status failed`
...
- run `elidia teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `elidia teams-pipeline run`
...
- [ ] `elidia teams-pipeline validate` returns an OK snapshot
- [ ] `elidia teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Elidia cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
