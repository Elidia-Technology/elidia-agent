---
sidebar_position: 1
title: "Run Elidia Agent with Elidia Portal"
description: "Start-to-finish walkthrough: subscribe, set up, switch models, enable gateway tools, and verify routing"
---

# Run Elidia Agent with Elidia Portal

This guide walks you through running Elidia Agent on a [Elidia Portal](https://developer.aiutils.io) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [Elidia Portal integration page](/integrations/elidia-portal). This page is the task script.

## Prerequisites

- Elidia Agent installed ([Quickstart](/getting-started/quickstart))
...
Open [developer.aiutils.io/manage-subscription](https://developer.aiutils.io/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: elidia` in `~/.elidia/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Elidia is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
elidia setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
elidia auth add elidia --type oauth --manual-paste
# Then re-run `elidia setup --portal` to wire the provider + gateway
```
...
If any line shows something other than "via Elidia Portal" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
```
Hey, search the web for "Elidia Agent release notes" and summarize the top 3 hits.
```

You should see Elidia call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`elidia setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
The gateway is opt-in per tool, not all-or-nothing. If you already have a Browserbase account and want to keep using it while routing web search and image generation through Elidia, that's supported:

```bash
elidia tools
# → Web search       → "Elidia Subscription"     (recommended)
# → Image generation → "Elidia Subscription"     (recommended)
# → Browser          → "Browserbase"           (your existing key)
# → TTS              → "Elidia Subscription"     (recommended)
```

These rows appear in `elidia tools` even before you've logged into Elidia Portal — if you pick "Elidia Subscription" without an active session, Elidia runs the Portal login inline (without changing your inference provider or your other tools).
...
You'll see per-tool routing — `via Elidia Portal` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
```bash
elidia setup voice
# → pick "Elidia Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Elidia will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
elidia cron create "every day at 9am" \
...
If you use [Elidia profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.elidia/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `elidia portal info` shows "not logged in" after `elidia setup --portal`
...
### "Model: currently openrouter" (or some other provider) instead of "using Elidia as inference provider"

Your local config drifted. The OAuth worked but `model.provider` is still pointing at a different provider. Fix:

```bash
elidia config set model.provider elidia
```
...
Re-verify with `elidia portal info`.

### Tool Gateway tools showing partner names instead of "via Elidia Portal"

Per-tool config is overriding the gateway. Run:

```bash
elidia tools
# pick "Elidia Subscription" for any tool you want gateway-routed
```

Some users intentionally mix — e.g. routing web through Elidia but using their own Browserbase key for browser. If that's intentional, leave it alone. If not, this command fixes it.

### "Re-authentication required" mid-session

Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Elidia doesn't replay it endlessly. Just log in again:

```bash
elidia auth add elidia
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/Elidia-Technology/elidia-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`elidia portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:

- `model.provider` set to `openrouter`/`anthropic`/etc. instead of `elidia`
- An OAuth refresh failure that fell back to a different configured provider
- Multiple Elidia profiles where you're using the wrong one (check `elidia profile current`)

### Want to revoke and start clean

```bash
elidia auth remove elidia       # wipes the local refresh token
...
- **[Elidia Portal integration page](/integrations/elidia-portal)** — Overview of what's in the subscription
- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway-routed tool
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Elidia tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Elidia configurations
