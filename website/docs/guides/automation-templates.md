...
Copy-paste recipes for common automation patterns. Each template uses Elidia's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`elidia webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `elidia webhook subscribe`) |
...
```bash
elidia cron create "0 2 * * *" \
  "You are a project manager triaging the AiUtils/elidia-agent GitHub repo.

1. Run: gh issue list --repo AiUtils/elidia-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
elidia webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
elidia cron create "0 9 * * 1" \
  "Scan the AiUtils/elidia-agent repo for documentation drift.

1. Run: gh pr list --repo AiUtils/elidia-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (elidia_cli/commands.py, elidia_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (elidia_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
elidia cron create "0 6 * * *" \
  "Run a dependency security audit on the elidia-agent project.

1. cd ~/.elidia/elidia-agent && source .venv/bin/activate
...
```bash
elidia webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
elidia webhook subscribe alert-triage \
...
```python title="~/.elidia/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Elidia-Monitor/1.0"})
...
elidia cron create "every 30m" \
  ...
  --script ~/.elidia/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
elidia cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Elidia Agent development." \
...
```bash
elidia webhook subscribe github-issues \
  --events "issues" \
...
```bash
elidia webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
elidia webhook subscribe stripe-payments \
...
```bash
elidia cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the elidia-agent codebase.
