...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Elidia Agent — no API key required"
---

# MiniMax OAuth

Elidia Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Elidia automatically refreshes your session.
...
# → Elidia opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.elidia/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.elidia/.env
```
...
```bash
elidia auth add minimax-oauth --no-browser
```

Elidia will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Elidia implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Elidia generates a PKCE verifier / challenge pair and a random state value.
...
4. Elidia polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.elidia/auth.json` under the `minimax-oauth` key.
...
```bash
elidia config set model.default MiniMax-M2.7
elidia config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.elidia/config.yaml` will contain entries similar to:
...
```bash
elidia --provider minimax-oauth    # canonical
elidia --provider minimax-portal   # alias
elidia --provider minimax-global   # alias
elidia --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `elidia setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
elidia --provider minimax-oauth
```
...
Elidia refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Elidia marks the session as requiring re-login.
...
**Fix:** run `elidia auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Elidia raises a timeout error.

**Fix:** re-run `elidia auth add minimax-oauth` (or `elidia model`). The flow starts fresh.

### State mismatch (possible CSRF)

Elidia detected that the `state` value returned by the authorization server does not match what it sent.
...
If `elidia` cannot open a browser window, use `--no-browser`:

```bash
elidia auth add minimax-oauth --no-browser
```

Elidia prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `elidia model` and select MiniMax (OAuth), or run `elidia auth add minimax-oauth`.
...
```bash
elidia auth remove minimax-oauth
```
...
- [elidia doctor](../reference/cli-commands.md)
