---
sidebar_position: 7
title: "Use SOUL.md with Elidia"
description: "How to use SOUL.md to shape Elidia Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Elidia

`SOUL.md` is the **primary identity** for your Elidia instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Elidia to feel like the same assistant every time you talk to it — or if you want to replace the Elidia persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Elidia should be
- what Elidia should avoid stylistically
- how Elidia should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Elidia is and how Elidia speaks
...
Elidia now uses only the global SOUL file for the current instance:

```text
~/.elidia/SOUL.md
```

If you run Elidia with a custom home directory, it becomes:
...
Elidia automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Elidia does not overwrite it
- if the file exists but is empty, Elidia adds nothing from it to the prompt

## How Elidia uses it

When Elidia starts a session, it reads `SOUL.md` from `ELIDIA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Elidia feels.
...
Elidia already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Elidia should sound.

# Avoid
What Elidia should not do.

# Defaults
How Elidia should behave when ambiguity appears.
```
...
Then restart Elidia or start a new session.
...
### I edited SOUL.md but Elidia still sounds the same

Check:
- you edited `~/.elidia/SOUL.md` or `$ELIDIA_HOME/SOUL.md`
...
### Elidia is ignoring parts of my SOUL.md
