---
sidebar_position: 8
title: "Use Voice Mode with Elidia"
description: "A practical guide to setting up and using Elidia voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Elidia
...
:::tip
[Elidia Portal](/integrations/elidia-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Elidia sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Elidia.
...
## Step 1: make sure normal Elidia works first
...
Elidia supports both local and cloud speech stacks.
...
### If you use `elidia setup`

If you choose NeuTTS in the setup wizard, Elidia checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Elidia transcribes and responds
...
- asking Elidia to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Elidia loop.

## Tuning CLI behavior

### Silence threshold

If Elidia starts/stops too aggressively, tune:
...
Elidia stays a normal chat bot, but can speak replies.
...
- you want Elidia to function like a portable research or ops assistant
...
Elidia joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Elidia detects speech boundaries
- transcripts are posted in the associated text channel
- Elidia responds in text and audio
...
1. get text Elidia working
2. install `elidia-agent[voice]`
