...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `elidia send`."
---

# Pipe Script Output to Messaging Platforms

`elidia send` is a small, scriptable CLI that pushes a message to any
messaging platform Elidia is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | elidia send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `elidia
...
elidia send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
elidia send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
elidia send --list

# Filter by platform
elidia send --list telegram
```
...
Any platform Elidia ships adapters for works as a target:
...
`elidia send` resolves the message body in this order:

1. **Positional argument** — `elidia send --to telegram "hi"`
2. **`--file PATH`** — `elidia send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | elidia send --to telegram`

When stdin is a TTY (no pipe), Elidia does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  elidia send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `elidia send` reuses your Elidia config, the same script works on
any host where Elidia is installed — no need to export bot tokens into
...
`elidia send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  elidia send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | elidia send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.elidia/bin/elidia send \
...
```bash
./train.py --epochs 200 && \
  elidia send --to telegram "training done" || \
  elidia send --to telegram "training failed (exit $?)"
```
...
elidia send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(elidia send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `elidia send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `elidia send` calls
the platform's REST endpoint directly using credentials from
`~/.elidia/.env` and `~/.elidia/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `elidia gateway start` and retry.
...
# Machine-readable
elidia send --list --json
```

The listing is built from `~/.elidia/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`elidia gateway
...
| Approach | Multi-platform | Reuses Elidia creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `elidia send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`elidia send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `elidia send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `elidia send` shares with cron delivery.
