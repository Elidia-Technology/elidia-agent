---
sidebar_position: 6
title: "Use MCP with Elidia"
description: "A practical guide to connecting MCP servers to Elidia Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Elidia

This guide shows how to actually use MCP with Elidia Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Elidia tool
- you want Elidia to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Elidia to internal APIs, databases, or company systems without modifying Elidia core

Do not use MCP when:
- a built-in Elidia tool already solves the job well
...
- Elidia remains the agent
- MCP servers contribute tools
- Elidia discovers those tools at startup or reload time
...
If you installed Elidia with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Elidia banner/status should show MCP integration when configured
- ask Elidia what tools it has available
...
## WSL2: bridge Elidia in WSL to Windows Chrome

This is the practical setup when:

- Elidia runs inside WSL2
...
In this setup, Elidia does **not** connect to Chrome directly. Instead:

- Elidia runs in WSL
- Elidia starts a local stdio MCP server
...
```text
Elidia (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Elidia stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Elidia core browser transport
...
```bash
elidia mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
elidia mcp test chrome-devtools-win
```

Then start a fresh Elidia session or run:
...
Once loaded, Elidia can use the MCP-prefixed browser tools directly. For example:
...
If Elidia runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Elidia from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Elidia from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Elidia:
...
2. Elidia-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Elidia will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Elidia to reason over a bounded workspace.
...
Now Elidia can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Elidia core.
...
Because Elidia now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
