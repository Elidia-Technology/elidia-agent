...
This tutorial walks you through setting up a Telegram bot powered by Elidia Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Elidia Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.elidia/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Elidia itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Elidia Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_elidia_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Elidia Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Elidia Gateway...
...
# View live logs
journalctl --user -u elidia-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `elidia gateway install` to update the plist.
:::
...
```bash
elidia gateway stop && elidia gateway start
```
...
elidia pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.elidia/.env`:
...
Control how much detail the bot shows when using tools. In `~/.elidia/config.yaml`:
...
Customize how the bot communicates by editing `~/.elidia/SOUL.md`:

For a full guide, see [Use SOUL.md with Elidia](/guides/use-soul-with-elidia).
...
```bash
# From the CLI
elidia cron list          # View all scheduled jobs
elidia cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u elidia-gateway` (Linux) or `~/.elidia/logs/gateway.log` (macOS) |
| Cron job output | `~/.elidia/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.elidia/cron/jobs.json` |
| Pairing data | `~/.elidia/pairing/` |
| Session history | `~/.elidia/sessions/` |
