"""Centralized logging setup for Elidia Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.elidia/logs/`` (profile-aware via ``get_elidia_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``elidia_cli.web_server``,
    ``elidia_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from elidia_constants import get_config_path, get_elidia_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_elidia_session_injector", False):
    ...
    _session_record_factory._elidia_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``elidia logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "elidia_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("elidia_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "elidia_cli.web_server",
        "elidia_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Elidia logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    elidia_home
        Override for the Elidia home directory.  Falls back to
        ``get_elidia_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_elidia_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._elidia_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the elidia group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from elidia_cli.config import is_managed
