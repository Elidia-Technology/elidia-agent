...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_elidia_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.elidia/config.yaml (primary user-facing config)
    3. ~/.elidia/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_elidia_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``elidia config set gateway.streaming.*``
