...
There are two ways to add a platform to the Elidia gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.elidia/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Elidia code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `elidia gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `elidia_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Elidia core.
...
And add it to the `elidia-gateway` composite:

```python
"elidia-gateway": {
    "includes": [..., "elidia-your-platform"]
}
```
...
## 12. Status Display (`elidia_cli/status.py`)
...
## 13. Gateway Setup Wizard (`elidia_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ elidia_cli/ toolsets.py \
  --include="*.py" -l | sort -u
