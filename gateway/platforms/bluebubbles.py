...
# group mention gating without custom aliases, use conservative Elidia wake
# words so `require_mention: true` is a one-line enablement path.
DEFAULT_MENTION_PATTERNS = [
    r"(?<![\w@])@?elidia\s+agent\b[,:\-]?",
    r"(?<![\w@])@?elidia\b[,:\-]?",
]
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Elidia defaults).
        """
        if raw is None:
