...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Elidia command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Elidia accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Elidia ``/command`` text."""
...
# Uses get_elidia_home() so each profile gets its own Matrix store.
from elidia_constants import get_elidia_dir as _get_elidia_dir

_STORE_DIR = _get_elidia_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ELIDIA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ELIDIA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "elidia"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Elidia Agent" become "Agent".
        """
        if not body:
            return ""
