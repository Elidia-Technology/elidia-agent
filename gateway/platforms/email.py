"""
Email platform adapter for the Elidia gateway.

Allows users to interact with Elidia by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "elidia-agent" "version" "{_elidia_version}" '
            '"vendor" "AiUtils" '
            '"support-email" "noreply@aiutils.io")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Elidia Agent")
        ...
        msg_id = f"<elidia-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
