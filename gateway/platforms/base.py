...
# Audio file extensions Elidia recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Elidia' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Elidia-created Telegram private-chat
...
from elidia_constants import get_default_elidia_root, get_elidia_dir, get_elidia_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.elidia/.env manually."
)
...
# Default location: {ELIDIA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_elidia_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ElidiaAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_elidia_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_elidia_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_elidia_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_elidia_dir("cache/screenshots", "browser_screenshots")
_ELIDIA_HOME = get_elidia_home()
_ELIDIA_ROOT = get_default_elidia_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ELIDIA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ELIDIA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ELIDIA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ELIDIA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ELIDIA_HOME / "image_cache",
    _ELIDIA_HOME / "audio_cache",
    _ELIDIA_HOME / "video_cache",
    _ELIDIA_HOME / "document_cache",
    _ELIDIA_HOME / "browser_screenshots",
    ...
    _ELIDIA_HOME / "cache" / "images",
    _ELIDIA_HOME / "cache" / "audio",
    _ELIDIA_HOME / "cache" / "videos",
    _ELIDIA_HOME / "cache" / "documents",
    _ELIDIA_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Elidia profile and shared Elidia root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for elidia_root in (_ELIDIA_HOME, _ELIDIA_ROOT):
        denied.append(elidia_root / ".env")
        denied.append(elidia_root / "auth.json")
        denied.append(elidia_root / "credentials")
        denied.append(elidia_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ELIDIA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under an Elidia-managed cache, under an operator-allowlisted root
    (``ELIDIA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.elidia/.env,
    # ~/.elidia/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?elidia\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+elidia[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ELIDIA_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ELIDIA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ELIDIA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.elidia/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from elidia_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ELIDIA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ELIDIA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ELIDIA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ELIDIA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ELIDIA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ELIDIA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _elidia_run_generation is set on
