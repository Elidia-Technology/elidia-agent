"""
Session-scoped context variables for the Elidia gateway.

Replaces the previous ``os.environ``-based session state
(``ELIDIA_SESSION_PLATFORM``, ``ELIDIA_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["ELIDIA_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("ELIDIA_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("ELIDIA_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("ELIDIA_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("ELIDIA_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("ELIDIA_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("ELIDIA_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("ELIDIA_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("ELIDIA_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("ELIDIA_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("ELIDIA_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("ELIDIA_SESSION_MESSAGE_ID", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("ELIDIA_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("ELIDIA_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("ELIDIA_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "ELIDIA_SESSION_PLATFORM": _SESSION_PLATFORM,
    "ELIDIA_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "ELIDIA_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "ELIDIA_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "ELIDIA_SESSION_USER_ID": _SESSION_USER_ID,
    "ELIDIA_SESSION_USER_NAME": _SESSION_USER_NAME,
    "ELIDIA_SESSION_KEY": _SESSION_KEY,
    "ELIDIA_SESSION_ID": _SESSION_ID,
    "ELIDIA_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "ELIDIA_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "ELIDIA_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "ELIDIA_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``ELIDIA_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("ELIDIA_SESSION_*", default)``.
