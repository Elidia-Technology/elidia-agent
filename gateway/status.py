...
The PID file lives at ``{ELIDIA_HOME}/gateway.pid``.  ELIDIA_HOME defaults to
``~/.elidia`` but can be overridden via the environment variable.  This means
separate ELIDIA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ELIDIA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ELIDIA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "elidia" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Elidia gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ELIDIA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``elidia.service`` + ``elidia-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``elidia gateway stop`` on Windows would be misclassified as an
