# Contributing to Elidia Agent

Thank you for contributing to Elidia Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Elidia to work everywhere.
...
Bundled skills (in `skills/`) ship with every Elidia install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `elidia skills browse` (labeled "official") and install it with `elidia skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [AiUtils Discord](https://discord.gg/AiUtils). Users can install it with `elidia skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.elidia/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(elidia_home, config)` for setup-wizard integration
...
- Integrate with `elidia memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.elidia/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.elidia/config.yaml
touch ~/.elidia/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.elidia/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/elidia" ~/.local/bin/elidia
...
├── cli.py                    # ElidiaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (elidia-cli, elidia-telegram, etc.)
├── elidia_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── elidia_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Elidia Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.elidia/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (aiutils.io)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.elidia/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.elidia/`)

| Path | Purpose |
|------|---------|
| `~/.elidia/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.elidia/.env` | API keys and secrets |
| `~/.elidia/auth.json` | OAuth credentials (Elidia Portal) |
| `~/.elidia/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.elidia/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.elidia/state.db` | SQLite session database |
| `~/.elidia/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.elidia/cron/` | Scheduled job data |
| `~/.elidia/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`elidia_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.elidia/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Elidia Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_ELIDIA_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.elidia`:
...
The user may skip setup and keep loading the skill. Elidia only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `elidia setup` or update `~/.elidia/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Elidia Agent" is the secondary collaborator. If the contributor's commit shows "Elidia Agent" as author because they used Elidia to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Elidia tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `elidia --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Elidia uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.elidia/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `elidia_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `elidia_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Elidia runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the elidia wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `elidia_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `elidia_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `elidia_cli/gateway_windows.py::
...
Elidia has terminal access. Security matters.
...
2. **Test manually**: Run `elidia` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Elidia-Technology/elidia-agent/issues)
- Include: OS, Python version, Elidia version (`elidia version`), full error traceback
...
- **Discord**: [discord.gg/AiUtils](https://discord.gg/AiUtils) — for questions, showcasing projects, and sharing skills
