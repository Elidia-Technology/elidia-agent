# Elidia Agent Has Had "Routines" Since March
...
Elidia equivalent — works today:
```bash
elidia webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Elidia implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Elidia Agent |
|---|---|---|
...
## Things Elidia Does That Routines Can't
...
Elidia has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Elidia Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
elidia gateway setup    # enable webhooks
elidia webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [aiutils.io/elidia/guides/automation-templates](https://aiutils.io/elidia/guides/automation-templates)

Documentation: [aiutils.io](https://aiutils.io)

GitHub: [github.com/Elidia-Technology/elidia-agent](https://github.com/Elidia-Technology/elidia-agent)

---

*Elidia Agent is built by [AiUtils](https://aiutils.io). Open source, model-agnostic, runs on your infrastructure.*
