#!/usr/bin/env python3
"""
SQLite State Store for Elidia Agent.
...
# elidia_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``elidia_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see elidia_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple elidia processes (gateway + CLI sessions + worktree agents)
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `elidia update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._elidia_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._elidia_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Elidia) receive
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Elidia, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        An Elidia session may only be linked to one Telegram topic in MVP.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ELIDIA_DISABLE_FTS_TRIGRAM`` or not yet created), so
