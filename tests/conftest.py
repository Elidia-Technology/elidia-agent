"""Shared fixtures for the elidia-agent test suite.
...
2. **Isolated ELIDIA_HOME.** ELIDIA_HOME points to a per-test tempdir so
   code reading ``~/.elidia/*`` via ``get_elidia_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".elidia"`` instead of the canonical
   ``get_elidia_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ELIDIA_SESSION_* inheritance** — the agent's current gateway
...
# ELIDIA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ELIDIA_BEHAVIORAL_VARS = frozenset({
    "ELIDIA_YOLO_MODE",
    "ELIDIA_INTERACTIVE",
    "ELIDIA_QUIET",
    "ELIDIA_TOOL_PROGRESS",
    "ELIDIA_TOOL_PROGRESS_MODE",
    "ELIDIA_MAX_ITERATIONS",
    "ELIDIA_SESSION_PLATFORM",
    "ELIDIA_SESSION_CHAT_ID",
    "ELIDIA_SESSION_CHAT_NAME",
    "ELIDIA_SESSION_THREAD_ID",
    "ELIDIA_SESSION_SOURCE",
    "ELIDIA_SESSION_KEY",
    "ELIDIA_GATEWAY_SESSION",
    "ELIDIA_CRON_SESSION",
    "_ELIDIA_GATEWAY",
    "ELIDIA_PLATFORM",
    "ELIDIA_MODEL",
    "ELIDIA_INFERENCE_MODEL",
    "ELIDIA_INFERENCE_PROVIDER",
    "ELIDIA_TUI_PROVIDER",
    "ELIDIA_MANAGED",
    "ELIDIA_DEV",
    "ELIDIA_CONTAINER",
    ...
    "ELIDIA_TIMEZONE",
    "ELIDIA_REDACT_SECRETS",
    "ELIDIA_BACKGROUND_NOTIFICATIONS",
    "ELIDIA_EXEC_ASK",
    "ELIDIA_HOME_MODE",
    "ELIDIA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.elidia/kanban.db instead of the per-test ELIDIA_HOME.
    "ELIDIA_KANBAN_DB",
    "ELIDIA_KANBAN_BOARD",
    "ELIDIA_KANBAN_HOME",
    "ELIDIA_KANBAN_WORKSPACES_ROOT",
    "ELIDIA_KANBAN_LOGS_ROOT",
    "ELIDIA_KANBAN_TASK",
    "ELIDIA_KANBAN_WORKSPACE",
    "ELIDIA_KANBAN_RUN_ID",
    "ELIDIA_KANBAN_CLAIM_LOCK",
    "ELIDIA_KANBAN_DISPATCH_IN_GATEWAY",
    "ELIDIA_TENANT",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `elidia` plugin auto-registers itself on plugin discovery,
    ...
    "ELIDIA_DASHBOARD_OAUTH_CLIENT_ID",
    "ELIDIA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ELIDIA_HOME to per-test tempdirs so code that
    reads ``~/.elidia/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ELIDIA_* vars that could change test semantics.
    for name in _ELIDIA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ELIDIA_HOME to a per-test tempdir. Code that reads
    #    ``~/.elidia/*`` via ``get_elidia_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.elidia/*`` via
    #    ``Path.home() / ".elidia"`` instead of ``get_elidia_home()``
    #    is a bug to fix at the callsite.
    fake_elidia_home = tmp_path / "elidia_test"
    fake_elidia_home.mkdir()
    (fake_elidia_home / "sessions").mkdir()
    (fake_elidia_home / "cron").mkdir()
    (fake_elidia_home / "memories").mkdir()
    (fake_elidia_home / "skills").mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(fake_elidia_home))
    ...
    #    ~/.elidia/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import elidia_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_elidia_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal elidia config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``elidia-gateway`` process
...
# correlated with a ``tests/elidia_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> elidia-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart elidia-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting elidia/python patterns are also blocked.
    """
    ...
    def _matches_elidia_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ELIDIA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_elidia_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch elidia-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m elidia_cli.main".
                if (
                    "elidia" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live elidia-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting elidia/python could hit the live gateway. "
