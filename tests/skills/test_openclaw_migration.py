...
"""
    ...
    assert "Tyler Williams > Active Projects: Elidia Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `elidia cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # elidia_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.elidia/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "elidia_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Elidia``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Elidia prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Elidia to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Elidia config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Elidia uses tools well"
    # All-lowercase matches → lowercase ``elidia``; this preserves the
    # real filesystem path ``~/.elidia`` (Elidia home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "elidia should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Elidia remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "elidia prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Elidia was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "elidia uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Elidia said to ask Elidia about Elidia settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.elidia`` (the real Elidia home), not the broken
    ``.Elidia``.
    ...
    rewritten to ``~/.Elidia/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.elidia/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .elidia directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.elidia'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "elidia config path: ~/.elidia/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Elidia" in result
