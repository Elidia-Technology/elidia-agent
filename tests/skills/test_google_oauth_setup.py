...
class TestElidiaConstantsFallback:
    """Tests for _elidia_home.py fallback when elidia_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_elidia_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _elidia_home.py with elidia_constants blocked."""
        monkeypatch.setitem(sys.modules, "elidia_constants", None)
        spec = importlib.util.spec_from_file_location("_elidia_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_elidia_home_env_var(self, monkeypatch, tmp_path):
        """When elidia_constants is missing, ELIDIA_HOME comes from env var."""
        monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "custom-elidia"))
        module = self._load_helper(monkeypatch)
        assert module.get_elidia_home() == tmp_path / "custom-elidia"

    def test_fallback_defaults_to_dot_elidia(self, monkeypatch):
        """When elidia_constants is missing and ELIDIA_HOME unset, default to ~/.elidia."""
        monkeypatch.delenv("ELIDIA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_elidia_home() == Path.home() / ".elidia"

    def test_fallback_ignores_empty_elidia_home(self, monkeypatch):
        """Empty/whitespace ELIDIA_HOME is treated as unset."""
        monkeypatch.setenv("ELIDIA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_elidia_home() == Path.home() / ".elidia"

    def test_fallback_display_elidia_home_shortens_path(self, monkeypatch):
        """Fallback display_elidia_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ELIDIA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_elidia_home() == "~/.elidia"

    def test_fallback_display_elidia_home_profile_path(self, monkeypatch):
        """Fallback display_elidia_home() handles profile paths under ~/."""
        monkeypatch.setenv("ELIDIA_HOME", str(Path.home() / ".elidia/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_elidia_home() == "~/.elidia/profiles/coder"

    def test_fallback_display_elidia_home_custom_path(self, monkeypatch):
        """Fallback display_elidia_home() returns full path for non-home locations."""
        monkeypatch.setenv("ELIDIA_HOME", "/opt/elidia-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_elidia_home() == "/opt/elidia-custom"

    def test_delegates_to_elidia_constants_when_available(self):
        """When elidia_constants IS importable, _elidia_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_elidia_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import elidia_constants
        assert module.get_elidia_home is elidia_constants.get_elidia_home
        assert module.display_elidia_home is elidia_constants.display_elidia_home
