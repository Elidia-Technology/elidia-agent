...
Tests cover:
- managed_elidia_tools_enabled() subscription-based gate
...
# managed_elidia_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedElidiaToolsEnabled:
    """Subscription-based gate: True for paid Elidia subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "elidia_cli.elidia_account.get_elidia_portal_account_info",
            lambda: ElidiaPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_elidia_tools_enabled() is False
    ...
    def test_enabled_for_paid_subscriber(self, monkeypatch):
        ...
        assert managed_elidia_tools_enabled() is True
    ...
    def test_force_fresh_is_forwarded(self, monkeypatch):
        ...
        assert managed_elidia_tools_enabled(force_fresh=True) is True
        assert calls == [True]
...
class TestElidiaToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "elidia_cli.elidia_account.get_elidia_portal_account_info",
            lambda force_fresh=False: ElidiaPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
                paid_service_access=False,
        ...
        message = elidia_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, elidia_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_elidia_tools_enabled",
            lambda: elidia_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, elidia_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_auto_falls_back_to_direct(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=False, elidia_enabled=True)
    ...
    def test_auto_managed_ready_but_elidia_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, elidia_enabled=False)
        assert result["selected_backend"] == "direct"

    def test_auto_nothing_when_only_managed_and_elidia_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=False, managed_ready=True, elidia_enabled=False)
        assert result["selected_backend"] is None

    # --- direct mode ---

    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, elidia_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, elidia_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---

    def test_managed_selects_managed_when_ready_and_enabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, elidia_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_managed_none_when_not_ready(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=False, elidia_enabled=True)
        assert result["selected_backend"] is None

    def test_managed_blocked_when_elidia_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, elidia_enabled=False)
        assert result["selected_backend"] is None
