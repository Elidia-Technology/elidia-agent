...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ELIDIA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ELIDIA_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ELIDIA_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ELIDIA_INTERACTIVE", raising=False)
    monkeypatch.delenv("ELIDIA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ELIDIA_CRON_SESSION", "1")
    monkeypatch.delenv("ELIDIA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_elidia_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ELIDIA_HOME": "/h", "ELIDIA_PROFILE": "p",
        "ELIDIA_CONFIG": "/c.yaml", "ELIDIA_ENV": "/e",
        # other ELIDIA_* → dropped (broad prefix removed)
        "ELIDIA_BASE_URL": "https://x", "ELIDIA_INTERACTIVE": "1",
        "ELIDIA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ELIDIA_HOME", "ELIDIA_PROFILE", "ELIDIA_CONFIG", "ELIDIA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ELIDIA_BASE_URL", "ELIDIA_INTERACTIVE", "ELIDIA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_elidia_vars(caplog):
    """Dropping a non-allowlisted, non-secret ELIDIA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ELIDIA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ELIDIA_HOME": "/h",          # allowlisted → kept, not logged
        "ELIDIA_BASE_URL": "https://x",   # dropped → logged
        "ELIDIA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ELIDIA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ELIDIA_HOME" in out and "PATH" in out
    assert "ELIDIA_BASE_URL" not in out and "ELIDIA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ELIDIA_BASE_URL" in msgs and "ELIDIA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ELIDIA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ELIDIA_* vars."""
    import logging
