...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_elidia_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "elidia-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.aiutils.io"
    assert result.elidia_user_token == "elidia-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_elidia_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "aiutils.io",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "aiutils.io"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_elidia_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "elidia-token",
        )
...
def test_read_elidia_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    assert managed_tool_gateway.read_elidia_access_token() == "fresh-token"
