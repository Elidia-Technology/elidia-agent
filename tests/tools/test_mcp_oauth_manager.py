...
def test_elidia_provider_subclass_exists():
    """ElidiaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ELIDIA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ELIDIA_PROVIDER_CLS is not None
    assert issubclass(_ELIDIA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_elidia_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ElidiaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ELIDIA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    ...
    assert _ELIDIA_PROVIDER_CLS is not None
    assert isinstance(provider, _ELIDIA_PROVIDER_CLS)
    assert provider._elidia_server_name == "srv"
