...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps Elidia Portal billing
...
class TestManagedGatewayErrorTranslation:
    """4xx from the Elidia managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + elidia tools."""
        ...
        managed_gateway.elidia_user_token = "test-token"
