...
Verifies:
  - Tools are gated on ELIDIA_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `elidia chat` sessions (no ELIDIA_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("ELIDIA_KANBAN_TASK", raising=False)
    ...
    schema = registry.get_definitions(set(resolve_toolset("elidia-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ELIDIA_KANBAN_TASK", "t_fake")
    home = tmp_path / ".elidia"
    home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the ELIDIA_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: ELIDIA_HOME isolated, ELIDIA_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".elidia"
    home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(home))
    monkeypatch.setenv("ELIDIA_PROFILE", "test-worker")
    monkeypatch.delenv("ELIDIA_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ELIDIA_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["elidia_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ELIDIA_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to ELIDIA_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``ELIDIA_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``elidia-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "elidia-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from ELIDIA_PROFILE in the fixture, not the
        # caller-supplied "elidia-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ELIDIA_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no ELIDIA_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ELIDIA_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from elidia_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no ELIDIA_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (ELIDIA_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has ELIDIA_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no ELIDIA_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # ELIDIA_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("ELIDIA_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("ELIDIA_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no ELIDIA_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via ELIDIA_KANBAN_BOARD env var,
...
# restarting Elidia. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Elidia home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    ELIDIA_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".elidia"
    home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(home))
    # Make sure neither ELIDIA_KANBAN_DB nor ELIDIA_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("ELIDIA_KANBAN_DB", raising=False)
    monkeypatch.delenv("ELIDIA_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("ELIDIA_KANBAN_TASK", raising=False)
    monkeypatch.setenv("ELIDIA_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning ELIDIA_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".elidia"
    home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(home))
    monkeypatch.setenv("ELIDIA_PROFILE", "alt-worker")
    monkeypatch.delenv("ELIDIA_KANBAN_DB", raising=False)
    monkeypatch.delenv("ELIDIA_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
