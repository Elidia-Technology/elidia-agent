...
class TestWriteDenyExactPaths:
    ...
    def test_elidia_env(self):
        # ``.env`` under the active ELIDIA_HOME (profile-aware, not just
        # ``~/.elidia``) must be write-denied. The hermetic test conftest
        # points ELIDIA_HOME at a tempdir — resolve via get_elidia_home()
        # to match the denylist.
        from elidia_constants import get_elidia_home
        path = str(get_elidia_home() / ".env")
        assert _is_write_denied(path) is True

    def test_elidia_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.elidia/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "elidia_root"
        ...
        # Sanity check: ELIDIA_HOME does point to the profile dir, not the root.
        from elidia_constants import get_elidia_home, get_default_elidia_root
        assert get_elidia_home() == profile_home
        assert get_default_elidia_root() == root
...
class TestWriteAllowed:
    ...
    def test_elidia_config_not_env(self):
        path = os.path.join(str(Path.home()), ".elidia", "config.yaml")
