...
def _restore_tool_modules():
    original_elidia_home = os.environ.get("ELIDIA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_elidia_home is None:
            os.environ.pop("ELIDIA_HOME", None)
        else:
            os.environ["ELIDIA_HOME"] = original_elidia_home
        _reset_modules(("tools", "elidia_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    elidia_cli = types.ModuleType("elidia_cli")
    elidia_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["elidia_cli"] = elidia_cli
    elidia_home = tmp_path / "elidia-home"
    os.environ["ELIDIA_HOME"] = str(elidia_home)
    sys.modules["elidia_cli.config"] = types.SimpleNamespace(
        get_elidia_home=lambda: elidia_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="elidia-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": elidia_home / "modal_snapshots.json",
