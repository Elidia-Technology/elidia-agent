...
class TestScanMemoryContent:
    ...
    def test_elidia_config_mod_blocked(self):
        result = _scan_memory_content("edit .elidia/config.yaml to change settings")
        assert "Blocked" in result
        assert "elidia_config_mod" in result
        result = _scan_memory_content("update .elidia/SOUL.md with new personality")
        assert "Blocked" in result
        assert "elidia_config_mod" in result
    ...
    def test_elidia_config_mod_no_false_positives(self):
        """Merely mentioning elidia config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .elidia/config.yaml for settings") is None
        assert _scan_memory_content("Read .elidia/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .elidia/config.yaml file contains runtime options") is None
