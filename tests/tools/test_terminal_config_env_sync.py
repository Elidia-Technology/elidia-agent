...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. elidia_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``elidia config set …``)
...
dicts) keeps the test independent of the user's ~/.elidia/config.yaml and
mirrors the pattern used in tests/elidia_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``elidia config set foo bar``."""
    from elidia_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``elidia config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`elidia config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in elidia_cli/config.py:set_config_value."
    )
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Elidia exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Elidia sweeps stale
