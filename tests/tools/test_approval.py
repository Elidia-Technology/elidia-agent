...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("elidia_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("elidia_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ELIDIA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_elidia_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.elidia/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_elidia_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ELIDIA_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_elidia_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ELIDIA_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestElidiaConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.elidia/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.elidia/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.elidia/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.elidia/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.elidia/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.elidia/config.yaml")
        assert dangerous is True
        assert "elidia config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.elidia/config.yaml")
        assert dangerous is True

    def test_custom_elidia_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ELIDIA_HOME/config.yaml")
        assert dangerous is True

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.elidia/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Elidia config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_elidia_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ELIDIA_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.elidia/elidia-agent && source venv/bin/activate && python -m elidia_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m elidia_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m elidia_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "elidia_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart elidia-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_elidia_detected(self):
        """pkill targeting elidia/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_elidia_detected(self):
        cmd = "killall elidia"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep elidia) bypasses the pkill/killall name-matching
    ...
    def test_pkill_elidia_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ELIDIA_GATEWAY_SESSION", "ELIDIA_CRON_SESSION",
                      "ELIDIA_YOLO_MODE",
                      "ELIDIA_SESSION_KEY", "ELIDIA_INTERACTIVE")
        }
        os.environ.pop("ELIDIA_YOLO_MODE", None)
        os.environ.pop("ELIDIA_INTERACTIVE", None)
        # ELIDIA_CRON_SESSION takes priority over ELIDIA_GATEWAY_SESSION in
        ...
        os.environ.pop("ELIDIA_CRON_SESSION", None)
        os.environ["ELIDIA_GATEWAY_SESSION"] = "1"
        os.environ["ELIDIA_SESSION_KEY"] = self.SESSION_KEY
