...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `elidia-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.elidia/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.elidia/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because elidia uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("ELIDIA_INTERACTIVE", raising=False)
        monkeypatch.delenv("ELIDIA_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("ELIDIA_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ELIDIA_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ELIDIA_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ELIDIA_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ELIDIA_INTERACTIVE", "ELIDIA_GATEWAY_SESSION", "ELIDIA_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ELIDIA_INTERACTIVE", "ELIDIA_GATEWAY_SESSION", "ELIDIA_EXEC_ASK"):
