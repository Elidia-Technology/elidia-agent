...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        elidia_home.mkdir()
        (elidia_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(elidia_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.elidia/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        (elidia_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        (elidia_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        (elidia_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.elidia/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        elidia_home = tmp_path / ".elidia"
        (elidia_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ELIDIA_HOME": str(elidia_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.elidia")

        assert mounts[0]["container_path"] == "/home/user/.elidia/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.elidia/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.elidia/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ELIDIA_HOME."""
        monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / ".elidia"))
        (tmp_path / ".elidia").mkdir()

        # Create a sensitive file one level above elidia_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside elidia_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ELIDIA_HOME must still be registered."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        (elidia_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_elidia_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ELIDIA_HOME must be allowed."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        subdir = elidia_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ELIDIA_HOME pointing outside must be rejected."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

        # Create a sensitive file outside elidia_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside elidia_home pointing outside
        symlink = elidia_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ELIDIA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, elidia_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ELIDIA_HOME."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        ...
        sensitive.write_text("{}")
        self._write_config(elidia_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(elidia_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ELIDIA_HOME via config must still mount."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

        (elidia_home / "oauth.json").write_text("{}")
        self._write_config(elidia_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        (elidia_home / "cache" / "documents").mkdir(parents=True)
        (elidia_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.elidia/cache/documents" in paths
        assert "/root/.elidia/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.elidia/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        # Use legacy dir name — get_elidia_dir prefers old if it exists
        (elidia_home / "document_cache").mkdir()
        (elidia_home / "image_cache").mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(elidia_home / "document_cache") in host_paths
        assert str(elidia_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.elidia/cache/documents" in container_paths
        assert "/root/.elidia/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        doc_dir = elidia_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        ss_dir = elidia_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.elidia/cache/screenshots/session_abc/screen1.png"
