...
ELIDIA_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
ELIDIA_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-elidia-setup, which runs lexicographically BEFORE
...
This mirrors the existing ELIDIA_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$ELIDIA_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "ELIDIA_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $ELIDIA_HOME.

    ``env_value`` is the ELIDIA_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export ELIDIA_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset ELIDIA_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'ELIDIA_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when ELIDIA_GATEWAY_BOOTSTRAP_STATE is unset"
