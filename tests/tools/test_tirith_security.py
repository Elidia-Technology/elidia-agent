...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._elidia_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(elidia_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(elidia_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_elidia_bin(self):
        """After _INSTALL_FAILED, manual install in ELIDIA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        elidia_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(elidia_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(elidia_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._elidia_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == elidia_bin
            assert _tirith_mod._resolved_path == elidia_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ELIDIA_HOME isolation
# ---------------------------------------------------------------------------

class TestElidiaHomeIsolation:
    def test_elidia_bin_dir_respects_elidia_home(self):
        """_elidia_bin_dir must use ELIDIA_HOME, not hardcoded ~/.elidia."""
        from tools.tirith_security import _elidia_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ELIDIA_HOME": tmpdir}):
            result = _elidia_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_elidia_home(self):
        """_failure_marker_path must use ELIDIA_HOME, not hardcoded ~/.elidia."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ELIDIA_HOME": "/custom/elidia"}):
            result = _failure_marker_path()
        assert result == "/custom/elidia/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ELIDIA_HOME; verify it's active."""
        elidia_home = os.getenv("ELIDIA_HOME")
        assert elidia_home is not None, "ELIDIA_HOME should be set by conftest"
        assert "elidia_test" in elidia_home, "Should point to test temp dir"

    def test_get_elidia_home_fallback(self):
        """Without ELIDIA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_elidia_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ELIDIA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".elidia")
            result = _get_elidia_home()
