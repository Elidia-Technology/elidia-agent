...
def _clean_state():
    ...
    saved = {}
    for k in ("ELIDIA_INTERACTIVE", "ELIDIA_GATEWAY_SESSION", "ELIDIA_EXEC_ASK", "ELIDIA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ELIDIA_INTERACTIVE"] = "1"
        session_key = os.getenv("ELIDIA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ELIDIA_INTERACTIVE or ELIDIA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
