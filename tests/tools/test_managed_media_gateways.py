...
@pytest.fixture(autouse=True)
def _enable_managed_elidia_tools(monkeypatch):
    """Patch the source modules so managed_elidia_tools_enabled() returns True
...
def test_managed_fal_submit_uses_gateway_origin_and_elidia_token(monkeypatch):
    captured = {}
    _install_fake_tools_package()
    _install_fake_fal_client(captured)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "elidia-token")
    ...
    assert captured["client_key"] == "elidia-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "aiutils.io")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "elidia-token")
    ...
    assert captured["api_key"] == "elidia-token"
    assert captured["base_url"] == "https://openai-audio-gateway.aiutils.io/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.aiutils.io/v1"
...
def test_video_gen_managed_fal_submit_uses_gateway(monkeypatch):
    ...
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "elidia-video-token")
    ...
    assert captured["submit_via"] == "managed_client"
    assert captured["client_key"] == "elidia-video-token"
