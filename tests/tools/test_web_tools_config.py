...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_elidia_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_elidia_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_elidia_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "aiutils.io"}):
            with patch("tools.web_tools._read_elidia_access_token", return_value="elidia-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="elidia-token",
                        api_url="https://firecrawl-gateway.aiutils.io",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "aiutils.io",
            ...
            with patch("tools.web_tools._read_elidia_access_token", return_value="elidia-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="elidia-token",
                        api_url="http://firecrawl-gateway.aiutils.io",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_default_gateway_domain_targets_elidia_production_origin(self):
    ...
    def test_elidia_auth_token_respects_elidia_home_override(self, tmp_path):
        """Auth lookup should read from ELIDIA_HOME/auth.json, not ~/.elidia/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".elidia").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "ELIDIA_HOME": str(elidia_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_elidia_access_token() == "elidia-token"
...
class TestBackendSelection:
    ...
    ``elidia tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_returns_true(self):
        with patch("tools.web_tools._peek_elidia_access_token", return_value="elidia-token"):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "elidia_cli.auth.resolve_elidia_access_token",
            _record_refresh,
        )
