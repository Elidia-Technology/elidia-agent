...
def _enable_managed_elidia_tools(monkeypatch):
    """Ensure managed_elidia_tools_enabled() returns True even after module reloads.
    ...
    elidia_cli.elidia_account — so the function body returns True.
    """
    monkeypatch.setattr(
...
def test_browser_use_explicit_local_mode_stays_local_even_when_managed_gateway_is_ready(tmp_path):
    ...
    env.update({
        ...
        "TOOL_GATEWAY_USER_TOKEN": "elidia-token",
...
def test_browser_use_availability_skips_refresh_for_expired_cached_gateway_token(tmp_path, monkeypatch):
    ...
    (tmp_path / "auth.json").write_text(
        '{"providers":{"elidia":{"access_token":"expired-token","refresh_token":"refresh-token","expires_at":"%s"}}}'
        % expired_at,
        encoding="utf-8",
    )
    refresh_calls = []
...
def test_browser_use_managed_gateway_adds_idempotency_key_and_persists_external_call_id():
    ...
    assert sent_headers["X-Browser-Use-API-Key"] == "elidia-token"
