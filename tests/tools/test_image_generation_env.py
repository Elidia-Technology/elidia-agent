...
def test_no_backend_message_mentions_fal_signup_and_plugins(monkeypatch):
    ...
    monkeypatch.setattr(
        image_generation_tool, "managed_elidia_tools_enabled", lambda: False
    )
    ...
    assert "elidia tools" in msg or "elidia plugins" in msg
...
def test_no_backend_message_mentions_managed_gateway_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        image_generation_tool, "managed_elidia_tools_enabled", lambda: True
    )

    msg = image_generation_tool._build_no_backend_setup_message()

    assert "managed FAL gateway" in msg
    assert "Elidia account" in msg or "elidia setup" in msg
