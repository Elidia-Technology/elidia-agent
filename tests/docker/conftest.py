...
Override the image with ``ELIDIA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ELIDIA_TEST_IMAGE", "elidia-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ELIDIA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ELIDIA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged elidia user
# ---------------------------------------------------------------------------
#
# Background: every Elidia runtime path inside the container drops to UID
# 10000 (the ``elidia`` user) via ``s6-setuidgid elidia``. ``docker exec``
...
# fails (PermissionError swallowed) for elidia, so a test that ran as root
...
# helpers below run every probe under ``-u elidia`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: elidia).
