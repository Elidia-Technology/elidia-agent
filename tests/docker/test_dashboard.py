"""Harness: dashboard opt-in via ELIDIA_DASHBOARD.

Today (tini): dashboard starts once when ELIDIA_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``elidia`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without ELIDIA_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "elidia dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without ELIDIA_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without ELIDIA_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `elidia doctor` and any other health check).
    ...
    ELIDIA_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without ELIDIA_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with ELIDIA_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # ELIDIA_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
...
def test_dashboard_opt_in_starts(
    ...
    """With ELIDIA_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ELIDIA_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'elidia dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with ELIDIA_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """ELIDIA_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ELIDIA_DASHBOARD=1", "-e", "ELIDIA_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "elidia dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as elidia, so the
    # elidia user can kill it (same UID).
...
# whenever `ELIDIA_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled elidia
    provider activates whenever ``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``elidia`` in the
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ELIDIA_DASHBOARD=1",
         "-e", "ELIDIA_DASHBOARD_HOST=0.0.0.0",
         "-e", "ELIDIA_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "elidia" in provider_names, (
        "Bundled dashboard_auth/elidia provider should register when "
        f"ELIDIA_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and ELIDIA_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``ELIDIA_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    envs (``ELIDIA_DASHBOARD``, ``ELIDIA_DASHBOARD_TUI``).
    ...
    assert status.get("auth_required") is False, (
        "ELIDIA_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
