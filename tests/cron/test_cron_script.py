...
- Path resolution (absolute, relative to ELIDIA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ELIDIA_HOME."""
    elidia_home = tmp_path / ".elidia"
    elidia_home.mkdir()
    (elidia_home / "cron").mkdir()
    (elidia_home / "cron" / "output").mkdir()
    (elidia_home / "scripts").mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ELIDIA_DIR", elidia_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", elidia_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", elidia_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", elidia_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.elidia/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ELIDIA_SESSION_PLATFORM") is None
        assert os.environ.get("ELIDIA_SESSION_CHAT_ID") is None
        assert os.environ.get("ELIDIA_SESSION_CHAT_NAME") is None
