...
def server():
    with patch.dict(
        "sys.modules",
        {
            "elidia_constants": MagicMock(
                get_elidia_home=MagicMock(return_value="/tmp/elidia_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'elidia-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'elidia-release' patched"
    }
