...
binary download are exercised manually by `elidia secrets bitwarden
...
def elidia_home(tmp_path, monkeypatch):
    """Point Elidia at an isolated home directory."""
    home = tmp_path / ".elidia"
    home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(home))
    # Some modules cache get_elidia_home; clear if needed.
    import elidia_constants
    if hasattr(elidia_constants, "_ELIDIA_HOME_CACHE"):
        elidia_constants._ELIDIA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(elidia_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(elidia_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(elidia_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from elidia_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under elidia_home/cache/."""
    home = tmp_path / ".elidia"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
