"""Tests for ACP Registry metadata shipped with Elidia."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "elidia-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Elidia Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/Elidia-Technology/elidia-agent"
    assert data["website"].startswith("https://aiutils.io/")
    assert data["authors"] == ["AiUtils"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"elidia-agent-cli[acp]=={data['version']}"
    assert uvx["args"] == ["elidia-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"elidia-agent-cli[acp]=={_pyproject_version()}"
    )
