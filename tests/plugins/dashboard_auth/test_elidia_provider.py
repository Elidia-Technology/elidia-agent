"""Tests for the bundled Elidia dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.elidia/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.elidia as elidia_plugin
from elidia_cli.dashboard_auth import (
    InvalidCodeError,
    LoginStart,
    ProviderError,
    RefreshExpiredError,
...
def _patched_jwks(provider: elidia_plugin.ElidiaDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(elidia_plugin.ElidiaDashboardAuthProvider)

    def test_name_and_display(self):
        p = elidia_plugin.ElidiaDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "elidia"
        assert p.display_name == "AiUtils"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            elidia_plugin.ElidiaDashboardAuthProvider(
                client_id="elidia-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("ELIDIA_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "ELIDIA_DASHBOARD_OAUTH_CLIENT_ID" in elidia_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: ELIDIA_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Elidia Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("ELIDIA_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, elidia_plugin.ElidiaDashboardAuthProvider)
        assert registered._portal_url == "https://developer.aiutils.io"
        # Skip reason cleared on successful registration.
        assert elidia_plugin.LAST_SKIP_REASON == ""

    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "elidia-dashboard")
        monkeypatch.setenv("ELIDIA_DASHBOARD_PORTAL_URL", "https://p.example")
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason names the offending value + contract shape.
        assert "agent:" in elidia_plugin.LAST_SKIP_REASON
        assert "elidia-dashboard" in elidia_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("ELIDIA_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("ELIDIA_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://developer.aiutils.io"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID``
    and ``ELIDIA_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``elidia_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "elidia_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        ELIDIA_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "ELIDIA_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("ELIDIA_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        elidia_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in elidia_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {elidia_plugin.LAST_SKIP_REASON!r}"
        )
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "elidia_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        elidia_plugin.register(ctx)
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return elidia_plugin.ElidiaDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://elidia.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://elidia.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "elidia_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["elidia_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["elidia_session_pkce"] != b.cookie_payload[
            "elidia_session_pkce"
        ]
    ...
    def test_rejects_http_with_non_localhost(self, provider):
        with pytest.raises(ProviderError, match="localhost"):
            provider.start_login(
                redirect_uri="http://elidia.fly.dev/auth/callback"
            )
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.elidia.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "elidia"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.elidia.httpx.post",
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between ELIDIA_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
        token = _mint_token(rsa_keypair, iss="https://evil.example")
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.elidia"):
            session = provider.verify_session(access_token=token)
        assert session is not None
        assert any(
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.elidia.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-elidia-refresh-token"] == "rt_old_value"
    ...
    def test_refresh_empty_token_raises_refresh_expired_without_network(self, provider):
        ...
        with patch("plugins.dashboard_auth.elidia.httpx.post") as mock_post:
