...
Run from the PR worktree:
    ...
    cd ~/.elidia/elidia-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.elidia/elidia-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.elidia/elidia-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like an elidia-agent checkout"
)
...
# Isolated ELIDIA_HOME for the config write.
