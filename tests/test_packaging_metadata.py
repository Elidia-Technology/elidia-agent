...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``elidia_cli``) for the package
    itself and ``elidia_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``elidia_cli`` without ``elidia_cli.*``,
    so the wheel shipped ``elidia_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'elidia_cli.dashboard_auth'``.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (elidia_cli/plugins.py) registers each bundled plugin by
