"""Tests for elidia_constants module."""
...
class TestGetDefaultElidiaRoot:
    """Tests for get_default_elidia_root() — Docker/custom deployment awareness."""

    def test_no_elidia_home_returns_native(self, tmp_path, monkeypatch):
        """When ELIDIA_HOME is not set, returns ~/.elidia."""
        monkeypatch.delenv("ELIDIA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_elidia_root() == tmp_path / ".elidia"

    def test_elidia_home_is_native(self, tmp_path, monkeypatch):
        """When ELIDIA_HOME = ~/.elidia, returns ~/.elidia."""
        native = tmp_path / ".elidia"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(native))
        assert get_default_elidia_root() == native

    def test_elidia_home_is_profile(self, tmp_path, monkeypatch):
        """When ELIDIA_HOME is a profile under ~/.elidia, returns ~/.elidia."""
        native = tmp_path / ".elidia"
    ...
    def test_elidia_home_is_docker(self, tmp_path, monkeypatch):
        """When ELIDIA_HOME points outside ~/.elidia (Docker), returns ELIDIA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(docker_home))
        assert get_default_elidia_root() == docker_home

    def test_elidia_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ELIDIA_HOME outside ~/.elidia is treated as the root."""
        custom = tmp_path / "my-elidia-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(custom))
        assert get_default_elidia_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ELIDIA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(profile))
        assert get_default_elidia_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(elidia_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(elidia_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".elidia"
