...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    elidia_state = ModuleType("elidia_state")
    elidia_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "elidia_state", elidia_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Elidia"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Elidia"
