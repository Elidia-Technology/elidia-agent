"""Test that ELIDIA_SESSION_ID is exposed as an env var and ContextVar."""
...
def _cleanup_env():
    """Remove ELIDIA_SESSION_ID before/after each test."""
...
def test_session_id_env_set_on_init():
    """AIAgent.__init__ sets ELIDIA_SESSION_ID in the environment."""
    agent = AIAgent(
        api_key="test-key",
    ...
    assert os.environ.get("ELIDIA_SESSION_ID") == agent.session_id
    assert len(agent.session_id) > 0


def test_session_id_env_uses_provided_id():
    """When session_id is passed explicitly, ELIDIA_SESSION_ID reflects it."""
    ...
    assert os.environ["ELIDIA_SESSION_ID"] == custom_id
