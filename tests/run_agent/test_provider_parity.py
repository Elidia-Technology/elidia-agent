...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_elidia_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "elidia",
...
class TestBuildApiKwargsElidiaPortal:
    def test_includes_elidia_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == elidia_portal_tags()
...
# ── Chat completions response handling (OpenRouter/Elidia) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_elidia_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_elidia_auth", return_value=elidia_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("elidia_cli.models.get_elidia_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_elidia(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Elidia keys are available.
