...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._elidia_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ELIDIA_REDACT_SECRETS state.
...
class TestBuildSystemPrompt:
    ...
    def test_includes_elidia_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_elidia_subscription_prompt", lambda tool_names: "ELIDIA SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "ELIDIA SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_elidia_route(self, agent):
        agent.provider = "elidia"
        agent.base_url = "https://inference-api.aiutils.io/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("elidia_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("elidia_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "elidia_cli.plugins.get_pre_tool_call_block_message",
            lambda *args, **kwargs: None,
        )
        monkeypatch.setattr(
            "elidia_cli.plugins.invoke_hook",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Elidia tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "elidia_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("elidia_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("elidia_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("elidia_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("elidia_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("elidia_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_elidia_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "elidia"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_elidia_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("elidia_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("elidia_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ELIDIA_KANBAN_TASK
...
class TestElidiaCredentialRefresh:
    """Verify Elidia credential refresh rebuilds the runtime client."""

    def test_try_refresh_elidia_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "elidia_cli.auth.resolve_elidia_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_elidia_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_refresh"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-elidia-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.aiutils.io/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_elidia_gpt5_stays_on_chat_completions(self, agent):
        """Elidia serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "elidia"
        agent.base_url = "https://inference-api.aiutils.io/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Elidia Agent" in agent._cached_system_prompt
