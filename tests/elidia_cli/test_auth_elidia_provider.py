"""Regression tests for Elidia OAuth refresh and inference JWT interactions."""
...
from elidia_cli.auth import AuthError, get_provider_auth_state, resolve_elidia_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from elidia_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ELIDIA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_elidia_ca_bundle_env_falls_back(self, monkeypatch):
        from elidia_cli.auth import _resolve_verify

        monkeypatch.setenv("ELIDIA_CA_BUNDLE", "/nonexistent/elidia-ca.pem")
...
def _setup_elidia_auth(
    ...
    *,
    ...
    (elidia_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_elidia_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    creds = auth_mod.resolve_elidia_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.ELIDIA_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.ELIDIA_AUTH_PATH_INVOKE_JWT

    payload = json.loads((elidia_home / "auth.json").read_text())
    ...
    pool_entries = payload["credential_pool"]["elidia"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.ELIDIA_DEVICE_CODE_SOURCE


def test_resolve_elidia_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import elidia_cli.auth as auth_mod
    ...
    auth_path = elidia_home / "auth.json"
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_elidia_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["elidia"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_elidia_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import elidia_cli.auth as auth_mod
...
def test_resolve_elidia_runtime_credentials_does_not_apply_agent_key_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import elidia_cli.auth as auth_mod
    ...
    assert creds["source"] == auth_mod.ELIDIA_AUTH_PATH_INVOKE_JWT
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert payload["providers"]["elidia"]["agent_key"] == token
    assert payload["credential_pool"]["elidia"][0]["agent_key"] == token


def test_resolve_elidia_runtime_credentials_refreshes_legacy_agent_key_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import elidia_cli.auth as auth_mod
    ...
    assert payload["credential_pool"]["elidia"][0]["agent_key"] == refreshed_token


def test_resolve_elidia_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import elidia_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_elidia_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert payload["providers"]["elidia"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("elidia")


def test_elidia_device_code_login_does_not_retry_legacy_scope_when_invoke_refused(monkeypatch):
    import elidia_cli.auth as auth_mod
    ...
    with pytest.raises(httpx.HTTPStatusError):
        auth_mod._elidia_device_code_login(
            portal_base_url="https://portal.example.com",
            inference_base_url="https://inference.example.com/v1",
            open_browser=False,
            timeout_seconds=1,
    ...
    assert scopes == [auth_mod.DEFAULT_ELIDIA_SCOPE]
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    result = auth_mod._elidia_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
...
def test_elidia_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.INFO, logger="elidia_cli.auth")
    auth_mod.resolve_elidia_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_elidia_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_elidia_auth_status() should find Elidia credentials in the pool
    even when the auth store has no Elidia provider entry — this is the
    ...
    elidia_home = tmp_path / "elidia"
    elidia_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Elidia provider entry
    (elidia_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    # Seed the credential pool with an Elidia entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("elidia")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("elidia", {
        "access_token": token,
    ...
    status = get_elidia_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_elidia_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from elidia_cli.auth import get_elidia_auth_status, invalidate_elidia_auth_status_cache
...
def test_get_elidia_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_elidia_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from elidia_cli.auth import get_elidia_auth_status

    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, access_token="at-123")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    monkeypatch.setattr(
...
def test_get_elidia_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from elidia_cli.auth import get_elidia_auth_status
    from agent.credential_pool import PooledCredential, load_pool

    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, access_token="at-fresh")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    pool = load_pool("elidia")
    stale = PooledCredential.from_dict("elidia", {
...
def test_get_elidia_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from elidia_cli.auth import get_elidia_auth_status
    ...
    def _boom(**kwargs):
        raise AuthError("Refresh session has been revoked", provider="elidia", relogin_required=True)

    monkeypatch.setattr("elidia_cli.auth.resolve_elidia_runtime_credentials", _boom)
...
def test_get_elidia_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_elidia_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth._refresh_access_token", _fake_refresh_access_token)

    with pytest.raises(AuthError) as exc:
        resolve_elidia_runtime_credentials()
    assert exc.value.code == "missing_inference_invoke_scope"

    state_after_failure = get_provider_auth_state("elidia")
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid Elidia refresh token must not be replayed forever."""
    ...
    assert load_pool("elidia").select() is not None
    ...
    auth_mod._write_shared_elidia_state(shared_state)
    ...
    state_after_failure = auth_mod.get_provider_auth_state("elidia")
    assert state_after_failure is not None
    ...
    assert auth_mod._read_shared_elidia_state() is None
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert payload.get("credential_pool", {}).get("elidia") == []
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, refresh_token="refresh-old")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    from agent.credential_pool import load_pool
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_elidia_access_token()
...
# =============================================================================
# _login_elidia: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginElidiaSkipKeepsCurrent:
    """When a user runs `elidia model` → Elidia Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "elidia" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        config_path = elidia_home / "config.yaml"
        ...
        return elidia_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_elidia doesn't hit network."""
        import elidia_cli.auth as auth_mod
        import elidia_cli.models as models_mod
        import elidia_cli.elidia_subscription as ns

        fake_auth_state = {
            "access_token": "fake-elidia-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.aiutils.io",
        ...
        def _check_elidia_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_elidia_free_tier", _check_elidia_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_elidia_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Elidia creds still saved."""
        import argparse
        import yaml
        from elidia_cli.auth import PROVIDER_REGISTRY, _login_elidia

        elidia_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_elidia(args, PROVIDER_REGISTRY["elidia"])
        ...
        # auth.json: active_provider restored to openrouter, but Elidia creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "elidia" in auth_after["providers"]
        assert auth_after["providers"]["elidia"]["access_token"] == "fake-elidia-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_elidia(self, tmp_path, monkeypatch):
        """User picks an Elidia model → provider flips to elidia with that model."""
        import argparse
        import yaml
        from elidia_cli.auth import PROVIDER_REGISTRY, _login_elidia
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as elidia."""
        import argparse
        import yaml
        from elidia_cli.auth import PROVIDER_REGISTRY, _login_elidia
        ...
        auth_path = elidia_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "elidia" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Elidia creds are still saved
        assert "elidia" in auth_after.get("providers", {})


# =============================================================================
# persist_elidia_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _elidia_device_code_login /
    refresh_elidia_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_elidia_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.elidia AND providers.elidia.

    Regression guard: before this helper existed, `elidia auth add elidia`
    wrote only the pool. After the Elidia agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_elidia_runtime_credentials
    which reads providers.elidia, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from elidia_cli.auth import persist_elidia_credentials, ELIDIA_DEVICE_CODE_SOURCE
    ...
    entry = persist_elidia_credentials(state)

    assert entry is not None
    assert entry.provider == "elidia"
    assert entry.source == ELIDIA_DEVICE_CODE_SOURCE

    payload = json.loads((elidia_home / "auth.json").read_text())

    # providers.elidia populated with the full state (new behaviour)
    singleton = payload["providers"]["elidia"]
    ...
    # credential_pool.elidia has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["elidia"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == ELIDIA_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_elidia_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_elidia_runtime_credentials
    must succeed (not raise "Elidia is not logged into Elidia Portal").

    This is the exact path that run_agent.py's `_try_refresh_elidia_client_credentials`
    calls after an Elidia 401 — before the fix it would raise AuthError because
    providers.elidia was empty.
    """
    from elidia_cli.auth import (
        persist_elidia_credentials,
        resolve_elidia_runtime_credentials,
    )
    ...
    persist_elidia_credentials(_full_state_fixture())
    ...
    # doesn't raise "Elidia is not logged into Elidia Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_elidia_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.elidia and lets seeding
    ...
    persist_elidia_credentials(first)
    ...
    persist_elidia_credentials(second)

    payload = json.loads((elidia_home / "auth.json").read_text())

    # providers.elidia reflects the latest write (singleton semantics)
    assert payload["providers"]["elidia"]["access_token"] == second_token
    assert payload["providers"]["elidia"]["agent_key"] == second_token

    # credential_pool.elidia has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["elidia"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == ELIDIA_DEVICE_CODE_SOURCE
...
def test_persist_elidia_credentials_reloads_pool_after_singleton_write(tmp_path, monkeypatch):
...
def test_persist_elidia_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.elidia and the pool.

    Previously `elidia auth add elidia --type oauth --label <name>` silently
    dropped the label because persist_elidia_credentials() ignored it and
    _seed_from_singletons always auto-derived via label_from_token().  The
    fix stashes the label inside providers.elidia so seeding prefers it.
    """
    from elidia_cli.auth import persist_elidia_credentials, ELIDIA_DEVICE_CODE_SOURCE
    ...
    entry = persist_elidia_credentials(_full_state_fixture(), label="my-personal")
    assert entry is not None
    assert entry.source == ELIDIA_DEVICE_CODE_SOURCE
    assert entry.label == "my-personal"

    # providers.elidia carries the label so re-seeding on the next load_pool
    # doesn't overwrite it with the auto-derived fingerprint.
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert payload["providers"]["elidia"]["label"] == "my-personal"


def test_persist_elidia_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    persist_elidia_credentials(_full_state_fixture(), label="work-acct")
...
def test_persist_elidia_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_elidia_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.elidia when the caller didn't supply one.
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["elidia"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Elidia Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Elidia must surface an
    ...
    re-authenticate" string led users to report this as an Elidia persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from elidia_cli.auth import _refresh_access_token
    ...
    assert "elidia auth add elidia" in message.lower()
...
def test_refresh_token_reuse_error_code_is_terminal():
    """Elidia may return refresh_token_reused as the OAuth error code itself."""
    from elidia_cli import auth as auth_mod
    ...
    assert exc_info.value.relogin_required is True
    assert auth_mod._is_terminal_elidia_refresh_error(exc_info.value) is True


def test_refresh_token_exchange_sends_refresh_token_header():
    """Elidia refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-elidia-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Elidia token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ELIDIA_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Elidia store — the
    ...
    monkeypatch.setenv("ELIDIA_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ELIDIA_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.elidia/shared/`` across CI runs.
    """
    from elidia_cli.auth import _elidia_shared_store_path

    monkeypatch.delenv("ELIDIA_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Elidia auth store"):
        _elidia_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ELIDIA_SHARED_AUTH_DIR must redirect the path."""
    from elidia_cli.auth import _elidia_shared_store_path, ELIDIA_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ELIDIA_SHARED_AUTH_DIR", str(custom_dir))

    path = _elidia_shared_store_path()
    assert path == custom_dir / ELIDIA_SHARED_STORE_FILENAME


def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file → ``_read_shared_elidia_state()`` returns None."""
    from elidia_cli.auth import _read_shared_elidia_state

    assert _read_shared_elidia_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file → None, not an exception."""
    from elidia_cli.auth import _elidia_shared_store_path, _read_shared_elidia_state
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_elidia_state()
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from elidia_cli.auth import _elidia_shared_store_path, _write_shared_elidia_state
    ...
    assert not _elidia_shared_store_path().is_file()


def test_persist_elidia_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_elidia_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `elidia auth add elidia
    ...
    elidia_home = tmp_path / "elidia"
    elidia_home.mkdir(parents=True, exist_ok=True)
    (elidia_home / "auth.json").write_text(
    ...
    payload = json.loads((elidia_home / "auth.json").read_text())
    assert "elidia" in payload.get("providers", {})
    ...
    assert str(_elidia_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from elidia_cli.auth import _try_import_shared_elidia_state

    assert _try_import_shared_elidia_state() is None
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    portal down), _try_import_shared_elidia_state must return None so the
    login flow falls back to a fresh device-code run.
    """
    from elidia_cli import auth as auth_mod

    # Seed the shared store
    auth_mod._write_shared_elidia_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_elidia_oauth_from_state", _boom)

    assert auth_mod._try_import_shared_elidia_state() is None
    assert auth_mod._read_shared_elidia_state() is None
...
def test_try_import_shared_persists_rotated_token_when_jwt_validation_fails(
    ...
    shared_after = auth_mod._read_shared_elidia_state()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_elidia_credentials() needs.
    """
    from elidia_cli import auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "refresh_elidia_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_elidia_state()
    ...
    assert result["client_id"] == "elidia-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ELIDIA_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(profile_a))
    auth_mod.persist_elidia_credentials(_full_state_fixture())

    # Profile A's auth.json has elidia
    a_payload = json.loads((profile_a / "auth.json").read_text())
    assert "elidia" in a_payload.get("providers", {})

    # Profile B: fresh ELIDIA_HOME, no auth yet, but the shared store
    # persists — _read_shared_elidia_state() must still return the tokens.
    profile_b = tmp_path / "profile_b"
    profile_b.mkdir(parents=True, exist_ok=True)
    (profile_b / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("ELIDIA_HOME", str(profile_b))

    # B's own auth.json has no elidia
    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "elidia" not in b_payload.get("providers", {})
    ...
    # And a successful rehydrate + persist lands elidia into profile B
    ...
    auth_mod.persist_elidia_credentials(result)

    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "elidia" in b_payload.get("providers", {})
    assert b_payload["providers"]["elidia"]["refresh_token"] == "b-refresh-tok"
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use Elidia refresh token.
    ...
    profile_state = auth_mod.get_provider_auth_state("elidia")
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_elidia_access_token() == "shared-fresh-access"
