...
from elidia_cli import setup as setup_mod
from elidia_cli.setup import setup_model_provider
...
def test_setup_syncs_elidia_from_disk(tmp_path, monkeypatch):
    """Elidia OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "elidia", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "elidia"
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(elidia_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("elidia_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("elidia_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "elidia_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "elidia_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_elidia_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("elidia_cli.setup.managed_elidia_tools_enabled", lambda: True)
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("elidia_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("elidia_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "elidia_cli.setup.get_elidia_subscription_features",
        lambda config: type("Features", (), {"elidia_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from elidia_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("elidia_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("elidia_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "elidia_cli.setup.get_elidia_subscription_features",
        lambda config: type("Features", (), {"elidia_auth_present": True})(),
