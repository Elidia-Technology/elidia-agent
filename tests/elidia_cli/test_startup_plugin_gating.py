...
``elidia_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``elidia --help``, ``elidia version``,
``elidia logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``elidia --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from elidia_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["elidia", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["elidia", "-w", "--tui"], None),
        (["elidia", "version"], "version"),
        (["elidia", "--tui", "chat"], "chat"),
        (["elidia", "-w", "logs"], "logs"),
        (["elidia", "chat", "hello world"], "chat"),
        (["elidia", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["elidia", "-m", "gpt5", "chat"], "chat"),
        (["elidia", "--model", "gpt5", "chat", "hi"], "chat"),
        (["elidia", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["elidia", "-z", "hello world"], None),
        (["elidia", "-z", "hello", "chat"], "chat"),
        (["elidia", "--model=gpt5", "chat"], "chat"),     # inline form
        (["elidia", "--", "chat"], "chat"),               # -- terminator
        (["elidia", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["elidia", "some-plugin-cmd"], "some-plugin-cmd"),
        (["elidia", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["elidia"],                          # bare → chat
        ["elidia", "--help"],                # top-level help
        ["elidia", "-h"],
        ["elidia", "version"],               # known built-in
        ["elidia", "logs"],
        ["elidia", "gateway", "run"],
        ["elidia", "--tui"],
        ["elidia", "-w", "--tui"],
        ["elidia", "chat", "hi"],
        ["elidia", "help"],                  # accepted built-in-ish
        ["elidia", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["elidia", "meet", "join"],          # potential google_meet plugin
        ["elidia", "honcho", "status"],      # potential memory plugin
        ["elidia", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``elidia help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"elidia_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
