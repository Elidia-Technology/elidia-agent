"""Tests for elidia_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_elidia_home):
    """Drop the example-dashboard fixture into the per-test ELIDIA_HOME
    ...
    isolated ``ELIDIA_HOME``.

    The user-plugin source is preferred over a transient
    ``ELIDIA_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, elidia-achievements, model providers
    ...
    from elidia_cli import web_server
    ...
    #      and start serving requests against a torn-down ELIDIA_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Elidia vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Elidia env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints ELIDIA_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import elidia_cli.web_server as ws

        monkeypatch.setenv("ELIDIA_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("ELIDIA_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_elidia_home):
        """Create a TestClient and isolate the state DB under the test ELIDIA_HOME."""
        try:
        ...
        import elidia_state
        from elidia_constants import get_elidia_home
        from elidia_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(elidia_state, "DEFAULT_DB_PATH", get_elidia_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("elidia_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from elidia_cli.web_server import app
    ...
    def test_elevenlabs_voices_unavailable_without_key(self, monkeypatch):
        import elidia_cli.web_server as web_server
    ...
    def test_update_elidia_returns_docker_guidance_without_spawning(self, monkeypatch):
        import elidia_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn elidia update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_elidia_action", fail_spawn)
        web_server._ACTION_PROCS.pop("elidia-update", None)
        web_server._ACTION_RESULTS.pop("elidia-update", None)

        resp = self.client.post("/api/elidia/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "elidia-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull aiutils/elidia-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/elidia-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull aiutils/elidia-agent:latest" in line for line in status_data["lines"])

    def test_update_elidia_spawns_on_non_docker_install(self, monkeypatch):
        import elidia_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_elidia_action", fake_spawn)
        web_server._ACTION_PROCS.pop("elidia-update", None)
        web_server._ACTION_RESULTS.pop("elidia-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "elidia-update"}
        assert calls == [(["update"], "elidia-update")]
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from elidia_cli.web_server import _channel_managed_env_keys
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from elidia_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from elidia_cli.config import save_env_value
        from elidia_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from elidia_cli.config import save_env_value
        from elidia_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from elidia_cli.config import load_config, load_env
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/elidia_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
            assert "FastAPI" not in resp.text  # Should not serve the actual source

    def test_set_model_main_elidia_applies_gateway_defaults(self, monkeypatch):
        """Switching the main provider to Elidia calls apply_elidia_managed_defaults
        ...
        monkeypatch.setattr(ns, "apply_elidia_managed_defaults", fake_apply)

        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "elidia", "model": "hermes-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["provider"] == "elidia"
        assert data["gateway_tools"] == ["web"]
        assert called["force_fresh"] is True

    def test_set_model_main_non_elidia_skips_gateway_defaults(self, monkeypatch):
        """Non-Elidia providers must NOT trigger Tool Gateway auto-routing."""
        import elidia_cli.elidia_subscription as ns

        def boom(*args, **kwargs):  # pragma: no cover - must not be called
            raise AssertionError("apply_elidia_managed_defaults called for non-elidia provider")

        monkeypatch.setattr(ns, "apply_elidia_managed_defaults", boom)
    ...
    def test_recommended_default_elidia_honors_free_tier(self, monkeypatch):
        """For a free-tier Elidia user, the recommended default must be a free
        model (mirroring `elidia model`), not the first curated paid entry."""
        import elidia_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_elidia_model_ids", lambda: ["paid/expensive", "free/cheap"])
        monkeypatch.setattr(
            models_mod, "get_pricing_for_provider",
            lambda provider: {"paid/expensive": {"input": "1"}, "free/cheap": {"input": "0"}},
        )
        monkeypatch.setattr(models_mod, "check_elidia_free_tier", lambda *, force_fresh=False: True)
        monkeypatch.setattr(
        ...
        resp = self.client.get("/api/model/recommended-default?provider=elidia")
        assert resp.status_code == 200
        data = resp.json()
        assert data["provider"] == "elidia"
    ...
    def test_recommended_default_elidia_paid_uses_curated_default(self, monkeypatch):
        """A paid Elidia user gets the first curated/paid-augmented model."""
        import elidia_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_elidia_model_ids", lambda: ["top/model", "other/model"])
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda provider: {})
        monkeypatch.setattr(models_mod, "check_elidia_free_tier", lambda *, force_fresh=False: False)
        monkeypatch.setattr(
    ...
    def test_recommended_default_handles_failure_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr(models_mod, "get_curated_elidia_model_ids", boom)
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from elidia_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from elidia_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_elidia_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from elidia_constants import get_elidia_home
        get_elidia_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from elidia_constants import get_elidia_home
        import elidia_cli.profiles as profiles_mod
        ...
        named = elidia_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_elidia_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_elidia_for_default_profile(self):
        from elidia_constants import get_elidia_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "elidia setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_elidia_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_elidia_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_elidia_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_elidia_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import elidia_cli.skills_config as skills_config
        import elidia_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import elidia_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from elidia_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from elidia_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from elidia_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.elidia/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``elidia_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_elidia_home):
        ...
        monkeypatch.setattr(
            elidia_state, "DEFAULT_DB_PATH", get_elidia_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in elidia_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_elidia_home):
        ...
        # Pin the SessionDB to the isolated ELIDIA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_elidia_home, _install_example_plugin):
        ...
        installs it into the per-test ``ELIDIA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ELIDIA_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, elidia-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (elidia-achievements is loaded by default).
        resp = self.client.get("/api/plugins/elidia-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``elidia --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import elidia_cli.main as main_mod
        ...
        assert env["ELIDIA_TUI_INLINE"] == "1"
        assert env["ELIDIA_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"elidia-ws-ok" in buf:
                    break
            assert b"elidia-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from elidia_cli.pty_bridge import PtyUnavailableError
        ...
        import elidia_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        """When /api/pty is opened with ?channel=, the PTY child gets an ELIDIA_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from elidia_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ELIDIA_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_elidia_home, _install_example_plugin):
        ...
        the per-test ``ELIDIA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
