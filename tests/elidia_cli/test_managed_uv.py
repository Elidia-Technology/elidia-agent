"""Tests for elidia_cli.managed_uv — one path, no guessing."""
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path), \
             patch("elidia_cli.managed_uv.platform.system", return_value="Linux"):
            from elidia_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path), \
             patch("elidia_cli.managed_uv.platform.system", return_value="Windows"):
            from elidia_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    def test_already_installed_no_bootstrap(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path):
            from elidia_cli.managed_uv import ensure_uv
    ...
    def test_installs_if_missing_sets_bootstrap_flag(self, tmp_path):
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path), \
             patch("elidia_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_failure_returns_none_false(self, tmp_path):
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path), \
             patch("elidia_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from elidia_cli.managed_uv import ensure_uv
            path, fresh = ensure_uv()
            assert path is None
            assert fresh is False
...
class TestRebuildVenv:
    def test_removes_old_venv_and_creates_new(self, tmp_path):
        ...
        with patch("elidia_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("elidia_cli.managed_uv.shutil.rmtree") as mock_rmtree:
            from elidia_cli.managed_uv import rebuild_venv
    ...
    def test_rebuild_failure_returns_false(self, tmp_path):
        ...
        with patch("elidia_cli.managed_uv.subprocess.run") as mock_run, \
             patch("elidia_cli.managed_uv.shutil.rmtree"):
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path):
            from elidia_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("elidia_cli.managed_uv.get_elidia_home", return_value=tmp_path), \
             patch("elidia_cli.managed_uv.subprocess.run") as mock_run:
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("elidia_cli.managed_uv._install_uv_posix") as mock_posix:
            from elidia_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("elidia_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("elidia_cli.managed_uv._install_uv_windows") as mock_windows:
            from elidia_cli.managed_uv import _install_uv
            _install_uv(target)
