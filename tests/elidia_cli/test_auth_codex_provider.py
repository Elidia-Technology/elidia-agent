"""Tests for Codex auth — tokens stored in Elidia auth store (~/.elidia/auth.json)."""
...
def _setup_elidia_auth(elidia_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Elidia auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_elidia_auth(elidia_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_elidia_auth(elidia_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    monkeypatch.setattr("elidia_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((elidia_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``elidia auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Elidia auth store, not ~/.codex/."""
    elidia_home = tmp_path / "elidia"
    codex_home = tmp_path / "codex-cli"
    elidia_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "elidia-at", "refresh_token": "elidia-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Elidia store
    assert not (codex_home / "auth.json").exists()

    # Elidia auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "elidia-at"


def test_resolve_returns_elidia_auth_store_source(tmp_path, monkeypatch):
    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run elidia auth".
    """
    from elidia_cli.auth import (
    ...
    assert "elidia auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from elidia_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from elidia_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("elidia_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("elidia_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
