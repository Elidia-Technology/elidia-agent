...
Profile-aware location: ``$ELIDIA_HOME/logs/dashboard-auth.log``.
...
from elidia_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $ELIDIA_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="elidia", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="elidia", user_id="u1",
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="elidia", access_token="should-not-appear",
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point ELIDIA_HOME at a file (not a dir) so mkdir/open will fail.
    broken = tmp_path / "not-a-dir"
    broken.write_text("blocking file")
    monkeypatch.setenv("ELIDIA_HOME", str(broken))
    # Should NOT raise.
    audit_log(AuditEvent.LOGIN_FAILURE, provider="elidia", reason="x")
...
def test_audit_creates_logs_dir_if_missing(tmp_path, monkeypatch):
    ...
    audit_log(AuditEvent.LOGIN_START, provider="elidia")
