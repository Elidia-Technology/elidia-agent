...
def test_ungrouped_providers_pass_through_in_order():
    rows = group_providers(["elidia", "openrouter", "deepseek"])
    assert all(r["kind"] == "single" for r in rows)
    assert [r["slug"] for r in rows] == ["elidia", "openrouter", "deepseek"]
...
def test_group_appears_at_first_member_position():
    ...
    rows = group_providers(["elidia", "minimax", "deepseek", "minimax-cn"])
...
def test_duplicate_slugs_ignored():
    rows = group_providers(["elidia", "elidia", "minimax", "minimax"])
    assert [r.get("slug") or r["group_id"] for r in rows] == ["elidia", "minimax"]
