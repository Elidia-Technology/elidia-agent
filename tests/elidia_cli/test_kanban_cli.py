"""Tests for the kanban CLI surface (elidia_cli.kanban)."""
...
from elidia_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`elidia kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from elidia_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("ELIDIA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ELIDIA_KANBAN_BOARD") == "beta"
