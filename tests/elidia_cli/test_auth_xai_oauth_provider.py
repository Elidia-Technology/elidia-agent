"""Tests for xAI Grok OAuth — tokens stored in Elidia auth store (~/.elidia/auth.json)."""
...
def _setup_elidia_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Elidia auth store at the given root."""
    ...
    auth_file = elidia_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("elidia_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_elidia_agent():
    """Attribution: xAI's OAuth server can identify Elidia-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "elidia-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "elidia auth add xai-oauth" in body
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, access_token="")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    elidia_home = tmp_path / "elidia"
    _setup_elidia_auth(elidia_home, refresh_token="")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    elidia_home = tmp_path / "elidia"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_elidia_auth(elidia_home, access_token=fresh)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    assert creds["source"] == "elidia-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_elidia_auth(elidia_home, access_token=fresh)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    monkeypatch.setenv("ELIDIA_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``ELIDIA_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    elidia_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    elidia_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    elidia_home = tmp_path / "elidia"
    _seed_xai_oauth_state(elidia_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="elidia")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    monkeypatch.setattr("elidia_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((elidia_home / "auth.json").read_text())
    ...
    assert raw["active_provider"] == "elidia"
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    elidia_home = tmp_path / "elidia"
    _seed_xai_oauth_state(elidia_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    monkeypatch.setattr("elidia_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``elidia model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``elidia model``
    ...
    assert "elidia model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from elidia_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Elidia that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `elidia model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `elidia auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    elidia_home = tmp_path / "elidia"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_elidia_auth(elidia_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_credential_pool_does_not_seed_when_singleton_missing_access_token(tmp_path, monkeypatch):
    ...
    (elidia_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`elidia auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `elidia auth remove`.
    from elidia_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``elidia auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from elidia_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `elidia auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((elidia_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Elidia invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_elidia_auth(elidia_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    monkeypatch.setattr("elidia_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from elidia_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    elidia_home = tmp_path / "elidia"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_elidia_auth(elidia_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Elidia process refreshed the singleton
    ...
    Mirrors the proactive sync codex/elidia already perform for the same
    reason, and is what makes the pool actually safe to share across
    profiles + Elidia processes."""
    from agent.credential_pool import load_pool

    elidia_home = tmp_path / "elidia"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_elidia_auth(elidia_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    (elidia_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_elidia_auth(elidia_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``elidia model`` -> xAI Grok OAuth (or another process
    ...
    cooldown elapses.  Mirrors the codex/elidia self-heal pattern."""
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
    from dataclasses import replace

    elidia_home = tmp_path / "elidia"
    stale_at = _jwt_with_exp(int(time.time()) + 3600)
    _setup_elidia_auth(elidia_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    # Simulate the user re-running `elidia model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``elidia auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_elidia_auth(elidia_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`elidia auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    Elidia, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/Elidia refresh in a
    multi-provider setup silently overrides the user's chosen active
    provider (visible to ``elidia auth status``, ``elidia setup``, and the
    ``elidia`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    elidia_home = tmp_path / "elidia"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_elidia_auth(elidia_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "Elidia token rotations silently take over multi-provider users' "
