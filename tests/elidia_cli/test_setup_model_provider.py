...
from elidia_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from elidia_cli.config import load_config, save_config, save_env_value
from elidia_cli.elidia_subscription import ElidiaFeatureState, ElidiaSubscriptionFeatures
from elidia_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("elidia_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("elidia_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside elidia setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("elidia_cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("elidia_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("elidia_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("elidia_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'elidia setup' to configure" not in output
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "elidia_cli.setup.get_elidia_subscription_features",
        lambda config: ElidiaSubscriptionFeatures(
            ...
            features={
                ...
                "browser": ElidiaFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": ElidiaFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
    ...
    monkeypatch.setattr(
        ...
        lambda config: ElidiaSubscriptionFeatures(
            ...
            features={
                ...
                "video_gen": ElidiaFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                "tts": ElidiaFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": ElidiaFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": ElidiaFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
