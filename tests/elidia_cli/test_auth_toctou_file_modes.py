"""Regression tests for TOCTOU-safe credential file writers in ``elidia_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.elidia/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Elidia shared-credential store write (inside _write_shared_elidia_state)
# ---------------------------------------------------------------------------


def test_shared_elidia_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Elidia shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    # _elidia_shared_store_path() refuses to touch the real shared store during
    ...
    # home" reference — which currently tracks ELIDIA_HOME via
    # get_default_elidia_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("ELIDIA_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from elidia_cli import auth as auth_mod

        state = {
            "access_token": "elidia-access-xxx",
            "refresh_token": "elidia-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_elidia_state(state)
        path = auth_mod._elidia_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared Elidia store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Elidia shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Elidia shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )

    data = json.loads(path.read_text())
    assert data["refresh_token"] == "elidia-refresh-xxx"
