"""Tests for elidia_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("elidia_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from elidia_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("elidia_cli.config.load_config")
    ...
    @patch("elidia_cli.config.load_config")
    @patch.dict("os.environ", {"ELIDIA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ELIDIA_SESSION_PLATFORM should be used when ELIDIA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_elidia_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ELIDIA_PLATFORM should win over ELIDIA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
        monkeypatch.setenv("ELIDIA_PLATFORM", "telegram")
        monkeypatch.setenv("ELIDIA_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from elidia_cli.skills_config import _get_categories
        skills = [
