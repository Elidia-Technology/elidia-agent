"""Tests for the auxiliary-model configuration UI in ``elidia model``.
...
from elidia_cli.config import DEFAULT_CONFIG, load_config
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"provider": "elidia", "model": "gemini-3-flash"}, "elidia · gemini-3-flash"),
        (
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".elidia").mkdir(exist_ok=True)
...
def test_save_aux_choice_preserves_timeout(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="elidia", model="gemini-3-flash")
...
def test_save_aux_choice_creates_missing_task_entry(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="elidia", model="gemini-3-flash")
    cfg = load_config()
    assert cfg["auxiliary"]["vision"]["provider"] == "elidia"
...
def test_reset_aux_to_auto_clears_routing_preserves_timeouts(tmp_path, monkeypatch):
    ...
    _save_aux_choice("compression", provider="elidia", model="gemini-3-flash")
    from elidia_cli.config import save_config
