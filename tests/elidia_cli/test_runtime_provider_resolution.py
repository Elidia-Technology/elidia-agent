...
from elidia_cli import runtime_provider as rp
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    ...
    entry = rp._get_named_custom_provider("elidia")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "elidia"
...
def test_auto_detected_elidia_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks Elidia but credentials are revoked, fall through to OpenRouter."""
    from elidia_cli.auth import AuthError
    ...
    # resolve_provider returns "elidia" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "elidia")
    ...
    # Elidia credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_elidia_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="elidia", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_elidia_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests Elidia and auth fails, the error should propagate."""
    from elidia_cli.auth import AuthError
    import pytest
    ...
    # With explicit "elidia", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="elidia")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.elidia/.env — mock it to return None
        # so the resolver can't find a key there either.
        import elidia_cli.config as cfg_mod
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from elidia_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from elidia_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from elidia_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from elidia_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
