...
class TestSlackNativeSlashes:
    ...
    def test_elidia_catchall_is_first(self):
        """``/elidia`` must be reserved as the first slot so the legacy
        ``/elidia <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "elidia"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /elidia <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `elidia slack manifest`)."""
...
from elidia_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch elidia_cli.plugins.get_plugin_commands() to a fixed dict."""
        from elidia_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/elidia metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
