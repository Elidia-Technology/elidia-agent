...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("elidia_cli.config.get_managed_system", return_value=None), \
         patch("elidia_cli.config.get_elidia_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When ELIDIA_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("elidia_cli.config.get_managed_system", return_value="NixOS"), \
         patch("elidia_cli.config.get_elidia_home", return_value=tmp_path):
        from elidia_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from elidia_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``elidia update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("elidia_cli.config.get_managed_system", return_value=None), \
         patch("elidia_cli.config.get_elidia_home", return_value=tmp_path), \
         patch("elidia_constants.is_container", return_value=True):
        from elidia_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from elidia_cli import banner

    hh = tmp_path / ".elidia"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("elidia_cli.config.get_elidia_home", return_value=hh), \
         patch("elidia_constants.get_elidia_home", return_value=hh):
        buf = io.StringIO()
