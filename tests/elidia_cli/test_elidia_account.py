"""Tests for normalized Elidia Portal account entitlement helpers."""
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "elidia-cli",
            "product_id": "elidia-agent",
            "elidia_client": "elidia-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("elidia_cli.auth.get_provider_auth_state", lambda provider: _state(token))

    info = get_elidia_portal_account_info()
    ...
    assert info.product_id == "elidia-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("elidia_cli.auth.resolve_elidia_access_token", lambda: "fresh-token")
    monkeypatch.setattr("elidia_cli.elidia_account._fetch_elidia_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, ElidiaPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("elidia_cli.auth.get_provider_auth_state", lambda provider: {})
    ...
    assert info.credential_source == "pool:manual-elidia"
    assert info.paid_service_access is None
...
def test_entitlement_message_returns_none_for_paid_access():
    ...
    assert format_elidia_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Elidia inference credentials are configured" in message
    assert "cannot verify your Elidia Portal paid access" in message
    assert "Log in with `elidia model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_elidia_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_elidia_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `elidia model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find an Elidia Portal account or organisation" in message
