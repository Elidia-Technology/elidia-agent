"""Tests for `elidia portal` dispatch.

`elidia portal` (no subcommand) is the human-readable alias for the Elidia Portal
one-shot onboarding (`elidia auth add elidia --type oauth` / `elidia setup
--portal`). The prior status default moved to `elidia portal info`, with
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`elidia portal`, `elidia portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr(
        "elidia_cli.setup._run_portal_one_shot", fake_one_shot
    )
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`elidia portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "elidia_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.setup._run_portal_one_shot", boom)
    monkeypatch.setattr(portal_cli, "load_config", lambda: {})
...
def test_one_shot_delegates_to_model_flow_elidia(monkeypatch):
    """`elidia portal` must run the quick-setup Elidia flow (login + MODEL PICK +
    provider + Tool Gateway), i.e. delegate to `_model_flow_elidia` — not the
    ...
    # _model_flow_elidia lives in elidia_cli.main and is imported lazily inside
    # _run_portal_one_shot, so patch it at the source module.
    monkeypatch.setattr("elidia_cli.main._model_flow_elidia", fake_model_flow)
    # Keep the disk re-sync a no-op so the test never touches real config.
    monkeypatch.setattr("elidia_cli.config.load_config", lambda: {})

    setup_mod._run_portal_one_shot({})

    assert calls["model_flow"] == 1, (
        "`elidia portal` must route through _model_flow_elidia so the model "
...
def test_one_shot_swallows_cancel_and_systemexit(monkeypatch, exc):
    """A cancel/abort from the delegated Elidia flow must NOT escape and kill the
    CLI. `_login_elidia` raises SystemExit(130)/(1) on cancel/failure, and the
    expired-session re-login path inside `_model_flow_elidia` only catches
    ...
    monkeypatch.setattr("elidia_cli.main._model_flow_elidia", boom)
    monkeypatch.setattr("elidia_cli.config.load_config", lambda: {})
