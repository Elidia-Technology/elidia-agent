"""Tests for ``elidia update`` / ``--check`` inside the Docker container.
...
fix, ``elidia update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Elidia, not an update to the running container, so the message
...
from elidia_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("elidia_cli.config.is_managed", return_value=False)
@patch("elidia_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``elidia update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull aiutils/elidia-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``elidia update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull aiutils/elidia-agent:latest" in msg
    ...
    assert "ELIDIA_HOME" in msg or "/opt/data" in msg, (
