"""Tests for the update check mechanism in elidia_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "elidia-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("elidia_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import elidia_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "elidia_cli" / "banner.py"
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    monkeypatch.delenv("ELIDIA_REVISION", raising=False)
    with patch("elidia_cli.banner.subprocess.run") as mock_run, \
         patch("elidia_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    with patch("elidia_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    with patch("elidia_cli.banner.subprocess.run") as mock_run:
        with patch("elidia_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ELIDIA_HOME has no git repo."""
    import elidia_cli.banner as banner
    ...
    # Point ELIDIA_HOME at a temp dir with no elidia-agent/.git
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    with patch("elidia_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from elidia_cli.main import _invalidate_update_cache

    # Build a fake ~/.elidia with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ELIDIA_HOME": str(default_home)}):
