"""Comprehensive tests for elidia_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ELIDIA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.elidia/profiles)
    * ELIDIA_HOME  -> tmp_path/.elidia  (so get_elidia_home() agrees)
    * Creates the bare-minimum ~/.elidia directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["elidia", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Elidia install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_elidia_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".elidia" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".elidia" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.elidia must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".elidia"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes elidia-agent, .worktrees,
        ...
        (default_home / "elidia-agent" / ".git").mkdir(parents=True)
        (default_home / "elidia-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "elidia-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "elidia-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `elidia profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `elidia update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("elidia_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".elidia" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_elidia_home_returns_default(self, profile_env):
        # ELIDIA_HOME points to tmp_path/.elidia which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ELIDIA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ELIDIA_HOME", str(custom))
        # With Docker-aware roots, a custom ELIDIA_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("elidia")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from elidia_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "elidia -p mybot" in content
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from elidia_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from elidia_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "elidia -p redqueen" in content
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "elidia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["elidia_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["elidia_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "elidia.ssi_health": {"workspace": "elidia", "enabled": True}
            }
        }))
        ...
        assert "elidia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["elidia_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["elidia_heimdall"]["workspace"] == "elidia"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "elidia.ssi_health": {"aiPeer": "ssi_health"},
                "elidia_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["elidia.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["elidia_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("elidia-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".elidia_history",
        ...
        excluded_prefixes = [
            "default/elidia-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.elidia_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultElidiaHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_elidia_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".elidia" / "profiles"

    def test_default_elidia_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_elidia_home()
        assert home == tmp_path / ".elidia"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ELIDIA_HOME outside ~/.elidia), profiles go under ELIDIA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_elidia_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_elidia_home() returns ELIDIA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ELIDIA_HOME under ~/.elidia), profiles root is still ~/.elidia/profiles."""
        native = tmp_path / ".elidia"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ELIDIA_HOME."""
        from elidia_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ELIDIA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELIDIA_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".elidia" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from elidia_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".elidia"
