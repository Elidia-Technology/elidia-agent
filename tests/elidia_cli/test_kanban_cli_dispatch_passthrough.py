...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh ELIDIA_HOME with a clean kanban DB."""
...
def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: elidia kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "elidia_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.config.load_config", lambda: fake_config)
