"""Tests for IRC gateway configuration via `elidia setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Elidia install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Elidia install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#elidia")
            monkeypatch.setenv("IRC_NICKNAME", "elidia-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `elidia setup gateway` experience with IRC present."""
