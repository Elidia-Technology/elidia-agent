...
These guard the contract:
  ...
  * OpenRouter and Elidia Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Elidia) semantics.
...
appear in ``/model`` without an Elidia release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see elidia_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
...
class TestOpenRouterAndElidiaUnchanged:
    """Per Teknium: openrouter and elidia are NEVER merged with models.dev."""

    def test_openrouter_not_in_preferred_set(self):
        assert "openrouter" not in _MODELS_DEV_PREFERRED

    def test_elidia_not_in_preferred_set(self):
        assert "elidia" not in _MODELS_DEV_PREFERRED

    def test_openrouter_does_not_call_merge(self):
        """openrouter takes its own live path — merge helper must NOT run."""
        with patch(
            "elidia_cli.models._merge_with_models_dev",
