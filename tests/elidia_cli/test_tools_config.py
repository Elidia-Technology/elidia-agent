"""Tests for elidia_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `elidia tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "elidia_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "elidia_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `elidia tools`,
    ...
    # User explicitly opted into spotify but not x_search via `elidia tools`.
    config = {"platform_toolsets": {"cli": ["elidia-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[elidia-cli, spotify]`` (composite + configurable) must keep the full
    ``elidia-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``elidia-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when elidia-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["elidia-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``elidia-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["elidia-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _ELIDIA_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `elidia tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("elidia_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/Elidia-Technology/elidia-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (elidia-cli, elidia-telegram, etc.) must NOT
    ...
    ``elidia tools``, but elidia-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "elidia-cli",  # <-- the culprit
    ...
    # elidia-cli must NOT survive — it's a platform default, not an MCP server
    assert "elidia-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_elidia_telegram():
    """Same bug for Telegram — elidia-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "elidia-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "elidia-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "elidia-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_elidia_subscription_when_logged_in(monkeypatch):
    config = {"model": {"provider": "elidia"}}

    monkeypatch.setattr(
        "elidia_cli.elidia_subscription.get_elidia_portal_account_info",
        lambda: ElidiaPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
    ...
    # The managed Elidia row is listed (not necessarily first — "Local Browser"
    # sorts first so a fresh-install Enter lands on the free local backend).
    assert any(p["name"].startswith("Elidia Subscription") for p in providers)
    # "Local Browser" must be the index-0 default so pressing Enter never
    # walks a user into a paid Elidia Portal login.
    assert providers[0]["name"] == "Local Browser"


def test_visible_providers_show_elidia_subscription_when_logged_out(monkeypatch):
    """Elidia-managed Tool Gateway rows are always listed, even logged out.
...
def test_visible_providers_show_elidia_subscription_when_paid_access_is_false(monkeypatch):
    ...
    shown; ``ensure_elidia_portal_access`` blocks activation if still unpaid.
    """
    config = {"model": {"provider": "elidia"}}
...
def test_visible_providers_force_fresh_shows_elidia_subscription_after_upgrade(monkeypatch):
    calls = []

    def fake_subscription_features(config, *, force_fresh=False):
        calls.append(("features", force_fresh))
        return SimpleNamespace(
            elidia_auth_present=True,
            account_info=ElidiaPortalAccountInfo(
                logged_in=True,
    ...
    monkeypatch.setattr(
        "elidia_cli.tools_config.get_elidia_subscription_features",
        fake_subscription_features,
    )

    providers = _visible_providers(
        TOOL_CATEGORIES["browser"],
        {"model": {"provider": "elidia"}},
        force_fresh=True,
    )

    # The managed Elidia row reappears after the entitlement upgrade. It is no
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.tools_config._run_post_setup", lambda key: None)
    _configure_provider(local_provider, config)

    assert config["browser"]["cloud_provider"] == "local"


def test_fresh_install_browser_default_is_free_local_not_paid_elidia():
    """On a fresh install the browser picker must default to the free local
    backend, never the paid Elidia Subscription gateway.

    Regression: the Elidia row used to sort first, so the menu cursor defaulted
    to index 0 (Elidia) and pressing Enter walked users straight into an Elidia
    Portal login for a paid offering (Javier's bug, June 2026).
    """
    from elidia_cli.tools_config import _detect_active_provider_index
...
def test_fresh_install_tts_default_is_free_edge_not_paid_elidia():
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("elidia_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "elidia_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("elidia_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_elidia_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("elidia_cli.elidia_subscription.managed_elidia_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "elidia"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "elidia_cli.tools_config._prompt_toolset_checklist",
    ...
    # apply_elidia_managed_defaults per platform; a second iteration sees values
    # set by the first as "explicit" and skips them.
    monkeypatch.setattr(
        "elidia_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "elidia_cli.elidia_subscription.get_elidia_portal_account_info",
        lambda *args, **kwargs: ElidiaPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=True,
    ...
    configured = []
    monkeypatch.setattr(
        "elidia_cli.tools_config._configure_toolset",
...
def test_first_install_elidia_auto_configures_video_gen(monkeypatch):
    """When an Elidia subscriber checks video_gen in the toolset checklist,
    apply_elidia_managed_defaults must write video_gen.provider and
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from elidia_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """elidia-gateway includes list should cover all messaging platforms."""
        from elidia_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["elidia-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from elidia_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from elidia_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/Elidia-Technology/elidia-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `elidia tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's ELIDIA_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"ELIDIA_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"ELIDIA_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from elidia_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Elidia Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from elidia_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from elidia_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("elidia_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("elidia_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("elidia_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`elidia tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["elidia-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "elidia-_test_platform"},
    }

    with mock_patch("elidia_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'elidia-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `elidia tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from elidia_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `elidia tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from elidia_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `elidia tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from elidia_cli.tools_config import _get_effective_configurable_toolsets
...
@pytest.mark.parametrize("provider,config_key,expected", [
    ...
    ({"name": "T", "tts_provider": "elevenlabs", "managed_elidia_feature": "tts", "env_vars": []}, "tts", True),
    ({"name": "B", "browser_provider": "browserbase", "managed_elidia_feature": "browser", "env_vars": []}, "browser", True),
    ({"name": "W", "web_backend": "tavily", "managed_elidia_feature": "web", "env_vars": []}, "web", True),
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "elidia_cli.elidia_subscription.ensure_elidia_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("elidia_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("elidia_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("elidia_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("elidia_cli.tools_config.save_env_value", lambda k, v: None)
...
# ---------------------------------------------------------------------------
# Inline Elidia Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------


def test_configure_managed_provider_blocks_when_not_entitled(monkeypatch):
    """Selecting an Elidia-managed backend without paid access writes no config."""
    monkeypatch.setattr(
        "elidia_cli.elidia_subscription.ensure_elidia_portal_access",
        lambda **kwargs: False,
    )
    provider = {
        "name": "Elidia Subscription (Firecrawl)",
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    """A self-hosted provider must never trigger the Elidia Portal login gate."""
    called = {"gate": False}
    ...
    monkeypatch.setattr(
        "elidia_cli.elidia_subscription.ensure_elidia_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from elidia_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from elidia_cli import tools_config
