"""Tests for elidia_cli.auth._update_config_for_provider clearing stale fields.
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_elidia_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("elidia", "https://inference-api.aiutils.io/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "elidia"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
