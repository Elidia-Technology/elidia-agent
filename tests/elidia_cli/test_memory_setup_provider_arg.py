"""Tests for `elidia memory setup [provider]` routing.
...
``elidia memory setup honcho``) without the interactive picker — which matters
because the per-provider ``elidia <provider>`` subcommand is only registered
...
from elidia_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "elidia memory setup" in out
