...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   elidia gateway" in output
    assert "systemd (user)" not in output


def test_show_status_reports_elidia_auth_error(monkeypatch, capsys, tmp_path):
    from elidia_cli import status as status_mod
    import elidia_cli.auth as auth_mod
    import elidia_cli.gateway as gateway_mod
...
def test_show_status_reports_elidia_inference_key_without_portal_login(monkeypatch, capsys, tmp_path):
    from elidia_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "managed_elidia_tools_enabled", lambda: False, raising=False)
    monkeypatch.setattr(auth_mod, "get_codex_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_qwen_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda exclude_pids=None: [], raising=False)
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in elidia status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.elidia/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.elidia/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: elidia auth add xai-oauth)" in out
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """Elidia/Codex/MiniMax rows must still appear when xAI import fails."""
        import elidia_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_elidia_auth_status",
