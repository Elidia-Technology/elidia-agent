...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_elidia_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:elidia_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:elidia_pkce"
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from elidia_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`elidia logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (elidia_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `elidia logout` should target configured Codex if auth has no active provider."""
    elidia_home = tmp_path / "elidia"
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (elidia_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((elidia_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from elidia_cli.auth import _reset_config_provider
    ...
    with patch("elidia_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from elidia_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "elidia_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("elidia_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("elidia_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("elidia_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "elidia_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((elidia_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from elidia_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from elidia_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `elidia auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    elidia_home = tmp_path / "elidia"
    elidia_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `elidia auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((elidia_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`elidia auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.elidia/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.elidia/.env (not the shell), the
    ...
    (elidia_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (elidia_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `elidia auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from elidia_cli.auth import is_source_suppressed
    from elidia_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `elidia auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Elidia reads from has a
...
def test_seed_from_singletons_respects_elidia_suppression(tmp_path, monkeypatch):
    """elidia device_code must not re-seed from auth.json when suppressed."""
    elidia_home = tmp_path / "elidia"
    elidia_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    (elidia_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"elidia": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"elidia": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("elidia", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import elidia_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_elidia_pkce_suppression(tmp_path, monkeypatch):
    """anthropic elidia_pkce must not re-seed from ~/.elidia/.anthropic_oauth.json when suppressed."""
    elidia_home = tmp_path / "elidia"
    elidia_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    import yaml
    (elidia_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (elidia_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["elidia_pkce"]},
    }))

    # Stub the readers so only elidia_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_elidia_oauth_credentials", lambda: {
    ...
    # elidia_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "elidia_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (elidia_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.elidia/.anthropic_oauth.json",
        "auth.json providers.elidia",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "elidia_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `elidia auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `elidia auth add openai-codex`)
