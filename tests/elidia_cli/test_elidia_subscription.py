"""Tests for Elidia subscription feature detection."""

from elidia_cli.elidia_account import ElidiaPortalAccountInfo, ElidiaToolAccessInfo
from elidia_cli import elidia_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> ElidiaPortalAccountInfo:
    return ElidiaPortalAccountInfo(
        logged_in=logged_in,
...
def _pool_account() -> ElidiaPortalAccountInfo:
    ...
    return ElidiaPortalAccountInfo(
        ...
        tool_access=ElidiaToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_elidia_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_elidia_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_elidia is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_elidia_subscription_features_force_fresh_forwards_account_request(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(ns, "get_elidia_portal_account_info", fake_account_info)
    ...
    features = ns.get_elidia_subscription_features({}, force_fresh=True)
...
def test_get_elidia_subscription_features_prefers_managed_modal_in_auto_mode(monkeypatch):
    monkeypatch.setattr("tools.tool_backend_helpers.managed_elidia_tools_enabled", lambda: True)
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
    ...
    features = ns.get_elidia_subscription_features(
        {"terminal": {"backend": "modal", "modal_mode": "auto"}}
    )

    assert features.modal.available is True
    assert features.modal.active is True
    assert features.modal.managed_by_elidia is True
    assert features.modal.direct_override is False


def test_get_elidia_subscription_features_marks_browser_use_as_managed_when_gateway_ready(monkeypatch):
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
    ...
    assert features.browser.managed_by_elidia is True
    assert features.browser.direct_override is False
    assert features.browser.current_provider == "Browser Use"


def test_get_elidia_subscription_features_uses_direct_browserbase_when_no_managed_gateway(monkeypatch):
    ...
    assert features.browser.available is True
    assert features.browser.active is True
    assert features.browser.managed_by_elidia is False
    assert features.browser.direct_override is True
    assert features.browser.current_provider == "Browserbase"


def test_get_elidia_subscription_features_prefers_camofox_over_managed_browser_use(monkeypatch):
...
def test_get_elidia_subscription_features_requires_agent_browser_for_browserbase(monkeypatch):
    env = {
        "BROWSERBASE_API_KEY": "bb-key",
        "BROWSERBASE_PROJECT_ID": "bb-project",
    }
...
def test_get_elidia_subscription_features_does_not_treat_quoted_false_as_gateway_opt_in(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}
...
def test_get_gateway_eligible_tools_ignores_quoted_false_opt_in(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda **kw: _account(logged_in=True, paid=True)
    )
    monkeypatch.setattr(
        ns,
...
def test_get_gateway_eligible_tools_pool_excludes_video(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_elidia_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
...
def test_get_gateway_eligible_tools_empty_when_not_entitled(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda **kw: _account(logged_in=True, paid=False)
    )
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import elidia_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "elidia_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_paid_user_offers_video(monkeypatch):
    ...
    ns.prompt_enable_tool_gateway({"model": {"provider": "elidia"}})

    blob = " ".join(captured["items"]).lower()
    assert "video" in blob


def test_apply_elidia_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_elidia_managed_defaults must write video_gen.provider and
    video_gen.use_gateway when an Elidia subscriber selects video_gen
    without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_elidia_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "elidia"}}
    changed = ns.apply_elidia_managed_defaults(
...
def test_apply_elidia_managed_defaults_writes_image_gen_config(monkeypatch):
    """apply_elidia_managed_defaults must write image_gen.use_gateway
    when an Elidia subscriber selects image_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_elidia_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
...
def test_apply_elidia_managed_defaults_skips_fal_tools_when_key_present(monkeypatch):
    """When FAL_KEY is set, apply_elidia_managed_defaults should not touch
...
def test_apply_elidia_managed_defaults_preserves_existing_video_gen_section(monkeypatch):
...
# ---------------------------------------------------------------------------
# ensure_elidia_portal_access — inline login gate for `elidia tools`
# ---------------------------------------------------------------------------


def test_ensure_elidia_portal_access_fast_path_when_already_paid(monkeypatch):
    ...
    monkeypatch.setattr(ns, "_run_elidia_portal_login_only", _login)

    assert ns.ensure_elidia_portal_access() is True
    assert login_called["v"] is False


def test_ensure_elidia_portal_access_logs_in_then_grants(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info", lambda **kw: next(states),
    )
    monkeypatch.setattr(ns, "_run_elidia_portal_login_only", lambda **kw: True)

    assert ns.ensure_elidia_portal_access() is True


def test_ensure_elidia_portal_access_returns_false_when_login_declined(monkeypatch):
    monkeypatch.setattr(
        ns, "get_elidia_portal_account_info",
        lambda **kw: _account(logged_in=False, paid=None),
    )
    monkeypatch.setattr(ns, "_run_elidia_portal_login_only", lambda **kw: False)

    assert ns.ensure_elidia_portal_access() is False


def test_ensure_elidia_portal_access_false_when_logged_in_but_unpaid(monkeypatch):
