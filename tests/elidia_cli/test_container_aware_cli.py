...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ELIDIA_HOME with .container-mode file."""
    elidia_home = tmp_path / ".elidia"
    elidia_home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    monkeypatch.delenv("ELIDIA_DEV", raising=False)

    container_mode = elidia_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=elidia-agent\n"
        "exec_user=elidia\n"
        "elidia_bin=/data/current-package/bin/elidia\n"
    )
    return elidia_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("elidia_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "elidia-agent"
    assert info["exec_user"] == "elidia"
    assert info["elidia_bin"] == "/data/current-package/bin/elidia"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("elidia_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_elidia_dev(container_env, monkeypatch):
    """Returns None when ELIDIA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ELIDIA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_elidia_dev_zero(container_env, monkeypatch):
    """ELIDIA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ELIDIA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        elidia_home = Path(tmpdir) / ".elidia"
        elidia_home.mkdir()
        (elidia_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("elidia_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_elidia_home": lambda: elidia_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELIDIA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=elidia-custom\n"
        "exec_user=myuser\n"
        "elidia_bin=/opt/elidia/bin/elidia\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "elidia-custom"
    assert info["exec_user"] == "myuser"
    assert info["elidia_bin"] == "/opt/elidia/bin/elidia"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from elidia_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "elidia"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "elidia-agent" in cmd
    assert "/data/current-package/bin/elidia" in cmd
    assert "chat" in cmd
