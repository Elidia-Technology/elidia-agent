"""Tests for the `elidia proxy` subcommand and its upstream adapters."""
...
from elidia_cli.proxy.adapters import ADAPTERS, get_adapter
from elidia_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from elidia_cli.proxy.adapters.elidia_portal import ElidiaPortalAdapter
from elidia_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_registry_lists_elidia():
    assert "elidia" in ADAPTERS
...
def test_get_adapter_returns_instance():
    adapter = get_adapter("elidia")
    assert isinstance(adapter, ElidiaPortalAdapter)
...
def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("ELIDIA"), ElidiaPortalAdapter)
    assert isinstance(get_adapter("  Elidia  "), ElidiaPortalAdapter)
...
def _write_auth_store(elidia_home: Path, elidia_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given elidia state into a hermetic ELIDIA_HOME."""
    auth_path = elidia_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"elidia": elidia_state},
    }))
    return auth_path


def test_elidia_adapter_metadata():
    adapter = ElidiaPortalAdapter()
    assert adapter.name == "elidia"
    assert adapter.display_name == "Elidia Portal"
...
def test_elidia_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ELIDIA_HOME is already set by conftest, but make doubly sure
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    adapter = ElidiaPortalAdapter()
    assert not adapter.is_authenticated()


def test_elidia_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    (tmp_path / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    }))
    assert not ElidiaPortalAdapter().is_authenticated()


def test_elidia_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "agent_key": "ov-test-key",
        "agent_key_expires_at": "2099-01-01T00:00:00Z",
        "inference_base_url": "https://inference-api.aiutils.io/v1",
    })
    assert ElidiaPortalAdapter().is_authenticated()


def test_elidia_adapter_authenticated_with_refresh_token_only(tmp_path, monkeypatch):
...
def test_elidia_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
    ...
    with patch(
        "elidia_cli.proxy.adapters.elidia_portal.resolve_elidia_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.aiutils.io/v1"
    assert cred.expires_at == "2099-01-01T00:00:00Z"
    assert cred.token_type == "Bearer"


def test_elidia_adapter_retry_credential_force_refreshes_on_jwt_401(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
...
def test_elidia_adapter_retry_credential_skips_non_401(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "jwt-access",
        "refresh_token": "refresh-tok",
...
def test_elidia_adapter_get_credential_raises_on_refresh_failure(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
...
def test_elidia_adapter_quarantines_terminal_refresh_failure(tmp_path, monkeypatch):
    ...
    elidia_state = stored["providers"]["elidia"]
    ...
    assert stored.get("credential_pool", {}).get("elidia") == []


def test_elidia_adapter_get_credential_raises_when_no_jwt_returned(tmp_path, monkeypatch):
...
def test_elidia_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic ELIDIA_HOME."""
    auth_path = elidia_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from elidia_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    from elidia_cli.proxy.cli import cmd_proxy_status

    args = MagicMock()
    rc = cmd_proxy_status(args)
    assert rc == 0
    out = capsys.readouterr().out
    assert "elidia" in out
    assert "Elidia Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from elidia_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from elidia_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    args = MagicMock()
    args.provider = "elidia"
    args.host = None
    args.port = None
    rc = cmd_proxy_start(args)
    assert rc == 2
    err = capsys.readouterr().err
    assert "elidia auth add elidia" in err
