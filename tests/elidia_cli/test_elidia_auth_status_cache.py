"""Tests for the get_elidia_auth_status() process-level cache.

The cache avoids re-validating Elidia credentials on every menu paint —
`elidia tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against developer.aiutils.io during one render. The cache is keyed
on auth.json mtime so login/logout flows invalidate naturally; tests
and other writers can also call invalidate_elidia_auth_status_cache().
"""
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test ELIDIA_HOME.
...
def test_get_elidia_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    auth_mod.invalidate_elidia_auth_status_cache()
    ...
    with patch.object(auth_mod, "_compute_elidia_auth_status", side_effect=fake_compute):
        first = auth_mod.get_elidia_auth_status()
        second = auth_mod.get_elidia_auth_status()
        third = auth_mod.get_elidia_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_elidia_auth_status was called {call_count['n']}× — "
        "cache is not deduplicating within TTL."
    )
    # Each call returns a copy so callers can't mutate the cached dict.
    assert first == second == third
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_elidia_auth_status()

    auth_mod.invalidate_elidia_auth_status_cache()


def test_get_elidia_auth_status_invalidates_on_auth_file_mtime(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_elidia_auth_status", side_effect=fake_compute):
        auth_mod.get_elidia_auth_status()
...
def test_invalidate_elidia_auth_status_cache_forces_recompute(tmp_path, monkeypatch):
...
def test_get_elidia_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too — that's where the cost was.

    Teknium's case: ~31 cache misses per `elidia tools` "All Platforms"
