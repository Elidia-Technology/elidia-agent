...
Before the fix, ``elidia whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``elidia gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    elidia = home / ".elidia"
    elidia.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia))
...
def _env_value(elidia_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``elidia whatsapp`` with an existing paired session and
