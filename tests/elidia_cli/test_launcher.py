"""Tests for the top-level `./elidia` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./elidia` should use `elidia_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "elidia"
    called = []

    fake_main_module = types.ModuleType("elidia_cli.main")

    def fake_main():
        called.append("elidia_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "elidia_cli.main", fake_main_module)
