...
columns + Free/Pro badges and gate paid models on free Elidia accounts, the
same way the `elidia model` CLI picker does.
"""

import elidia_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_elidia_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_elidia_free_tier_gates_paid_models(monkeypatch):
    """A free-tier Elidia account marks paid models unavailable and sets the flag."""
    _patch_pricing(
        monkeypatch,
        free_tier=True,
        pricing={
    ...
    rows = [{"slug": "elidia", "models": ["free/model", "paid/model"]}]
    inv._apply_pricing(rows)
...
def test_apply_pricing_elidia_paid_tier_no_gating(monkeypatch):
    """A paid Elidia account gates nothing."""
    _patch_pricing(
        monkeypatch,
        free_tier=False,
        pricing={"elidia": {"x/model": {"prompt": "0.000001", "completion": "0.000002"}}},
    )
    rows = [{"slug": "elidia", "models": ["x/model"]}]
    inv._apply_pricing(rows)
