"""Tests for elidia_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ELIDIA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Elidia-managed node dir.
"""
...
import elidia_cli.uninstall as uninstall
...
def _make_elidia_node(elidia_home: Path) -> Path:
    """Create a fake $ELIDIA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = elidia_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_elidia_node(fake_home):
    elidia_home = fake_home / ".elidia"
    node_bin = _make_elidia_node(elidia_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(elidia_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(elidia_home) == []


def test_removes_dangling_symlink_into_elidia_node(fake_home):
    """A link into the Elidia node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Elidia links that exist; ignores the ones that don't."""
    elidia_home = fake_home / ".elidia"
    node_bin = _make_elidia_node(elidia_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Elidia-managed; node is a real user binary.
