...
def test_store_provider_state_can_skip_active_provider() -> None:
    auth_store = {"active_provider": "elidia", "providers": {}}
    ...
    assert auth_store["active_provider"] == "elidia"
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    assert auth_mod.get_active_provider() == "elidia"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from elidia_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes ELIDIA_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    ...
    assert "ELIDIA_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "ELIDIA_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "ELIDIA_SPOTIFY_CLIENT_ID" not in env_path.read_text()
