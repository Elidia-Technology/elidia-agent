"""Tests for `_can_open_graphical_browser()` in elidia_cli.auth.

Guards the fix for the May 2026 report where `elidia auth add xai-oauth`
...
from elidia_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("elidia_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("elidia_cli.auth.sys.platform", "darwin")
