...
`ELIDIA_REDACT_SECRETS` at module-import time. `elidia_cli/main.py` at
...
in .env), the toggle was silently ignored in both `elidia chat` and
`elidia gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `ELIDIA_REDACT_SECRETS`
env var in `elidia_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports elidia_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("ELIDIA_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import elidia_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('ELIDIA_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["ELIDIA_HOME"] = str(elidia_home)
    env.pop("ELIDIA_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env ELIDIA_REDACT_SECRETS takes precedence over config.yaml."""
    elidia_home = tmp_path / ".elidia"
    elidia_home.mkdir()
    (elidia_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (elidia_home / ".env").write_text("ELIDIA_REDACT_SECRETS=true\n")
