...
When ``ELIDIA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.elidia/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.elidia            (has its own auth.json fixture)
    * Profile     -> tmp_path/.elidia/profiles/coder   (active, ELIDIA_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "elidia-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("elidia")
    assert state is not None
    assert state["access_token"] == "elidia-global"
...
def test_provider_auth_state_profile_wins_when_present(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "elidia-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "elidia-profile"},
    }))

    state = get_provider_auth_state("elidia")
    assert state is not None
    assert state["access_token"] == "elidia-profile"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("elidia") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_elidia_runtime_credentials`` and
# ``resolve_elidia_access_token``) call ``_load_provider_state`` directly with
# a profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers raise ``"Elidia is not
# logged into Elidia Portal"`` even though the user has a valid global Elidia
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from elidia_cli.auth import _load_auth_store, _load_provider_state

    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "global-elidia-token", "refresh_token": "rt"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))
    ...
    assert state is not None
    assert state["access_token"] == "global-elidia-token"
...
def test_load_provider_state_profile_wins_over_global(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "global-token"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "profile-token"},
    }))
...
def test_load_provider_state_returns_none_when_neither_has_it(profile_env):
    ...
    auth_store = _load_auth_store()
    assert _load_provider_state(auth_store, "elidia") is None
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    elidia_home = tmp_path / "classic"
    elidia_home.mkdir()
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    _write(elidia_home / "auth.json", _make_auth_store(providers={
        "elidia": {"access_token": "classic-token"},
    }))
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ELIDIA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.elidia as the "real home" — which is NOT equal
    # to the ELIDIA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    elidia_home = tmp_path / "classic"
    ...
    _write(elidia_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from elidia_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ELIDIA_HOME is set to a custom path that is NOT under
    # ~/.elidia/profiles/ — get_default_elidia_root() returns ELIDIA_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from elidia_cli.auth import read_credential_pool, write_credential_pool
