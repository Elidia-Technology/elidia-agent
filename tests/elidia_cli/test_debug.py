"""Tests for ``elidia debug`` CLI command and debug utilities."""
...
@pytest.fixture
def elidia_home(tmp_path, monkeypatch):
    """Set up an isolated ELIDIA_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from elidia_cli.debug import _upload_paste_rs
        ...
        with patch("elidia_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "elidia_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from elidia_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from elidia_cli.debug import upload_to_pastebin

        with patch("elidia_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("elidia_cli.debug._upload_paste_rs",
             ...
             patch("elidia_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, elidia_home):
        from elidia_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, elidia_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (elidia_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, elidia_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = elidia_home / "logs" / "agent.log"
        from elidia_cli import debug
    ...
    def test_truncates_large_file(self, elidia_home):
        """Files larger than max_bytes get tail-truncated."""
        from elidia_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (elidia_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, elidia_home):
        ...
        (elidia_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, elidia_home):
    ...
    def test_unknown_log_returns_none(self, elidia_home):
        from elidia_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, elidia_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from elidia_cli.debug import _capture_log_snapshot

        logs_dir = elidia_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, elidia_home):
    ...
    def test_falls_back_when_primary_empty(self, elidia_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ELIDIA_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def elidia_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ELIDIA_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".elidia"
        home.mkdir()
        monkeypatch.setenv("ELIDIA_HOME", str(home))
        ...
        monkeypatch.delenv("ELIDIA_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, elidia_home_with_secret):
        from elidia_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, elidia_home_with_secret):
        from elidia_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, elidia_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ELIDIA_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ELIDIA_REDACT_SECRETS", "false")

        from elidia_cli.debug import _capture_log_snapshot

        assert os.environ.get("ELIDIA_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, elidia_home_with_secret
    ):
        from elidia_cli.debug import _capture_log_snapshot

        log_path = elidia_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, elidia_home_with_secret):
        from elidia_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from elidia_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, elidia_home):
        from elidia_cli.debug import collect_debug_report

        with patch("elidia_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- elidia dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- elidia dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, elidia_home):
        from elidia_cli.debug import collect_debug_report

        with patch("elidia_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, elidia_home):
        from elidia_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, elidia_home):
        from elidia_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, elidia_home):
        from elidia_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, elidia_home, capsys):
        ...
        with patch("elidia_cli.dump.run_dump"), \
             patch("elidia_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("elidia_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, elidia_home, capsys):
        ...
        with patch("elidia_cli.dump.run_dump"), \
             patch(
                 "elidia_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, elidia_home, capsys):
    ...
    def test_share_uploads_four_pastes(self, elidia_home, capsys):
        ...
        with patch("elidia_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- elidia dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- elidia dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- elidia dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        desktop_paste = uploaded_content[3]
        assert "--- elidia dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, elidia_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from elidia_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("elidia_cli.dump.run_dump"), \
             patch("elidia_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("elidia_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, elidia_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, elidia_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, elidia_home_with_secret, capsys
    ):
        ...
        with patch("elidia_cli.dump.run_dump"), \
             patch("elidia_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("elidia_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "elidia debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, elidia_home):
        from elidia_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from elidia_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from elidia_cli.debug import delete_paste
        ...
        with patch("elidia_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.elidia/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``elidia debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, elidia_home):
        ...
        import ast
        import inspect
        from elidia_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, elidia_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from elidia_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, elidia_home):
    ...
    def test_merges_with_existing_pending(self, elidia_home):
        """Subsequent calls merge into existing pending.json."""
        from elidia_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, elidia_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, elidia_home):
    ...
    def test_sweep_deletes_expired_entries(self, elidia_home):
        ...
        with patch("elidia_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, elidia_home):
        from elidia_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("elidia_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, elidia_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, elidia_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, elidia_home):
        from elidia_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("elidia_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, elidia_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: elidia debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from elidia_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("elidia_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("elidia_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, elidia_home, capsys):
        from elidia_cli.debug import run_debug_share
        ...
        with patch("elidia_cli.dump.run_dump"), \
             ...
             patch("elidia_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, elidia_home, capsys):
        from elidia_cli.debug import run_debug_share
        ...
        with patch("elidia_cli.dump.run_dump"), \
             patch("elidia_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("elidia_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, elidia_home, capsys):
        from elidia_cli.debug import run_debug_share
