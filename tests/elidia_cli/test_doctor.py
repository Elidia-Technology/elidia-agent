...
import elidia_cli.doctor as doctor
import elidia_cli.gateway as gateway_cli
from elidia_cli import doctor as doctor_mod
from elidia_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `elidia doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ELIDIA_HOME", elidia_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ELIDIA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_elidia_home(self, tmp_path, provider=""):
        """Create a minimal ELIDIA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_elidia_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "ELIDIA_HOME", home)
        ...
        try:
            from elidia_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_elidia_auth_status", lambda: {})
...
def test_run_doctor_accepts_elidia_provider_ids_that_catalog_aliases(
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_elidia_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from elidia_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("elidia_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "elidia_cli.doctor", raising=False)
        from elidia_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Elidia / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from elidia_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_elidia_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Elidia / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".elidia"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Elidia Portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ELIDIA_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir(parents=True)
        (elidia_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"ELIDIA_MAX_ITERATIONS={ghost}\n")
        (elidia_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ELIDIA_HOME", elidia_home)
        monkeypatch.setattr(doctor_mod, "get_elidia_home", lambda: elidia_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ELIDIA_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ELIDIA_MAX_ITERATIONS" in out
        env_after = (elidia_home / ".env").read_text(encoding="utf-8")
        assert "ELIDIA_MAX_ITERATIONS" not in env_after
