"""Tests for ``elidia gui`` desktop launcher wiring."""
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Elidia.app" / "Contents" / "MacOS" / "Elidia"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Elidia.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "elidia"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("elidia_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("elidia_cli.main._desktop_build_needed", return_value=True), \
         patch("elidia_cli.main._write_desktop_build_stamp"), \
         patch("elidia_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("elidia_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    elidia_root = tmp_path / "custom-elidia"
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("elidia_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("elidia_cli.main._desktop_build_needed", return_value=True), \
         patch("elidia_cli.main._write_desktop_build_stamp"), \
         patch("elidia_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("elidia_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(
            fake_boot=True,
            ignore_existing=True,
            elidia_root=str(elidia_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["ELIDIA_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["ELIDIA_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["ELIDIA_DESKTOP_ELIDIA_ROOT"] == str(elidia_root)
    assert launch_env["ELIDIA_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("elidia_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value=None), \
         patch("elidia_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("elidia_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("elidia_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("elidia_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("elidia_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("elidia_cli.main._desktop_build_needed", return_value=True), \
         patch("elidia_cli.main._write_desktop_build_stamp"), \
         patch("elidia_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main._desktop_build_needed", return_value=False), \
         patch("elidia_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("elidia_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("elidia_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("elidia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("elidia_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("elidia_cli.main._desktop_build_needed", return_value=False), \
         patch("elidia_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("elidia_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("elidia_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"elidia"}', encoding="utf-8")
