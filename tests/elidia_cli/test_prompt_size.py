"""Tests for the ``elidia prompt-size`` diagnostic (issue #34667)."""

import json

import pytest

from elidia_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(elidia_home, memory_text="", user_text=""):
    mem_dir = elidia_home / "memories"
...
def _seed_skill(elidia_home, name, description):
    skill_dir = elidia_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "ELIDIA_API_KEY",
                "ANTHROPIC_API_KEY"):
