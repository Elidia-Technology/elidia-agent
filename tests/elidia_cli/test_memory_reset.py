"""Tests for the `elidia memory reset` CLI command.
...
- Profile-scoped reset (uses ELIDIA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ELIDIA_HOME with memory files."""
    elidia_home = tmp_path / ".elidia"
    memories = elidia_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nElidia repo is at ~/.elidia/elidia-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `elidia memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        (elidia_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ELIDIA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_elidia_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
