"""Regression tests for #34107 — Docker UID/GID handling in ensure_elidia_home.

When Elidia runs in Docker with ``ELIDIA_UID=1000`` / ``ELIDIA_GID=911``,
the entrypoint chowns the top-level ``ELIDIA_HOME`` once at startup. But
...
The fix is a ``_chown_to_elidia_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_elidia_uid_gid
# ---------------------------------------------------------------------------


class TestResolveElidiaUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_UID", "1000")
        monkeypatch.setenv("ELIDIA_GID", "911")
        from elidia_cli.config import _resolve_elidia_uid_gid
        uid, gid = _resolve_elidia_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ELIDIA_UID", raising=False)
        monkeypatch.delenv("ELIDIA_GID", raising=False)
        from elidia_cli.config import _resolve_elidia_uid_gid
        uid, gid = _resolve_elidia_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_UID", "not-a-number")
        monkeypatch.setenv("ELIDIA_GID", "911")
        from elidia_cli.config import _resolve_elidia_uid_gid
        uid, gid = _resolve_elidia_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ELIDIA_UID", "")
        monkeypatch.setenv("ELIDIA_GID", "")
        from elidia_cli.config import _resolve_elidia_uid_gid
        uid, gid = _resolve_elidia_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_elidia_uid
# ---------------------------------------------------------------------------


class TestChownToElidiaUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ELIDIA_UID", "1000")
        monkeypatch.setenv("ELIDIA_GID", "911")
        from elidia_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_elidia_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_elidia_uid(d)  # must not raise
