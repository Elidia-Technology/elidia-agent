"""Tests for elidia_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ELIDIA_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from elidia_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "elidia": {
                "metadata": {"display_name": "Elidia Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from elidia_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    when the primary URL fails. Regression: the aiutils.io-hosted manifest can
    be unreachable (CDN hiccup, DNS, transient 5xx); without a fallback URL
    the user's disk cache freezes and new model releases never reach the
    picker.
    """

    PRIMARY = "https://share.aiutils.io/elidia-hermes/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/Elidia-Technology/elidia-agent"
...
class TestCuratedAccessors:
    ...
    def test_elidia_returns_ids(self, isolated_home):
        from elidia_cli import model_catalog
        with patch.object(
            model_catalog, "_fetch_manifest", return_value=_valid_manifest()
        ):
            result = model_catalog.get_curated_elidia_models()
    ...
    def test_elidia_returns_none_when_catalog_empty(self, isolated_home):
        from elidia_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_elidia_models() is None
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in elidia_cli.models."""

    def test_curated_elidia_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from elidia_cli import model_catalog
        from elidia_cli.models import get_curated_elidia_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_elidia_model_ids()

        assert result == list(_PROVIDER_MODELS["elidia"])

    def test_curated_elidia_ids_prefers_manifest(self, isolated_home):
        from elidia_cli import model_catalog
        from elidia_cli.models import get_curated_elidia_model_ids
    ...
    def test_picker_elidia_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["elidia"]``
        list in curated order — matching the ``elidia model`` CLI — not the live
        ...
        # ``ELIDIA_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ELIDIA_HOME directly instead.
        import importlib
        from elidia_cli import model_catalog
        from elidia_cli.models import get_curated_elidia_model_ids
        importlib.reload(model_catalog)
        try:
            from elidia_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["ELIDIA_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"elidia": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["elidia"] = get_curated_elidia_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("elidia_cli.models.check_elidia_free_tier", return_value=False), patch(
                "elidia_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "elidia_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_elidia_model_ids()
                picker = list_picker_providers(
                    current_provider="elidia", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        elidia_row = next((r for r in picker if r["slug"] == "elidia"), None)
        assert elidia_row is not None, "elidia row must appear when authed"
        assert elidia_row["models"] == expected
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["elidia"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['elidia'] / OPENROUTER_MODELS. "
