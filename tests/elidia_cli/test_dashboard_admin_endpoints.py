...
def _client():
    ...
    # Keep the state DB under the isolated ELIDIA_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_elidia_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_elidia_home):
        ...
        (get_elidia_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "elidia_version"):
...
class TestUpdateCheckEndpoint:
    """``GET /api/elidia/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/elidia/update`` runs ``elidia update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/elidia/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/elidia/update/check").json()
