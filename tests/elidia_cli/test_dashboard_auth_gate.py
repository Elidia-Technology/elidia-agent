...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Elidia-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__ELIDIA_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("elidia-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from elidia_cli.web_server import should_require_auth
...
def test_start_server_public_without_insecure_records_auth_required(monkeypatch):
    ...
    """
    from elidia_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_start_server_surfaces_elidia_skip_reason_when_unconfigured(monkeypatch):
    """When the bundled Elidia plugin loaded but skipped registration (no
    ...
    'set ELIDIA_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from elidia_cli.dashboard_auth import clear_providers
    from plugins.dashboard_auth import elidia as elidia_plugin
    ...
    elidia_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
    ...
    # than the generic "Install the default Elidia provider" boilerplate.
    msg = str(exc_info.value)
    assert "ELIDIA_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "elidia:" in msg
