...
class TestProviderMapping:
    ...
    def test_unmapped_provider_not_in_dict(self):
        assert "elidia" not in PROVIDER_TO_MODELS_DEV
...
class TestLookupModelsDevContext:
    ...
    def test_provider_not_mapped(self, mock_fetch):
        mock_fetch.return_value = SAMPLE_REGISTRY
        assert lookup_models_dev_context("elidia", "some-model") is None
...
class TestFetchModelsDev:
    ...
    def test_force_refresh_skips_disk_cache(self, mock_get):
        ...
        disk-cache fast path. Used by ``elidia config refresh`` and
