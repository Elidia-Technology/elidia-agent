...
from agent.auxiliary_client import (
    ...
    _refresh_elidia_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "ELIDIA_INFERENCE_BASE_URL",
...
class TestAuxiliaryMaxTokensParam:
    def test_uses_max_completion_tokens_for_github_copilot_custom_base(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com", "key", None)), \
             patch("agent.auxiliary_client._read_elidia_auth", return_value=None):
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("elidia", "hermes-4", "https://inference-api.aiutils.io/v1"),
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("elidia_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        elidia_home = tmp_path / "elidia"
        elidia_home.mkdir(parents=True, exist_ok=True)
        (elidia_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``elidia auth status`` already reports these as logged in; compression
        ...
        from elidia_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
        ...
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        monkeypatch.delenv("ELIDIA_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        monkeypatch.setenv("ELIDIA_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``elidia model``, no per-task override is
    ...
    fallback chain (OpenRouter / Elidia / etc.), so aux tasks billed
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        """anthropic / elidia / openrouter / etc.: catalog default wins
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._read_elidia_auth", return_value=None
            ),
        ):
...
class TestExpiredCodexFallback:
    ...
    def test_elidia_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-elidia-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("elidia_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        with patch("agent.auxiliary_client._read_elidia_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_elidia_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    def test_try_elidia_uses_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("elidia_cli.models.get_elidia_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_elidia

            client, model = _try_elidia()
    ...
    def test_try_elidia_uses_portal_recommendation_for_text(self):
        """When the Portal recommends a compaction model, _try_elidia honors it."""
        fresh_base = "https://inference-api.aiutils.io/v1"
        with (
            patch("agent.auxiliary_client._read_elidia_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("elidia_cli.models.get_elidia_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_elidia

            mock_openai.return_value = MagicMock()
            client, model = _try_elidia(vision=False)

        assert client is not None
        assert model == "minimax/minimax-m2.7"
        assert mock_rec.call_args.kwargs["vision"] is False

    def test_try_elidia_uses_portal_recommendation_for_vision(self):
        """Vision tasks should ask for the vision-specific recommendation."""
        fresh_base = "https://inference-api.aiutils.io/v1"
        with (
            patch("agent.auxiliary_client._read_elidia_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("elidia_cli.models.get_elidia_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_elidia
            client, model = _try_elidia(vision=True)

        assert client is not None
        assert model == "google/gemini-3-flash-preview"
        assert mock_rec.call_args.kwargs["vision"] is True

    def test_try_elidia_falls_back_when_recommendation_lookup_raises(self):
        """If the Portal lookup throws, we must still return a usable model."""
        fresh_base = "https://inference-api.aiutils.io/v1"
        with (
            patch("agent.auxiliary_client._read_elidia_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("elidia_cli.models.get_elidia_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_elidia
            client, model = _try_elidia()
    ...
    def test_call_llm_retries_elidia_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.aiutils.io/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale elidia key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.aiutils.io/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("elidia", "elidia-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "elidia-model")),
            patch("agent.auxiliary_client.OpenAI", return_value=fresh_client),
            patch("agent.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", "https://inference-api.aiutils.io/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_elidia_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", "https://inference-api.aiutils.io/v1")),
            patch(
                "elidia_cli.elidia_account.get_elidia_portal_account_info",
                return_value=ElidiaPortalAccountInfo(
                    logged_in=True,
                    source="account_api",
                    fresh=True,
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_retries_elidia_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.aiutils.io/v1"
        stale_client.chat.completions.create = AsyncMock(side_effect=_Auth401("stale elidia key"))

        fresh_async_client = MagicMock()
        fresh_async_client.base_url = "https://inference-api.aiutils.io/v1"
        fresh_async_client.chat.completions.create = AsyncMock(return_value={"ok": True})

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("elidia", "elidia-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "elidia-model")),
            patch("agent.auxiliary_client._to_async_client", return_value=(fresh_async_client, "elidia-model")),
            patch("agent.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("agent.auxiliary_client._resolve_elidia_runtime_api", return_value=("fresh-agent-key", "https://inference-api.aiutils.io/v1")),
        ):
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_refreshes_elidia_after_free_tier_block_when_account_paid(self):
        from elidia_cli.elidia_account import ElidiaPortalAccountInfo
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://developer.aiutils.io or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestIsModelNotFoundError:
    ...
    def test_elidia_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Elidia → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshElidiaRecommendedModel:
    """_refresh_elidia_recommended_model picks a fresh model after a stale 404."""

    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "elidia_cli.models.get_elidia_recommended_aux_model",
            lambda **kw: "stepfun/step-3.7-flash:free",
        )
        out = _refresh_elidia_recommended_model(
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "elidia_cli.models.get_elidia_recommended_aux_model", _boom)
        out = _refresh_elidia_recommended_model(
...
class TestIsRateLimitError:
    ...
    def test_429_with_resets_in_message(self):
        """Elidia-style 429: 'resets in 3508s'."""
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "elidia", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_elidia", return_value=(mock_client, "elidia-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "elidia-model"
        assert label == "elidia"

    def test_returns_none_when_no_fallback(self):
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_elidia", return_value=(None, None)), \
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Elidia/custom/api-key all fail, payment-fallback returns None —
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from elidia_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("elidia_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "elidia_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("elidia_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/Elidia-Technology/elidia-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/Elidia-Technology/elidia-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ElidiaAgent"
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        elidia_client = MagicMock()
        # Mark OpenRouter unhealthy → chain should skip it and pick elidia.
        _mark_provider_unhealthy("openrouter")
        with patch("agent.auxiliary_client._read_main_provider", return_value=""), \
             ...
             patch("agent.auxiliary_client._try_elidia", return_value=(elidia_client, "elidia-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        assert client is elidia_client
        assert model == "elidia-model"
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._try_openrouter") as or_try, \
             patch("agent.auxiliary_client._try_elidia", return_value=(elidia_client, "n-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        # Step-1 was bypassed — resolve_provider_client never invoked
        step1.assert_not_called()
        # Step-2 also skipped openrouter and landed on elidia
        or_try.assert_not_called()
        assert client is elidia_client
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find elidia.
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        elidia_client = MagicMock()
        elidia_resp = MagicMock()
        elidia_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        elidia_client.chat.completions.create.return_value = elidia_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._resolve_task_provider_model",
                    return_value=("auto", "google/gemini-3-flash-preview", None, None, None)), \
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(elidia_client, "n-model", "elidia")), \
