...
Prior to this change, aggregator users (OpenRouter / Elidia Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_elidia_main_uses_main_model_for_aux(self, monkeypatch):
        """Elidia Portal main user → aux uses their picked Elidia model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY → ensures if main failed we'd fall to chain
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="elidia",
        ), patch(
            "agent.auxiliary_client._read_main_model",
        ...
        assert mock_resolve.call_args.args[0] == "elidia"
...
class TestResolveVisionMainFirst:
    ...
    def test_elidia_main_vision_uses_paid_elidia_vision_backend(self):
        """Paid Elidia main → aux vision uses the dedicated Elidia vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="elidia",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_elidia_main_vision_uses_free_tier_elidia_vision_backend(self):
        """Free-tier Elidia main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="elidia",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "elidia_cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/Elidia strict backends."""
        ...
        assert provider in {"openrouter", "elidia"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("elidia", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "elidia-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "elidia" override → uses strict backend, NOT main model path
        assert provider == "elidia"
        mock_strict.assert_called_once_with("elidia", None)
