...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Elidia passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ELIDIA_HOME", "/users/alice/.elidia/profiles/backend-worker")
        monkeypatch.setenv("ELIDIA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ELIDIA_KANBAN_DB",
            "/users/alice/.elidia/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.elidia/kanban/boards/smoke"]'
            in cmd
        )
