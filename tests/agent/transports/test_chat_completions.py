...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Elidia-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_elidia_tags(self, transport):
        ...
        assert kw["extra_body"]["tags"] == elidia_portal_tags()
    ...
    def test_elidia_omits_disabled_reasoning(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("elidia")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # Elidia rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Elidia has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (Elidia, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
