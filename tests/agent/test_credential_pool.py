...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (elidia_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "elidia" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "elidia" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "elidia" / "auth.json").read_text()
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "elidia_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from elidia_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://elidia/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://elidia/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "elidia" / "auth.json").read_text())["credential_pool"]["elidia"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.elidia/.env when
    ...
    # User edited ~/.elidia/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.elidia/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (elidia_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_migrates_elidia_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_mirrors_elidia_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    ...
    auth_payload = json.loads((tmp_path / "elidia" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["elidia"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_elidia_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential
...
def test_elidia_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    monkeypatch.setenv("ELIDIA_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    pool.add_entry(PooledCredential.from_dict("elidia", {
        "id": "legacy-seeded",
        "source": "manual:device_code",
        "auth_type": "oauth",
    ...
    pool.add_entry(PooledCredential.from_dict("elidia", {
        ...
        "source": "manual",
        "auth_type": "api_key",
        "access_token": "manual-elidia-key",
    }))

    monkeypatch.setattr(auth_mod, "resolve_elidia_runtime_credentials", _terminal_refresh_failure)
    ...
    auth_payload = json.loads((tmp_path / "elidia" / "auth.json").read_text())
    elidia_state = auth_payload["providers"]["elidia"]
    assert not elidia_state.get("refresh_token")
    assert not elidia_state.get("access_token")
    assert not elidia_state.get("agent_key")
    assert elidia_state["last_auth_error"]["code"] == "invalid_grant"
    assert [entry["id"] for entry in auth_payload["credential_pool"]["elidia"]] == ["manual-key"]

    assert pool.try_refresh_current() is None
    assert refresh_calls["count"] == 1


def test_load_pool_removes_elidia_device_code_when_singleton_quarantined(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_elidia_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_elidia_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    _write_auth_store(
        tmp_path,
        {
    ...
    auth_payload = json.loads((tmp_path / "elidia" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["elidia"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/elidia-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("elidia_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:elidia_pkce", "elidia_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `elidia setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Elidia PKCE
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_elidia_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `elidia_pkce` into auth.json) and later switches to the API key at
    `elidia setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("elidia_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_elidia_oauth_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `elidia setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    (tmp_path / "elidia").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "elidia" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "elidia_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_elidia_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.elidia`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-elidia-AAAA"
...
# ── PR #10160 salvage: Elidia OAuth cross-process sync tests ─────────────────

def test_sync_elidia_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    synced = pool._sync_elidia_entry_from_auth_store(entry)
...
def test_sync_elidia_entry_noop_when_tokens_match(tmp_path, monkeypatch):
...
def test_elidia_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted Elidia entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / "elidia"))
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `elidia auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `elidia model` to reauth, but the pool
    ...
    # Simulate `elidia model` / `elidia auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from elidia_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
    ...
    # Elidia error does not trigger xAI check
    assert not _is_terminal_xai_oauth_refresh_error(
        AuthError("Revoked", provider="elidia", code="invalid_grant", relogin_required=True)
    )
    # Generic exception
...
def test_is_terminal_codex_oauth_refresh_error():
    from elidia_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
