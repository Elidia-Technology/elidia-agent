...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_elidia_env(self):
        """Elidia' own .env inside ELIDIA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: elidia internal .env is in ~/.elidia/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.elidia/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.elidia/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Elidia cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        elidia_home = tmp_path / ".elidia"
        cache = elidia_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._elidia_home_path", return_value=elidia_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Elidia cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        elidia_home = tmp_path / ".elidia"
        hub = elidia_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_elidia_home(self, tmp_path):
        """The env basename guard does not depend on ELIDIA_HOME resolution."""
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        elidia_home = tmp_path / ".elidia"
        cache = elidia_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
