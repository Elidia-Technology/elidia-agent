"""Tests for agent/elidia_rate_guard.py — cross-session Elidia Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    elidia_home = str(tmp_path / ".elidia")
    os.makedirs(elidia_home, exist_ok=True)
    monkeypatch.setenv("ELIDIA_HOME", elidia_home)
    # Clear any cached module-level imports
    return elidia_home


class TestRecordElidiaRateLimit:
    """Test recording rate limit state."""

    def test_records_with_header_reset(self, rate_guard_env):
        from agent.elidia_rate_guard import record_elidia_rate_limit, _state_path

        headers = {"x-ratelimit-reset-requests-1h": "1800"}
        record_elidia_rate_limit(headers=headers)
    ...
    def test_falls_back_to_default_cooldown(self, rate_guard_env):
        ...
        record_elidia_rate_limit(headers=None)
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_elidia_rate_limit(headers=None, default_cooldown=120.0)
    ...
    def test_creates_directory_if_missing(self, rate_guard_env):
        ...
        record_elidia_rate_limit(headers={"retry-after": "10"})
        assert os.path.exists(_state_path())


class TestElidiaRateLimitRemaining:
    ...
    def test_returns_none_when_no_file(self, rate_guard_env):
        ...
        assert elidia_rate_limit_remaining() is None

    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.elidia_rate_guard import record_elidia_rate_limit, elidia_rate_limit_remaining

        record_elidia_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = elidia_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.elidia_rate_guard import elidia_rate_limit_remaining, _state_path
...
class TestClearElidiaRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_elidia_rate_limit(headers={"retry-after": "600"})
        assert elidia_rate_limit_remaining() is not None
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.elidia_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.elidia_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_elidia_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.elidia_rate_guard import record_elidia_rate_limit

        # Record a rate limit
        record_elidia_rate_limit(headers={"retry-after": "600"})

        # Mock _read_elidia_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_elidia_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.elidia.test/v1",
        })

        result = aux._try_elidia()
        assert result == (None, None)

    def test_try_elidia_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_elidia should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_elidia_auth", lambda: None)
        result = aux._try_elidia()
        assert result == (None, None)


class TestIsGenuineElidiaRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    Elidia Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Elidia).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.elidia_rate_guard import is_genuine_elidia_rate_limit
        ...
        assert is_genuine_elidia_rate_limit(headers=headers) is True
    ...
    def test_healthy_headers_on_429_are_upstream_capacity(self):
        # Classic upstream-capacity symptom: Elidia edge reports plenty of
        ...
        assert is_genuine_elidia_rate_limit(headers=headers) is False

    def test_bare_429_with_no_headers_is_upstream(self):
        from agent.elidia_rate_guard import is_genuine_elidia_rate_limit

        assert is_genuine_elidia_rate_limit(headers=None) is False
        assert is_genuine_elidia_rate_limit(headers={}) is False
        assert is_genuine_elidia_rate_limit(
    ...
    def test_last_known_state_with_exhausted_bucket_triggers_genuine(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="elidia")
        assert is_genuine_elidia_rate_limit(
