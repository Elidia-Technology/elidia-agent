...
engine plugins (e.g. elidia-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``elidia plugins`` picker."""
    from elidia_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "elidia_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "elidia_cli.plugins.get_plugin_context_engine",
