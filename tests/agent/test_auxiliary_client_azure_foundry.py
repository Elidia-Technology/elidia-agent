...
that delegates to :func:`elidia_cli.runtime_provider._resolve_azure_foundry_runtime`
...
class TestResolveProviderClientAzureFoundry:
    ...
    def test_warns_and_returns_none_on_failure(
        ...
        clear warning pointing at ``elidia doctor``."""
        import logging
        ...
        assert resolved is None
        assert any(
            "azure-foundry" in rec.message and "elidia doctor" in rec.message
