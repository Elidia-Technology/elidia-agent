...
def curator_env(tmp_path, monkeypatch):
    """Isolated ELIDIA_HOME + freshly reloaded curator + skill_usage modules."""
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `elidia curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """elidia curator unpin must refuse bundled/hub skills too (matches pin)."""
    from elidia_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `elidia model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_model_new_slot_wins_over_legacy(curator_env):
    ...
    cfg = {
        "model": {"provider": "openrouter", "default": "openai/gpt-5.5"},
        "auxiliary": {
            "curator": {"provider": "elidia", "model": "new-winner"},
        },
        "curator": {
            "auxiliary": {"provider": "openrouter", "model": "legacy-loser"},
        },
    }
    assert curator._resolve_review_model(cfg) == ("elidia", "new-winner")
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from elidia_cli.main import _AUX_TASKS
    from elidia_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. elidia_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. elidia_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
