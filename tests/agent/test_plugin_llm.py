...
""",
        ...
        from elidia_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        elidia_home = tmp_path / ".elidia"
        elidia_home.mkdir()
        (elidia_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        from elidia_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from elidia_cli.plugins import PluginContext, PluginManifest, PluginManager
