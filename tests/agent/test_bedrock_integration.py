...
Note: Tests that import ``elidia_cli.auth`` or ``elidia_cli.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("elidia_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("elidia_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("elidia_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("elidia_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("elidia_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("elidia_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into elidia_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from elidia_cli.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from elidia_cli.providers import TRANSPORT_TO_API_MODE
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "elidia-agent[bedrock]" not in extras["all"]
