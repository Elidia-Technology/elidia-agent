...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ELIDIA_HOME under tmp_path."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / ".elidia"))
    (tmp_path / ".elidia").mkdir()

    # Force the constants/image cache helpers to re-read ELIDIA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("elidia_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_elidia_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ELIDIA_HOME — gateway cleanup
