...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Elidia Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_elidia_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path / ".elidia"))

    elidia_env = tmp_path / ".elidia" / ".env"
    elidia_env.parent.mkdir(parents=True)
    elidia_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.elidia/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
