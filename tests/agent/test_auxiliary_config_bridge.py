...
class TestAuxiliaryConfigBridge:
    ...
    def test_web_extract_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "web_extract": {"provider": "elidia", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_PROVIDER") == "elidia"
    ...
    def test_all_tasks_with_overrides(self, monkeypatch):
        config = {
            "auxiliary": {
                ...
                "web_extract": {"provider": "elidia", "model": "gemini-3-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_VISION_PROVIDER") == "openrouter"
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in elidia_cli/config.py has correct auxiliary structure."""
