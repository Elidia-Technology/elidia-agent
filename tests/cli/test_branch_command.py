...
def session_db(tmp_path):
    ...
    os.environ["ELIDIA_HOME"] = str(tmp_path / ".elidia")
    os.makedirs(tmp_path / ".elidia", exist_ok=True)
    from elidia_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".elidia" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal ElidiaCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        ElidiaCLI._handle_branch_command(cli_instance, "/branch refactor approach")
