...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("elidia/elidia-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned elidia/* and pr-* branches."""

    def test_prunes_orphaned_elidia_branch(self, git_repo):
        """elidia/elidia-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "elidia/elidia-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "elidia/elidia-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "elidia/elidia-deadbeef" in result.stdout
        ...
        assert "elidia/elidia-deadbeef" in orphaned
        ...
        assert "elidia/elidia-deadbeef" not in result.stdout
