"""Tests for elidia_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Elidia entry point
(elidia, elidia-agent, elidia-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Elidia entry point imports elidia_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported elidia_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("elidia_bootstrap", None)
    import elidia_bootstrap  # noqa: WPS433
    return elidia_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Elidia CLI
...
class TestEntryPointsImportBootstrap:
    """Every Elidia entry point must import elidia_bootstrap as its
    ...
    # Entry points that invoke Elidia as a process.  Each one must
    # import elidia_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "elidia_cli/main.py",   # elidia CLI (console_script)
        "run_agent.py",          # elidia-agent (console_script)
        "acp_adapter/entry.py",  # elidia-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import elidia_bootstrap' and that
        ...
        half-finished ``elidia update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``elidia_bootstrap`` as a top-level module) leaves elidia
        ...
        """
        # Resolve relative to the elidia-agent repo root.  Tests live
        # at tests/test_elidia_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # elidia start even when elidia_bootstrap hasn't been
        ...
        assert first_import_name == "elidia_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'elidia_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import elidia_bootstrap' line to be the first import."
        )
