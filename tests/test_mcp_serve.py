"""
Tests for mcp_serve — Elidia MCP server.
...
def _isolate_elidia_home(tmp_path, monkeypatch):
    """Redirect ELIDIA_HOME to a temp directory."""
    monkeypatch.setenv("ELIDIA_HOME", str(tmp_path))
    try:
        import elidia_constants
        monkeypatch.setattr(elidia_constants, "get_elidia_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking elidia_state schema."""
