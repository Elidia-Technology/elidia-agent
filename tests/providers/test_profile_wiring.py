...
class TestElidiaProfileParity:
    def test_tags(self, transport):
        legacy = transport.build_kwargs(
            model="hermes-3", messages=_msgs(), tools=None, provider_profile=get_provider_profile("elidia"),
        )
        profile = transport.build_kwargs(
            model="hermes-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("elidia"),
        )
    ...
    def test_reasoning_omitted_when_disabled(self, transport):
        ...
        legacy = transport.build_kwargs(
            model="hermes-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("elidia"), supports_reasoning=True, reasoning_config=rc,
        )
        profile = transport.build_kwargs(
            model="hermes-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("elidia"),
...
class TestRequestOverridesParity:
    ...
    def test_extra_body_override_merges_with_provider_body(self, transport):
        ...
        assert kw["extra_body"]["tags"] == elidia_portal_tags()  # from profile
