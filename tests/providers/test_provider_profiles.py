...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("elidia-portal").name == "elidia"
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike Elidia)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
...
class TestElidiaProfile:
    def test_tags(self):
        from agent.portal_tags import elidia_portal_tags
        p = get_provider_profile("elidia")
        body = p.build_extra_body()
