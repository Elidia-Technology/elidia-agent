...
class TestGetDefaultModelForProvider:
    """Unit tests for elidia_cli.models.get_default_model_for_provider."""
    ...
    def test_aiutils_returns_deepseek_v4_flash(self):
        """The aiutils Developer API provider defaults to deepseek-v4-flash.

        Regression guard: the aiutils provider has no static entry in
        _PROVIDER_MODELS and no REST catalog, so it must fall back to the
        profile's explicit default_model rather than sending an empty model
        string (which the gateway rejects with HTTP 400).
        """
        from elidia_cli.models import get_default_model_for_provider
        assert get_default_model_for_provider("aiutils") == "deepseek-v4-flash"
