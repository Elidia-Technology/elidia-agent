"""Tests for elidia_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    elidia_logging._logging_initialized = False
    ...
    elidia_logging._install_session_record_factory()
    yield
    ...
    elidia_logging._logging_initialized = False
    elidia_logging.clear_session_context()


@pytest.fixture
def elidia_home(tmp_path, monkeypatch):
    """Provide an isolated ELIDIA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_elidia_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["ELIDIA_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, elidia_home):
        log_dir = elidia_logging.setup_logging(elidia_home=elidia_home)
        assert log_dir == elidia_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        elidia_logging.setup_logging(elidia_home=elidia_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        elidia_logging.setup_logging(elidia_home=elidia_home, force=True)
    ...
    def test_custom_log_level(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, elidia_home):
        elidia_logging.setup_logging(
            elidia_home=elidia_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
    ...
    def test_writes_to_agent_log(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)

        test_logger = logging.getLogger("test_elidia_logging.write_test")
        ...
        agent_log = elidia_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)

        test_logger = logging.getLogger("test_elidia_logging.warning_test")
        ...
        agent_log = elidia_home / "logs" / "agent.log"
        errors_log = elidia_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)

        test_logger = logging.getLogger("test_elidia_logging.info_test")
    ...
    def test_explicit_params_override_config(self, elidia_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (elidia_home / "config.yaml").write_text(yaml.dump(config))

        elidia_logging.setup_logging(elidia_home=elidia_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, elidia_home):
        """The custom record factory injects session_tag on all records."""
        elidia_logging.setup_logging(elidia_home=elidia_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_elidia_session_injector", False), (
            "Record factory should have _elidia_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, elidia_home):
        ...
        gw_log = elidia_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, elidia_home):
    ...
    def test_gateway_log_receives_gateway_records(self, elidia_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, elidia_home):
    ...
    def test_agent_log_still_receives_all(self, elidia_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="cli")
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="gui")

        logging.getLogger("elidia_cli.web_server").info("dashboard online")
        ...
        gui_log = elidia_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, elidia_home):
        """When session context is set, log lines include [session_id]."""
        elidia_logging.setup_logging(elidia_home=elidia_home)
        elidia_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, elidia_home):
    ...
    def test_clear_session_context(self, elidia_home):
        """After clearing, session tag disappears."""
        elidia_logging.setup_logging(elidia_home=elidia_home)
        elidia_logging.set_session_context("xyz789")
        elidia_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, elidia_home):
        ...
        def thread_a():
            elidia_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            elidia_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        elidia_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        elidia_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = elidia_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = elidia_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in elidia_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # elidia_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "elidia_plugins") == elidia_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = elidia_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == elidia_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = elidia_logging.COMPONENT_PREFIXES["cli"]
        assert "elidia_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == elidia_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = elidia_logging.COMPONENT_PREFIXES["gui"]
        assert "elidia_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        elidia_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_elidia_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, elidia_home):
        elidia_logging.setup_logging(elidia_home=elidia_home)
        elidia_logging.setup_verbose_logging()
        elidia_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        elidia_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = elidia_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        elidia_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("elidia_cli.config.is_managed", return_value=True):
                elidia_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, elidia_home):
        level, max_size, backup = elidia_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, elidia_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, elidia_home):
        import yaml
        config = {"model": "test"}
        (elidia_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> elidia_logging._ManagedRotatingFileHandler:
        handler = elidia_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that elidia_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        elidia_logging.setup_logging(elidia_home=elidia_home, mode="gateway")
        gw_path = elidia_home / "logs" / "gateway.log"
        rotated = elidia_home / "logs" / "gateway.log.1"
