...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.elidia/media/'
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("ELIDIA_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("ELIDIA_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ELIDIA_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ELIDIA_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ELIDIA_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ELIDIA_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ELIDIA_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ELIDIA_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.elidia/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("ELIDIA_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("ELIDIA_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_elidia_credentials(self, tmp_path, monkeypatch):
        """~/.elidia/.env and ~/.elidia/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        elidia_dir = fake_home / ".elidia"
        elidia_dir.mkdir(parents=True)
        env_file = elidia_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._ELIDIA_HOME",
            elidia_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(env_file)) is None

    def test_denylist_blocks_elidia_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        elidia_dir = fake_home / ".elidia"
        elidia_dir.mkdir(parents=True)
        config_file = elidia_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_elidia_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Elidia root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".elidia" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        elidia_root = fake_home / ".elidia"
        config_file = elidia_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ELIDIA_ROOT",
            elidia_root,
        )
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting ELIDIA_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``ELIDIA_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("ELIDIA_MEDIA_DELIVERY_STRICT", raw)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"ELIDIA_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELIDIA_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"ELIDIA_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "ELIDIA_HUMAN_DELAY_MODE": "natural",
            "ELIDIA_HUMAN_DELAY_MIN_MS": "oops",
            "ELIDIA_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "ELIDIA_HUMAN_DELAY_MODE": "custom",
            "ELIDIA_HUMAN_DELAY_MIN_MS": "100",
            "ELIDIA_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"ELIDIA_MEDIA_DELIVERY_STRICT": "1",
                                     "ELIDIA_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELIDIA_MEDIA_DELIVERY_STRICT", "0")
