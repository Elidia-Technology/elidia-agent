...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "elidia_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_elidia_home() / config.yaml`` — point it here.
    monkeypatch.setattr("elidia_constants.get_elidia_home", lambda: elidia_home)
    monkeypatch.setattr("elidia_cli.config.get_elidia_home", lambda: elidia_home)
    return cfg_path
