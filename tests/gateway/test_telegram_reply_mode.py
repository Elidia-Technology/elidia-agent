...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        elidia_home.mkdir()
        (elidia_home / "config.yaml").write_text(content, encoding="utf-8")
        return elidia_home
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        elidia_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        elidia_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("ELIDIA_HOME", str(elidia_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/Elidia-Technology/elidia-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Elidia-created DM topic
