...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.elidia."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /elidia, /btw, /stop,
        ...
        for expected in ("/elidia", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@elidia`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@elidia"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@elidia")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /elidia subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /elidia btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_elidia_prefix_still_works(self, adapter):
        """Backward compat: /elidia btw foo must still route to /btw foo.

        Old workspace manifests only declared /elidia as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_elidia_freeform_question(self, adapter):
        """/elidia <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/elidia",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_elidia_slash_stashes_context(self, adapter):
        """Legacy /elidia <subcommand> also stashes context."""
        command = {
            "command": "/elidia",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_elidia_question_does_not_stash_context(self, adapter):
        """Free-form /elidia <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/elidia",
