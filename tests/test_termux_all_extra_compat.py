...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"elidia-agent-cli[termux]"' in text
    assert '"elidia-agent-cli[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"elidia-agent-cli[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
