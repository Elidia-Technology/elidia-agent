...
class TestToolsetConsistency:
    ...
    def test_elidia_platforms_share_core_tools(self):
        """All elidia-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on elidia-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["elidia-cli", "elidia-telegram", "elidia-discord", "elidia-whatsapp", "elidia-slack", "elidia-signal", "elidia-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_elidia_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("elidia-whatsapp")
