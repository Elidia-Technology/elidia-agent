# ============================================================================
# Elidia Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/Elidia-Technology/elidia-agent-cli-v2/master/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ElidiaHome = $(if ($env:ELIDIA_HOME) { $env:ELIDIA_HOME } else { "$env:LOCALAPPDATA\elidia" }),
    [string]$InstallDir = $(if ($env:ELIDIA_HOME) { "$env:ELIDIA_HOME\elidia-agent" } else { "$env:LOCALAPPDATA\elidia\elidia-agent" }),
    ...
    # builds apps/desktop into a launchable Elidia.exe.
    #
    # Why opt-in:
    #   * Elidia-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Elidia.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Elidia.exe
    ...
    #     `elidia desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:Elidia-Technology/elidia-agent-cli-v2.git"
$RepoUrlHttps = "https://github.com/Elidia-Technology/elidia-agent-cli-v2.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Elidia Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by AiUtils.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ElidiaHome)) {
        New-Item -ItemType Directory -Force -Path $ElidiaHome | Out-Null
    }
    $envFile = Join-Path $ElidiaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ElidiaHome "node"
...
function Install-Uv {
    # Elidia owns its own uv at $ElidiaHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (elidia_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `elidia update` stay in sync.
    $managedUv = Join-Path $ElidiaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ElidiaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ElidiaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ElidiaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ElidiaHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ELIDIA_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Elidia uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\elidia\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Elidia needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Elidia copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\elidia\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ELIDIA_GIT_BASH_PATH`` (User scope) so Elidia can find it in a fresh
    ...
    # Download PortableGit into $ElidiaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ElidiaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Elidia features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Elidia needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ELIDIA_GIT_BASH_PATH`` (User env scope) so Elidia can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ElidiaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ElidiaHome\git\usr\bin\bash.exe
    $candidates += "$ElidiaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ElidiaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ELIDIA_GIT_BASH_PATH", $candidate, "User")
            $env:ELIDIA_GIT_BASH_PATH = $candidate
            Write-Info "Set ELIDIA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Elidia may not find Git Bash."
    Write-Info "If needed, set ELIDIA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ElidiaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ElidiaHome\node;$env:Path"
        Write-Success "Node.js $version found (Elidia-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Elidia-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ElidiaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ElidiaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\elidia-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ElidiaHome\node") { Remove-Item -Recurse -Force "$ElidiaHome\node" }
                Move-Item $extractedDir.FullName "$ElidiaHome\node"
                ...
                $version = & "$ElidiaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ElidiaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\elidia-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running elidia processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Elidia-Technology/elidia-agent-cli-v2/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Elidia-Technology/elidia-agent-cli-v2/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Elidia-Technology/elidia-agent-cli-v2/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\elidia-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\elidia-agent-extract"
    ...
    # next `elidia update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `elidia.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install elidia-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and elidia.exe broken with
    ...
    # here, not 30 seconds later when the user runs `elidia`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `elidia dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `elidia dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `elidia dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up elidia command..."
    
    if ($NoVenv) {
        $elidiaBin = "$InstallDir"
    } else {
        $elidiaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so elidia is globally available
    # On Windows, the elidia.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$elidiaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$elidiaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $elidiaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ELIDIA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\elidia instead
    # of the Unix default ~/.elidia
    $currentElidiaHome = [Environment]::GetEnvironmentVariable("ELIDIA_HOME", "User")
    if (-not $currentElidiaHome -or $currentElidiaHome -ne $ElidiaHome) {
        [Environment]::SetEnvironmentVariable("ELIDIA_HOME", $ElidiaHome, "User")
        Write-Success "Set ELIDIA_HOME=$ElidiaHome"
    }
    $env:ELIDIA_HOME = $ElidiaHome
    
    # Update current session
    $env:Path = "$elidiaBin;$env:Path"
    
    Write-Success "elidia command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.elidia-bootstrap-complete which tells the Elidia
    ...
    # Elidia-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `elidia update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".elidia-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.elidia directory structure
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ElidiaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ElidiaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.elidia/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.elidia/.env"
        }
    } else {
        Write-Info "~/.elidia/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ElidiaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.elidia/config.yaml from template"
        }
    } else {
        Write-Info "~/.elidia/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Elidia's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.elidia/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.elidia/"
    
    # Seed bundled skills into ~/.elidia/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.elidia/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.elidia/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ElidiaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.elidia/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Elidia-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'elidia setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\elidia-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\elidia-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\elidia-npm-tui-$(Get-Random).log"
        [void](_Run-NpmInstall "TUI" $tuiDir $tuiLog $npmExe)
    }
}

function Install-Desktop {
    # Build apps/desktop into a launchable Elidia.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_elidia_desktop command
    # resolves apps/desktop/release/win-unpacked/Elidia.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Elidia-managed
    ...
    # The --dir mode produces an unpacked Elidia.exe in
    ...
    # are NOT trying to work around). The Elidia icon + product name are
    # stamped onto Elidia.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\elidia-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Elidia.exe",
        "$desktopDir\release\win-arm64-unpacked\Elidia.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Elidia.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Elidia icon + identity are stamped onto Elidia.exe by the
    ...
    #    Elidia.exe. We deliberately do NOT point them at `elidia desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Elidia.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Elidia.lnk')
        )
        ...
        # with the Elidia icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.elidia/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'elidia setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m elidia_cli.main setup
    } else {
        python -m elidia_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $elidiaCmd = "$InstallDir\venv\Scripts\elidia.exe"
    if (-not (Test-Path $elidiaCmd)) {
        $elidiaCmd = "elidia"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ElidiaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'elidia whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: elidia gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ElidiaHome\logs\gateway.log"
            Start-Process -FilePath $elidiaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ElidiaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: elidia gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: elidia gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ElidiaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ElidiaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ElidiaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ElidiaHome\elidia-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   elidia              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   elidia setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   elidia config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   elidia config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   elidia gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   elidia update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Elidia repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Elidia-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Elidia to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Elidia-Technology/elidia-agent-cli-v2/master/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
