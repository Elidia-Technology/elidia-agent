#!/usr/bin/env python3
"""Drive the Elidia TUI under ELIDIA_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.elidia/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ELIDIA_TUI_RESUME set so it
bypasses the elidia_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ELIDIA_PERF_LOG     (default ~/.elidia/perf.log)
  ELIDIA_PERF_NODE    (default node from $PATH)
  ELIDIA_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ELIDIA_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_elidia_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("ELIDIA_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".elidia"

DEFAULT_TUI_DIR = Path(
    os.environ.get("ELIDIA_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("ELIDIA_PERF_LOG", str(get_elidia_home() / "perf.log")))
DEFAULT_STATE_DB = get_elidia_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — ELIDIA_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["ELIDIA_DEV_PERF"] = "1"
    env["ELIDIA_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ELIDIA_DEV_PERF_LOG"] = str(log)
    env["ELIDIA_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ELIDIA_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ELIDIA_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "elidia-ink" / "src"
