...
# Bootstrap Open WebUI against Elidia Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.elidia/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-elidia.sh
...
#   ELIDIA_API_PORT=8642
#   ELIDIA_API_HOST=127.0.0.1
#   ELIDIA_API_MODEL_NAME='Elidia Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Elidia Agent WebUI}"
...
ELIDIA_ENV_FILE="${ELIDIA_ENV_FILE:-$HOME/.elidia/.env}"
ELIDIA_API_PORT="${ELIDIA_API_PORT:-8642}"
ELIDIA_API_HOST="${ELIDIA_API_HOST:-127.0.0.1}"
ELIDIA_API_CONNECT_HOST="${ELIDIA_API_CONNECT_HOST:-127.0.0.1}"
ELIDIA_API_MODEL_NAME="${ELIDIA_API_MODEL_NAME:-Elidia Agent}"
ELIDIA_API_BASE_URL="http://${ELIDIA_API_CONNECT_HOST}:${ELIDIA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-elidia.sh"
LOG_DIR="$HOME/.elidia/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ELIDIA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ELIDIA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.elidia.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.elidia</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.elidia"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.elidia"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-elidia.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Elidia Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-elidia.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.elidia/logs/openwebui.log
StandardError=append:%h/.elidia/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-elidia.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ELIDIA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Elidia API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ELIDIA_ENV_FILE"
  upsert_env API_SERVER_HOST "$ELIDIA_API_HOST" "$ELIDIA_ENV_FILE"
  upsert_env API_SERVER_PORT "$ELIDIA_API_PORT" "$ELIDIA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ELIDIA_API_MODEL_NAME" "$ELIDIA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ELIDIA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Elidia gateway so API server settings take effect...'
  elidia gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ELIDIA_API_CONNECT_HOST}:${ELIDIA_API_PORT}/health" >/dev/null; then
    log 'Elidia API server did not answer on the first check. Trying to start gateway in the background...'
    nohup elidia gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ELIDIA_API_CONNECT_HOST}:${ELIDIA_API_PORT}/health" >/dev/null
  ...
  log "Elidia API endpoint: ${ELIDIA_API_BASE_URL}"
