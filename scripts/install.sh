#!/bin/bash
# ============================================================================
# Elidia Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Elidia-Technology/elidia-agent-cli-v2/master/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Elidia terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Elidia-Technology/elidia-agent-cli-v2.git"
REPO_URL_HTTPS="https://github.com/Elidia-Technology/elidia-agent-cli-v2.git"
ELIDIA_HOME="${ELIDIA_HOME:-$HOME/.elidia}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ELIDIA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ELIDIA_INSTALL_DIR"
...
#   code at /usr/local/lib/elidia-agent, command at /usr/local/bin/elidia,
#   data still at /root/.elidia (ELIDIA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ELIDIA_HOME/.no-bundled-skills so future"
            echo "                   'elidia update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Elidia.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.elidia/elidia-agent"
            echo "                   default (root, Linux): /usr/local/lib/elidia-agent"
            echo "  --elidia-home PATH  Data directory (default: ~/.elidia, or \$ELIDIA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Elidia installs the code under"
            echo "  /usr/local/lib/elidia-agent and links the command into"
            echo "  /usr/local/bin/elidia (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ELIDIA_HOME"
            echo "  (default /root/.elidia).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ELIDIA_HOME/elidia-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs elidia setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ✦ Elidia Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by AiUtils.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `elidia update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Elidia-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Elidia Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install elidia command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `elidia` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ELIDIA_HOME/elidia-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ELIDIA_HOME/elidia-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/elidia-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ELIDIA_HOME/elidia-agent — then preserve it)
#
# Always no-op when the user set --dir or $ELIDIA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ELIDIA_HOME.
    if is_termux; then
        INSTALL_DIR="$ELIDIA_HOME/elidia-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ELIDIA_HOME/elidia-agent/.git" ]; then
            INSTALL_DIR="$ELIDIA_HOME/elidia-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/elidia-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/elidia-agent"
        ...
        # /usr/local/bin/elidia wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/elidia"
        log_info "  Data:    $ELIDIA_HOME (unchanged)"
...
get_elidia_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/elidia" ]; then
        echo "$link_dir/elidia"
    else
        echo "elidia"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Elidia-Technology/elidia-agent-cli-v2/master/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Elidia owns its own uv at $ELIDIA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (elidia_cli/managed_uv.py) looks in the same
    # place, so install.sh and `elidia update` stay in sync.
    local _managed_uv="$ELIDIA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ELIDIA_HOME/bin ..."
    mkdir -p "$ELIDIA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/elidia-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/elidia-uv-installer.$$.sh")"
    ...
    # directly into $ELIDIA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ELIDIA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Elidia-
...
check_node() {
    ...
    # Prefer an Elidia-managed Node from a previous run over a too-old system one.
    if [ -x "$ELIDIA_HOME/node/bin/node" ] && node_satisfies_build "$("$ELIDIA_HOME/node/bin/node" --version)"; then
        export PATH="$ELIDIA_HOME/node/bin:$PATH"
        log_success "Node.js $("$ELIDIA_HOME/node/bin/node" --version) found (Elidia-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Elidia-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.elidia/node/..."
    ...
    # Place into ~/.elidia/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ELIDIA_HOME/node"
    mkdir -p "$ELIDIA_HOME"
    mv "$extracted_dir" "$ELIDIA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ELIDIA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ELIDIA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ELIDIA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ELIDIA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ELIDIA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.elidia/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Elidia install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Elidia Agent itself does not require or retain root access."
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # update path. Fork users customize via `elidia update`, which keeps
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"elidia-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up elidia command..."

    if [ "$USE_VENV" = true ]; then
        ELIDIA_BIN="$INSTALL_DIR/venv/bin/elidia"
    else
        ELIDIA_BIN="$(which elidia 2>/dev/null || echo "")"
        if [ -z "$ELIDIA_BIN" ]; then
            log_warn "elidia not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$ELIDIA_BIN" ]; then
        log_warn "elidia entry point not found at $ELIDIA_BIN"
    ...
    # Create a user-facing shim for the elidia command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ELIDIA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ELIDIA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/elidia"
    cat > "$command_link_dir/elidia" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ELIDIA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/elidia"
    log_success "Installed elidia launcher → $command_link_display_dir/elidia"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "elidia command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v elidia' \
                >/dev/null 2>&1; then
        ...
        log_info "elidia not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Elidia Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Elidia Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Elidia Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so elidia works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.elidia directory structure (config at top level, code in subdir)
    mkdir -p "$ELIDIA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.elidia/.env (top level, easy to find)
    if [ ! -f "$ELIDIA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ELIDIA_HOME/.env"
            log_success "Created ~/.elidia/.env from template"
        else
            touch "$ELIDIA_HOME/.env"
            log_success "Created ~/.elidia/.env"
        fi
    else
        log_info "~/.elidia/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ELIDIA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.elidia/config.yaml (top level, easy to find)
    if [ ! -f "$ELIDIA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ELIDIA_HOME/config.yaml"
            log_success "Created ~/.elidia/config.yaml from template"
        fi
    else
        log_info "~/.elidia/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ELIDIA_HOME/SOUL.md" ]; then
        cat > "$ELIDIA_HOME/SOUL.md" << 'SOUL_EOF'
# Elidia Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Elidia communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.elidia/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.elidia/"

    # Seed bundled skills into ~/.elidia/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `elidia update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'elidia update'." \
            > "$ELIDIA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ELIDIA_HOME/.no-bundled-skills"
        log_info "  Future 'elidia update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.elidia/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.elidia/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ELIDIA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ELIDIA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.elidia/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$ELIDIA_HOME/.env"
    ...
    {
        echo ""
        echo "# Elidia Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Elidia will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (elidia --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `elidia update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'elidia setup' after install."
        return 0
    fi
    ...
    # Run elidia setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m elidia_cli.main setup < /dev/tty
    else
        python -m elidia_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ELIDIA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Elidia to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ELIDIA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'elidia whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ELIDIA_CMD="$(get_elidia_command_path)"
                $ELIDIA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'elidia whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'elidia gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ELIDIA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ELIDIA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: elidia gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: elidia gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ELIDIA_CMD gateway > "$ELIDIA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.elidia/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: elidia gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: elidia gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ELIDIA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ELIDIA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ELIDIA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}elidia${NC}              Start chatting"
    echo -e "   ${GREEN}elidia setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}elidia config${NC}       View/edit configuration"
    echo -e "   ${GREEN}elidia config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}elidia gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}elidia update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'elidia' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'elidia' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'elidia' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ELIDIA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ELIDIA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ELIDIA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ELIDIA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ELIDIA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Elidia for pip install"
    ...
    ELIDIA_CMD="$(command -v elidia 2>/dev/null || echo "")"
    if [ -n "$ELIDIA_CMD" ]; then
        log_info "Running elidia setup..."
        "$ELIDIA_CMD" setup
    else
        log_warn "elidia command not found on PATH"
        log_info "Try: python -m elidia_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Elidia-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Elidia" ]; then
            app="$desktop_dir/release/linux-unpacked/Elidia"
        elif [ -x "$desktop_dir/release/linux-unpacked/elidia" ]; then
            app="$desktop_dir/release/linux-unpacked/elidia"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Elidia.app" \
            "$desktop_dir/release/mac/Elidia.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Elidia is damaged and can't be opened").
    ...
    # checkout stays clean for the next `elidia update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Elidia-managed Node
            # provisioned during prerequisites/node-deps (at $ELIDIA_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$ELIDIA_HOME/.install_method"
            ;;
        *)
