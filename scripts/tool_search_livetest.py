#!/usr/bin/env python3
"""Live test harness for Elidia Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any elidia imports.
ORIGINAL_HOME = os.environ.get("ELIDIA_HOME")
ORIGINAL_AUTH = Path.home() / ".elidia" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.elidia/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.elidia/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="elidia_ts_live_"))
    elidia_home = home_dir / ".elidia"
    elidia_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, elidia_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".elidia" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, elidia_home / ".env")
        ...
        load_elidia_dotenv(elidia_home=str(Path.home() / ".elidia"))
    ...
    (elidia_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return elidia_home
...
def reset_module_state():
    """Drop cached modules so the new ELIDIA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "elidia_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ELIDIA_HOME"] = str(home)
...
def main():
    ...
    # Restore original ELIDIA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ELIDIA_HOME"] = ORIGINAL_HOME
    else:
