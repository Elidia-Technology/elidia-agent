...
## 5. Elidia Configuration

### Config location

`$ELIDIA_HOME/config.yaml` (defaults to `~/.elidia/config.yaml` when `ELIDIA_HOME` is unset)
...
Restart the Elidia session for changes to take effect. The MCP connection is
