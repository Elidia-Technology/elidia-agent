#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Elidia Agent.
...
# Ensure sibling modules (_elidia_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _elidia_home import display_elidia_home, get_elidia_home

ELIDIA_HOME = get_elidia_home()
TOKEN_PATH = ELIDIA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ELIDIA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ELIDIA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Elidia profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'elidia-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Elidia home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_elidia_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Elidia")
