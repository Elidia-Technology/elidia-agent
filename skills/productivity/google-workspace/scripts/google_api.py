#!/usr/bin/env python3
"""Google Workspace API CLI for Elidia Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Elidia-facing JSON contract and falls back to the Python client
...
from _elidia_home import get_elidia_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ELIDIA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Elidia Agent")
