...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ELIDIA_HOME", os.path.expanduser("~/.elidia")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ELIDIA_HOME", Path.home() / ".elidia")) / "skills" / "red-teaming" / "godmode"
...
ELIDIA_HOME = Path(os.getenv("ELIDIA_HOME", Path.home() / ".elidia"))
CONFIG_PATH = ELIDIA_HOME / "config.yaml"
PREFILL_PATH = ELIDIA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Elidia/Elidia models — already uncensored, just needs clean prompt
    "elidia": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "elidia" in model_lower or "elidia" in model_lower:
        return "elidia"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Elidia config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.elidia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Elidia for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Elidia for changes to take effect.")
