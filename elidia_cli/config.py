"""
Configuration management for Elidia Agent.

Config files are stored in ~/.elidia/ for easy access:
- ~/.elidia/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.elidia/.env         - API keys and secrets

This module provides:
- elidia config          - Show current configuration
- elidia config edit     - Open config in editor
- elidia config set      - Set a specific value
- elidia config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.elidia/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``elidia logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  elidia config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Elidia makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Elidia itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``elidia update``, the TUI build.
...
#   on every plugin install / ``elidia update``.
...
# * ``ELIDIA_HOME`` / ``ELIDIA_PROFILE`` / ``ELIDIA_CONFIG`` /
#   ``ELIDIA_ENV`` — Elidia runtime location flags. Writing these into
...
#
# IMPORTANT: ``ELIDIA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ELIDIA_GEMINI_CLIENT_ID,
# ELIDIA_LANGFUSE_PUBLIC_KEY, ELIDIA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Elidia runtime location — never via dashboard env writer.
    # NOT an ELIDIA_* blanket: integration credentials (ELIDIA_GEMINI_*,
    # ELIDIA_LANGFUSE_*, ELIDIA_SPOTIFY_*, ...) ARE allowed.
    "ELIDIA_HOME", "ELIDIA_PROFILE", "ELIDIA_CONFIG", "ELIDIA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Elidia runtime location "
            "(ELIDIA_HOME, ELIDIA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `elidia plugins enable
    # observability/langfuse` or `elidia tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ELIDIA_LANGFUSE_ENV",
    "ELIDIA_LANGFUSE_RELEASE",
    "ELIDIA_LANGFUSE_SAMPLE_RATE",
    "ELIDIA_LANGFUSE_MAX_CHARS",
    "ELIDIA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from elidia_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ELIDIA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_elidia_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Elidia is running in package-manager-managed mode.

    Two signals: the ELIDIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ELIDIA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade elidia-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Elidia was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.elidia/.install_method`` file (written by installers)
    2. ELIDIA_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``aiutils/elidia-agent`` image stamps ``docker``
    ...
    so ``elidia update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_elidia_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.elidia/.install_method."""
    stamp = get_elidia_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Elidia lives in a ``uv tool`` layout.

    ``uv tool install elidia-agent`` places the install at
    ``.../uv/tools/elidia-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``elidia-agent`` happens to be uv-tool-installed on the machine while
    the *active* Elidia is a regular pip/venv install, causing
    ``elidia update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/elidia-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade elidia-agent"
    if method == "docker":
        return "docker pull aiutils/elidia-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade elidia-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade elidia-agent"
        return "pip install --upgrade elidia-agent"
    return "elidia update"
...
# Long-form text for ``elidia update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# elidia_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Elidia, it doesn't update
...
✗ ``elidia update`` doesn't apply inside the Docker container.

Elidia Agent runs as a published image (aiutils/elidia-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull aiutils/elidia-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate elidia-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm aiutils/elidia-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/aiutils/elidia-agent/tags
  • Your config and session history live under ``$ELIDIA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``elidia update`` inside Docker.
...
def format_managed_message(action: str = "modify this Elidia installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ELIDIA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Elidia installation is managed by NixOS "
            f"(ELIDIA_MANAGED={env_hint}).\n"
            "Edit services.elidia-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Elidia installation is managed by Homebrew "
            f"(ELIDIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade elidia-agent"
        )

    return (
        f"Cannot {action}: this Elidia installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Elidia."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ELIDIA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, elidia_bin
    or None if container mode is not active, we're already inside the
    container, or ELIDIA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ELIDIA_DEV") == "1":
        return None

    from elidia_constants import is_container
    if is_container():
        return None

    container_mode_file = get_elidia_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "elidia-agent")
    exec_user = info.get("exec_user", "elidia")
    elidia_bin = info.get("elidia_bin", "/data/current-package/bin/elidia")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "elidia_bin": elidia_bin,
    }
...
# Re-export from elidia_constants — canonical definition lives there.
from elidia_constants import get_elidia_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_elidia_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_elidia_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_elidia_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ELIDIA_UID / ELIDIA_GID env vars set by Docker deployments.

    Docker containers running Elidia commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ELIDIA_HOME once, but
    subdirectories created at runtime by ``ensure_elidia_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ELIDIA_UID", "").strip()
    gid_str = os.environ.get("ELIDIA_GID", "").strip()
    try:
...
def _chown_to_elidia_uid(path) -> None:
    """Chown ``path`` to ``ELIDIA_UID:ELIDIA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_elidia_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_elidia_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the elidia group can
    share state with the gateway service.

    The mode can be overridden via the ELIDIA_HOME_MODE environment variable
    (e.g. ELIDIA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ELIDIA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ELIDIA_UID``/``ELIDIA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ELIDIA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_elidia_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Elidia runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ELIDIA_CONTAINER") or os.environ.get("ELIDIA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ELIDIA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ELIDIA_HOME if the user doesn't have one yet."""
...
def ensure_elidia_home():
    """Ensure ~/.elidia directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_elidia_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_elidia_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ELIDIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Elidia-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Elidia
        ...
        # identity slot (SOUL.md). Empty by default. The ELIDIA_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Elidia auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Elidia sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Elidia
        ...
        # bundled Elidia image, which drops to the `elidia` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Elidia sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Elidia should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``elidia chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.elidia/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: elidia sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Elidia truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | elidia | codex | custom
        ...
        # Invoked by ``elidia kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``elidia kanban decompose`` and the kanban
        ...
        # ``elidia profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `elidia model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `elidia` (and `elidia chat`) launches by default:
        ...
        # REPL and `--tui` (or ELIDIA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `elidia --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `elidia -c` muscle memory.  Default off so existing
        # users aren't surprised.  ELIDIA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `elidia --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/elidia`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Elidia Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``ELIDIA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ELIDIA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://developer.aiutils.io).
        "oauth": {
            "client_id": "",  # agent:{instance_id} — Portal provisions this
            "portal_url": "",  # blank → use plugin default (production Portal)
        },
        # Public URL override (env: ``ELIDIA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/elidia``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.elidia/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.elidia/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Elidia, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Elidia feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Elidia auto-pauses the goal and
    ...
    # always goes to ~/.elidia/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ELIDIA_SKILL_DIR} and ${ELIDIA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `elidia curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `elidia update`, so pruning them only
        ...
        # skipped), snapshot ~/.elidia/skills/ into
        # ~/.elidia/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `elidia curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for elidia-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "✦ *Elidia Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ELIDIA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.elidia/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ELIDIA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `elidia doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``elidia_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Elidia to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ELIDIA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `elidia -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `elidia` launches with
        ...
        # decomposition is manual via `elidia kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     elidia-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Elidia tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.elidia/logs/.
    ...
    # curated model lists for OpenRouter and Elidia Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping an elidia-agent release.
    # The default URL is the aiutils.io-hosted manifest.
    "model_catalog": {
        "enabled": True,
        "url": "https://share.aiutils.io/elidia-hermes/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `elidia model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.elidia/.env,
        ...
        # behavior: files must live under the Elidia cache, under
        ...
        # user. Bridged to ELIDIA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Elidia
        # cache (~/.elidia/cache/{documents,images,audio,video,screenshots})
        ...
        # to ELIDIA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ELIDIA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ELIDIA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.elidia/state.db.
    ...
    "sessions": {
        ...
        # default of ``elidia sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.elidia/sessions/session_{sid}.json`` on every turn
    ...
    # ``elidia update`` behaviour.
    "updates": {
        # Run a full ``elidia backup``-style zip of ELIDIA_HOME before every
        # ``elidia update``.  Backups land in ``<ELIDIA_HOME>/backups/`` and
        # can be restored with ``elidia import <path>``.  Off by default —
        # on large ELIDIA_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ELIDIA_HOME>/lsp/bin/`` on first use.
    ...
    # `elidia tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.elidia/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.elidia/.env (or your shell) and never in config.yaml.
            ...
            # ~/.elidia/bin/ on first use.  When False you must install
            ...
            # `elidia secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (Elidia Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "ELIDIA_BASE_URL": {
        "description": "Elidia Portal base URL override",
        "prompt": "Elidia Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'elidia model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Elidia Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Elidia Subscribers only, used to derive vendor hosts, e.g. aiutils.io -> firecrawl-gateway.aiutils.io",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Elidia Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Elidia Subscriber access token for tool-gateway requests (optional; otherwise read from the Elidia auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @elidia:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ELIDIA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #elidia)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: elidia-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'elidia-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Elidia API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Elidia API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Elidia API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ELIDIA_TOOL_PROGRESS and ELIDIA_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ELIDIA_TOOL_PROGRESS": {
    ...
    "ELIDIA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.elidia.config`` entries, then checks
    ...
    try:
        ...
        # should never break `elidia update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Elidia versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``elidia_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'elidia setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Elidia won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'elidia doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ELIDIA_HOME", "~/.elidia")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ELIDIA_TOOL_PROGRESS")
            old_mode = get_env_value("ELIDIA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ELIDIA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ELIDIA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ELIDIA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ELIDIA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ELIDIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_elidia_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `elidia plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `elidia curator status` has no
    ...
    #   3. Creates `~/.elidia/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_elidia_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_elidia_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: elidia config set <key> <value>")
    ...
    # metadata.elidia.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _elidia_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.elidia/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.elidia/config.yaml.
    ...
    (which change ``ELIDIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — elidia auth) — OpenAI Codex
#   elidia         (OAuth — elidia auth) — Elidia Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.elidia/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.elidia/.env.
    ...
    interactive menu render (`elidia tools`, `elidia setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `elidia tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Elidia env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.elidia/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.elidia/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.elidia/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.elidia/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Elidia — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Elidia vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.elidia/.env or environment."""
...
def show_config():
    ...
    print(color("│              ✦ Elidia Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale ELIDIA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ELIDIA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ELIDIA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'elidia doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  elidia config edit     # Edit config file", Colors.DIM))
    print(color("  elidia config set <key> <value>", Colors.DIM))
    print(color("  elidia setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: elidia config set <key> <value>")
            print()
            print("Examples:")
            print("  elidia config set model anthropic/claude-sonnet-4")
            print("  elidia config set terminal.backend docker")
            print("  elidia config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'elidia config migrate' to add them")
        ...
        print("  elidia config           Show current configuration")
        print("  elidia config edit      Open config in editor")
        print("  elidia config set <key> <value>   Set a config value")
        print("  elidia config check     Check for missing/outdated config")
        print("  elidia config migrate   Update config with new options")
        print("  elidia config path      Show config file path")
        print("  elidia config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``elidia config`` UI so users
