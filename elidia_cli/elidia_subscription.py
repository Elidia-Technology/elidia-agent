"""Helpers for Elidia subscription managed-tool capabilities."""
...
from elidia_cli.config import get_env_value, load_config
from elidia_cli.elidia_account import (
    ElidiaPortalAccountInfo,
...
# Maps a tools_config provider's ``managed_elidia_feature`` to the tool-pool
# coverage category (elidia_cli.elidia_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `elidia tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_elidia_subscription_features.
...
class ElidiaFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_elidia: bool
...
@dataclass(frozen=True)
class ElidiaSubscriptionFeatures:
    subscribed: bool
    elidia_auth_present: bool
    provider_is_elidia: bool
    features: Dict[str, ElidiaFeatureState]
    account_info: Optional[ElidiaPortalAccountInfo] = None

    @property
    def web(self) -> ElidiaFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> ElidiaFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> ElidiaFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> ElidiaFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> ElidiaFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> ElidiaFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[ElidiaFeatureState]:
...
def get_elidia_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> ElidiaSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_elidia = str(model_cfg.get("provider") or "").strip().lower() == "elidia"

    try:
        if force_fresh:
            account_info = get_elidia_portal_account_info(force_fresh=True)
        else:
            account_info = get_elidia_portal_account_info()
    except Exception:
        account_info = None
    ...
    elidia_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_elidia or elidia_auth_present
    ...
    # Tool Gateway via `elidia model`, so direct credentials should NOT
    ...
    features = {
        "web": ElidiaFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_elidia=web_managed,
        ...
        "image_gen": ElidiaFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_elidia=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("Elidia Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": ElidiaFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_elidia=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if direct_fal_video else ("Elidia Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": ElidiaFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_elidia=tts_managed,
        ...
        "browser": ElidiaFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_elidia=browser_managed,
        ...
        "modal": ElidiaFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_elidia=modal_managed,
    ...
    return ElidiaSubscriptionFeatures(
        subscribed=subscribed,
        elidia_auth_present=elidia_auth_present,
        provider_is_elidia=provider_is_elidia,
        features=features,
...
def apply_elidia_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_elidia:
        return set()
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Elidia subscriber or
    is not using Elidia as their provider.
    """
    ...
    # Quick provider check without the heavy get_elidia_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "elidia":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_elidia which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from elidia_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_elidia_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Elidia subscription"
    ...
    if pool_only:
        title = "Your free Elidia tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Elidia subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Elidia Portal login for the Tool Gateway picker (`elidia tools`)
# ---------------------------------------------------------------------------


def ensure_elidia_portal_access(
    *,
    capability: str = "the Elidia Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Elidia Tool Gateway, logging in if
    needed.

    Used by ``elidia tools`` when a user selects an Elidia-managed Tool Gateway
    backend (e.g. "Firecrawl (Elidia Portal)").  Unlike ``elidia model``'s Elidia
    login, this:
    ...
    It only performs the Elidia Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_elidia_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_elidia_portal_login_only(capability=capability):
            return False
        try:
            info = get_elidia_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_elidia_portal_login_only(*, capability: str) -> bool:
    """Run the Elidia Portal device-code OAuth and persist credentials only.
    ...
    try:
        from elidia_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _elidia_device_code_login,
            _read_shared_elidia_state,
            _save_auth_store,
            _save_provider_state,
            _sync_elidia_pool_from_auth_store,
            _try_import_shared_elidia_state,
            _write_shared_elidia_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Elidia Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires an Elidia Portal login.")
    try:
        proceed = input("  Log in to Elidia Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Elidia Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Elidia.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Elidia OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_elidia_state(timeout_seconds=15.0)
        ...
        _sync_elidia_pool_from_auth_store()
        print("  Elidia Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _elidia_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Elidia Portal login failed: {exc}")
        return False
