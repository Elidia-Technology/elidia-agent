...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from elidia_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from elidia_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Elidia' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from elidia_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from elidia_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("ELIDIA_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``elidia``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/elidia_cli/test_runtime_provider_resolution.py
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'elidia model' or "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "elidia":
        state = auth_mod.get_provider_auth_state("elidia") or {}
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_ELIDIA_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_elidia_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, int(os.getenv("ELIDIA_AIUTILS_MIN_KEY_TTL_SECONDS", "1800"))),
            )
            else ""
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For Elidia, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `elidia auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_elidia_runtime_credentials() which handles refresh.
        if provider == "elidia" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("ELIDIA_AIUTILS_MIN_KEY_TTL_SECONDS", "1800")))
            elidia_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(elidia_state, min_ttl):
                logger.debug("Elidia pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "elidia":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Elidia but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Elidia provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "elidia-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from elidia_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Elidia canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Elidia-compatible importers).
