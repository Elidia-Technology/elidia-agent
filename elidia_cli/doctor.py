"""
Doctor command for elidia CLI.

Diagnoses issues with Elidia Agent setup.
"""
...
from elidia_cli.config import get_project_root, get_elidia_home, get_env_path
...
_DHH = display_elidia_home()  # user-facing display path (e.g. ~/.elidia or ~/.elidia/profiles/coder)

# Load environment variables from ~/.elidia/.env so API key checks work
_env_path = get_env_path()
load_elidia_dotenv(elidia_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from elidia_cli.colors import Colors, color
from elidia_cli.models import _ELIDIA_USER_AGENT
from elidia_constants import OPENROUTER_MODELS_URL
...
from elidia_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.elidia/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ELIDIA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from elidia_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from elidia_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches elidia_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``elidia --version`` reporting a stale version while
    ...
    """
    try:
        from elidia_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != elidia_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'elidia update', or set "
            "elidia_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-elidia and dashboard static services are up
    ...
    for static in ("main-elidia", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `elidia profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from elidia_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `elidia`.
    os.environ.setdefault("ELIDIA_INTERACTIVE", "1")

    # Handle `elidia doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.elidia/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Elidia Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`elidia doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and elidia_cli/__init__.py versions
    ...
    # Check ~/.elidia/.env (primary location for user config)
    env_path = ELIDIA_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'elidia setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'elidia setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'elidia setup' to create one")
                issues.append("Run 'elidia setup' to create .env")
    
    # Check ~/.elidia/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from elidia_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from elidia_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'elidia config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from elidia_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.elidia/.env or run 'elidia setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'elidia setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'elidia config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from elidia_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from elidia_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'elidia setup' to migrate config")
                else:
                    issues.append("Run 'elidia doctor --fix' or 'elidia setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'elidia doctor --fix'")
        except Exception:
            pass

        # Detect stale ELIDIA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ELIDIA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from elidia_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("ELIDIA_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ELIDIA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `elidia setup` run)",
                )
                if should_fix:
                    if remove_env_value("ELIDIA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ELIDIA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ELIDIA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ELIDIA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ELIDIA_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'elidia doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        from elidia_cli.auth import (
            get_elidia_auth_status,
            get_codex_auth_status,
            get_gemini_oauth_auth_status,
            get_minimax_oauth_auth_status,
        )

        elidia_status = get_elidia_auth_status()
        if elidia_status.get("logged_in"):
            check_ok("Elidia Portal auth", "(logged in)")
        else:
            check_warn("Elidia Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Elidia' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed Elidia/Codex/Gemini/MiniMax rows above.
    try:
        from elidia_cli.auth import get_xai_oauth_auth_status
    ...
    elidia_home = ELIDIA_HOME
    if elidia_home.exists():
    ...
    for subdir_name in expected_subdirs:
        subdir_path = elidia_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Elidia a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Elidia Agent Persona\n\n"
                "<!-- Edit this file to customize how Elidia communicates. -->\n\n"
                "You are Elidia, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = elidia_home / "memories"
    ...
    state_db_path = elidia_home / "state.db"
    ...
    wal_path = elidia_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'elidia doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "elidia"
        ...
        _cmd_link = _cmd_link_dir / "elidia"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(elidia not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/elidia → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/elidia points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/elidia → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/elidia — run 'elidia doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/elidia exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/elidia not found",
                    "(elidia command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/elidia → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/elidia symlink — run 'elidia doctor --fix'")
    ...
    try:
        from elidia_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `elidia doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'elidia config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``elidia doctor``.
    ...
    # never the legitimate source for `elidia doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'elidia setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ELIDIA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: elidia skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ELIDIA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: elidia memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: elidia memory setup",
                    "No Honcho API key — run 'elidia memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run elidia memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: elidia memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: elidia memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from elidia_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "elidia -p" in content:
                            _m = _re.search(r"elidia -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'elidia doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
