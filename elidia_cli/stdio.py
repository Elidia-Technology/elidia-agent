...
on Japanese locales, etc.).  Elidia's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``elidia_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Elidia is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ELIDIA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ELIDIA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Elidia-managed Git install directories so
    ...
    # already-running shells don't see the change — which means elidia
    ...
    # install without Elidia-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Elidia's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Elidia's
    ``elidia config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Elidia (User env var in Windows Settings, or
    export in a PowerShell profile) and Elidia picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Elidia-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\elidia\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``elidia`` from right after install) retain their old PATH.

    Any subprocess Elidia spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Elidia-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "elidia", "git", "cmd"),
        os.path.join(local_appdata, "elidia", "git", "bin"),
        os.path.join(local_appdata, "elidia", "git", "usr", "bin"),
        # Elidia venv Scripts directory — host of the elidia.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes elidia, but harmless to include.
        os.path.join(local_appdata, "elidia", "elidia-agent", "venv", "Scripts"),
