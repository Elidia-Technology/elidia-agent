"""
Gateway subcommand for elidia CLI.

Handles: elidia gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_elidia_home is imported lazily at call sites to avoid ImportError
# when elidia_constants is cached from a pre-update version during `elidia update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "elidia CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``elidia gateway status`` from falsely counting the ``elidia`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``elidia gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "elidia_cli.main gateway",
        "elidia_cli.main --profile",
        "elidia_cli.main -p",
        "elidia_cli/main.py gateway",
        "elidia_cli/main.py --profile",
        "elidia_cli/main.py -p",
        "elidia gateway",
        ...
        # ``elidia.exe --profile`` / ``elidia.exe -p`` would also match
        # ``elidia.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "elidia.exe gateway",
        "elidia-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_elidia_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"elidia_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ELIDIA_HOME
        ...
        # ELIDIA_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "elidia_home=" in command_lc
            and f"elidia_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``elidia update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Elidia profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Elidia profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from elidia_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "elidia_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``elidia update`` users
    # with no running gateway until they re-invoke ``elidia gateway``
...
def _sync_elidia_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ELIDIA_HOME``.

    Under ``sudo``, ``ELIDIA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_elidia_home` falls back to ``/root/.elidia`` — the wrong
    profile. The unit file pins ``ELIDIA_HOME`` for the actual gateway
    ...
    unit_home = env.get("ELIDIA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ELIDIA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ELIDIA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}elidia gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}elidia gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from elidia_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `elidia gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``elidia gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from elidia_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ELIDIA_HOME-scoped).
...
from elidia_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Elidia Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``elidia gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ELIDIA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ELIDIA_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "elidia-gateway"
SERVICE_DESCRIPTION = "Elidia Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ELIDIA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ELIDIA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from elidia_constants import get_default_elidia_root

    home = get_elidia_home().resolve()
    default = get_default_elidia_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ELIDIA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(elidia_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ELIDIA_HOME is a named profile.

    For ``~/.elidia/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        elidia_home: Optional explicit ELIDIA_HOME path. Defaults to the current
            ``get_elidia_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from elidia_constants import get_default_elidia_root

    home = Path(elidia_home or str(get_elidia_home())).resolve()
    default = get_default_elidia_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ELIDIA_HOME.

    Default ``~/.elidia`` returns ``elidia-gateway`` (backward compatible).
    Profile ``~/.elidia/profiles/coder`` returns ``elidia-gateway-coder``.
    Any other ELIDIA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Elidia installs that predate the
# elidia-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (elidia-gateway-*.service) and unrelated third-party
# "elidia" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("elidia.service",)
...
def _find_legacy_elidia_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Elidia gateway units.

    Detects unit files installed by older Elidia versions that used a
    different service name (e.g. ``elidia.service`` before the rename to
    ``elidia-gateway.service``). When both a legacy unit and the current
    ``elidia-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``elidia-gateway-coder.service`` and unrelated third-party
      ``elidia-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``elidia.service`` running an unrelated
...
def has_legacy_elidia_units() -> bool:
    """Return True when any legacy Elidia gateway unit files exist."""
    return bool(_find_legacy_elidia_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Elidia gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_elidia_units()
    if not legacy:
        return
    print_warning("Legacy Elidia gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current elidia-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    elidia gateway migrate-legacy")


def remove_legacy_elidia_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Elidia gateway unit files.

    Iterates over whatever ``_find_legacy_elidia_units()`` returns — which is
    ...
    legacy = _find_legacy_elidia_units()
    if not legacy:
        print("No legacy Elidia gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Elidia gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: elidia gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo elidia gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    elidia gateway uninstall")
    print_info("    sudo elidia gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Elidia can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo elidia gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo elidia gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo elidia gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Elidia often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.elidia`` → ``ai.elidia.gateway.plist`` (backward compatible).
    Profile ``~/.elidia/profiles/coder`` → ``ai.elidia.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.elidia.gateway-{suffix}" if suffix else "ai.elidia.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.elidia/elidia-agent  -> /home/alice/.elidia/elidia-agent
      /opt/elidia                 -> /opt/elidia  (kept as-is)
...
def _elidia_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ELIDIA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_elidia_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.elidia                    → /home/alice/.elidia
      /root/.elidia/profiles/coder     → /home/alice/.elidia/profiles/coder
      /opt/custom-elidia               → /opt/custom-elidia  (kept as-is)
    """
    current_elidia = get_elidia_home().resolve()
    current_default = (Path.home() / ".elidia").resolve()
    target_default = Path(target_home_dir) / ".elidia"

    # Default ~/.elidia → remap to target user's default
    if current_elidia == current_default:
        return str(target_default)

    # Profile or subdir of ~/.elidia → preserve the relative structure
    try:
        relative = current_elidia.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.elidia) — keep as-is
        return str(current_elidia)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    elidia_home = get_elidia_home()
    elidia_node = elidia_home / "node" / "bin"
    if _is_dir(elidia_node):
        candidates.append(str(elidia_node))
    elidia_nm = elidia_home / "node_modules" / ".bin"
    if _is_dir(elidia_nm):
        candidates.append(str(elidia_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m elidia_cli.main``, so module
    ...
    dir, or a clone that ``elidia update`` later relocates/removes — the path
    ...
    ``ELIDIA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ELIDIA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        elidia_home = _elidia_home_for_target_user(home_dir)
        profile_arg = _profile_arg(elidia_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ELIDIA_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(elidia_home) if elidia_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m elidia_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ELIDIA_HOME={elidia_home}"
Restart=always
RestartSec=5
...
"""
    ...
    elidia_home = str(get_elidia_home().resolve())
    profile_arg = _profile_arg(elidia_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ELIDIA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ELIDIA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../elidia_test`` path into
    # ``Environment="ELIDIA_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Elidia install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo elidia gateway uninstall --system")
    print_info("         elidia gateway install")
    print_info("         elidia gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (elidia.service from pre-rename installs)
    # before installing the new elidia-gateway.service. If both remain, they
    ...
    if has_legacy_elidia_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_elidia_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}elidia gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}elidia gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}elidia gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_elidia_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `elidia gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `elidia gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}elidia gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}elidia gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}elidia gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}elidia gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.elidia.gateway-{suffix}" if suffix else "ai.elidia.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Elidia install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  elidia gateway status             # Check status")
...
def launchd_stop():
    ...
    # `elidia gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Elidia install")
    else:
        print("⚠ Service definition is stale relative to the current Elidia install")
        print("  Run: elidia gateway start")
    ...
    if deep:
        log_file = get_elidia_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ELIDIA_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Elidia gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'elidia' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/elidia/docker/entrypoint.sh before the Elidia command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ELIDIA_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ELIDIA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `elidia gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ELIDIA_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `elidia gateway restart` which already
    ...
    # `elidia gateway start/restart` — leaving the gateway vulnerable to
    ...
    # ELIDIA_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ELIDIA_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Elidia will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @elidia:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. elidia) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Elidia delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Elidia agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Elidia will use (e.g., elidia@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Elidia connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: elidia pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Elidia will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``elidia setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run elidia under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.elidia/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_elidia_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'elidia pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: elidia pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'elidia gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from elidia_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from elidia_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: elidia pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'elidia gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from elidia_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Elidia will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Elidia will store the returned account_id/token in ~/.elidia/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `elidia gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_elidia_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `elidia pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `elidia pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "ElidiaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``elidia_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from elidia_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.elidia/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.elidia/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: elidia gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: elidia gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: elidia gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo elidia gateway install --system"
                        )
                    print_info("  Or run in foreground:  elidia gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: elidia gateway run")
                print_info(
                    "  For persistence:   tmux new -s elidia 'elidia gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: elidia gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup elidia gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'elidia gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`elidia_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from elidia_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ELIDIA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``elidia gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``elidia gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``elidia gateway run`` are unaffected.
      2. ``ELIDIA_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``elidia gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ELIDIA_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ELIDIA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ELIDIA_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ELIDIA_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ELIDIA_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ELIDIA_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-elidia service's
    # pattern in docker/s6-rc.d/main-elidia/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: elidia gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: elidia gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s elidia 'elidia gateway run'"
                )
                print()
            ...
            print()
            print(
                "  elidia gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s elidia 'elidia gateway run'         # persistent via tmux"
            )
            print(
                "  nohup elidia gateway run > ~/.elidia/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  elidia profile create <name>     # creates the s6 service slot")
                print("  elidia -p <name> gateway start   # bring it up via s6")
                print("  elidia status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: elidia gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: elidia gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: elidia gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  elidia profile delete <name>     # tears down the s6 service slot")
                print("  elidia -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `elidia -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: elidia gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ELIDIA_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `elidia gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ELIDIA_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `elidia gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: elidia gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  elidia gateway install")
                else:
                    print("To install as a service:")
                    print("  elidia gateway install")
                    print("  sudo elidia gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  elidia gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup elidia gateway run > ~/.elidia/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s elidia 'elidia gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  elidia gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  elidia gateway install  # Install as user service")
                    print(
                        "  sudo elidia gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Elidia gateway unit files from
        # pre-rename installs (e.g. elidia.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_elidia_units(interactive=not yes, dry_run=dry_run)
