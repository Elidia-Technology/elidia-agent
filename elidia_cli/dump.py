"""
Dump command for elidia CLI.

Outputs a compact, plain-text summary of the user's Elidia setup
...
from elidia_cli.config import get_elidia_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.elidia_build_sha`` by the Dockerfile's
    ``ELIDIA_GIT_SHA`` build-arg (see ``elidia_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``elidia dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from elidia_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(elidia_home: Path) -> int:
...
def _cron_summary(elidia_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = elidia_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"elidia_home:      {display_elidia_home()}")
    ...
    toolsets = config.get("toolsets", ["elidia-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(elidia_home)}")
    lines.append(f"  skills:             {_count_skills(elidia_home)}")
