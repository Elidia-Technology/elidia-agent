"""CLI subcommand: ``elidia send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"elidia send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"elidia send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"elidia send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"elidia send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"elidia send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `elidia gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.elidia/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_elidia_env() -> None:
    """Populate ``os.environ`` from ``~/.elidia/.env`` AND bridge top-level
    ...
    startup that ``elidia send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.elidia/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.elidia/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``elidia send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.elidia/.env and ~/.elidia/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "elidia send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  elidia send --to telegram \"hello\"\n"
            "  elidia send --to discord:#ops --file report.md\n"
            "  elidia send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "elidia send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `elidia send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Elidia "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.elidia/.env + ~/.elidia/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `elidia send --list telegram`.",
    )
