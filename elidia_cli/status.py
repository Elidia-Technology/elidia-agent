"""
Status command for elidia CLI.

Shows the status of all Elidia Agent components.
"""
...
from elidia_cli.auth import AuthError, resolve_provider
from elidia_cli.colors import Colors, color
from elidia_cli.config import get_env_path, get_env_value, get_elidia_home, load_config
from elidia_cli.models import provider_label
...
from elidia_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``elidia config``'s
...
def show_status(args):
    """Show status of all Elidia Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ✦ Elidia Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    elidia_account_info = None
    if (
        elidia_status.get("logged_in")
        or elidia_status.get("access_token")
        or elidia_status.get("portal_base_url")
        or elidia_status.get("inference_credential_present")
        or elidia_status.get("error_code")
    ):
        try:
            elidia_account_info = get_elidia_portal_account_info()
        except Exception:
            elidia_account_info = None

    elidia_logged_in = bool(
        elidia_status.get("logged_in")
        or (elidia_account_info and elidia_account_info.logged_in)
    )
    elidia_inference_present = bool(
        elidia_status.get("inference_credential_present")
        or (elidia_account_info and elidia_account_info.inference_credential_present)
    )
    elidia_error = elidia_status.get("error")
    if elidia_logged_in:
        elidia_label = "logged in"
    elif elidia_inference_present:
        elidia_label = "not logged in (Elidia inference key configured)"
    else:
        elidia_label = "not logged in (run: elidia portal)"
    print(
        f"  {'Elidia Portal':<12}  {check_mark(elidia_logged_in)} "
        f"{elidia_label}"
    )
    portal_url = elidia_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        elidia_status.get("inference_base_url")
        or (elidia_account_info.inference_base_url if elidia_account_info else None)
    )
    access_exp = _format_iso_timestamp(elidia_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(elidia_status.get("agent_key_expires_at"))
    refresh_label = "yes" if elidia_status.get("has_refresh_token") else "no"
    if elidia_logged_in or portal_url != "(unknown)" or elidia_error:
        print(f"    Portal URL: {portal_url}")
    if elidia_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if elidia_logged_in or elidia_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if elidia_logged_in or elidia_inference_present or elidia_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if elidia_logged_in or elidia_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if elidia_error:
        print(f"    Error:      {elidia_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: elidia model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: elidia auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Elidia/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: elidia auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Elidia Subscription Features
    # =========================================================================
    if managed_elidia_tools_enabled():
        ...
        for feature in features.items():
            ...
            elif feature.included_by_default and features.elidia_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.elidia_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_elidia)} {state}")
    elif elidia_logged_in or elidia_inference_present:
        # Elidia OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: elidia model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from elidia_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from elidia_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   elidia gateway")
    ...
    jobs_file = get_elidia_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_elidia_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'elidia doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'elidia setup' to configure", Colors.DIM))
    print()
