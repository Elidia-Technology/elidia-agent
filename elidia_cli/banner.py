...
Pure display functions with no ElidiaCLI state dependency.
"""
...
from elidia_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _banner_hero(accent: str) -> str:
    """Left-column hero for the welcome banner.
    ...
    try:
        image = render_icon(cells_w=16, cells_h=16)
    except Exception:
        image = None
    if image:
        # The inline image advances the cursor by `cells_h` rows; pad the cell
        # so Rich aligns the right column to the same height.
        return image + os.linesep * (16 - 1)
    return os.linesep.join(f"[{accent}]{line}[/]" for line in ELIDIA_ICON_ART.splitlines())
...
# (e.g. nix-built elidia — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Elidia-Technology/elidia-agent.git"
...
def _fetch_pypi_latest(package: str = "elidia-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether an Elidia update is available.

    Two paths: if ``ELIDIA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = elidia_home / ".update_check"
    embedded_rev = os.environ.get("ELIDIA_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $ELIDIA_HOME/elidia-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = elidia_home / "elidia-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Elidia git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ELIDIA_HOME/elidia-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``elidia_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from elidia_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/Elidia-Technology/elidia-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Elidia checkout. Cached per-process. Release URL always points at the
    canonical AiUtils/elidia-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Elidia Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with the Elidia mark on left and info on right.
    ...
    try:
        ...
        _bskin = None
    if _bskin and getattr(_bskin, "banner_hero", ""):
        _hero = _bskin.banner_hero
    else:
        _hero = _banner_hero(accent)
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]AiUtils[/]")

    if os.getenv("ELIDIA_YOLO_MODE"):
    ...
    try:
        from elidia_cli.codex_runtime_switch import get_current_runtime
        from elidia_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from elidia_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built elidia; we know an update
    ...
    # Pip-install warning — `pip install elidia-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ELIDIA_AGENT_LOGO
        console.print(_logo)
        console.print()
