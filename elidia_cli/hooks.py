"""elidia hooks — inspect and manage shell-script hooks.

Usage::

    elidia hooks list
    elidia hooks test <event> [--for-tool X] [--payload-file F]
    elidia hooks revoke <command>
    elidia hooks doctor

Consent records live under ``~/.elidia/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.elidia/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``elidia hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: elidia hooks {list|test|revoke|doctor}")
        print("Run 'elidia hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.elidia/config.yaml.")
        print("See `elidia hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `elidia hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `elidia hooks test` and `elidia hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Elidia wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `elidia hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `elidia hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `elidia hooks doctor`.")
