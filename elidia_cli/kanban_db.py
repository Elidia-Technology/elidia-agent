...
``<root>`` is the **shared Elidia root** (the parent of any active
...
``elidia -p <profile>`` joins the same board as the dispatcher that
...
the profile's Elidia home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ELIDIA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ELIDIA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``elidia kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.elidia``. In Docker / custom
deployments where ``ELIDIA_HOME`` points outside ``~/.elidia`` (e.g.
``/opt/elidia``), ``<root>`` is ``ELIDIA_HOME``. Legacy env-var
overrides still work:

* ``ELIDIA_KANBAN_DB`` — pin the database file path directly.
* ``ELIDIA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ELIDIA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ELIDIA_KANBAN_DB``,
``ELIDIA_KANBAN_WORKSPACES_ROOT``, and ``ELIDIA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/elidia-kanban-v1-spec.pdf`` for the full
...
# ``ELIDIA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ELIDIA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ELIDIA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ELIDIA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ELIDIA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ELIDIA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ELIDIA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``elidia-agent``
...
def kanban_home() -> Path:
    """Return the shared Elidia root that anchors the kanban board.

    Resolution order:

    1. ``ELIDIA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_elidia_root()``, which already returns ``<root>``
       when ``ELIDIA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ELIDIA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ELIDIA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ELIDIA_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from elidia_constants import get_default_elidia_root
    return get_default_elidia_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``elidia kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ELIDIA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``elidia kanban boards
    ...
    """
    env = os.environ.get("ELIDIA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``elidia kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ELIDIA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ELIDIA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ELIDIA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ELIDIA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ELIDIA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("ELIDIA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``elidia kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``ELIDIA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ELIDIA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ELIDIA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Elidia treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ELIDIA_KANBAN_DB`` env → ``ELIDIA_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See elidia_state._WAL_INCOMPAT_MARKERS for detection logic.
                from elidia_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``elidia kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ELIDIA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from elidia_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``elidia --skills ...`` alongside the built-in
    ...
    # `elidia --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``elidia kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `elidia kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``elidia kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``elidia kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``elidia kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``elidia kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ELIDIA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Elidia' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Elidia-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``elidia kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``ELIDIA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than an Elidia
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``elidia kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Elidia profile.
    ...
    if not rows:
        return False
    try:
        from elidia_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from elidia_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Elidia profile.
        # `_default_spawn` invokes ``elidia -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than an Elidia
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `elidia kanban tail` — without
...
def _module_elidia_argv() -> list[str]:
    """Return the interpreter-bound Elidia CLI invocation."""
    # ``elidia_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``elidia`` package — there is no
    # ``elidia`` package to import.
    return [sys.executable, "-m", "elidia_cli.main"]


def _absolute_elidia_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Elidia shim."""
...
def _elidia_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Elidia executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_elidia_argv()
    return [_absolute_elidia_path(path)]


def _resolve_elidia_argv() -> list[str]:
    """Resolve the ``elidia`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ELIDIA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("elidia")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\elidia.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m elidia_cli.main`` — fallback for setups where
       Elidia is launched from a venv and the ``elidia`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_elidia_bin`` for the same reason. Kept
    local (not imported from gateway) because ``elidia_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ELIDIA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _elidia_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _elidia_path_argv(resolved_env_bin)
        return _module_elidia_argv()

    elidia_bin = _safe_which_no_cwd("elidia") if _IS_WINDOWS else shutil.which("elidia")
    if elidia_bin:
        return _elidia_path_argv(elidia_bin)
    return _module_elidia_argv()


def _kanban_worker_skill_available(elidia_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``elidia -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset ELIDIA_HOME means the worker falls back to the default root
    # home (``~/.elidia``), which ships the bundled skill.
    base = _Path(elidia_home) if elidia_home else (_Path.home() / ".elidia")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``elidia -p <profile> chat -q ...`` subprocess.
    ...
    ``ELIDIA_KANBAN_DB`` / ``ELIDIA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ELIDIA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `elidia -p <name>` the
    # _apply_profile_override() runs *before* elidia_constants is imported.
    # If ELIDIA_HOME is absent from the child's env, get_elidia_home() falls
    # back to Path.home() / ".elidia" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from elidia_cli.profiles import resolve_profile_env
    try:
        env["ELIDIA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ELIDIA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ELIDIA_HOME never had profiles created.
        pass
    if task.tenant:
        env["ELIDIA_TENANT"] = task.tenant
    env["ELIDIA_KANBAN_TASK"] = task.id
    env["ELIDIA_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["ELIDIA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ELIDIA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ELIDIA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ELIDIA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ELIDIA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`elidia -p <name>`
    # rewrites ELIDIA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_elidia_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ELIDIA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ELIDIA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ELIDIA_KANBAN_BOARD"] = resolved_board
    # ELIDIA_PROFILE is the author the kanban_comment tool defaults to.
    # `elidia -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ELIDIA_PROFILE"] = profile_arg

    cmd = [
        *_resolve_elidia_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ELIDIA_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("ELIDIA_HOME")):
    ...
    # `elidia kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`elidia` executable not found on PATH. "
            "Install Elidia Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``elidia kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ELIDIA_PROFILE values like "elidia-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Elidia root exists

    Reads profile paths directly so this module has no import dependency on
    ``elidia_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from elidia_constants import get_default_elidia_root
        default_root = get_default_elidia_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``elidia kanban assignees`` for the terminal.
