"""``elidia logs`` — view and filter Elidia log files.
...
Usage examples::
    ...
    elidia logs                    # last 50 lines of agent.log
    elidia logs -f                 # follow agent.log in real time
    elidia logs errors             # last 50 lines of errors.log
    elidia logs gateway -n 100    # last 100 lines of gateway.log
    elidia logs gui -f            # follow gui.log (dashboard/pty/ws)
    elidia logs desktop -f        # follow desktop.log (Electron app boot/backend)
    elidia logs --level WARNING    # only WARNING+ lines
    elidia logs --session abc123   # filter by session ID substring
    elidia logs --component tools  # only tool-related lines
    elidia logs --since 1h         # lines from the last hour
    elidia logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_elidia_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Elidia runs — try 'elidia chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_elidia_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_elidia_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_elidia_home()}/logs/")
        return

    print(f"Log files in {display_elidia_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'elidia chat' to generate logs)")
