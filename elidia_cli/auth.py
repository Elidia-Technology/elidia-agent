"""
Multi-provider authentication system for Elidia Agent.

Supports OAuth device code flows (Elidia Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.elidia/auth.json with cross-process file locking.
...
from elidia_cli.config import get_elidia_home, get_config_path, read_raw_config
from elidia_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Elidia Portal defaults
DEFAULT_ELIDIA_PORTAL_URL = "https://developer.aiutils.io"
DEFAULT_ELIDIA_INFERENCE_URL = "https://inference-api.aiutils.io/v1"
DEFAULT_ELIDIA_CLIENT_ID = "elidia-cli"
ELIDIA_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_ELIDIA_SCOPE = ELIDIA_INFERENCE_INVOKE_SCOPE
ELIDIA_DEVICE_CODE_SOURCE = "device_code"
ELIDIA_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
ELIDIA_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
SPOTIFY_DOCS_URL = "https://aiutils.io/elidia/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://aiutils.io/elidia/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://aiutils.io/elidia/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "elidia": ProviderConfig(
        id="elidia",
        name="Elidia Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_ELIDIA_PORTAL_URL,
        inference_base_url=DEFAULT_ELIDIA_INFERENCE_URL,
        client_id=DEFAULT_ELIDIA_CLIENT_ID,
        scope=DEFAULT_ELIDIA_SCOPE,
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from elidia_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``elidia auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `elidia model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "elidia":
            return _format_elidia_entitlement_auth_error(error)
...
def _format_elidia_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        account_info = get_elidia_portal_account_info(force_fresh=True)
        message = format_elidia_portal_entitlement_message(
            account_info,
            capability="Elidia model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Elidia Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ELIDIA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.elidia/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_elidia_home() / "auth.json"
    # Seat belt: if pytest is running and ELIDIA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ELIDIA_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".elidia" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ELIDIA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ELIDIA_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_elidia_root()
    except Exception:
        return None
    profile_home = get_elidia_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.elidia/auth.json`` even when ELIDIA_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_elidia_root()`` for a profile-shaped ELIDIA_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".elidia" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Elidia store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_elidia_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Elidia lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "elidia_portal" in systems:
            providers["elidia"] = systems["elidia_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "elidia" if providers else None}
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``elidia``) that were only authenticated at
    global scope. Once the user runs ``elidia auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``elidia auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Elidia device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Elidia.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `elidia logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from elidia_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'elidia doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'elidia model' for available providers, or run 'elidia doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'elidia model' to choose a "
        "provider and model, or set an API key (ELIDIA_KEY, OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.elidia/.env.",
...
# Allowlist of hosts the Elidia Portal proxy is willing to forward inference
...
# (ELIDIA_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_ELIDIA_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.aiutils.io",
})


def _validate_elidia_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``ELIDIA_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "elidia: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_ELIDIA_INFERENCE_HOSTS:
        logger.warning(
            "elidia: refusing inference URL host %r from Portal response "
...
def _elidia_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = ELIDIA_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if ELIDIA_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _elidia_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_elidia_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Elidia Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: elidia auth add elidia",
        provider="elidia",
        code=reason,
...
def _log_elidia_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("Elidia inference auth: using NAS invoke JWT")
    _oauth_trace(
        "elidia_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _elidia_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_elidia_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _elidia_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_elidia_invoke_jwt(
    ...
    *,
    ...
    _set_elidia_agent_key_from_invoke_jwt(state)
    _log_elidia_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_ELIDIA_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _elidia_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _ELIDIA_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ELIDIA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `elidia model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.elidia/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `elidia auth list` / `elidia status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.elidia/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("ELIDIA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ELIDIA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ELIDIA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ELIDIA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ELIDIA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Elidia times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``ELIDIA_LOG_LEVEL=INFO`` (or check
            # ``~/.elidia/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>elidia auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `elidia auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `elidia auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `elidia auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `elidia auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.elidia/.env, and return it.
    ...
    print("       App name:     anything (e.g. elidia-agent)")
    ...
    # Persist so subsequent `elidia auth spotify` runs skip the wizard.
    save_env_value("ELIDIA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ELIDIA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ELIDIA_SPOTIFY_CLIENT_ID to ~/.elidia/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ELIDIA_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Elidia:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Elidia is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.elidia/auth.json (not ~/.codex/)
#
# Elidia maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Elidia auth store (~/.elidia/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `elidia auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `elidia auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `elidia auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `elidia auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``elidia setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``elidia auth add openai-codex``
      ...
      ``elidia auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Elidia auth store (~/.elidia/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Elidia auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run elidia auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `elidia auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Elidia auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Elidia's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            base_url = (
                os.getenv("ELIDIA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("ELIDIA_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Elidia processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.elidia/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `elidia model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `elidia model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `elidia model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `elidia model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.elidia/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `elidia model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``ELIDIA_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `elidia model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Elidia (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `elidia model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``elidia model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("ELIDIA_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("ELIDIA_XAI_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# Elidia Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Elidia token store — lets OAuth credentials persist across profiles
# so a new `elidia --profile <name> auth add elidia --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${ELIDIA_SHARED_AUTH_DIR}/elidia_auth.json, defaulting to
# ``<elidia-root>/shared/elidia_auth.json`` where ``<elidia-root>`` is what
# ``get_default_elidia_root()`` returns — ``~/.elidia`` on Linux/macOS,
# ``%LOCALAPPDATA%\elidia`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's ELIDIA_HOME so named profiles (which
# typically live under ``<elidia-root>/profiles/<name>/``) all see the
# same file.
#
...
ELIDIA_SHARED_STORE_FILENAME = "elidia_auth.json"
_elidia_shared_lock_holder = threading.local()


def _elidia_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Elidia token store.

    Honors ``ELIDIA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<elidia-root>/shared/``, where ``<elidia-root>`` is what
    :func:`elidia_constants.get_default_elidia_root` returns — so
    Linux/macOS classic installs land at ``~/.elidia/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\elidia\\shared\\``, and
    Docker / custom ``ELIDIA_HOME`` deployments at
    ``<ELIDIA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("ELIDIA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from elidia_constants import get_default_elidia_root
    return get_default_elidia_root() / "shared"


def _elidia_shared_store_path() -> Path:
    path = _elidia_shared_auth_dir() / ELIDIA_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Elidia root, refuse rather than silently corrupt cross-profile
    # state. Tests must set ELIDIA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from elidia_constants import get_default_elidia_root
        real_home_shared = (
            get_default_elidia_root() / "shared" / ELIDIA_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Elidia auth store during test run: "
                f"{path}. Set ELIDIA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _elidia_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Elidia OAuth store.
    ...
    ``_try_import_shared_elidia_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _elidia_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No ELIDIA_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _elidia_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Elidia auth lock",
    ):
        yield


def _merge_shared_elidia_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Elidia state."""
    shared = _read_shared_elidia_state()
    if not shared:
        return False
...
def _write_shared_elidia_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Elidia OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_ELIDIA_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_ELIDIA_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_ELIDIA_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_ELIDIA_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _elidia_shared_store_lock():
            path = _elidia_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Elidia
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "elidia_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Elidia auth store: %s", exc)


def _read_shared_elidia_state() -> Optional[Dict[str, Any]]:
    """Return the shared Elidia OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Elidia auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_elidia_state(reason: str) -> None:
    """Remove the shared Elidia OAuth store after a terminal token failure."""
    try:
        with _elidia_shared_store_lock():
            path = _elidia_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("elidia_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Elidia auth store: %s", exc)


def _is_terminal_elidia_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Elidia refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "elidia"
...
def _quarantine_elidia_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_elidia_state(reason)
    invalidate_elidia_auth_status_cache()


def _quarantine_elidia_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Elidia pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("elidia")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {ELIDIA_DEVICE_CODE_SOURCE, f"manual:{ELIDIA_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["elidia"] = retained
        _oauth_trace(
            "elidia_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_elidia_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Elidia OAuth state from the shared store.
    ...
    for ``persist_elidia_credentials()``.
    ...
    """
    try:
        with _elidia_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_elidia_state()
            if not shared:
                return None

            # Build a full state dict so refresh_elidia_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_ELIDIA_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_ELIDIA_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_ELIDIA_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_ELIDIA_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_elidia_state(updated_state)

            refreshed = refresh_elidia_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_elidia_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "elidia_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_elidia_refresh_error(exc):
            _clear_shared_elidia_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Elidia import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "elidia_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-elidia-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="elidia", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="elidia", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Elidia portal
    ...
    # called POST /api/oauth/token with Elidia's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Elidia's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Elidia Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Elidia install sharing "
            "~/.elidia/auth.json) called POST /api/oauth/token with Elidia's "
            "refresh token without persisting the rotated token back.\n"
            "Elidia refresh tokens are single-use — only Elidia may call the "
            "refresh endpoint. For health checks, use `elidia auth status` "
            "instead.\n"
            "Re-authenticate with: elidia auth add elidia"
        )
        relogin = True

    raise AuthError(description, provider="elidia", code=code, relogin_required=relogin)


def fetch_elidia_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Elidia inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="elidia", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Elidia models — they're not reliable for agentic tool-calling
            if "elidia" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _elidia_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_elidia_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware Elidia Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "elidia")

        if not state:
            raise AuthError(
                "Elidia is not logged into Elidia Portal.",
                provider="elidia",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("ELIDIA_PORTAL_BASE_URL")
            or os.getenv("ELIDIA_PORTAL_BASE_URL")
            or DEFAULT_ELIDIA_PORTAL_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_ELIDIA_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _elidia_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_elidia_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Elidia Portal login.",
                    provider="elidia",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "elidia", state)
                    _save_auth_store(auth_store)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_elidia_refresh_error(exc):
                        _quarantine_elidia_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_elidia_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state(auth_store, "elidia", state)
            _save_auth_store(auth_store)
            _write_shared_elidia_state(state)
            return state["access_token"]


def refresh_elidia_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_ELIDIA_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh Elidia OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_ELIDIA_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_ELIDIA_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_ELIDIA_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_ELIDIA_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _elidia_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Elidia Portal.",
                    provider="elidia",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
            state["scope"] = refreshed.get("scope") or state.get("scope")
            refreshed_url = _validate_elidia_inference_url_from_network(refreshed.get("inference_base_url"))
            if refreshed_url:
        ...
        _assert_elidia_inference_jwt_usable(state)
        _select_elidia_invoke_jwt(state)

    return state


def refresh_elidia_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Elidia OAuth from a state dict. Thin wrapper around refresh_elidia_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_elidia_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "elidia-cli"),
        state.get("portal_base_url", DEFAULT_ELIDIA_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_ELIDIA_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_ELIDIA_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_elidia_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Elidia OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Elidia credentials are read at runtime from two independent locations:

    - ``providers.elidia``: singleton state read by
      ``resolve_elidia_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.elidia``: used by the runtime ``pool.select()`` path.

    Historically ``elidia auth add elidia`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.elidia``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.elidia`` then calls ``load_pool("elidia")`` so
    ...
    ``elidia auth add elidia --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("elidia")`` instead of the
    ...
    # these credentials via `elidia auth add elidia --type oauth`. Best-
    # effort: any I/O failure is logged and swallowed (the per-profile
    # auth.json is still the source of truth).
    _write_shared_elidia_state(state)

    pool = load_pool("elidia")
    return next(
        (e for e in pool.entries() if e.source == ELIDIA_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_elidia_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.elidia changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("elidia")
    except Exception as exc:
        logger.debug("Failed to sync Elidia credential pool from auth store: %s", exc)


def resolve_elidia_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Elidia inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Elidia is not logged into Elidia Portal.",
                            provider="elidia", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("ELIDIA_INFERENCE_BASE_URL")
            or DEFAULT_ELIDIA_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_ELIDIA_CLIENT_ID)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Elidia auth-status cache warm during read paths.
            if (
                _elidia_effective_provider_state(state)
                == _elidia_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "elidia_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_elidia_state.
            _write_shared_elidia_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "elidia_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Elidia Portal login.",
                                provider="elidia", relogin_required=True)

            invoke_jwt_status = _elidia_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _elidia_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_elidia_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _elidia_invoke_jwt_status(
                            access_token,
            ...
            _assert_elidia_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_elidia_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_elidia_runtime_credentials_final")

    if state_persisted:
        _sync_elidia_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve an Elidia inference API key",
                        provider="elidia", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": ELIDIA_AUTH_PATH_INVOKE_JWT,
        "auth_path": ELIDIA_AUTH_PATH_INVOKE_JWT,
    }
...
def _empty_elidia_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_elidia_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_elidia_auth_status()
...
# ── Process-level memo for get_elidia_auth_status() ──
# get_elidia_auth_status() validates state by calling resolve_elidia_runtime_credentials(),
# which does a synchronous OAuth refresh POST to developer.aiutils.io. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`elidia tools`, status panels,
# subscription-feature checks) call it many times per render — `elidia tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `elidia auth login/logout/add/remove` invalidate naturally on the next call.
_ELIDIA_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_elidia_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_elidia_auth_status_cache() -> None:
    """Clear the get_elidia_auth_status() process-level memo.

    Call this from any code path that mutates Elidia auth state without going
    through resolve_elidia_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _elidia_auth_status_cache
    _elidia_auth_status_cache = None


def get_elidia_auth_status() -> Dict[str, Any]:
    """Status snapshot for Elidia auth.
    ...
    ``invalidate_elidia_auth_status_cache()`` explicitly.
    """
    global _elidia_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _elidia_auth_status_cache
    if cached is not None:
        cached_at, cached_mtime, cached_status = cached
        if (
            cached_mtime == mtime
            and (now - cached_at) < _ELIDIA_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_elidia_auth_status()
    _elidia_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_elidia_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_elidia_auth_status(). See that function."""
    ...
    if state:
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("elidia") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_elidia_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `elidia auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `elidia auth` and
    # `elidia model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ELIDIA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ELIDIA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``elidia doctor`` runs the live
    ...
    try:
        from elidia_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Elidia' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `elidia doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ELIDIA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `elidia logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"elidia", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from elidia_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_ELIDIA_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from elidia_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'elidia model' or 'elidia setup' instead."""
    print("The 'elidia login' command has been removed.")
    print("Use 'elidia auth' to manage credentials,")
    print("'elidia model' to select a provider, or 'elidia setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.elidia/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Elidia-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Elidia auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Elidia will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("ELIDIA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Elidia will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Elidia gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Elidia creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Elidia auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from elidia_constants import display_elidia_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Elidia auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Elidia creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=elidia-agent` lets xAI attribute Elidia-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Elidia — please report at "
            "https://github.com/Elidia-Technology/elidia-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Elidia with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Elidia auth store (~/.elidia/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Elidia login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Elidia / xAI-OAuth / Codex-OAuth quarantine pattern
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `elidia model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``elidia status`` that just want to know
...
def _elidia_device_code_login(
    *,
    ...
    """Run the Elidia device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["elidia"]
    portal_base_url = (
        portal_base_url
        or os.getenv("ELIDIA_PORTAL_BASE_URL")
        or os.getenv("ELIDIA_PORTAL_BASE_URL")
    ...
    print(f"Starting Elidia login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_elidia_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `elidia model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_elidia(args, pconfig: ProviderConfig) -> None:
    """Elidia Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Elidia credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_elidia_state()
        if shared:
            try:
                shared_path = _elidia_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Elidia OAuth credentials at {shared_path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                auth_state = _try_import_shared_elidia_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        if auth_state is None:
            auth_state = _elidia_device_code_login(
        ...
        # overwrites it to "elidia".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "elidia", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        _write_shared_elidia_state(auth_state)
        _sync_elidia_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=elidia but model
        ...
        try:
            ...
            from elidia_cli.models import (
                get_curated_elidia_model_ids, get_pricing_for_provider,
                check_elidia_free_tier, partition_elidia_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Elidia builds still see
            ...
            if model_ids:
                ...
                _url = (_portal or DEFAULT_ELIDIA_PORTAL_URL).rstrip("/")
        ...
        # them to Elidia with a mismatched model.  The Elidia OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "elidia".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Elidia credentials saved for future use.")
            print("  Run `elidia model` again to switch to Elidia Portal.")
            return

        config_path = _update_config_for_provider(
            "elidia", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=elidia)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Elidia will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `elidia model` or configure an API key to use Elidia.")
        else:
