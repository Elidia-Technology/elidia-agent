#!/usr/bin/env python3
"""
Elidia CLI - Main entry point.

Usage:
    elidia                     # Interactive chat (default)
    elidia chat                # Interactive chat
    elidia gateway             # Run gateway in foreground
    elidia gateway start       # Start gateway as service
    elidia gateway stop        # Stop gateway service
    elidia gateway status      # Show gateway status
    elidia gateway install     # Install gateway service
    elidia gateway uninstall   # Uninstall gateway service
    elidia setup               # Interactive setup wizard
    elidia logout              # Clear stored authentication
    elidia status              # Show status of all components
    elidia cron                # Manage cron jobs
    elidia cron list           # List cron jobs
    elidia cron status         # Check if cron scheduler is running
    elidia doctor              # Check configuration and dependencies
    elidia honcho setup                    # Configure Honcho AI memory integration
    elidia honcho status                   # Show Honcho config and connection status
    elidia honcho sessions                 # List directory → session name mappings
    elidia honcho map <name>               # Map current directory to a session name
    elidia honcho peer                     # Show peer names and dialectic settings
    elidia honcho peer --user NAME         # Set user peer name
    elidia honcho peer --ai NAME           # Set AI peer name
    elidia honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    elidia honcho mode                     # Show current memory mode
    elidia honcho mode [hybrid|honcho|local]  # Set memory mode
    elidia honcho tokens                   # Show token budget settings
    elidia honcho tokens --context N       # Set session.context() token cap
    elidia honcho tokens --dialectic N     # Set dialectic result char cap
    elidia honcho identity                 # Show AI peer identity representation
    elidia honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    elidia honcho migrate                  # Step-by-step migration guide: OpenClaw native → Elidia + Honcho
    elidia version             Show version
    elidia update              Update to latest version
    elidia uninstall           Uninstall Elidia Agent
    elidia acp                 Run as an ACP server for editor integration
    elidia sessions browse     Interactive session picker with search

    elidia claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: elidia_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``elidia_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``elidia update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``elidia_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, elidia crashes on import and the user can't run
# ``elidia update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import elidia_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'elidia' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``elidia tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``elidia.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("elidia")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"elidia", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"elidia")
        # Windows: the .exe name is already ``elidia.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* elidia_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ELIDIA_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".elidia", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ELIDIA_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ELIDIA_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ELIDIA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ELIDIA_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`elidia --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from elidia_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Elidia Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``elidia --version`` before config/logging imports on Termux."""
    if os.environ.get("ELIDIA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ELIDIA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (elidia tools, elidia setup, elidia model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'elidia {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any elidia module import.
#
# Many modules cache ELIDIA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ELIDIA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.elidia/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ELIDIA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors elidia_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ELIDIA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.elidia/profiles/coder or
    # /opt/data/profiles/coder).  If ELIDIA_HOME points to the elidia root
    # instead (e.g. systemd hardcodes ELIDIA_HOME=/root/.elidia), we must
    # still read active_profile — the user may have switched profiles via
    # `elidia profile use` and the gateway should honour that choice.
    # See issue #22502.
    elidia_home_env = os.environ.get("ELIDIA_HOME", "")
    if profile_name is None and elidia_home_env:
        if Path(elidia_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the elidia root
    if profile_name is None:
        try:
            from elidia_constants import get_default_elidia_root

            active_path = get_default_elidia_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ELIDIA_HOME
    if profile_name is not None:
        try:
            from elidia_cli.profiles import resolve_profile_env

            elidia_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent elidia from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ELIDIA_HOME"] = elidia_home
...
# Load .env from ~/.elidia/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from elidia_cli.config import get_elidia_home
from elidia_cli.env_loader import load_elidia_dotenv

load_elidia_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ELIDIA_REDACT_SECRETS env
# var BEFORE elidia_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_elidia_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "ELIDIA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ELIDIA_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `elidia` subcommands
...
try:
    from elidia_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from elidia_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if elidia_constants not importable yet
...
from elidia_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_elidia_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ELIDIA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ELIDIA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from elidia_cli.config import get_env_path, get_elidia_home, load_config
    from elidia_cli.auth import get_auth_status

    # Determine whether Elidia itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from elidia_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_elidia_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Elidia Portal OAuth credentials
    auth_file = get_elidia_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Elidia has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Elidia to use their tokens.
    if _has_elidia_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, elidia_bin
        cli_args: the original CLI arguments (everything after 'elidia')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    elidia_bin = container_info["elidia_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo elidia {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo elidia {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  elidia --tui --resume {target}")
    if title:
        print(f'  elidia --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @elidia/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@elidia" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/elidia-ink/package.json",
    "packages/elidia-ink/index.js",
    "packages/elidia-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ELIDIA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ELIDIA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ELIDIA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ELIDIA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    elidia_home = os.environ.get("ELIDIA_HOME") or str(Path.home() / ".elidia")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ELIDIA_HOME": elidia_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(elidia_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(elidia_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if elidia_cli_dir is None:
        elidia_cli_dir = Path(__file__).parent
    bundled = elidia_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ELIDIA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ELIDIA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from elidia_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ELIDIA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ELIDIA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ELIDIA_TUI_DIR (e.g. `unset ELIDIA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ELIDIA_QUIET"):
    ...
    if tui_dev:
        # Keep the local @elidia/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @elidia/ink resolves through
        # packages/elidia-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "elidia-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from elidia_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="elidia-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ELIDIA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ELIDIA_PYTHON_SRC_ROOT"] = os.environ.get(
        "ELIDIA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ELIDIA_PYTHON", sys.executable)
    env.setdefault("ELIDIA_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ELIDIA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ELIDIA_MODEL"] = model
        env["ELIDIA_INFERENCE_MODEL"] = model
    if provider:
        env["ELIDIA_TUI_PROVIDER"] = provider
        env["ELIDIA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ELIDIA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ELIDIA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ELIDIA_TUI_SKILLS"] = value
    if query:
        env["ELIDIA_TUI_QUERY"] = query
    if image:
        env["ELIDIA_TUI_IMAGE"] = image
    if checkpoints:
        env["ELIDIA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ELIDIA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ELIDIA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ELIDIA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ELIDIA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ELIDIA_ACCEPT_HOOKS"] = "1"
    ...
    # ELIDIA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `elidia --tui` try to
    ...
    # still set ELIDIA_TUI_RESUME themselves.
    env.pop("ELIDIA_TUI_RESUME", None)
    if resume_session_id:
        env["ELIDIA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `elidia update` so
    ...
    if code == 42:
        from elidia_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ELIDIA_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``elidia kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``elidia kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ELIDIA_KANBAN_BOARD"):
        return
    try:
        from elidia_cli.kanban_db import get_current_board

        os.environ["ELIDIA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.elidia/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Elidia — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Elidia still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ELIDIA_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ELIDIA_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'elidia sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from elidia_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from elidia_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'elidia doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Elidia isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  elidia setup")
        print()

        from elidia_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'elidia setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from elidia_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ELIDIA_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.elidia/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ELIDIA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ELIDIA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ELIDIA_SESSION_SOURCE"] = args.source
...
def cmd_proxy(args):
    ...
    from elidia_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from elidia_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Elidia?")
        print()
    ...
    # has no creds.json.  Every subsequent `elidia gateway` then paid a 30s
    ...
    session_dir = get_elidia_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: elidia gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `elidia gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  elidia gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '✦ Elidia Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  elidia gateway")
        ...
        print()
        print("  Or install as a service: elidia gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'elidia whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from elidia_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from elidia_cli.config import stamp_install_method
    from elidia_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("✦ Elidia post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from elidia_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``elidia model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("ELIDIA_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from elidia_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'elidia model' for "
                "available providers, or run 'elidia doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in elidia_cli/models.py). A multi-member group shows one
    ...
    elif selected_provider == "elidia":
        _model_flow_elidia(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.elidia/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.elidia/.env if the active provider is not 'custom'.
    ...
    """
    from elidia_cli.config import get_env_value, save_env_value, load_config
...
#
# Elidia uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `elidia model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `elidia model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`elidia_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from elidia_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from elidia_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Elidia only falls back to a lightweight backend (OpenRouter,")
        print("  Elidia Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``elidia model`` flow, then route aux tasks to them here.
    """
    from elidia_cli.config import load_config
    from elidia_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from elidia_cli.auth import _prompt_model_selection
    from elidia_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from elidia_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.elidia/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `elidia setup` from scratch. OpenRouter
    ...
    from elidia_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from elidia_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `elidia auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from elidia_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from elidia_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``elidia model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``elidia model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``elidia auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from elidia_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from elidia_cli.auth import _save_model_choice, deactivate_provider
    from elidia_cli.config import get_env_value, load_config, save_config
    from elidia_cli.secret_prompt import masked_secret_prompt
    ...
    from elidia_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `elidia model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from elidia_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Elidia URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from elidia_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from elidia_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from elidia_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Elidia will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Elidia will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        from elidia_cli.curses_ui import curses_radiolist
...
# downstream call sites read `elidia_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `elidia` invocation — including
# fast paths like `elidia --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from elidia_cli.config import save_env_value, load_config, save_config
    from elidia_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from elidia_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from elidia_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from elidia_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Elidia turns to `copilot --acp`.")
    print("  Elidia currently starts its own ACP subprocess for each request.")
    print("  Elidia uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ELIDIA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``elidia setup`` / ``elidia model``.
    ...
    recover from a malformed paste without editing ``~/.elidia/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from elidia_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from elidia_cli.config import save_env_value
    from elidia_cli.secret_prompt import masked_secret_prompt
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from elidia_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so elidia knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Elidia typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Elidia, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Elidia."
                )
                print()
    ...
    if provider_id == "lmstudio":
        from elidia_cli.auth import AuthError
        from elidia_cli.models import fetch_lmstudio_models
        ...
        from elidia_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from elidia_constants import display_elidia_home as _dhh_fn

            print(
                f"    Elidia will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from elidia_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'elidia model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Elidia CLI with a provider."""
    from elidia_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from elidia_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from elidia_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from elidia_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from elidia_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from elidia_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``elidia slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: elidia slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `elidia slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from elidia_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from elidia_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from elidia_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from elidia_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `elidia security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from elidia_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from elidia_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from elidia_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from elidia_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Elidia home directory to a zip file."""
    if getattr(args, "quick", False):
        from elidia_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from elidia_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore an Elidia backup from a zip file."""
    from elidia_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from elidia_cli.banner import check_for_updates
        from elidia_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Elidia Agent."""
    _require_tty("uninstall")
    from elidia_cli.uninstall import run_uninstall
...
# Critical files that every ``elidia`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``elidia update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "elidia_cli/main.py",
    "elidia_cli/config.py",
    "elidia_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``elidia`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="elidia-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``elidia update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``elidia_cli/web_dist/`` (per vite.config.ts
    outDir: "../elidia_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "elidia_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``elidia`` launcher present but ``elidia_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``elidia update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``elidia web``).
    ...
    # build path usable on a stock `py -m elidia_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (elidia web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``elidia update`` can unconditionally call ``elidia desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``elidia desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ELIDIA_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ELIDIA_HOME."""
    from elidia_constants import get_elidia_home
    return get_elidia_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``elidia update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Elidia.app/Contents/MacOS/Elidia"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Elidia.exe",
            release_dir / "win-ia32-unpacked" / "Elidia.exe",
            release_dir / "win-arm64-unpacked" / "Elidia.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "elidia",
            release_dir / "linux-unpacked" / "Elidia",
        ]
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Elidia is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Elidia.app/Contents/MacOS/Elidia  ->  app bundle = .../Elidia.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Elidia Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Elidia Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from elidia_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["ELIDIA_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ELIDIA_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "elidia_root", None):
        env["ELIDIA_DESKTOP_ELIDIA_ROOT"] = str(Path(args.elidia_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ELIDIA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  elidia gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Elidia is
    ...
    if source_mode:
        print("→ Launching Elidia Desktop from source build...")
    ...
    print(f"→ Launching packaged Elidia Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``elidia dashboard`` processes other than ourselves.

    ``elidia dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``elidia update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Elidia Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``elidia dashboard`` as
    ...
    environment variable ``ELIDIA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "elidia dashboard",
        "elidia_cli.main dashboard",
        "elidia_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "elidia.*dashboard"`) keeps us consistent
            # with `elidia_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `elidia update`.
    ...
    print("  Preview now:  elidia curator run --dry-run")
    print("  Pause it:     elidia curator pause")
    print(
        "  Docs:         https://aiutils.io/elidia/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``elidia update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``elidia update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: elidia curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``elidia dashboard`` processes.

    Called at the end of ``elidia update`` (default ``reason``) and also
    from ``elidia dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Elidia Desktop Electron app spawns this dashboard as a
    # backend child, it sets ELIDIA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ELIDIA_DESKTOP_CHILD_PID")
    if raw_pid:
        try:
            exclude = {int(raw_pid)}
    ...
    if killed:
        ...
        print("    elidia dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Elidia Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `elidia update "
            f"--branch {branch}`, or update against main with `elidia update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/Elidia-Technology/elidia-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="elidia-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"elidia-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for elidia-agent itself
        ...
        # GitHub ZIPs extract to elidia-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"elidia-agent-{branch}")
    ...
    from elidia_cli.managed_uv import ensure_uv, rebuild_venv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "elidia-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _clean_managed_worktree(git_cmd: list[str], cwd: Path) -> bool:
    ...
    On a managed install (%LOCALAPPDATA%\\elidia\\elidia-agent or
    ~/.elidia/elidia-agent) the user never edits the source tree, so any
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Elidia behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes elidia completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Elidia couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Elidia couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Elidia behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `elidia update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Elidia-Technology/elidia-agent.git",
    "git@github.com:AiUtils/elidia-agent.git",
    "https://github.com/Elidia-Technology/elidia-agent",
    "git@github.com:AiUtils/elidia-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Elidia-Technology/elidia-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_elidia_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_elidia_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Elidia repository.")
        print("  This means you may miss updates from AiUtils/elidia-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Elidia-Technology/elidia-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Elidia-Technology/elidia-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``elidia update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from elidia_constants import get_default_elidia_root

    default_home = get_default_elidia_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``elidia update`` is still progressing even if pip/uv itself is silent.
    """
...
def _elidia_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "elidia.exe",
        scripts_dir / "elidia-gateway.exe",
    ]


def _detect_concurrent_elidia_instances(
    ...
    The Elidia Desktop Electron app spawns ``elidia.EXE`` as a backend child,
    so during ``elidia update`` the user-invoked process and the desktop's
    ...
    shims (``elidia.exe`` / ``elidia-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``elidia update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``elidia.exe``) is a separate process from the Python
    ...
    for shim in _elidia_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated elidia.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``elidia update``
    ...
    #      elidia.exe sitting *under* a non-Elidia parent (e.g. an Elidia
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "elidia.exe"
    lines = ["✗ Another elidia.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Elidia Desktop, exit any open `elidia` REPLs, and")
    lines.append("  stop the gateway (`elidia gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `elidia update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_elidia_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``elidia.exe``.
    ...
    when ``elidia update`` runs, ``elidia.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``elidia.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next elidia invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Elidia Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Elidia Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _elidia_exe_shims(scripts_dir):
        ...
        print(
            "    Close Elidia Desktop, exit other `elidia` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `elidia update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_elidia_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``elidia.exe.old.*`` left by prior updates.

    Called early on every elidia invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``elidia update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `elidia update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``elidia.exe`` / ``elidia-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_elidia_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_elidia_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from elidia_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ELIDIA_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from elidia_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `elidia update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``elidia update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.elidia/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``elidia update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``elidia update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.elidia/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``elidia update --gateway``) the update is already
    ...
    try:
        ...
        # elidia_cli.config.get_elidia_home to simulate setup failure.
        from elidia_cli.config import get_elidia_home as _get_elidia_home

        logs_dir = _get_elidia_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``elidia update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from elidia_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``elidia update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from elidia_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from elidia_cli.config import recommended_update_command
        from elidia_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``elidia update`` without requiring a reinstall.  The
    ...
    ``elidia`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/elidia.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v elidia'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/elidia, code at /usr/local/lib/elidia-agent).
    fhs_link = Path("/usr/local/bin/elidia")
    ...
    path_comment = (
        "# Elidia Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ELIDIA_HOME before running the update.
    ...
    ELIDIA_HOME directories.  The ``--backup`` flag on ``elidia update``
    ...
    try:
        from elidia_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_elidia_home so the user sees ~/.elidia/...
    try:
        from elidia_constants import get_elidia_home, display_elidia_home

        home = get_elidia_home()
        try:
            display_path = f"{display_elidia_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  elidia import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``elidia update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Elidia Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Elidia Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `elidia update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Elidia via pip (for PyPI installs)."""
    from elidia_cli import __version__
    from elidia_cli.config import is_uv_tool_install

    print(f"→ Current version: {__version__}")
    print("→ Checking PyPI for updates...")

    from elidia_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "elidia-agent"]
        ...
        cmd = [pipx, "upgrade", "elidia-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "elidia-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "elidia-agent"]
    ...
    print("✓ Update complete! Restart elidia to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("✦ Updating Elidia Agent...")
    print()

    # On Windows, abort early if another elidia.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_elidia_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ELIDIA_HOME, this is cheap
        ...
        try:
            from elidia_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in elidia_cli/config.py bricked
        # every user who ran ``elidia update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``elidia
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``elidia update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_elidia_home added to elidia_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # build.  ``elidia desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``elidia desktop`` shouldn't be forced into a full
        # Electron build by ``elidia update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            build_result = subprocess.run(
                [sys.executable, "-m", "elidia_cli.main", "desktop", "--build-only"],
                cwd=PROJECT_ROOT,
                check=False,
            )
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `elidia desktop` to retry)")

        print()
        print("✓ Code updated!")

        # After git pull, source files on disk are newer than cached Python
        # modules in this process.  Reload elidia_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_elidia_home() added since the last release.
        try:
            import importlib
            import elidia_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ELIDIA_HOME so
        # it is not affected by sync_skills()'s module-level ELIDIA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ELIDIA_HOME env var points at the default or a named profile.
        try:
            from elidia_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'elidia config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: elidia config migrate")
            else:
                print()
                print("Skipped. Run 'elidia config migrate' later to configure.")
        else:
        ...
        try:
            from elidia_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `elidia curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``elidia update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from elidia_cli.tools_config import install_cua_driver
        ...
        # When running as ``elidia update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_elidia_home() / ".update_exit_code"
            try:
        ...
        try:
            from elidia_cli.gateway import (
                is_macos,
            ...
            # Discover all elidia-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. elidia-gateway.service or elidia-gateway-coder.service
                            ...
                            # path in `elidia gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: elidia gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: elidia -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Elidia gateway unit files are still installed.
        # When both elidia.service (from a pre-rename install) and the
        # current elidia-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `elidia update` surfaces the issue until the user migrates.
        try:
            from elidia_cli.gateway import (
                has_legacy_elidia_units,
                _find_legacy_elidia_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_elidia_units():
                print()
                print("⚠ Legacy Elidia gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_elidia_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (elidia.service) fight the current")
                print("  elidia-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    elidia gateway migrate-legacy")
                print()
        ...
        print("  elidia model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``elidia -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from elidia_constants import display_elidia_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `elidia profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_elidia_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → elidia -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.elidia)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  elidia profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     elidia -p {name} chat")
                else:
        ...
        from elidia_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from elidia_constants import get_elidia_home as _hh
        ...
        from elidia_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `elidia profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from elidia_cli.profiles import profile_exists
        ...
        from elidia_cli.profiles import rename_profile
        ...
        from elidia_cli.profiles import export_profile
        ...
        from elidia_cli.profiles import import_profile
        ...
        import tempfile
        from elidia_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="elidia_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  elidia -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      elidia -p {plan.manifest.name} chat")
        ...
        from elidia_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `elidia profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"elidia -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from elidia_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("elidia_requires"):
            print(f"Requires:     Elidia {data['elidia_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from elidia_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.elidia_requires:
        print(f"  Requires: Elidia {mf.elidia_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``elidia dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``elidia dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No elidia dashboard processes running.")
        return 0

    print(f"{len(pids)} elidia dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No elidia dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `elidia update`.
    ...
    # the same logs directory as every other Elidia surface.
    try:
        from elidia_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ELIDIA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ELIDIA_WEB_DIST"])
            if "ELIDIA_WEB_DIST" in os.environ
            else PROJECT_ROOT / "elidia_cli" / "web_dist"
        )
    ...
    # (e.g. plugins/dashboard_auth/elidia) registers BEFORE start_server's
    ...
    try:
        from elidia_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from elidia_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ELIDIA_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from elidia_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from elidia_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Elidia log files."""
    from elidia_cli.logs import tail_log, list_logs
...
# so that in ``elidia -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``elidia_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``elidia -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``elidia`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ELIDIA_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from elidia_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from elidia_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ELIDIA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ELIDIA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ELIDIA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `elidia --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for elidia CLI."""
    # Cosmetic: make the process show up as 'elidia' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from elidia_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``elidia.exe.old.*`` quarantine files left by previous
    # ``elidia update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_elidia_exe``.
    try:
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for Elidia login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for Elidia login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Elidia login (default: elidia-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for Elidia login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during Elidia login",
    )
    model_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    model_parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for Elidia login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for Elidia TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for Elidia login (testing only)",
    )
    ...
    # =========================================================================
    from elidia_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aiutils.io/elidia/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `elidia model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.elidia/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://aiutils.io/elidia/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from elidia_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from elidia_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ELIDIA_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy elidia.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Elidia gateway unit files "
            "(e.g. elidia.service) left over from older installs. Profile "
            "units (elidia-gateway-<profile>.service) and unrelated "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Elidia Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="elidia",
        help="Upstream provider: elidia or xai (default: elidia). See `elidia proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Elidia Agent with an interactive wizard. "
        "Run a specific section: elidia setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'elidia setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot Elidia Portal setup: log in via OAuth, pick an Elidia "
        "model, set Elidia as the inference provider, and opt into the Tool "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Elidia.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ELIDIA_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Elidia")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from elidia_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Elidia CLI",
    )
    login_parser.add_argument(
        "--provider",
        choices=["elidia", "openai-codex", "xai-oauth"],
        default=None,
        help="Provider to authenticate with (default: elidia)",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: elidia-cli)"
    )
    ...
    logout_parser.add_argument(
        "--provider",
        choices=["elidia", "openai-codex", "xai-oauth", "spotify"],
        default=None,
    ...
    auth_add.add_argument("--portal-url", help="Elidia portal base URL")
    auth_add.add_argument("--inference-url", help="Elidia inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Elidia with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ELIDIA_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Elidia Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.elidia/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Elidia profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.elidia/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Elidia profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    # portal command — Elidia Portal status + Tool Gateway routing
    # =========================================================================
    from elidia_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from elidia_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.elidia/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.elidia/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Elidia Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `elidia "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Elidia "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.elidia/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Elidia Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Elidia setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Elidia Agent. Use 'elidia debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    elidia debug share              Upload debug report and print URL
    elidia debug share --lines 500  Include more log lines
    elidia debug share --expire 30  Keep paste for 30 days
    elidia debug share --local      Print report locally (no upload)
    elidia debug share --no-redact  Disable upload-time secret redaction
    elidia debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'elidia debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Elidia home directory to a zip file",
        description="Create a zip archive of your entire Elidia configuration, "
        "skills, sessions, and data (excludes the elidia-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/elidia-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.elidia/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo elidia uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from elidia_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore an Elidia backup from a zip file",
        description="Extract a previously created Elidia backup into your "
        "Elidia home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Elidia Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from elidia_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.elidia/skills/.bundled_manifest) "
            "so future 'elidia update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`elidia update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `elidia update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from elidia_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from elidia_cli.skills_hub import skills_command
    ...
    from elidia_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/elidia-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `elidia plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from elidia_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``elidia --help``, ``elidia version``, ``elidia logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from elidia_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from elidia_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_elidia_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_elidia_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_elidia_home()}/memories/\n")
        else:
            from elidia_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'elidia tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # elidia tools list [--platform cli]
    ...
    # elidia tools disable <name...> [--platform cli]
    ...
    # elidia tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from elidia_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from elidia_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `elidia computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `elidia tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: elidia computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: elidia computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Elidia as an MCP server",
        description=(
            "Manage MCP server connections and run Elidia as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'elidia mcp add' to connect to a new server, or\n"
            "'elidia mcp serve' to expose Elidia conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Elidia as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `elidia mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Elidia-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `elidia mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Elidia-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `elidia mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from elidia_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch elidia --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Elidia",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Elidia",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.elidia/ (by default a "
        "single restore-point archive is written to ~/.elidia/backups/ "
        "before apply; restorable with 'elidia import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from elidia_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Elidia Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'elidia config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, elidia will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another elidia.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Elidia Agent",
        description="Remove Elidia Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Elidia Agent as an ACP (Agent Client Protocol) server",
        description="Start Elidia Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Elidia ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Elidia provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.elidia/node/ "
    ...
    def cmd_acp(args):
        """Launch Elidia Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Elidia instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `elidia update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `elidia profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install an Elidia profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Elidia Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `elidia --tui` via PTY/WebSocket). "
            "Alternatively set ELIDIA_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `elidia dashboard` cmdlines and SIGTERM them directly — the same
    # path `elidia update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running elidia dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running elidia dashboard processes and exit",
    )
    ...
    # for one release. The Elidia-Setup.exe success screen tells users to
    # run `elidia desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Elidia Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any elidia CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--elidia-root",
        help="Override the Elidia source root used by Desktop (sets ELIDIA_DESKTOP_ELIDIA_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets ELIDIA_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Elidia log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    elidia logs                    Show last 50 lines of agent.log
    elidia logs -f                 Follow agent.log in real time
    elidia logs errors             Show last 50 lines of errors.log
    elidia logs gateway -n 100     Show last 100 lines of gateway.log
    elidia logs gui -f             Follow gui.log in real time
    elidia logs desktop -f         Follow desktop.log (Electron app boot/backend)
    elidia logs --level WARNING    Only show WARNING and above
    elidia logs --session abc123   Filter by session ID
    elidia logs --component tools  Only show tool-related lines
    elidia logs --since 1h         Lines from the last hour
    elidia logs --since 30m -f     Follow, starting from 30 min ago
    elidia logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``elidia -c Pokemon Agent Dev`` → ``elidia -c 'Pokemon Agent Dev'``
    ...
    from elidia_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'elidia -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (elidia hooks list, cron
