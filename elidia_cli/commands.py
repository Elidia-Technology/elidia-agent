"""Slash command definitions and autocomplete for the Elidia CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Elidia config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Elidia works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Elidia is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.elidia/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Elidia Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`elidia_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/elidia`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Elidia
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``elidia skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``elidia skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``elidia skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/elidia <verb>`` subcommand.
    ...
    can still reach them via ``/elidia <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/elidia`` is always reserved as the first entry so the
    legacy ``/elidia <subcommand>`` form keeps working for anything that
    ...
    # Reserve /elidia as the catch-all top-level command.
    entries.append(("elidia", "Talk to Elidia or run a subcommand", "[subcommand] [args]"))
    seen.add("elidia")
...
def slack_app_manifest(request_url: str = "https://elidia-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /elidia handler.

    Maps both canonical names and aliases so /elidia bg do stuff works
    the same as /elidia background do stuff.

    Plugin-registered slash commands are included so ``/elidia <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from elidia_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
