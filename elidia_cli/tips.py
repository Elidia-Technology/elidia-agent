...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or elidia -c.",
    ...
    "Ctrl+Z suspends Elidia to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "elidia -c resumes your most recent CLI session. elidia -c \"project name\" resumes by title.",
    "elidia -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "elidia -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "elidia chat -t web,terminal enables only specific toolsets for a focused session.",
    "elidia chat -s github-pr-workflow preloads a skill at launch.",
    "elidia chat -q \"query\" runs a single non-interactive query and exits.",
    "elidia chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "elidia chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "elidia --yolo bypasses all dangerous command approval prompts for the entire session.",
    "elidia chat --source telegram tags the session for filtering in elidia sessions list.",
    "elidia -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "elidia doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "elidia dump outputs a compact setup summary — great for bug reports.",
    "elidia config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "elidia config edit opens config.yaml in your default editor.",
    "elidia config check scans for missing or stale configuration options.",
    "elidia sessions browse opens an interactive session picker with search.",
    "elidia sessions stats shows session counts by platform and database size.",
    "elidia sessions prune --older-than 30 cleans up old sessions.",
    "elidia skills search react --source skills-sh searches the skills.sh public directory.",
    "elidia skills check scans installed hub skills for upstream updates.",
    "elidia skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "elidia skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "elidia mcp add github --command npx adds MCP servers from the command line.",
    "elidia mcp serve runs Elidia itself as an MCP server for other agents.",
    "elidia auth add lets you add multiple API keys for credential pool rotation.",
    "elidia completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "elidia logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "elidia backup creates a zip backup of your entire Elidia home directory.",
    "elidia profile create coder creates an isolated profile that becomes its own command.",
    "elidia profile create work --clone copies your current config and keys to a new profile.",
    "elidia update syncs new bundled skills to ALL profiles automatically.",
    "elidia gateway install sets up Elidia as a system service (systemd/launchd).",
    "elidia memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "elidia webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: elidia tools disables unused tools, elidia skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "elidia models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as elidia -w).",
    ...
    "ELIDIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Elidia tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'elidia profile create coder' creates the 'coder' command.",
    "elidia profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Elidia prints a resume command with session ID and stats.",
    "elidia sessions export backup.jsonl exports all sessions for backup or analysis.",
    "elidia -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "elidia skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .elidia/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: elidia cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Elidia runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "elidia gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Elidia via a local macOS server.",
    ...
    "Context files (.elidia.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Elidia loads project context from .elidia.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "elidia mcp opens an interactive picker of Elidia-approved MCPs you can install in one keystroke.",
    "elidia mcp catalog lists Elidia-approved MCP servers shipped with the repo.",
    "elidia mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — elidia tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.elidia/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "elidia chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "elidia plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "elidia acp runs Elidia as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ELIDIA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "elidia auth add openai-codex sets up an OAuth-based provider; elidia auth add elidia stores an AiUtils Developer API key instead.",
    ...
    "elidia status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.elidia/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Elidia your own.",
    ...
    "Before a gateway session reset, Elidia auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.elidia/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ELIDIA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ELIDIA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ELIDIA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.elidia/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'elidia profile create ls' would be rejected.",
    "elidia profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "elidia claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ELIDIA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via elidia tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.elidia/.env.",
    "Skills from trusted repos (AiUtils) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.elidia/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Elidia auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Elidia config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Elidia is working.',
    ...
    '/reload reloads ~/.elidia/.env into the running session — pick up new API keys without restarting.',
    ...
    "ELIDIA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.elidia/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.elidia/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'elidia curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "elidia curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'elidia curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'elidia auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Elidia subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'ELIDIA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ELIDIA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'elidia dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'elidia dashboard --tui embeds the full Elidia TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.elidia/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.elidia/dashboard-plugins/ — no npm build required.',
    ...
    'ELIDIA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ELIDIA_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'ELIDIA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ELIDIA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.elidia/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.elidia/skills/ so drop-in skills appear without restarting the session.',
    ...
    'elidia -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'elidia chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'elidia chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'elidia chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "elidia chat --source tool tags programmatic chats so they don't clutter elidia sessions list.",
    'elidia dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'elidia sessions rename <ID> "new title" renames any past session; elidia sessions delete <ID> removes one.',
    'elidia import restores a session export or profile archive produced by sessions export or profile export.',
    'elidia fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'elidia pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'elidia setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'elidia status --deep runs the full health sweep across every component; plain elidia status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ELIDIA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ELIDIA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.elidia/plugins/ — trust-gated by design.',
    "ELIDIA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ELIDIA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ELIDIA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ELIDIA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ELIDIA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ELIDIA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ELIDIA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ELIDIA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ELIDIA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ELIDIA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ELIDIA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Elidia block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from elidia sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.elidia/dashboard-plugins/.',
]
