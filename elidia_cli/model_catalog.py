...
The Elidia docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Elidia Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_URL = (
    "https://share.aiutils.io/elidia-hermes/model-catalog.json"
)
# Fallback fetch chain. If the aiutils.io-hosted manifest is unreachable the
# disk cache goes stale and new model releases never reach the picker. The
# raw GitHub URL is the same manifest published from the upstream repo and is
# not bot-gated, so we fall through to it whenever the primary URL fails.
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/Elidia-Technology/elidia-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_elidia_home() / "cache" / "model_catalog.json"
...
def get_curated_elidia_models() -> list[str] | None:
    """Return Elidia Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("elidia")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``elidia model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
