...
Add, remove, or reorder entries here — both `elidia setup` and
`elidia` provider-selection will pick up the change automatically.
"""
...
from elidia_cli import __version__ as _ELIDIA_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ELIDIA_USER_AGENT = f"elidia-cli/{_ELIDIA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `elidia model`
    flow without maintaining a separate static list.
    """
    from elidia_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ELIDIA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ELIDIA_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Elidia refreshes the cache from
...
# ---------------------------------------------------------------------------
# Elidia Portal free-model helper
# ---------------------------------------------------------------------------
# The Elidia Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Elidia Portal account tier detection
# ---------------------------------------------------------------------------
def is_elidia_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_elidia_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Elidia models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/elidia/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["elidia"]`` list happens to contain or whether the
    ...
    older Elidia that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/elidia/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["elidia"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Elidia that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_elidia_models_by_tier` keeps free models selectable;
...
def check_elidia_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Elidia Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_elidia_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Elidia Portal recommended models
#
...
ELIDIA_RECOMMENDED_MODELS_PATH = "/api/elidia/recommended-models"
_ELIDIA_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_elidia_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_elidia_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Elidia Portal's curated recommended-models payload.

    Hits ``<portal>/api/elidia/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_ELIDIA_RECOMMENDED_CACHE_TTL`` seconds; pass ``force_refresh=True`` to
    bypass the cache.
    ...
    """
    base = (portal_base_url or "https://developer.aiutils.io").rstrip("/")
    now = time.monotonic()
    cached = _elidia_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _ELIDIA_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{ELIDIA_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    _elidia_recommended_cache[base] = (data, now)
    return data


def _resolve_elidia_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from elidia_cli.auth import (
            DEFAULT_ELIDIA_PORTAL_URL,
        ...
        return str(DEFAULT_ELIDIA_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://developer.aiutils.io"
...
def get_elidia_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_elidia_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_elidia_portal_url()
    payload = fetch_elidia_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_elidia_free_tier()
        except Exception:
...
# this list:  elidia model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `elidia model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `elidia model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("elidia",           "Elidia Portal",              "Elidia Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Elidia provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `elidia model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``elidia model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``elidia model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``elidia auth add openai-codex`` without
    ``elidia model``).
    """
    models = _PROVIDER_MODELS.get(provider, [])
    if models:
        return models[0]
    # Plugin providers not in _PROVIDER_MODELS may declare an explicit
    # ``default_model`` (e.g. ``aiutils``) so an empty model string never
    # reaches the gateway (which rejects it with HTTP 400). Providers with a
    # dynamic catalog (OpenRouter, etc.) omit it and keep returning "" here.
    try:
        from providers import get_provider_profile
        _p = get_provider_profile(provider)
        if _p is not None and _p.default_model:
            return _p.default_model
    except Exception:
        pass
    return ""
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    elidia-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Elidia Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from elidia_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — elidia-agent
...
def get_curated_elidia_model_ids() -> list[str]:
    """Return the curated Elidia Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["elidia"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from elidia_cli.model_catalog import get_curated_elidia_models
        remote = get_curated_elidia_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("elidia", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Elidia Portal).
    """
...
_DEFAULT_ELIDIA_INFERENCE_BASE = "https://inference-api.aiutils.io"


def _resolve_elidia_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Elidia Portal pricing.

    The Elidia inference ``/v1/models`` endpoint exposes pricing without
    ...
    try:
        ...
        pass
    return ("", _DEFAULT_ELIDIA_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, elidia, novita)."""
    ...
    if normalized == "elidia":
        api_key, base_url = _resolve_elidia_pricing_credentials()
        if base_url:
            # Elidia base_url typically looks like https://inference-api.aiutils.io/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``elidia model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from elidia_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        elidia:hermes-3                           →  ("elidia", "hermes-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Elidia, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"elidia", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from elidia_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model elidia` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Elidia' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``elidia_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Elidia should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``elidia auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.elidia/.env``.
    ...
    """
    try:
        from elidia_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from elidia_cli.copilot_auth import (
...
#   - "elidia": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Elidia),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without an Elidia release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # an Elidia release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "elidia":
        # Try live Elidia Portal /models endpoint
        try:
            from elidia_cli.auth import fetch_elidia_models, resolve_elidia_runtime_credentials
            creds = resolve_elidia_runtime_credentials()
            if creds:
                live = fetch_elidia_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["elidia"] snapshot — so newly
        # added Portal models still surface without an Elidia release.
        manifest_ids = get_curated_elidia_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``elidia model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ELIDIA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_elidia_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, elidia portal), the
    relevant tokens live in ``$ELIDIA_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ELIDIA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Elidia' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ELIDIA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from elidia_constants import get_elidia_home
    return get_elidia_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Elidia will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Elidia cannot verify the model name."
