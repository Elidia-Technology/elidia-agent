"""`elidia checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.elidia/checkpoints/``.  Actions:

    elidia checkpoints               # same as `status`
    elidia checkpoints status        # total size, project count, breakdown
    elidia checkpoints list          # per-project checkpoint counts + workdir
    elidia checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    elidia checkpoints clear [-f]    # nuke the entire base (asks first)
    elidia checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    elidia checkpoints
    elidia checkpoints prune --retention-days 3 --max-size-mb 200
    elidia checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: elidia checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``elidia checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `elidia checkpoints` → status
