"""Normalized Elidia Portal account entitlement helpers."""
...
ElidiaAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (elidia-account-service
...
_account_info_cache: tuple[str, float, "ElidiaPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class ElidiaPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class ElidiaPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class ElidiaPortalAccountInfo:
    logged_in: bool
    source: ElidiaAccountInfoSource
    fresh: bool
    user_id: Optional[str] = None
    org_id: Optional[str] = None
    client_id: Optional[str] = None
    product_id: Optional[str] = None
    elidia_client: Optional[str] = None
    ...
    subscription: Optional[ElidiaPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[ElidiaPaidServiceAccessInfo] = None
    tool_access: Optional[ElidiaToolAccessInfo] = None
...
def elidia_portal_billing_url(account_info: Optional[ElidiaPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Elidia account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_ELIDIA_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def format_elidia_portal_entitlement_message(
    account_info: Optional[ElidiaPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Elidia tool-gateway entitlement.
    ...
    """
    billing_url = elidia_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Elidia Portal "
    ...
    if account_info is None:
        return (
            f"Elidia could not verify your Elidia Portal entitlement, so {capability} "
            f"is unavailable. Run `elidia model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Elidia inference credentials are configured, but Elidia cannot verify "
                f"your Elidia Portal paid access for {capability}. Log in with "
                f"`elidia model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Elidia Portal to use {capability}: run `elidia model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Elidia could not verify your Elidia Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `elidia model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Elidia could not find an Elidia Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `elidia model` to "
            f"authenticate again; if the problem persists, contact Elidia support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `elidia model` to refresh Elidia."
        return message

    return (
        f"Your Elidia Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Elidia Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Elidia Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Elidia Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Elidia Portal account has no usable paid credits{credit_detail}, so "
...
def _fresh_account_info(
    *,
    ...
    try:
        from elidia_cli.auth import get_provider_auth_state, resolve_elidia_access_token

        access_token = resolve_elidia_access_token()
        ...
        payload = _fetch_elidia_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[ElidiaPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Elidia keys."""
    try:
        entry = _select_elidia_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_elidia_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_elidia_pool_entry() -> Optional[Any]:
...
def _fetch_elidia_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from elidia_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = ElidiaPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return ElidiaPortalAccountInfo(
        ...
        elidia_client=_coerce_str(claims.get("elidia_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[ElidiaToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`ElidiaToolAccessInfo`. Fails closed: a non-object value
    ...
    return ElidiaToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[ElidiaPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return ElidiaPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[ElidiaPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return ElidiaPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
