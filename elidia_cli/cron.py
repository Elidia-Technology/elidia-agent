"""
Cron subcommand for elidia CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(elidia\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*elidia)"
    r"|(systemctl\s+(restart|stop|start)\s+.*elidia)"
    r"|(p?kill\s+.*elidia.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'elidia cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: elidia gateway install", Colors.DIM))
        print(color("                    sudo elidia gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    elidia gateway install    # Install as a user service")
        print("    sudo elidia gateway install --system  # Linux servers: boot-time system service")
        print("    elidia gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `elidia gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: elidia cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
