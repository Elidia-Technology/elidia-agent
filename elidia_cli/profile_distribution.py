"""Profile distributions — shareable, packaged Elidia profiles via git.

A distribution is an Elidia profile published as a git repository (or
...
* ``elidia profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``elidia skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``elidia profile``, not a parallel tree):

    elidia profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "elidia-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            elidia_requires=str(data.get("elidia_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.elidia_requires:
            out["elidia_requires"] = self.elidia_requires
        if self.author:
...
def check_elidia_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Elidia {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``elidia profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not an Elidia profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not an Elidia distribution."
        )

    # Version check up-front so we fail fast
    check_elidia_requires(manifest.elidia_requires, elidia_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.elidia).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="elidia_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `elidia profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`elidia profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="elidia_dist_update_") as tmp:
        plan = plan_install(
