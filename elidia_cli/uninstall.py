"""
Elidia Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.elidia/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Elidia PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing elidia-agent or elidia PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Elidia Agent" comment and following line
                if '# Elidia Agent' in line or '# elidia-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('elidia' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing elidia
                if 'elidia' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the elidia wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "elidia",
        Path("/usr/local/bin/elidia"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains elidia_cli reference)
                content = wrapper.read_text()
                if 'elidia_cli' in content or 'elidia-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(elidia_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $ELIDIA_HOME/node/bin/node
        ~/.local/bin/npm  -> $ELIDIA_HOME/node/bin/npm
        ~/.local/bin/npx  -> $ELIDIA_HOME/node/bin/npx
    ...
    when the link still resolves into this Elidia home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Elidia) is left untouched so we never break unrelated tooling.
    """
    node_dir = (elidia_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``elidia gateway run`` processes
    ...
    try:
        from elidia_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from elidia_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ELIDIA_HOME`` and ``ELIDIA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\elidia\git\cmd``, ``%LOCALAPPDATA%\elidia\git\bin``,
#      ``%LOCALAPPDATA%\elidia\git\usr\bin``, ``%LOCALAPPDATA%\elidia\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\elidia\git\`` and Node to
#      ``%LOCALAPPDATA%\elidia\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``elidia dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\elidia\gateway-service\`` and sometimes
...
def _elidia_path_markers(elidia_home: Path) -> list[str]:
    """Path-entry substrings that identify Elidia-owned User-PATH entries."""
    root = str(elidia_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare elidia-agent install dir.
    markers = [root + "\\elidia-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ELIDIA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "elidia".  We don't want to catch
    # unrelated entries like "celidia-foo" or "ephermeral", so we look for
    # backslash-elidia as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(elidia_home: Path) -> list[str]:
    """Strip Elidia-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _elidia_path_markers(elidia_home)
            kept: list[str] = []
...
def remove_elidia_env_vars_windows() -> list[str]:
    """Delete ELIDIA_HOME and ELIDIA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ELIDIA_HOME", "ELIDIA_GIT_BASH_PATH"):
                try:
...
def _is_default_elidia_home(elidia_home: Path) -> bool:
    """Return True when ``elidia_home`` points at the default (non-profile) root."""
    try:
        from elidia_constants import get_default_elidia_root
        return elidia_home.resolve() == get_default_elidia_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ELIDIA_HOME directory.

    We shell out to ``elidia -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ELIDIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m elidia_cli.main` so we don't depend on a `elidia`
    #    wrapper that may be half-removed mid-uninstall.
    elidia_invocation = [_sys.executable, "-m", "elidia_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                elidia_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ELIDIA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.elidia/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.elidia/ for future reinstall
    """
    project_root = get_project_root()
    elidia_home = get_elidia_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ELIDIA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_elidia_home(elidia_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ✦ Elidia Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {elidia_home / 'config.yaml'}")
    print(f"  Secrets: {elidia_home / '.env'}")
    print(f"  Data:    {elidia_home / 'cron/'}, {elidia_home / 'sessions/'}, {elidia_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ELIDIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Elidia data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Elidia code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in elidia_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\elidia\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(elidia_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Elidia-owned PATH entries in User environment")

        log_info("Removing ELIDIA_HOME / ELIDIA_GIT_BASH_PATH User env vars...")
        removed_env = remove_elidia_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Elidia-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing elidia command...")
    ...
    #     (only when they still point into this Elidia home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Elidia-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(elidia_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Elidia-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.elidia/, just remove the elidia-agent subdir
            if elidia_home in project_root.parents or project_root.parent == elidia_home:
    ...
    #     under ELIDIA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(elidia_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(elidia_home)
    ...
    # 5. Optionally remove ~/.elidia/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ELIDIA_HOME dirs live under
        ...
        try:
            if elidia_home.exists():
                shutil.rmtree(elidia_home)
                log_success(f"Removed {elidia_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {elidia_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {elidia_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Elidia-Technology/elidia-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Elidia-Technology/elidia-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Elidia Agent! ✦")
    print()
