...
Used by ``elidia profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``elidia_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Elidia Agent kanban board.
...
Rules:
  ...
  - Never write "Elidia Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_elidia_home())
        else:
