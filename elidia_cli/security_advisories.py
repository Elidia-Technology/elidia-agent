"""
Security advisory checker for Elidia Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``elidia doctor --ack <id>``; the ack is persisted to
...
1. ``elidia doctor`` (and ``elidia doctor --ack <id>``)
...
run in environments where the rest of Elidia failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including elidia when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.elidia/.env (OpenRouter, Anthropic, OpenAI, "
            "Elidia, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: elidia doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Elidia requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'elidia doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.elidia/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_elidia_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``elidia doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `elidia doctor` on the gateway host for details.")
