...
Pure logic: walks an Elidia config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `elidia doctor` and a future `elidia migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in an Elidia config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in an Elidia config and return retirement issues.
