...
from elidia_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from elidia_cli.dashboard_auth.cookies import read_session_cookies
from elidia_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /elidia``, the
    ``login_url`` is prefixed (``/elidia/login?next=...``) so the
    ...
    """
    from elidia_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["elidia.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``elidia_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from elidia_cli.dashboard_auth.cookies import clear_session_cookies
        from elidia_cli.dashboard_auth.prefix import prefix_from_request
