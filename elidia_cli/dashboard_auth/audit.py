...
from ``elidia_constants`` or other elidia_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ELIDIA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``elidia_constants.get_elidia_home`` semantics: env var wins,
    else ``~/.elidia``. A local copy avoids an import cycle with the
    middleware which lives below ``elidia_cli``.
    """
    home = os.environ.get("ELIDIA_HOME") or str(Path.home() / ".elidia")
