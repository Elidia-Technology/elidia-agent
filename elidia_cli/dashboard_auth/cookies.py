...
Three cookies in play:
  - elidia_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL)
  - elidia_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 30 days)
                         **DEPRECATED in OAuth contract v1** — Elidia Portal
  ...
  - elidia_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/elidia) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/elidia`` covers
...
setter wrote ``__Secure-elidia_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "elidia_session_at"
SESSION_RT_COOKIE = "elidia_session_rt"
PKCE_COOKIE = "elidia_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /elidia`` we want ``Path=/elidia`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    SKIPPED when empty — Elidia Portal contract v1 issues no refresh tokens
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/elidia``)
