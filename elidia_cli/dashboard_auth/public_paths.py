...
* ``elidia_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``elidia_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``elidia-account-service`` ``fly-provider.ts``
