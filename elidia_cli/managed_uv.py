...
Elidia owns its own uv binary at ``$ELIDIA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ELIDIA_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Elidia keeps *its* uv binary.

    ``$ELIDIA_HOME/bin/uv`` on POSIX, ``$ELIDIA_HOME\\bin\\uv.exe`` on
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``elidia update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$ELIDIA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
