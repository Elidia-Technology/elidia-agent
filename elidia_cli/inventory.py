...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Elidia /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Elidia, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``elidia model`` CLI picker. Adds network calls
      (pricing fetch + Elidia tier check); only set for interactive pickers.
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from elidia_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `elidia model` to configure ({auth_type})"
        )
...
def _apply_pricing(rows: list[dict]) -> None:
    ...
    # Resolve Elidia free-tier once (cached in models.py for the TTL window).
    elidia_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        if slug == "elidia":
            try:
                if elidia_free_tier is None:
                    elidia_free_tier = check_elidia_free_tier(force_fresh=True)
                row["free_tier"] = bool(elidia_free_tier)
                if elidia_free_tier:
                    _selectable, unavailable = partition_elidia_models_by_tier(
