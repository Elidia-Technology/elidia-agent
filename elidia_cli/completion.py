"""Shell completion script generation for elidia CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_elidia_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Elidia Agent bash completion
# Add to ~/.bashrc:
#   eval "$(elidia completion bash)"

_elidia_profiles() {{
    local profiles_dir="$HOME/.elidia/profiles"
...
_elidia_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_elidia_profiles)" -- "$cur"))
        return
    fi
...
complete -F _elidia_completion elidia
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef elidia
# Elidia Agent zsh completion
# Add to ~/.zshrc:
...
_elidia_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.elidia/profiles" ]]; then
        profiles+=($HOME/.elidia/profiles/*(N/:t))
    fi
...
_elidia() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_elidia_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'elidia command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Elidia Agent fish completion",
        "# Add to your config:",
        "#   elidia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __elidia_profiles",
        "    echo default",
        "    if test -d $HOME/.elidia/profiles",
        "        for d in $HOME/.elidia/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c elidia -f -s p -l profile"
        " -d 'Profile name' -xa '(__elidia_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__elidia_profiles)' -d 'Profile name'"
                )
