"""CLI handlers for the ``elidia proxy`` subcommand."""
...
from elidia_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "elidia proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "elidia"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"elidia auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Elidia proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Elidia proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: elidia proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``elidia proxy <subcommand>``."""
    ...
    print(
        "elidia proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  elidia proxy start [--provider elidia|xai] [--host 127.0.0.1] [--port 8645]\n"
