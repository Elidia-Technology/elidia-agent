...
# xAI's public API is OpenAI-compatible for the endpoints Elidia commonly
# uses. The Responses endpoint is included because Elidia' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Elidia-managed OAuth credentials."""

    auth_hint = "elidia auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`elidia auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`elidia auth reset xai-oauth` or re-authenticate with "
                    "`elidia auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `elidia auth add xai-oauth --type oauth`."
            )
