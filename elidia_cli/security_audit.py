"""On-demand supply-chain audit for Elidia Agent installs.

Scans three surfaces an Elidia user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Elidia venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.elidia/plugins``
...
def _discover_plugins(elidia_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.elidia/plugins``.
...
def run_audit(
    *,
    ...
    home = elidia_home or Path(get_elidia_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, elidia_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(elidia_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `elidia security audit`."""
    home = Path(get_elidia_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, elidia_home=home
    )
    if total == 0:
