"""
Elidia Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m elidia_cli.main web          # Start on http://127.0.0.1:9119
    python -m elidia_cli.main web --port 8080
"""
...
try:
    ...
    # running `elidia dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ELIDIA_WEB_DIST"]) if "ELIDIA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
app = FastAPI(title="Elidia Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# ELIDIA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ELIDIA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Elidia-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``elidia dashboard --tui``
# or ELIDIA_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
#
# This list is defined in ``elidia_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from elidia_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from elidia_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``elidia status`` and the
    # SPA's StatusPage can show "OAuth gate ON via AiUtils" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from elidia_cli.dashboard_auth import list_providers as _list_providers
    ...
    return {
        ...
        "elidia_home": str(get_elidia_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the elidia home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_elidia_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_elidia_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Elidia Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------


@app.get("/api/portal")
async def get_portal_status():
    _log.debug("Entered into get_portal_status")
    cfg = load_config() or {}
    # Elidia Portal authenticates with an AiUtils Developer API key (AIUT-3434);
    # the former OAuth session state is no longer the source of truth.
    api_key = ""
    try:
        from elidia_cli.auth import PROVIDER_REGISTRY, _resolve_api_key_provider_secret

        aiutils_cfg = PROVIDER_REGISTRY["aiutils"]
        api_key, _source = _resolve_api_key_provider_secret("aiutils", aiutils_cfg)
        inference_url = aiutils_cfg.inference_base_url
    except Exception:
        _log.exception("portal key resolution failed")
        inference_url = ""
    ...
    try:
        ...
        feats = get_elidia_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_elidia", False):
                    state = "via Elidia Portal"
    ...
    return {
        ...
        "subscription_url": "https://developer.aiutils.io/api-keys",
        "features": features,
    }
...
async def run_prompt_size():
    try:
        proc = _spawn_elidia_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_elidia_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_elidia_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.elidia/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_elidia_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "elidia-update": "elidia-update.log",
...
def _spawn_elidia_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``elidia <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``elidia_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "elidia_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ELIDIA_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``elidia gateway restart`` in the background."""
    try:
        proc = _spawn_elidia_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
@app.post("/api/elidia/update")
async def update_elidia():
    """Kick off ``elidia update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("elidia-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "elidia-update",
    ...
    try:
        proc = _spawn_elidia_action(["update"], "elidia-update")
    except Exception as exc:
        _log.exception("Failed to spawn elidia update")
...
@app.get("/api/elidia/update/check")
async def check_elidia_update(force: bool = False):
    """Report whether an Elidia update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/elidia/update`` actually runs ``elidia update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Elidia version string
    ...
    try:
        ...
        if force:
            try:
                (get_elidia_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.elidia/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Elidia supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in elidia_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    """
    try:
        from elidia_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `elidia model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Elidia this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Elidia and None otherwise. `model` may be
    ...
    if slug == "elidia":
        try:
            ...
            from elidia_cli.auth import get_provider_auth_state

            model_ids = get_curated_elidia_model_ids()
            pricing = get_pricing_for_provider("elidia") or {}
            free_tier = check_elidia_free_tier(force_fresh=True)

            portal_url = ""
            try:
                state = get_provider_auth_state("elidia") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_elidia_models_by_tier(
            ...
            return {"provider": "elidia", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (elidia) failed")
            return {"provider": "elidia", "model": "", "free_tier": None}

    # Non-Elidia: first curated model for the provider, matching prior behaviour.
    try:
        from elidia_cli.inventory import build_models_payload, load_picker_context
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.elidia/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # When switching the main provider to Elidia, mirror the CLI's
            # post-model-selection behaviour (elidia_cli/main.py
            # prompt_enable_tool_gateway / tools_config apply_elidia_managed_defaults):
            # auto-route any *unconfigured* tools through the Elidia Tool Gateway.
            # This is purely additive — apply_elidia_managed_defaults skips every
            ...
            if provider.strip().lower() == "elidia":
                try:
                    from elidia_cli.elidia_subscription import apply_elidia_managed_defaults
                    from elidia_cli.tools_config import _get_platform_tools

                    enabled = _get_platform_tools(
                        cfg, "cli", include_default_mcp_servers=False
                    )
                    changed = apply_elidia_managed_defaults(
                        cfg,
                    ...
                    # Portal lookup hiccups / non-subscriber / non-elidia gating
                    # must never block saving the model assignment.
                    _log.debug("apply_elidia_managed_defaults skipped", exc_info=True)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Elidia from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Elidia to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Elidia from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Elidia to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Elidia in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Elidia through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Elidia via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Elidia through an IMAP/SMTP mailbox.",
        "docs_url": "https://aiutils.io/elidia/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Elidia to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Elidia inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Elidia through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Elidia to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Elidia to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Elidia as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://aiutils.io/elidia/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://aiutils.io/elidia/user-guide/messaging/webhooks/",
...
# Anthropic, device-code for Codex/MiniMax) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``elidia auth add <provider>`` command so the dashboard
# can surface a one-click copy. Elidia Portal is not an OAuth provider —
# it authenticates with an AiUtils Developer API key (``elidia key store``).
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Elidia resolves Anthropic creds in this order at runtime:
    1. ``~/.elidia/.anthropic_oauth.json`` — Elidia-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_elidia_oauth_credentials,
            read_claude_code_credentials,
            _ELIDIA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_elidia_oauth_credentials = None  # type: ignore
        _ELIDIA_OAUTH_FILE = None  # type: ignore

    elidia_creds = None
    if read_elidia_oauth_credentials:
        try:
            elidia_creds = read_elidia_oauth_credentials()
        except Exception:
            elidia_creds = None
    if elidia_creds and elidia_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "elidia_pkce",
            "source_label": f"Elidia PKCE ({_ELIDIA_OAUTH_FILE})",
            "token_preview": _truncate_token(elidia_creds.get("accessToken")),
            "expires_at": elidia_creds.get("expiresAt"),
            "has_refresh_token": bool(elidia_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Elidia even
    when they also have a separate Elidia-managed PKCE login.
    """
    try:
...
#
# "elidia" (Elidia Portal) is intentionally NOT in this catalog. It used to
# be a device-code OAuth entry, but the OAuth identity server it pointed at
# (developer.aiutils.io/api/oauth/device/code) does not exist — the Portal
# now issues AiUtils Developer API keys instead. Both `/oauth/{id}/start`
# and the DELETE route special-case "elidia" below to explain that, rather
# than falling through to a generic 400/404.
_ELIDIA_API_KEY_GUIDANCE = (
    "Elidia Portal authenticates with an AiUtils Developer API key, not "
    "OAuth. Run `elidia key store` (or set AIUTILS_API_KEY / ELIDIA_KEY) "
    "instead of connecting it here."
)
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "elidia auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "elidia auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "elidia auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Elidia's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "elidia auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "elidia auth add xai-oauth",
        "docs_url": "https://aiutils.io/elidia/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from elidia_cli import auth as hauth
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("elidia_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"elidia", "elidia-portal"}:
        raise HTTPException(status_code=400, detail=_ELIDIA_API_KEY_GUIDANCE)
    ...
    # Anthropic and claude-code clear the same Elidia-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ELIDIA_OAUTH_FILE
            if _ELIDIA_OAUTH_FILE.exists():
                _ELIDIA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from elidia_cli.auth import clear_provider_auth
...
#          → persists to ~/.elidia/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (OpenAI Codex, MiniMax):
#     1. POST /api/providers/oauth/{openai-codex|minimax-oauth}/start
...
# Guarded so elidia web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Elidia file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``elidia auth add anthropic``.
    """
    from agent.anthropic_adapter import _ELIDIA_OAUTH_FILE
    payload = {
    ...
    _ELIDIA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ELIDIA_OAUTH_FILE.with_name(
        f"{_ELIDIA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ELIDIA_OAUTH_FILE)
        try:
            _ELIDIA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "elidia-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    "elidia" is deliberately not handled here — Elidia Portal has no
    device-code identity server (see ``start_oauth_login``, which rejects it
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Elidia's device-code
...
# the tokens exactly like `elidia auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `elidia auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_elidia_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Elidia. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``elidia auth add minimax-oauth``.
    """
    from elidia_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from elidia_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(provider_id: str, session_id: str):
    ...
    Shared by the device-code flows (OpenAI Codex, MiniMax) and the
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `elidia sessions stats`).
...
async def get_logs(
    ...
    from elidia_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_elidia_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from elidia_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from elidia_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ELIDIA_HOME)."""
    from elidia_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["elidia_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ELIDIA_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (elidia_cli.mcp_config), so
# servers managed here show up under `elidia mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from elidia_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from elidia_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from elidia_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from elidia_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    """Browse the Elidia-approved MCP catalog (the optional-mcps/ manifests).
    ...
    `elidia mcp catalog` / `elidia mcp install` read.
    """
    try:
        from elidia_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_elidia_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (elidia_cli.webhook); the webhook
...
async def list_webhooks():
    import elidia_cli.webhook as wh
...
# spawn the real `elidia gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_elidia_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_elidia_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from elidia_cli.auth import read_credential_pool
...
# `elidia memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `elidia memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_elidia_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_elidia_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_elidia_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_elidia_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from elidia_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from elidia_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <elidia_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_elidia_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_elidia_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_elidia_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_elidia_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_elidia_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_elidia_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "elidia setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ELIDIA_HOME override so the write lands in the target
    ...
    """
    from elidia_constants import set_elidia_home_override, reset_elidia_home_override

    token = set_elidia_home_override(str(profile_dir))
    try:
        cfg = load_config()
        model_cfg = cfg.get("model", {})
        if not isinstance(model_cfg, dict):
        ...
        reset_elidia_home_override(token)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``elidia profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ELIDIA_HOME).
    """
    from elidia_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``elidia profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ELIDIA_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``elidia profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from elidia_cli import profile_describer
...
async def get_skills():
    ...
    from elidia_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from elidia_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from elidia_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``elidia tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``elidia tools`` picker shows
    ...
    if cat:
        for prov in _visible_providers(cat, config, force_fresh=True):
            ...
            providers.append({
                ...
                "requires_elidia_auth": bool(prov.get("requires_elidia_auth")),
                "is_active": is_active,
            })
    return {
        "name": name,
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``elidia tools``
...
#
# The endpoint spawns the same ``elidia --tui`` binary the CLI uses, behind
...
try:
    from elidia_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from elidia_cli.dashboard_auth.audit import AuditEvent, audit_log
        from elidia_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``elidia --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ELIDIA_TUI_RESUME`` env var —
    matching what ``elidia_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ELIDIA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ELIDIA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from elidia_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ELIDIA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ELIDIA_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ELIDIA_TUI_RESUME"] = resume

    if sidecar_url:
        env["ELIDIA_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["ELIDIA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from elidia_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Elidia inside WSL2 to use the dashboard's /chat "
...
# ELIDIA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`elidia_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from elidia_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/elidia/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /elidia`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ELIDIA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/elidia``)
        ...
        ``__ELIDIA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ELIDIA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ELIDIA_BASE_PATH__="{prefix}";'
                f"window.__ELIDIA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ELIDIA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ELIDIA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ELIDIA_BASE_PATH__="{prefix}";'
                f"window.__ELIDIA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/elidia`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Elidia backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Elidia Teal",         "description": "Classic dark teal — the canonical Elidia look"},
    {"name": "default-large", "label": "Elidia Teal (Large)", "description": "Elidia Teal with bigger fonts and roomier spacing"},
    {"name": "elidia-blue",     "label": "Elidia Blue",           "description": "Light mode — vivid Elidia-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.elidia/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.elidia/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_elidia_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.elidia/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as elidia_cli.plugins):
    1. User plugins:    ~/.elidia/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.elidia/plugins/  (only if ELIDIA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from elidia_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_elidia_home() / "plugins", "user"),
    ...
    # ``elidia_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ELIDIA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".elidia" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from elidia_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_elidia_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"elidia auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from elidia_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from elidia_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from elidia_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from elidia_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.elidia/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.elidia/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"elidia_dashboard_plugin_{plugin['name']}"
...
from elidia_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from elidia_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing ELIDIA_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Elidia Agent exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import elidia as _elidia_plugin

                if _elidia_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • elidia: {_elidia_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            raise SystemExit(
                ...
                f"(was the dashboard_auth/elidia plugin removed?).\n"
    ...
    print(f"  Elidia Web UI → http://{host}:{port}")
