"""
elidia fallback — manage the fallback provider chain.
...
Subcommands:
  elidia fallback [list]   Show the current fallback chain (default when no subcommand)
  elidia fallback add      Pick provider + model via the same picker as `elidia model`,
                           then append the selection to the chain
  elidia fallback remove   Pick an entry to delete from the chain
  elidia fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.elidia/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from elidia_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `elidia model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  elidia fallback add")
        print()
        return
    ...
    print("  Docs: https://aiutils.io/elidia/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `elidia model`, then append the selection to the chain."""
    from elidia_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `elidia model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `elidia fallback list` to view, or `elidia fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``elidia fallback [subcommand]``."""
