...
``$ELIDIA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-elidia-setup
(the stage2 hook) has chowned the volume and seeded $ELIDIA_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ELIDIA_HOME``,
    not under ``profiles/``). The dispatcher in ``elidia_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``elidia gateway start`` (no ``-p``) targets. Without it,
    bare ``elidia gateway start`` inside the container would land on
    ...
    ``$ELIDIA_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        elidia_home: The container's ELIDIA_HOME (typically /opt/data).
            Profiles live under ``<elidia_home>/profiles/<name>/``;
            the default profile lives at ``<elidia_home>`` itself.
    ...
    # ``elidia gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(elidia_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(elidia_home)
    ...
    profiles_root = elidia_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `elidia profile create` (config.yaml
            # is not — that comes later via `elidia setup`). Use it as the
            ...
            # via ``elidia -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(elidia_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = elidia_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ELIDIA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "elidia":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `elidia -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit elidia
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the elidia user) won't hit EACCES. See
...
def _write_reconcile_log(
    elidia_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ELIDIA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    elidia_home = Path(os.environ.get("ELIDIA_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        elidia_home=elidia_home, scandir=scandir,
    )
    for a in actions:
        print(
