...
the existing module-level functions in elidia_cli.gateway and
elidia_cli.gateway_windows directly. This protocol is a thin facade
...
``elidia gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``elidia gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged elidia
    ...
    detection always fails. Since every Elidia runtime call inside the
    container drops to elidia via ``s6-setuidgid``, that silent failure
...
# in ``elidia_cli.gateway`` (systemd/launchd) and ``elidia_cli.gateway_windows``
...
# active (set via the ``elidia -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in elidia_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from elidia_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from elidia_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from elidia_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from elidia_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in elidia_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from elidia_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from elidia_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from elidia_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from elidia_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``elidia_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from elidia_cli import gateway_windows
        from elidia_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `elidia profile create`
# runs inside the container (Phase 4). Static services (main-elidia, dashboard)
...
# UID/GID of the in-image ``elidia`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the elidia user.
    ...
    (the ``elidia`` user — UID 10000 — running every Elidia runtime
    ...
    is dispatched as the elidia user.
    ...
    with elidia ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           elidia:elidia, 0755 (parent must already exist)
    ``svc_dir/event/``                     elidia:elidia, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 elidia:elidia, 0755
    ``svc_dir/supervise/event/``           elidia:elidia, 03730
    ``svc_dir/supervise/control``          elidia:elidia, 0660    (FIFO)
    ...
    slot's supervise/ was elidia-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ELIDIA_UID, _ELIDIA_GID)
        except PermissionError:
            # Running as the elidia user already — directory is elidia-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ELIDIA_UID, _ELIDIA_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is elidia-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ELIDIA_UID, _ELIDIA_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``elidia -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`elidia profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-elidia, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ELIDIA_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ELIDIA_HOME=/data/elidia`` is honored at run
          ...
          4. Drops to the elidia user and exec's
             ``elidia -p <profile> gateway run`` (or just ``elidia
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``elidia gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ELIDIA_HOME profile" (the implicit profile that exists at
        the top of $ELIDIA_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``elidia_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ELIDIA_HOME/profiles/default/`` — a
        ...
        ``elidia_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export ELIDIA_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "elidia gateway run"
        else:
            gateway_cmd = f"elidia -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid elidia {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ELIDIA_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ELIDIA_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ELIDIA_HOME=/data/elidia``
        gets its logs under /data/elidia/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``elidia logs`` reads and what
        ...
        return (
            ...
            f': "${{ELIDIA_HOME:=/opt/data}}"\n'
            f'log_dir="$ELIDIA_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R elidia:elidia "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid elidia s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with elidia ownership
            ...
            # the elidia user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is elidia-owned (see ``_seed_supervise_skeleton``), and on
