"""
Profile management for multiple isolated Elidia instances.

Each profile is a fully independent ELIDIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.elidia/profiles/<name>/`` by default.

The "default" profile is ``~/.elidia`` itself — backward compatible,
zero migration needed.

Usage::

    elidia profile create coder          # fresh profile + bundled skills
    elidia profile create coder --clone  # also copy config, .env, SOUL.md, skills
    elidia profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    elidia -p coder chat                 # or via flag
    elidia profile use coder             # set as sticky default
    elidia profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See elidia_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.elidia``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   elidia-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `elidia profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `elidia
...
# `elidia skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Elidia infrastructure directories that only the default profile
         (``~/.elidia``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_elidia_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.elidia) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "elidia-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "elidia", "default", "test", "tmp", "root", "sudo",
})

# Elidia subcommands that cannot be used as profile names/aliases
_ELIDIA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the elidia root, NOT to the current ELIDIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ELIDIA_HOME points outside
    ``~/.elidia``, profiles live under ``ELIDIA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_elidia_home() / "profiles"


def _get_default_elidia_home() -> Path:
    """Return the default (pre-profile) ELIDIA_HOME path.

    In standard deployments this is ``~/.elidia``.
    In Docker/custom deployments where ELIDIA_HOME is outside ``~/.elidia``
    (e.g. ``/opt/data``), returns ELIDIA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_elidia_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Elidia installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ELIDIA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_elidia_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, elidia subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ELIDIA_SUBCOMMANDS:
        return f"'{canon}' conflicts with an elidia subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``elidia profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Elidia config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_elidia_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``elidia update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.elidia)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.elidia/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `elidia update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`elidia profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `elidia profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `elidia -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ELIDIA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``elidia profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ELIDIA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.elidia).\n"
            "To remove everything, use: elidia uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from elidia_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ELIDIA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ELIDIA_HOME")
    try:
        os.environ["ELIDIA_HOME"] = str(profile_dir)
        from elidia_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ELIDIA_HOME"] = old_home
        elif "ELIDIA_HOME" in os.environ:
            del os.environ["ELIDIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.elidia/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: elidia profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ELIDIA_HOME.

    Returns ``"default"`` if ELIDIA_HOME is not set or points to ``~/.elidia``.
    Returns the profile name if ELIDIA_HOME points into ``~/.elidia/profiles/<name>``.
    Returns ``"custom"`` if ELIDIA_HOME is set to an unrecognized path.
    """
    from elidia_constants import get_elidia_home
    elidia_home = get_elidia_home()
    resolved = elidia_home.resolve()

    default_resolved = _get_default_elidia_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.elidia itself — its parent is ~/ and its
        # directory name is ".elidia", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: elidia profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.elidia itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.elidia). "
            "Specify a different name: elidia profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="elidia_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"elidia_{old_name}"
    legacy_old_host = f"elidia.{old_name}"
    new_host = f"elidia_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_elidia_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("elidia_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to an ELIDIA_HOME path string.

    Called early in the CLI entry point, before any elidia modules
    are imported, to set the ELIDIA_HOME environment variable.
    """
