"""
Interactive setup wizard for Elidia Agent.
...
from tools.tool_backend_helpers import managed_elidia_tools_enabled
...
_DOCS_BASE = "https://aiutils.io/elidia"
...
# display_elidia_home imported lazily at call sites (stale-module safety during elidia update)
...
from elidia_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from elidia_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("✦ Elidia Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Elidia using environment variables or config commands:")
    print_info("  elidia config set model.provider custom")
    print_info("  elidia config set model.base_url http://localhost:8080/v1")
    print_info("  elidia config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'elidia setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from elidia_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'elidia setup')")


def _print_setup_summary(config: dict, elidia_home):
    ...
    tool_status = []
    subscription_features = get_elidia_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'elidia setup' to configure"))
    ...
    if subscription_features.web.managed_by_elidia:
        tool_status.append(("Web Search & Extract (Elidia subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_elidia:
        tool_status.append(("Browser Automation (Elidia Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Elidia), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_elidia:
        tool_status.append(("Image Generation (Elidia subscription)", True, None))
    ...
    # Video generation — opt-in via `elidia tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_elidia:
        tool_status.append(("Video Generation (FAL via Elidia subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from elidia_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_elidia:
        tool_status.append(("Text-to-Speech (OpenAI via Elidia subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'elidia setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'elidia setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_elidia:
        tool_status.append(("Modal Execution (Elidia subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'elidia setup terminal'"))
    elif managed_elidia_tools_enabled() and subscription_features.elidia_auth_present:
        tool_status.append(("Modal Execution (optional via Elidia subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via elidia auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'elidia setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {elidia_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('elidia setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('elidia setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('elidia setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('elidia setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('elidia setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('elidia config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('elidia config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('elidia config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('elidia', Colors.GREEN)}              Start chatting")
    print(f"   {color('elidia gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('elidia doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `elidia tools` and `elidia setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``elidia model``)
    ...
    provider added to ``elidia model`` is automatically available here.
    ...
    # Delegate to the shared elidia model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from elidia_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: elidia model")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``elidia model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from elidia_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_elidia_tools_enabled() and subscription_features.elidia_auth_present:
        choices.append("Elidia Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("elidia-openai")
    choices.extend(
        [
    ...
    selected_via_elidia = selected == "elidia-openai"
    if selected == "elidia-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Elidia gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.elidia/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_elidia:
        ...
        # via the Elidia auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via elidia setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'elidia setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Elidia runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `elidia setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `elidia setup terminal`.
        ...
        managed_modal_available = bool(
            managed_elidia_tools_enabled()
            and
            get_elidia_subscription_features(config).elidia_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Elidia gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ELIDIA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ELIDIA_MAX_ITERATIONS")
    ...
    print_info("  Run `elidia setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ELIDIA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Elidia delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `elidia update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://aiutils.io/elidia/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ELIDIA_HOME, and print
    ...
    """
    try:
        from elidia_cli.slack_cli import _build_full_manifest
        from elidia_constants import get_elidia_home

        manifest = _build_full_manifest(
            bot_name="Elidia",
            bot_description="Your Elidia agent on Slack",
        )
        target = Path(get_elidia_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `elidia slack manifest --write` anytime to refresh after "
            "Elidia adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Elidia delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Elidia to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from elidia_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://aiutils.io/elidia/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://aiutils.io/elidia/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  elidia config edit")
    print_info("   Open config in your editor:  elidia config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from elidia_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Elidia from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'elidia setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     elidia config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_elidia_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo elidia gateway install --system")
                print_info("  Or run in foreground:  elidia gateway")
        else:
            from elidia_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   elidia gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   elidia gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `elidia setup tools` and `elidia tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from elidia_cli.config
    ...
    if section_key == "model":
        ...
        from elidia_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_elidia.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_elidia migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_elidia", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Elidia.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Elidia to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Elidia at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Elidia at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Elidia at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Elidia at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Elidia equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Elidia config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Elidia.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Elidia's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(elidia_home: Path) -> bool:
    ...
    print_info("Elidia can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: elidia claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=elidia_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: elidia claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Elidia configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Elidia config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Elidia (use elidia claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    provider write here) means ``elidia portal`` always offers a model picker,
    and there is a single source of truth for the Elidia onboarding steps.
    """
    ...
    print(color("│     ✦ Elidia Setup — Elidia Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    try:
        ...
        logger.debug("_model_flow_elidia error during `elidia portal`: %s", exc)
        print()
        print_error(f"  Elidia Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `elidia portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_elidia (and the
    ...
    print_info("  Run `elidia portal info` to inspect routing.")
    print_info("  Run `elidia` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      elidia setup           — full or quick (auto-detected)
      elidia setup model     — just model/provider
      elidia setup tts       — just text-to-speech
      elidia setup terminal  — just terminal backend
      elidia setup gateway   — just messaging platforms
      elidia setup tools     — just tool configuration
      elidia setup agent     — just agent settings
    """
    from elidia_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Elidia Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ✦ Elidia Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ✦ Elidia Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Elidia Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, elidia_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Elidia configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'elidia setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(elidia_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Elidia?",
            [
                "Quick Setup (Elidia Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, elidia_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {elidia_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'elidia config edit'")
    ...
    # Tune later with `elidia setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, elidia_home)


def _run_first_time_quick_setup(config: dict, elidia_home, is_existing: bool):
    """Streamlined first-time setup via Elidia Portal: OAuth, model, terminal & messaging.

    Routes straight to the Elidia Portal provider — runs the device-code OAuth
    login, picks an Elidia model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``elidia setup <section>``
    or switch providers with ``elidia model``.
    """
    from elidia_cli.config import load_config
    ...
    try:
        ...
        print()
        print_info("Elidia Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_elidia error during quick setup: %s", exc)
        print_warning(f"Elidia Portal setup encountered an error: {exc}")
        print_info("You can try again later with: elidia model")

    # Re-sync the wizard's config dict from disk — _model_flow_elidia (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'elidia setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    elidia setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  elidia setup gateway")
    print()

    _print_setup_summary(config, elidia_home)


def _run_quick_setup(config: dict, elidia_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'elidia setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Elidia to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'elidia setup gateway'.")
