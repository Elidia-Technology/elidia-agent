"""
Backup and import commands for elidia CLI.

`elidia backup` creates a zip archive of the entire ~/.elidia/ directory
(excluding the elidia-agent repo and transient files).

`elidia import` restores from a backup zip, overlaying onto the current
ELIDIA_HOME root.
"""
...
from elidia_constants import get_default_elidia_root, get_elidia_home, display_elidia_home
...
_EXCLUDED_DIRS = {
    "elidia-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to elidia root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ELIDIA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Elidia home directory."""
    elidia_root = get_default_elidia_root()

    if not elidia_root.is_dir():
        print(f"Error: Elidia home directory not found at {elidia_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"elidia-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"elidia-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_elidia_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(elidia_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(elidia_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(elidia_root)
    ...
    print(f"\nRestore with: elidia import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like an Elidia backup.
    ...
    # Look for telltale files that an elidia home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be an Elidia backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.elidia/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like an elidia dir name
        if prefix in {".elidia", "elidia"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_elidia_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (elidia_root / "config.yaml").exists()
        has_env = (elidia_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Elidia configuration.")
        ...
        for member in members:
            ...
            target = elidia_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(elidia_root.resolve())
            except ValueError:
        ...
        profiles_dir = elidia_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # elidia_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: elidia profile list  (after installing elidia)")

        # Guidance
        print()
        if not (elidia_root / "elidia-agent").is_dir():
            print("Note: The elidia-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: elidia update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  elidia -p {pname} gateway install")

        print("Done. Your Elidia configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and elidia backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ELIDIA_HOME).
...
# — `elidia update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(elidia_home: Optional[Path] = None) -> Path:
    home = elidia_home or get_elidia_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    elidia_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(elidia_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ELIDIA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``elidia update``.
    ...
    Args:
        ...
        elidia_home: Override for the Elidia home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(elidia_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for elidia backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_elidia_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, elidia_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``elidia_root`` to ``out_path``.
...
def _pre_update_backup_dir(elidia_home: Optional[Path] = None) -> Path:
    home = elidia_home or get_elidia_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ELIDIA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ELIDIA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``elidia update``) should continue even if the backup fails.
    """
    elidia_root = elidia_home or get_default_elidia_root()
    if not elidia_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(elidia_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, elidia_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `elidia claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ELIDIA_HOME under ``backups/`` before a
    ``elidia claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``elidia import <archive>``.  Writes to
    ``<ELIDIA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `elidia import` and the
