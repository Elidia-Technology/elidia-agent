"""MCP picker — interactive `elidia mcp picker` (also the default `elidia mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``elidia mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `elidia plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from elidia_cli.cli_output import prompt_yes_no
from elidia_cli.curses_ui import curses_single_select
from elidia_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Elidia session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from elidia_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `elidia mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: elidia mcp install <name>    Picker: elidia mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Elidia is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Elidia — run `elidia update` "
...
def show_catalog() -> None:
    """`elidia mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`elidia mcp picker` (and default `elidia mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`elidia mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `elidia mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
