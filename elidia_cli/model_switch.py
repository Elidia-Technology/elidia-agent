...
- ``elidia_cli.providers``        -- canonical provider identity + overlays
- ``elidia_cli.model_normalize``  -- per-provider name formatting
...
def is_elidia_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_ELIDIA_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``elidia config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from elidia_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "elidia")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "elidia")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Elidia, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'elidia model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'elidia doctor' — config issues detected:"
        ...
        from elidia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from elidia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from elidia_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in elidia_cli.runtime_provider.resolve_runtime_provider;
    ...
    elidia_warn = _check_elidia_model_warning(new_model)
    if elidia_warn:
        warnings.append(elidia_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from elidia_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from elidia_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by elidia provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "elidia" pulls from the remote model-catalog manifest published at
    # https://share.aiutils.io/elidia-hermes/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring an Elidia release. Falls back to the in-repo
    # _PROVIDER_MODELS["elidia"] snapshot when the manifest is unreachable.
    curated["elidia"] = get_curated_elidia_model_ids()
    ...
    # --- 1. Check Elidia-mapped providers ---
    from elidia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from elidia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for elidia_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(elidia_id)
        if (
            _alias_target
            and _alias_target != elidia_id
        ...
        pconfig = PROVIDER_REGISTRY.get(elidia_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ELIDIA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from elidia_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(elidia_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `elidia model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(elidia_id)
        if not model_ids:
            model_ids = curated.get(elidia_id, [])
            if elidia_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(elidia_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Elidia-only providers (elidia, openai-codex, copilot, opencode-go) ---
    from elidia_cli.providers import ELIDIA_OVERLAYS
    from elidia_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Elidia provider ID.
    # ELIDIA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Elidia IDs ("copilot").
    _mdev_to_elidia = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ELIDIA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Elidia slug — e.g. "github-copilot" → "copilot"
        elidia_slug = _mdev_to_elidia.get(pid, pid)
        if elidia_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(elidia_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, elidia_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or elidia_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(elidia_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", elidia_slug, exc)
        ...
        # configured.
        if not has_creds and elidia_slug == "anthropic":
            try:
                ...
                if (elidia_creds and elidia_creds.get("accessToken")) or \
        ...
        if elidia_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(elidia_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(elidia_slug)
                model_ids = _ids if _ids else (curated.get(elidia_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(elidia_slug, []) or curated.get(pid, [])
        elif elidia_slug == "elidia":
            # Elidia serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `elidia model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_elidia in main.py) so
            ...
            model_ids = curated.get("elidia", [])
            try:
                from elidia_cli.models import (
                    get_pricing_for_provider as _elidia_pricing,
                    check_elidia_free_tier as _elidia_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from elidia_cli.auth import get_provider_auth_state as _elidia_state

                _pricing = _elidia_pricing("elidia") or {}
                _portal = ""
                try:
                    _st = _elidia_state("elidia") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _elidia_free(force_fresh=True):
            ...
            if not model_ids:
                model_ids = curated.get(elidia_slug, []) or curated.get(pid, [])
                if elidia_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(elidia_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": elidia_slug,
            "name": get_label(elidia_slug),
            "is_current": elidia_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "elidia",
        ...
        seen_slugs.add(elidia_slug.lower())
        _record_builtin_endpoint(elidia_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ELIDIA_OVERLAYS (keeps /model in sync
    # with `elidia model`).
    try:
        from elidia_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Elidia's canonical write key (matches
            ...
            # Elidia writes ``models:`` as a dict keyed by model id
            # (see elidia_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Elidia's own writer uses; a hyphen variant is accepted
            ...
            # active model. Elidia's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, elidia_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`elidia_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from elidia_cli.models import fetch_openrouter_models
