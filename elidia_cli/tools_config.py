"""
Unified tool configuration for Elidia Agent.

`elidia tools` and `elidia setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.elidia/config.yaml under
...
from elidia_cli.elidia_subscription import (
    apply_elidia_managed_defaults,
    get_elidia_subscription_features,
)
from elidia_cli.elidia_account import format_elidia_portal_entitlement_message
...
from elidia_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ELIDIA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `elidia tools` → Video Generation, which walks
...
# `elidia tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "aiutils"}
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from elidia_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `elidia tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``elidia tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from elidia_cli.plugins import discover_plugins, get_plugin_toolsets
...
from elidia_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Elidia Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_elidia_auth": True,
                "managed_elidia_feature": "tts",
    ...
    "web": {
        ...
        #   - "Elidia Subscription" — managed Firecrawl billed via Elidia
        #     subscription (requires_elidia_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Elidia Subscription" — managed FAL billed via the Elidia
        #     subscription (requires_elidia_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_elidia_auth": True,
                "managed_elidia_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Elidia Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Elidia Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_elidia_auth": True,
                "managed_elidia_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Elidia Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Elidia routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Elidia Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Elidia Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Elidia subscription (requires_elidia_auth +
        ...
        "providers": [
            ...
            {
                "name": "Elidia Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_elidia_auth": True,
                "managed_elidia_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. ELIDIA_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ELIDIA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ELIDIA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ELIDIA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ELIDIA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ELIDIA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ELIDIA_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``elidia update`` and
      by ``elidia computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `elidia update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Elidia process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_elidia_home()}/elidia-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in elidia-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/aiutils/elidia-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.elidia/config.yaml")
        ...
        # Run the full `elidia auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.elidia/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from elidia_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: elidia auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `elidia auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: elidia auth spotify")
        except Exception as exc:
        ...
        try:
            from elidia_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: elidia plugins enable observability/langfuse")
        _print_info("    Restart Elidia for tracing to take effect.")
        _print_info("    Verify: elidia plugins list")
        ...
        try:
            ...
            _print_info("    Run later: elidia auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `elidia auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: elidia auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: elidia auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "elidia-cli" (which include all _ELIDIA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[elidia-cli, spotify]`` after enabling Spotify via ``elidia
        ...
        # (e.g. "elidia-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``elidia tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `elidia tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"elidia-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("elidia-")}
    ...
    # shipped as a bundled plugin but user must opt in via `elidia tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `elidia tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by elidia tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (elidia-cli, elidia-telegram, etc.)
    ...
    # Opening `elidia tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "browser"}:
        features = get_elidia_subscription_features(config, force_fresh=force_fresh)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_elidia):
            return True
...
# ("Elidia Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Elidia Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Elidia-managed Tool Gateway rows (``managed_elidia_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Elidia Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Elidia-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_elidia_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_elidia_auth")
            and not provider.get("managed_elidia_feature")
            and not features.elidia_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_elidia_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Elidia Subscription.
    ...
    # remaining hardcoded rows ("Elidia Subscription", "Firecrawl
    ...
    # the hardcoded "Elidia Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_elidia_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Elidia Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Elidia-managed rows were filtered out for unentitled
    ...
    (``ensure_elidia_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `elidia tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_elidia_message = _hidden_elidia_gateway_message(
        cat,
        config,
        f"the Elidia Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_elidia_message:
            for line in hidden_elidia_message.splitlines():
        ...
        # When the user is logged into Elidia, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Elidia.
        try:
            _elidia_logged_in = bool(
                get_elidia_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).elidia_auth_present
        ...
        for p in providers:
            ...
            # Mark Elidia-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Elidia Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_elidia_feature"):
                if _elidia_logged_in:
                    sub_marker = "  ★ Included with your Elidia subscription"
                else:
                    sub_marker = "  ★ via Elidia Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_elidia_feature"):
        video_cfg = config.get("video_gen", {})
        return isinstance(video_cfg, dict) and video_cfg.get("provider") == video_plugin_name

    managed_feature = provider.get("managed_elidia_feature")
    if managed_feature:
        features = get_elidia_subscription_features(config, force_fresh=force_fresh)
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_elidia
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_elidia
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_elidia and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_elidia and current == provider["web_backend"]
        return feature.managed_by_elidia
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Elidia auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Elidia Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # Elidia-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Elidia
    ...
    # "enable all tools" prompt (that lives in `elidia model`).
    if managed_feature:
        from elidia_cli.elidia_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_elidia_portal_access,
        )

        if not ensure_elidia_portal_access(
            capability=f"{provider.get('name', 'the Elidia Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Elidia Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_elidia_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_elidia_auth") and not managed_feature:
        features = get_elidia_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.elidia_auth_present or not entitled:
            message = format_elidia_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Elidia Subscription')}",
            )
            _print_warning(
                f"  {message or 'Elidia Subscription is only available after logging into Elidia Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Elidia subscription.")
    ...
    # Elidia-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Elidia.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_elidia_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_elidia_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_elidia_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.elidia_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Elidia Portal subscription.")
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Elidia Portal login + entitlement gate as _configure_provider:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `elidia tools` and `elidia setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("✦ Elidia Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://aiutils.io/elidia/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_elidia_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Elidia subscription defaults", Colors.GREEN))
    ...
    print()
    from elidia_constants import display_elidia_home
    print(color(f"  Tool configuration saved to {display_elidia_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'elidia' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, elidia mcp
