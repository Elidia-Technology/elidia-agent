"""
Elidia MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Elidia-specific extra)

Usage:
    elidia mcp serve
    elidia mcp serve --verbose
...
logger = logging.getLogger("elidia.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ELIDIA_HOME."""
    try:
        from elidia_constants import get_elidia_home
        return get_elidia_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ELIDIA_HOME", Path.home() / ".elidia")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from elidia_constants import get_elidia_home
        directory_file = get_elidia_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ELIDIA_HOME", Path.home() / ".elidia")
...
class EventBridge:
    ...
    This is the Elidia equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from elidia_constants import get_elidia_home
            db_file = get_elidia_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ELIDIA_HOME", Path.home() / ".elidia")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Elidia MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "elidia",
        instructions=(
            "Elidia Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Elidia MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
