...
from elidia_cli.timeouts import (
...
_loaded_env_paths = load_elidia_dotenv(elidia_home=_elidia_home, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"ElidiaAgent/{_ELIDIA_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[elidia-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=self.platform or os.environ.get("ELIDIA_SESSION_SOURCE", "cli"),
                model=self.model,
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("ELIDIA_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Elidia' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from elidia_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``elidia chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ELIDIA_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ELIDIA_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("ELIDIA_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ELIDIA_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ELIDIA_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        elidia-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See elidia-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Elidia serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "elidia":
            return False
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Elidia against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ELIDIA_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("elidia_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from elidia_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``ELIDIA_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.elidia/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ELIDIA_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.elidia/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ELIDIA_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ELIDIA_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ELIDIA_KANBAN_TASK set),
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``elidia auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_elidia_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "elidia":
            return False
        ...
        try:
            ...
            logger.debug("Elidia credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Elidia requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="elidia_credential_refresh"):
            return False
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from elidia_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Elidia Portal.
        """
        if base_url_host_matches(self._base_url_lower, "aiutils.io"):
            return True
        if (
            ...
            try:
                from elidia_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from elidia_cli.models import lmstudio_model_reasoning_options
